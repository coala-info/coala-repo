...
* [1 TL;DR (Too long; didn’t read)](#tldr-too-long-didnt-read)
* [2 Introduction to normR](#introduction-to-normr)
* [3 Toy Examples](#toy-examples)
  + [3.1 `enrichR()`: Calling Enrichment with an Input Control](#enrichr-calling-enrichment-with-an-input-control)
    - [3.1.1 Analyzing Results](#analyzing-results)
    - [3.1.2 Exporting Results](#exporting-results)
  + [3.2 `diffR()`: Calling Differential Enrichment without a Control Experiment](#diffr-calling-differential-enrichment-without-a-control-experiment)
  + [3.3 `regimeR()`: Identify Enrichment Regimes in ChIP-seq Experiments](#regimer-identify-enrichment-regimes-in-chip-seq-experiments)
* [4 Advanced Topics](#advanced-topics)
  + [4.1 Change Read Counting Strategy with `NormRCountConfig-class`](#change-read-counting-strategy-with-normrcountconfig-class)
  + [4.2 Analyzing Predefined Regions](#analyzing-predefined-regions)
  + [4.3 Post-processing of Difference Calls with CNV information](#post-processing-of-difference-calls-with-cnv-information)
...
actual \(\theta\_B\) is with ~0.09 much smaller than \(\theta^\*\) which allows for
...
![](data:image/png;base64...)
**IGV browser shot of Input (grey), H3K4me3 (green) and H3K36me3 (purple)
alignment data (bars), normalized enrichment, *i.e.* “bigWig” files, (lines)
and enriched regions, *i.e.* “bed” files (boxes below respective tracks).**
...
*i.e.* “bigWig” file, and differential regions are plotted as boxes, *i.e.*
