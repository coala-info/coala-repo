...
illuminaHumanv2ACCNUM Map Manufacturer identifiers to Accession Numbers

Description

illuminaHumanv2ACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
illuminaHumanv2ALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(illuminaHumanv2ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the illuminaHumanv2.db annotation Package. The purpose of this package is to provide
detailed information about the illuminaHumanv2 platform. This package is updated biannually.
...
illuminaHumanv2CHR is an R object that provides mappings between a manufacturer identifier and
...
illuminaHumanv2CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

illuminaHumanv2CHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
tt <- illuminaHumanv2CHRLENGTHS
# Length of chromosome 1
tt["1"]

4

illuminaHumanv2ENSEMBL

illuminaHumanv2CHRLOC Map Manufacturer IDs to Chromosomal Location

Description

illuminaHumanv2CHRLOC is an R object that maps manufacturer identifiers to the starting position
...
illuminaHumanv2ENSEMBL is an R object that contains mappings between manufacturer identi-
fiers and Ensembl gene accession numbers.

illuminaHumanv2ENTREZID
...
# Convert to a list
xx <- as.list(illuminaHumanv2ENSEMBL2PROBE)
if(length(xx) > 0){
...
illuminaHumanv2ENTREZID is an R object that provides mappings between manufacturer identi-
...
illuminaHumanv2ENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

illuminaHumanv2ENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers.
illuminaHumanv2ENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the illuminaHumanv2ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the illuminaHumanv2ENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In illuminaHumanv2ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert illuminaHumanv2ENZYME2PROBE to a list to see inside
xx <- as.list(illuminaHumanv2ENZYME2PROBE)
if(length(xx) > 0){
...
illuminaHumanv2GENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
illuminaHumanv2GO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(illuminaHumanv2GO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

illuminaHumanv2GO2ALLPROBES is an R object that provides mappings between a given GO
...
illuminaHumanv2GO2PROBE.

Details

If illuminaHumanv2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If illuminaHumanv2GO2ALLPROBES or illuminaHumanv2GO2PROBE is cast as a list, each GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(illuminaHumanv2GO2PROBE)
if(length(xx) > 0){
...
# Convert illuminaHumanv2GO2ALLPROBES to a list
xx <- as.list(illuminaHumanv2GO2ALLPROBES)
if(length(xx) > 0){
...
illuminaHumanv2MAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
illuminaHumanv2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package illuminaHumanv2.db.
...
We have used an extensive re-annotation of the illuminaHumanv2 probe sequences to provide ad-
...
illuminaHumanv2listNewMappings List all the custom re-annotation mappings provided by the

package

illuminaHumanv2fullReannotation Return all the re-annotation information as a matrix
illuminaHumanv2ARRAYADDRESS Array Address code used to identify the probe at the bead-

level

illuminaHumanv2NUID Lumi’s nuID (universal naming scheme for oligonucleotides) Reference:

Du et al. (2007), Biol Direct 2:16

illuminaHumanv2PROBESEQUENCE The 50 base sequence for the probe
illuminaHumanv2PROBEQUALITY Quality grade assigned to the probe: “Perfect” if it per-
...
illuminaHumanv2CODINGZONE Coding status of target sequence: intergenic / intronic / Tran-
scriptomic (“Transcriptomic” when the target transcript is non-coding or there is no informa-
tion on the coding sequence)

illuminaHumanv2GENOMICLOCATION Probe’s genomic coordinates (hg19 for human, mm9

for mouse or rn4 for rat)

illuminaHumanv2GENOMICMATCHSIMILARITY Percentage of similarity between the probe

and its best genomic match in the alignable region, taking the probe as reference

illuminaHumanv2SECONDMATCHES Genomic coordinates of second best matches between

the probe and the genome

illuminaHumanv2SECONDMATCHSIMILARITY Percentage of similarity between the probe
and its second best genomic match in the alignable region, taking the probe as reference
illuminaHumanv2TRANSCRIPTOMICMATCHSIMILARITY Percentage of similarity between

the probe and its target transcript in the alignable region, taking the probe as reference

illuminaHumanv2OTHERGENOMICMATCHES Genomic coordinates of sequences as alignable

with the probe (in terms of number of matching nucleotides) as its main target

illuminaHumanv2REPEATMASK Overlapping RepeatMasked sequences, with number of bases

overlapped by the repeat

illuminaHumanv2OVERLAPPINGSNP Overlapping annotated SNPs
illuminaHumanv2ENTREZREANNOTATED Entrez IDs
illuminaHumanv2ENSEMBLREANNOTATED Ensembl IDs
illuminaHumanv2SYMBOLREANNOTATED Gene symbol derived by re-annotation
illuminaHumanv2REPORTERGROUPID For probes marked as controls in Illuminas annota-

tion file, these gives the type of control

illuminaHumanv2REPORTERGROUPNAME Usually a more informative name for the control
...
illuminaHumanv2OMIM is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv2ORGANISM

15

Examples

x <- illuminaHumanv2OMIM
...
The Organism information for illuminaHumanv2

Description

illuminaHumanv2ORGANISM is an R object that contains a single item: a character string that
names the organism for which illuminaHumanv2 was built.
illuminaHumanv2ORGPKG is an R
...
manv2ORGANISM provides a simple way to programmatically extract the organism name. illumi-
naHumanv2ORGPKG provides a simple way to programmatically extract the name of the parent
...
illuminaHumanv2PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

illuminaHumanv2PATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the illuminaHumanv2PATH2PROBE object to a list
xx <- as.list(illuminaHumanv2PATH2PROBE)
if(length(xx) > 0){
...
illuminaHumanv2PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. illuminaHumanv2PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When illuminaHumanv2PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When illuminaHumanv2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object illuminaHumanv2PMID2PROBE to a list
xx <- as.list(illuminaHumanv2PMID2PROBE)
if(length(xx) > 0){
...
illuminaHumanv2REFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

illuminaHumanv2REFSEQ is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv2SYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

illuminaHumanv2SYMBOL is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv2UNIGENE

Map between Manufacturer Identifiers and UniGene cluster identifiers

Description

illuminaHumanv2UNIGENE is an R object that provides mappings between manufacturer identi-
...
illuminaHumanv2UNIPROT is an R object that contains mappings between the manufacturer iden-
...
illuminaHumanv2_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by illuminaHumanv2_dbconn
or you will break all the AnnDbObj objects defined in this package!

illuminaHumanv2_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).

illuminaHumanv2_dbschema prints the schema definition of the package annotation DB.

illuminaHumanv2_dbInfo prints other information about the package annotation DB.

Value

illuminaHumanv2_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.

illuminaHumanv2_dbfile: a character string with the path to the package annotation DB.

illuminaHumanv2_dbschema: none (invisible NULL).

illuminaHumanv2_dbInfo: none (invisible NULL).
...
illuminaHumanv2.db, 2
illuminaHumanv2_dbconn, 21
illuminaHumanv2ACCNUM, 1
illuminaHumanv2ALIAS2PROBE, 2
illuminaHumanv2CHR, 3
illuminaHumanv2CHRLENGTHS, 3
illuminaHumanv2CHRLOC, 4
illuminaHumanv2ENSEMBL, 4
illuminaHumanv2ENTREZID, 5
illuminaHumanv2ENZYME, 6
illuminaHumanv2GENENAME, 7
illuminaHumanv2GO, 8
illuminaHumanv2listNewMappings, 11
illuminaHumanv2MAP, 10
illuminaHumanv2MAPCOUNTS, 11
illuminaHumanv2OMIM, 14
illuminaHumanv2ORGANISM, 15
illuminaHumanv2PATH, 15
illuminaHumanv2PMID, 16
illuminaHumanv2REFSEQ, 18
illuminaHumanv2SYMBOL, 19
illuminaHumanv2UNIGENE, 19
illuminaHumanv2UNIPROT, 20
...
illuminaHumanv2 (illuminaHumanv2.db), 2
illuminaHumanv2.db, 2
illuminaHumanv2_dbconn, 21
...
illuminaHumanv2UNIGENE, 19
illuminaHumanv2UNIPROT, 20
illuminaHumanv2UNIPROT2PROBE

(illuminaHumanv2UNIPROT), 20

mappedkeys, 11

illuminaHumanv2GO2ALLPROBES, 9
illuminaHumanv2GO2ALLPROBES
...
illuminaHumanv2PATH, 15
illuminaHumanv2PATH2PROBE

(illuminaHumanv2PATH), 15

illuminaHumanv2PMID, 16
illuminaHumanv2PMID2PROBE
