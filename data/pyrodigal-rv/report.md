...
### Tool Description
pyrodigal-rv [-a trans_file] [-c] [-d nuc_file] [-f output_type]
...
### Original Help Text
```text
usage: pyrodigal-rv [-a trans_file] [-c] [-d nuc_file] [-f output_type]
                    [-g tr_table] [-i input_file] [-m] [-n] [-o output_file]
                    [-p mode] [-s start_file] [-t training_file] [-j jobs]
                    [-h] [-V] [--min-gene MIN_GENE]
                    [--min-edge-gene MIN_EDGE_GENE]
...
options:
  ...
  -t training_file      Write a training file (if none exists); otherwise,
                        ...
                        None)
  -j, --jobs jobs       The number of threads to use if input contains
