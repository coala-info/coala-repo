...
inputs:
  ...
  - id: featcol
    type:
      - 'null'
      - int
    doc: Column number in table in which desired accessions to summarize are 
      stored. First column number is 1. If not specified this will summarize to 
      PSMs themselves, i.e. only calculate the ratios and add those to the PSM 
      table.
    inputBinding:
      position: 101
      prefix: --featcol
  ...
  - id: split_multi_entries
    type:
      - 'null'
      - boolean
    doc: When summarizing isobaric quantification data, also use the PSMs that 
      have multiple mappings to their column, and split these, so that quant 
      data from a PSM mapping to two proteins is used for both proteins Normally
      these PSMs accessionould be skipped in quantification
    inputBinding:
      position: 101
      prefix: --split-multi-entries
