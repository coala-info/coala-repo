...
hgubeta7ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgubeta7ALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgubeta7ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgubeta7.db annotation Package. The purpose of this package is to provide detailed
information about the hgubeta7 platform. This package is updated biannually.
...
hgubeta7CHR is an R object that provides mappings between a manufacturer identifier and the
...
hgubeta7CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgubeta7CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgubeta7ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgubeta7ENSEMBL2PROBE)
if(length(xx) > 0){
...
hgubeta7ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgubeta7ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgubeta7ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgubeta7ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgubeta7ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgubeta7ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgubeta7ENZYME2PROBE to a list to see inside
xx <- as.list(hgubeta7ENZYME2PROBE)
if(length(xx) > 0){
...
hgubeta7GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgubeta7GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgu-
beta7GO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

hgubeta7GO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu-
beta7GO2PROBE.

Details

If hgubeta7GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgubeta7GO2ALLPROBES or hgubeta7GO2PROBE is cast as a list, each GO term maps to a
...
# Convert hgubeta7GO2ALLPROBES to a list
xx <- as.list(hgubeta7GO2ALLPROBES)
if(length(xx) > 0){
...
hgubeta7MAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgubeta7.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgubeta7MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgubeta7.db.
...
hgubeta7OMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgubeta7

Description

hgubeta7ORGANISM is an R object that contains a single item: a character string that names the
organism for which hgubeta7 was built. hgubeta7ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgubeta7ORGANISM
provides a simple way to programmatically extract the organism name. hgubeta7ORGPKG pro-
...
hgubeta7PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgubeta7PATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the hgubeta7PATH2PROBE object to a list
xx <- as.list(hgubeta7PATH2PROBE)
if(length(xx) > 0){
...
hgubeta7PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgubeta7PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgubeta7PMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgubeta7PMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgubeta7PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgubeta7PMID2PROBE to a list
xx <- as.list(hgubeta7PMID2PROBE)
if(length(xx) > 0){
...
hgubeta7PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgubeta7REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgubeta7SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgubeta7UNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgubeta7UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgubeta7_dbconn()
hgubeta7_dbfile()
hgubeta7_dbschema(file="", show.indices=FALSE)
hgubeta7_dbInfo()
...
hgubeta7_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgubeta7_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgubeta7_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgubeta7_dbschema prints the schema definition of the package annotation DB.
hgubeta7_dbInfo prints other information about the package annotation DB.

Value

hgubeta7_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
hgubeta7_dbfile: a character string with the path to the package annotation DB.
hgubeta7_dbschema: none (invisible NULL).
hgubeta7_dbInfo: none (invisible NULL).
...
hgubeta7_dbconn, 21
hgubeta7ACCNUM, 1
hgubeta7ALIAS2PROBE, 2
hgubeta7CHR, 3
hgubeta7CHRLENGTHS, 4
hgubeta7CHRLOC, 4
hgubeta7ENSEMBL, 5
hgubeta7ENTREZID, 6
hgubeta7ENZYME, 7
...
hgubeta7 (hgubeta7.db), 2
hgubeta7.db, 2
hgubeta7_dbconn, 21

hgubeta7_dbfile (hgubeta7_dbconn), 21
hgubeta7_dbInfo (hgubeta7_dbconn), 21
hgubeta7_dbschema (hgubeta7_dbconn), 21
hgubeta7ACCNUM, 1
hgubeta7ALIAS2PROBE, 2
hgubeta7CHR, 3
hgubeta7CHRLENGTHS, 4
hgubeta7CHRLOC, 4
hgubeta7CHRLOCEND (hgubeta7CHRLOC), 4
hgubeta7ENSEMBL, 5
hgubeta7ENSEMBL2PROBE

(hgubeta7ENSEMBL), 5

hgubeta7ENTREZID, 6
hgubeta7ENZYME, 7
hgubeta7ENZYME2PROBE (hgubeta7ENZYME), 7
hgubeta7GENENAME, 9
hgubeta7GO, 10
hgubeta7GO2ALLPROBES, 11
hgubeta7GO2ALLPROBES (hgubeta7GO), 10
hgubeta7GO2PROBE (hgubeta7GO), 10
hgubeta7LOCUSID (hgubeta7ENTREZID), 6
hgubeta7MAP, 12
hgubeta7MAPCOUNTS, 13
hgubeta7OMIM, 13
hgubeta7ORGANISM, 14
hgubeta7ORGPKG (hgubeta7ORGANISM), 14
hgubeta7PATH, 15
hgubeta7PATH2PROBE (hgubeta7PATH), 15
hgubeta7PFAM, 16
hgubeta7PMID, 16
hgubeta7PMID2PROBE (hgubeta7PMID), 16
hgubeta7PROSITE, 17
hgubeta7REFSEQ, 18
hgubeta7SYMBOL, 19
hgubeta7UNIGENE, 20
hgubeta7UNIPROT, 21
hgubeta7UNIPROT2PROBE
