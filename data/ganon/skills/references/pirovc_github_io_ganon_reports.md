...
* Reports (ganon report)
  + [Examples](#examples)
    - [Taxonomic profile with abundance estimation (default)](#taxonomic-profile-with-abundance-estimation-default)
    - [Sequence profile](#sequence-profile)
    - [Matches profile](#matches-profile)
...
# Reports[:)](#reports "Permanent link")
...
### Taxonomic profile with abundance estimation (default)[:)](#taxonomic-profile-with-abundance-estimation-default "Permanent link")

```
ganon report --db-prefix mydb --input results.rep --output-prefix tax_profile --report-type abundance
```

### Sequence profile[:)](#sequence-profile "Permanent link")

```
ganon report --db-prefix mydb --input results.rep --output-prefix seq_profile --report-type reads
```

### Matches profile[:)](#matches-profile "Permanent link")
...
### report type (--report-type)[:)](#report-type-report-type "Permanent link")
...
[Previous](../classification/ "Classification (ganon classify)")
[Next](../table/ "Table (ganon table)")
