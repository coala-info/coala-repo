...
- class: InitialWorkDirRequirement
  listing:
  - entryname: $(inputs.bed_file.basename)
...
- id: output_file
  ...
  outputBinding:
    glob: $(inputs.output_filename || 'clipped.bam')
- id: rejects_file
  type: File?
  doc: file to write filtered reads.
  outputBinding:
    glob: $(inputs.rejects_filename)
- id: primer_counts
  type: File?
  doc: file to write read counts per bed entry (bedgraph format).
  outputBinding:
    glob: $(inputs.primer_counts_filename)
stdout: clipped.bam
hints:
