...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Ffnaumenko%2FwigReg%2Fblob%2Fmaster%2FREADME.md)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=fnaumenko%2FwigReg)
...
[fnaumenko](/fnaumenko)
/
**[wigReg](/fnaumenko/wigReg)**
Public

* [Notifications](/login?return_to=%2Ffnaumenko%2FwigReg) You must be signed in to change notification settings
* [Fork
  0](/login?return_to=%2Ffnaumenko%2FwigReg)
* [Star
   1](/login?return_to=%2Ffnaumenko%2FwigReg)

* [Code](/fnaumenko/wigReg)
* [Issues
  0](/fnaumenko/wigReg/issues)
* [Pull requests
  0](/fnaumenko/wigReg/pulls)
* [Actions](/fnaumenko/wigReg/actions)
* [Projects](/fnaumenko/wigReg/projects)
* [Security
  0](/fnaumenko/wigReg/security)
* [Insights](/fnaumenko/wigReg/pulse)

Additional navigation options

* [Code](/fnaumenko/wigReg)
* [Issues](/fnaumenko/wigReg/issues)
* [Pull requests](/fnaumenko/wigReg/pulls)
* [Actions](/fnaumenko/wigReg/actions)
* [Projects](/fnaumenko/wigReg/projects)
* [Security](/fnaumenko/wigReg/security)
* [Insights](/fnaumenko/wigReg/pulse)
...
1. [wigReg](/fnaumenko/wigReg/tree/master)
...
[History](/fnaumenko/wigReg/commits/master/README.md)

History

176 lines (145 loc) · 9.2 KB
...
[Raw](https://github.com/fnaumenko/wigReg/raw/refs/heads/master/README.md)
...
**Wig**gle **Reg**ulator is a fast, memory-undemanding software to regulate wig-files after [MACS](http://liulab.dfci.harvard.edu/MACS/00README.html) and [PeakRanger](http://ranger.sourceforge.net/manual1.18.html).
...
`wget -O wigReg.gz https://github.com/fnaumenko/wigReg/releases/download/1.0/wigReg-Linux-x64.gz`
`gzip -d wigReg.gz`
`chmod +x wigReg`
...
Go to the desired directory and type commands:
`wget -O wigReg.zip https://github.com/fnaumenko/wigReg/archive/1.0.zip`
`unzip wigReg.zip`
`cd wigReg-1.0`
`make`

If **zlib** is not installed on your system, a linker message will be displayed.
In that case you can compile the program without the ability to work with .gz files:
open *makefile* in any text editor, uncomment last macro in the second line, comment third line, save *makefile*, and try `make` again.
...
If wiggle is generated by another program, and option `-p` has not `AUTO` value, the result is unpredictable.
...
`stdout`
Redirect output to standard output. It allows to build **wigReg** into pipe-line, for instance
...
`-p <PR|MACS|AUTO>`
Program what generates input wiggle. Lower case value is appropriate as well.
...
In such case you should define program-source manually.
By the way, wiggle file recovered from *bigWig* even lost required track type, which makes this wiggle invalid (it is true at least for wigToBigWig version from 06-Aug-2013).
To rehabilitate it you need to put into track type element *‘track type=wiggle\_0’*, f.e. by command
 `echo track type=wiggle_0 | cat – inFile.wig > outFile.wig`
...
Because in general it may needed to scan uncertain number of data lines, what breaks data flow processing and reduces efficiency.
Default: `AUTO`

`-f|--frag-len <int>`
Length of fragment (reads extension) in bp, needs to extend singleton features in wiggles from PeakRanger.
This option ignored for wiggles from MACS.
Default: 200

`-s|--space <int>`
The resolution for saving wiggle from PeakRanger.
This value defines the minimal span in bps from which raw tags will be saved.
...
For instance, run time and memory requirement for the comparison wiggles process are in inverse relation with their resolution.
Moreover resolution of 1 is surplus for most practical purposes.
This option ignored for wiggles from MACS since their resolution defined by the program itself.
Default: 10
...
it is made from [wiggle](https://genome.ucsc.edu/goldenpath/help/wiggle.html).
...
and [PeakRanger](http://ranger.sourceforge.net/manual1.18.html) (not so popular, but one of the best one).
...
This also allows the generation of wiggles for each strand separately.
...
Firstly, they look strange in genome browser by zooming, as it shows in [![figure](https://github.com/fnaumenko/wigReg/tree/master/pict/MACS-PeakRanger-comparison.png)](https://github.com/fnaumenko/wigReg/tree/master/pict/MACS-PeakRanger-comparison.png).
Secondly, and more substantially, they cannot be used in treatment, like comparison etc.
Moreover PeakRanger does not allow to manage resolution.
