...
* **-p , --pheight**  occupancy (reads density) P value cutoff for defining
  ...
  + individual protein binding position. Setting to a value 0 will disable this parameter and use the parameter -q as an instead (default: 0)
  + **-q , --height** occupancy (reads density) cutoff for defining individual protein binding position. Setting to a value 0 will disable this parameter and use the parameter -p as an instead (default: 5)
* **-t , --testcut**  P value cutoff for defining position based on **difference in protein binding occupancy** between each pair of datasets. Set as 0 will disable this parameter, then positions will be defined solely based on **occupancy** cutoff (-p or -q), but a P value of difference will still be calculated at each position for each pair of data sets. (default: 0)
...
* **-s , --save**  set to 0 if need not to save middle-stage files, otherwise set to 1 (default: 0)
  ...
  + **-b , --bg** The paths used to specify background data for experimental data. Background data is always called 'input', e.g., IgG ChIP-Seq data. Experimental data may comes from either the treatment or control experiment, e.g., the knockout sample or wild type sample. The parameter could be:
...
* **-jw , --width** the window size used to scan for the summit of each potential position. (default: 40)
* **-jd , --distance** minimal center-to-center distance between positions. positions closer than -jd will be merged as one single position (default: 100)
* **-jf , --position\_reference** A file contains a set of reference positions. Danpos will map each newly defined position to a reference position in the file. (default: None)
* **-R , --ratio** the valley to summit ratio of reads density at each position. Only positions with a ratio lower than this value will be used to detect position shift events. (default: 0.9)
* **-e , --edge**  set to 1 if need to detect edges for each position, else set to 0 (default: 0)
* **-g , --gapfill** if the gap size between two neighboring positions is close to the a position size, fill the gap with an additional position. set to 0 if need not to fill, otherwise set to 1 (default: 0)
