...
This value sets the *p*-value threshold for displaying search
results. If the *p*-value of a match is greater than this value,
...
When using RNA sequences you should select this option to ensure
that only the given strand is scanned for motifs.

[ close ]

When this option is selected, if the FASTA sequence ID of an input
sequence contains genomic coordinates in UCSC or Galaxy format, then its motif sites
will be output in genomic coordinates. If the sequence ID does
not contain valid coordinates, then its sites will be output with
the start of the sequence assumed to be position 1.
...
If you select a supported sequence database in
...
protein database (NRDB frequencies).

You can override the default background here by choosing the
uniform background model, the background model specified by the motifs,
or you may choose "Uploa