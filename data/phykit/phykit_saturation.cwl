...
baseCommand:
  - phykit
  - saturation
label: phykit_saturation
doc: Calculate saturation for a given tree and alignment. Saturation is defined 
  as sequences in multiple sequence alignments that have undergone numerous 
  substitutions such that the distances between taxa are underestimated.
inputs:
  - id: alignment
    type: File
  ...
  - id: exclude_gaps
    type:
      - 'null'
      - boolean
    doc: if a site has a gap, ignore it
    inputBinding:
      position: 101
      prefix: --exclude_gaps
  - id: plot
    type:
      - 'null'
      - boolean
    doc: optional argument to save a saturation scatter plot
    inputBinding:
      position: 101
      prefix: --plot
  - id: plot_output
    type: string
    doc: output path for saturation plot
    inputBinding:
      position: 101
      prefix: --plot-output
  - id: fig_width
