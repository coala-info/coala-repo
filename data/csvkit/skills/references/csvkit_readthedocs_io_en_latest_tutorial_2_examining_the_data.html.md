...
[View this page](../_sources/tutorial/2_examining_the_data.rst.txt "View this page")

# 2. Examining the data[¶](#examining-the-data "Link to this heading")

## 2.1. csvstat: statistics without code[¶](#csvstat-statistics-without-code "Link to this heading")

In the previous section we saw how we could use [csvlook](../scripts/csvlook.html) and [csvcut](../scripts/csvcut.html) to view slices of our data. This is a good tool for exploring a dataset, but in practice we usually need to get the broadest possible view before we can start diving into specifics.

[csvstat](../scripts/csvstat.html) is designed to give us just such a broad understanding of our data. Inspired by the `summary()` function from the computational statistics programming language [“R”](https://www.r-project.org/), [csvstat](../scripts/csvstat.html) will generate summary statistics for all the data in a CSV file.

Let’s examine summary statistics for a few columns from our dataset. As we learned in the last section, we can use [csvcut](../scripts/csvcut.html) and a pipe to pick out the columns we want:
...
[csvstat](../scripts/csvstat.html) infers the type of data in each column and then performs basic statistics on it. The particular statistics computed depend on the type of the column (numbers, text, dates, etc).
...
## 2.2. csvgrep: find the data you need[¶](#csvgrep-find-the-data-you-need "Link to this heading")

After reviewing the summary statistics you might wonder what equipment was received by a particular county. To get a simple answer to the question we can use [csvgrep](../scripts/csvgrep.html) to search for the state’s name amongst the rows. Let’s also use [csvcut](../scripts/csvcut.html) to just look at the columns we care about and [csvlook](../scripts/csvlook.html) to format the output:
...
## 2.3. csvsort: order matters[¶](#csvsort-order-matters "Link to this heading")

Now let’s use [csvsort](../scripts/csvsort.html) to sort the rows by the `total_cost` column, in reverse (descending) order:
...
## 2.4. Summing up[¶](#summing-up "Link to this heading")

At this point you should be able to use csvkit to investigate the basic properties of a dataset. If you understand this section, you should be ready to move onto [Power tools](3_power_tools.html).
...
1. Getting started](1_getting_started.html)
...
* 2. Examining the data
  + [2.1. csvstat: statistics without code](#csvstat-statistics-without-code)
  + [2.2. csvgrep: find the data you need](#csvgrep-find-the-data-you-need)
  + [2.3. csvsort: order matters](#csvsort-order-matters)
  + [2.4. Summing up](#summing-up)