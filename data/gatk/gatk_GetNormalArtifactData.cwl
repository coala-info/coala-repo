...
label: gatk_GetNormalArtifactData
doc: Collects data for training normal artifact filter
inputs:
  - id: input
    type:
      type: array
  ...
  - id: error_prob
    type:
      - 'null'
      - float
    doc: Error probability for p-values
    inputBinding:
      position: 101
      prefix: --error-prob
  ...
  - id: max_depth_per_sample
    type:
      - 'null'
      - int
    doc: Maximum number of reads to retain per sample per locus. Reads above 
