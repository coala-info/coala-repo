...
raex10stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

raex10stprobesetACCNUM is an R object that contains mappings between a manufacturer’s identi-
...
raex10stprobesetALIAS is an R object that provides mappings between common gene symbol iden-
...
xx <- as.list(raex10stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the raex10stprobeset.db annotation Package. The purpose of this package is to provide
detailed information about the raex10stprobeset platform. This package is updated biannually.
...
## use of select().
ls("package:raex10stprobeset.db")
...
raex10stprobesetCHR is an R object that provides mappings between a manufacturer identifier and
...
raex10stprobesetCHRLOC

raex10stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

raex10stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
raex10stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
raex10stprobesetENSEMBL is an R object that contains mappings between manufacturer identifiers
...
x <- raex10stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
raex10stprobesetENTREZID is an R object that provides mappings between manufacturer identi-
fiers and Entrez Gene identifiers.

raex10stprobesetENZYME
...
raex10stprobesetENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. raex10stprobesetENZYME2PROBE is an R object that maps Enzyme Commis-
...
When the raex10stprobesetENZYME maping viewed as a list, each manufacturer identifier maps to
...
will contain an NA.
For the raex10stprobesetENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In raex10stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert raex10stprobesetENZYME2PROBE to a list to see inside
x <- raex10stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
raex10stprobesetGENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
raex10stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(raex10stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

raex10stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
raex10stprobesetGO2PROBE.

Details

If raex10stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If raex10stprobesetGO2ALLPROBES or raex10stprobesetGO2PROBE is cast as a list, each GO
...
x <- raex10stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
raex10stprobesetMAPCOUNTS

Number of mapped keys for the maps in package raex10stprobeset.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

raex10stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package raex10stprobeset.db.
...
The Organism information for raex10stprobeset

Description

raex10stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which raex10stprobeset was built. raex10stprobesetORGPKG is an R object
...
raex10stprobesetPATH

Details

13

Although the package name is suggestive of the organism for which it was built, raex10stprobesetORGANISM
provides a simple way to programmatically extract the organism name. raex10stprobesetORGPKG
...
## Bimap interface:
raex10stprobesetORGANISM
raex10stprobesetORGPKG

raex10stprobesetPATH Mappings between probe identifiers and KEGG pathway identifiers
...
raex10stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

raex10stprobesetPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
x <- raex10stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the raex10stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
raex10stprobesetPFAM Maps between Manufacturer Identifiers and PFAM Identifiers

Description

raex10stprobesetPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
raex10stprobesetPMID

15

raex10stprobesetPMID Maps between Manufacturer Identifiers and PubMed Identifiers

Description

raex10stprobesetPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. raex10stprobesetPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When raex10stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When raex10stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- raex10stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object raex10stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
raex10stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

raex10stprobesetPROSITE is an R object that provides mappings between manufacturer identifiers
...
raex10stprobesetREFSEQ is an R object that provides mappings between manufacturer identifiers
...
raex10stprobesetSYMBOL is an R object that provides mappings between manufacturer identifiers
...
raex10stprobesetUNIPROT is an R object that contains mappings between the manufacturer iden-
...
raex10stprobeset_dbschema(file="", show.indices=FALSE)
...
raex10stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by raex10stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
raex10stprobeset_dbfile returns the path (character string) to the package annotation DB (this
is an SQLite file).
raex10stprobeset_dbschema prints the schema definition of the package annotation DB.
raex10stprobeset_dbInfo prints other information about the package annotation DB.

Value

raex10stprobeset_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.
raex10stprobeset_dbfile: a character string with the path to the package annotation DB.
raex10stprobeset_dbschema: none (invisible NULL).
raex10stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(raex10stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")
...
raex10stprobeset.db, 2
raex10stprobeset_dbconn, 19
raex10stprobesetACCNUM, 1
raex10stprobesetALIAS2PROBE, 2
raex10stprobesetCHR, 3
raex10stprobesetCHRLENGTHS, 4
raex10stprobesetCHRLOC, 4
raex10stprobesetENSEMBL, 5
raex10stprobesetENTREZID, 6
raex10stprobesetENZYME, 7
raex10stprobesetGENENAME, 9
raex10stprobesetGO, 10
raex10stprobesetMAPCOUNTS, 12
raex10stprobesetORGANISM, 12
raex10stprobesetPATH, 13
raex10stprobesetPFAM, 14
raex10stprobesetPMID, 15
raex10stprobesetPROSITE, 16
raex10stprobesetREFSEQ, 16
raex10stprobesetSYMBOL, 18
raex10stprobesetUNIPROT, 19
...
raex10stprobeset (raex10stprobeset.db),
...
raex10stprobesetCHRLENGTHS, 4
raex10stprobesetCHRLOC, 4
raex10stprobesetCHRLOCEND

(raex10stprobesetCHRLOC), 4

raex10stprobesetENSEMBL, 5
raex10stprobesetENSEMBL2PROBE

(raex10stprobesetENSEMBL), 5

raex10stprobesetENTREZID, 6
raex10stprobesetENZYME, 7
raex10stprobesetENZYME2PROBE

(raex10stprobesetENZYME), 7

raex10stprobesetGENENAME, 9
raex10stprobesetGO, 10
raex10stprobesetGO2ALLPROBES, 11
raex10stprobesetGO2ALLPROBES
...
raex10stprobesetPATH, 13
raex10stprobesetPATH2PROBE

(raex10stprobesetPATH), 13

raex10stprobesetPFAM, 14
raex10stprobesetPMID, 15
raex10stprobesetPMID2PROBE
