...
mouse4302ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mouse4302ALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mouse4302ALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mouse4302ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mouse4302.db annotation Package. The purpose of this package is to provide
detailed information about the mouse4302 platform. This package is updated biannually.
...
mouse4302CHR is an R object that provides mappings between a manufacturer identifier and the
...
mouse4302CHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mouse4302CHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mouse4302ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mouse4302ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mouse4302ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mouse4302ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mouse4302ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mouse4302ENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mouse4302ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mouse4302ENZYME2PROBE to a list to see inside

mouse4302GENENAME

9

x <- mouse4302ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mouse4302GENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mouse4302GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mouse4302GO2PROBE)
...
mouse4302GO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mouse4302GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mouse4302GO2ALLPROBES or mouse4302GO2PROBE is cast as a list, each GO term maps to
...
x <- mouse4302GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mouse4302GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package mouse4302.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mouse4302MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mouse4302.db.
...
mouse4302MGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mouse4302

Description

mouse4302ORGANISM is an R object that contains a single item: a character string that names
the organism for which mouse4302 was built. mouse4302ORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mouse4302ORGANISM
provides a simple way to programmatically extract the organism name. mouse4302ORGPKG pro-
...
mouse4302PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mouse4302PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- mouse4302PATH
mapped_probes <- mappedkeys(x)
# Now convert the mouse4302PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mouse4302PFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mouse4302PMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mouse4302PMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mouse4302PMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mouse4302PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- mouse4302PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mouse4302PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mouse4302PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mouse4302REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mouse4302SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mouse4302UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mouse4302_dbconn()
mouse4302_dbfile()
mouse4302_dbschema(file="", show.indices=FALSE)
mouse4302_dbInfo()
...
mouse4302_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mouse4302_dbconn or you will break all
the AnnDbObj objects defined in this package!

mouse4302_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

mouse4302_dbschema prints the schema definition of the package annotation DB.

mouse4302_dbInfo prints other information about the package annotation DB.

Value

mouse4302_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

mouse4302_dbfile: a character string with the path to the package annotation DB.

mouse4302_dbschema: none (invisible NULL).

mouse4302_dbInfo: none (invisible NULL).
...
dbGetQuery(mouse4302_dbconn(), "SELECT COUNT(*) FROM probes")
...
mouse4302_dbconn, 20
mouse4302ACCNUM, 1
mouse4302ALIAS2PROBE, 2
mouse4302CHR, 3
mouse4302CHRLENGTHS, 4
mouse4302CHRLOC, 4
mouse4302ENSEMBL, 5
mouse4302ENTREZID, 6
mouse4302ENZYME, 7
mouse4302GENENAME, 9
mouse4302GO, 10
mouse4302MAPCOUNTS, 12
mouse4302MGI, 12
mouse4302ORGANISM, 13
mouse4302PATH, 14
mouse4302PFAM, 15
mouse4302PMID, 15
mouse4302PROSITE, 17
...
mouse4302 (mouse4302.db), 2
mouse4302.db, 2
mouse4302_dbconn, 20
mouse4302_dbfile (mouse4302_dbconn), 20
mouse4302_dbInfo (mouse4302_dbconn), 20

22

mouse4302_dbschema (mouse4302_dbconn),
...
mouse4302CHRLENGTHS, 4
mouse4302CHRLOC, 4
mouse4302CHRLOCEND (mouse4302CHRLOC), 4
mouse4302ENSEMBL, 5
mouse4302ENSEMBL2PROBE
...
mouse4302GENENAME, 9
mouse4302GO, 10
mouse4302GO2ALLPROBES, 11
mouse4302GO2ALLPROBES (mouse4302GO), 10
mouse4302GO2PROBE (mouse4302GO), 10
mouse4302LOCUSID (mouse4302ENTREZID), 6
mouse4302MAPCOUNTS, 12
mouse4302MGI, 12
mouse4302MGI2PROBE (mouse4302MGI), 12
mouse4302ORGANISM, 13
mouse4302ORGPKG (mouse4302ORGANISM), 13
mouse4302PATH, 14
mouse4302PATH2PROBE (mouse4302PATH), 14
mouse4302PFAM, 15
mouse4302PMID, 15
mouse4302PMID2PROBE (mouse4302PMID), 15
mouse4302PROSITE, 17
mouse4302REFSEQ, 17
mouse4302SYMBOL, 18
mouse4302UNIPROT, 19
mouse4302UNIPROT2PROBE
