...
baseCommand:
  - phykit
  - quartet_network
label: phykit_quartet_network
doc: Quartet-based network inference (NANUQ-style). Computes quartet concordance
  factors from gene trees, classifies each quartet as tree-like, hybrid, or 
  unresolved using two chi-squared tests, and optionally visualizes the result.
inputs:
  - id: trees
    type: File
  ...
  - id: alpha
    type:
      - 'null'
      - float
    doc: significance level for the T3 tree model test
    inputBinding:
      position: 101
      prefix: --alpha
  - id: beta
    type:
      - 'null'
      - float
    doc: threshold for the star tree test; quartets with p_star > beta are 
      called unresolved
    inputBinding:
      position: 101
      prefix: --beta
  ...
  - id: plot_output
    type: string
    doc: output filename for the quartet network plot
    inputBinding:
      position: 101
      prefix: --plot-output
  - id: fig_width
