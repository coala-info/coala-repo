...
mgu74cv2ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mgu74cv2ALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mgu74cv2ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mgu74cv2.db annotation Package. The purpose of this package is to provide de-
tailed information about the mgu74cv2 platform. This package is updated biannually.
...
mgu74cv2CHR is an R object that provides mappings between a manufacturer identifier and the
...
mgu74cv2CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
mgu74cv2CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
mgu74cv2ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mgu74cv2ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgu74cv2ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mgu74cv2ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mgu74cv2ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mgu74cv2ENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In mgu74cv2ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgu74cv2ENZYME2PROBE to a list to see inside

mgu74cv2GENENAME

9

x <- mgu74cv2ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mgu74cv2GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
mgu74cv2GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgu74cv2GO2PROBE)
...
mgu74cv2GO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mgu74cv2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mgu74cv2GO2ALLPROBES or mgu74cv2GO2PROBE is cast as a list, each GO term maps to a
...
x <- mgu74cv2GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mgu74cv2GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package mgu74cv2.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgu74cv2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mgu74cv2.db.
...
mgu74cv2MGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for mgu74cv2

Description

mgu74cv2ORGANISM is an R object that contains a single item: a character string that names
the organism for which mgu74cv2 was built. mgu74cv2ORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgu74cv2ORGANISM
provides a simple way to programmatically extract the organism name. mgu74cv2ORGPKG pro-
...
mgu74cv2PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mgu74cv2PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- mgu74cv2PATH
mapped_probes <- mappedkeys(x)
# Now convert the mgu74cv2PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74cv2PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
mgu74cv2PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. mgu74cv2PMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When mgu74cv2PMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When mgu74cv2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- mgu74cv2PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mgu74cv2PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74cv2PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgu74cv2REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mgu74cv2SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mgu74cv2UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mgu74cv2_dbconn()
mgu74cv2_dbfile()
mgu74cv2_dbschema(file="", show.indices=FALSE)
mgu74cv2_dbInfo()
...
mgu74cv2_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgu74cv2_dbconn or you will break all
the AnnDbObj objects defined in this package!

mgu74cv2_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

mgu74cv2_dbschema prints the schema definition of the package annotation DB.

mgu74cv2_dbInfo prints other information about the package annotation DB.

Value

mgu74cv2_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

mgu74cv2_dbfile: a character string with the path to the package annotation DB.

mgu74cv2_dbschema: none (invisible NULL).

mgu74cv2_dbInfo: none (invisible NULL).
...
dbGetQuery(mgu74cv2_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgu74cv2_dbconn, 20
mgu74cv2ACCNUM, 1
mgu74cv2ALIAS2PROBE, 2
mgu74cv2CHR, 3
mgu74cv2CHRLENGTHS, 4
mgu74cv2CHRLOC, 4
mgu74cv2ENSEMBL, 5
mgu74cv2ENTREZID, 6
mgu74cv2ENZYME, 7
...
mgu74cv2 (mgu74cv2.db), 2
mgu74cv2.db, 2
mgu74cv2_dbconn, 20
mgu74cv2_dbfile (mgu74cv2_dbconn), 20
mgu74cv2_dbInfo (mgu74cv2_dbconn), 20

22

mgu74cv2_dbschema (mgu74cv2_dbconn), 20
mgu74cv2ACCNUM, 1
mgu74cv2ALIAS2PROBE, 2
mgu74cv2CHR, 3
mgu74cv2CHRLENGTHS, 4
mgu74cv2CHRLOC, 4
mgu74cv2CHRLOCEND (mgu74cv2CHRLOC), 4
mgu74cv2ENSEMBL, 5
mgu74cv2ENSEMBL2PROBE

(mgu74cv2ENSEMBL), 5

mgu74cv2ENTREZID, 6
mgu74cv2ENZYME, 7
mgu74cv2ENZYME2PROBE (mgu74cv2ENZYME), 7
mgu74cv2GENENAME, 9
mgu74cv2GO, 10
mgu74cv2GO2ALLPROBES, 11
mgu74cv2GO2ALLPROBES (mgu74cv2GO), 10
mgu74cv2GO2PROBE (mgu74cv2GO), 10
mgu74cv2LOCUSID (mgu74cv2ENTREZID), 6
mgu74cv2MAPCOUNTS, 12
mgu74cv2MGI, 12
mgu74cv2MGI2PROBE (mgu74cv2MGI), 12
mgu74cv2ORGANISM, 13
mgu74cv2ORGPKG (mgu74cv2ORGANISM), 13
mgu74cv2PATH, 14
mgu74cv2PATH2PROBE (mgu74cv2PATH), 14
mgu74cv2PFAM, 15
mgu74cv2PMID, 15
mgu74cv2PMID2PROBE (mgu74cv2PMID), 15
mgu74cv2PROSITE, 17
mgu74cv2REFSEQ, 17
mgu74cv2SYMBOL, 18
mgu74cv2UNIPROT, 19
mgu74cv2UNIPROT2PROBE
