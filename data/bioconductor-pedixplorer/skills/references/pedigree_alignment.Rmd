...
At this point the most common situation will be what is shown in the next
...
```{r, align3, echo = FALSE, fig.dim = c(8, 6), fig.alt = "Horizontal ordering impact", fig.align = 'center', out.width="600px", out.height="300px"}
#
# A simple pedigree to illustrate autohint's code
#
...
plot(ped4, title = "Horder plot 1")
plot(ped3, title = "Horder plot 2")
```

Multiple marriages can lead to unanchored subjects.
In the first plot above we have two
...
The first plot shows a case where
...
In the second plot, subject 2 has an anchor variable of `(0,1)`.
