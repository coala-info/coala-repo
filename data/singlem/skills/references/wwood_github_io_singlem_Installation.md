...
```
conda create -c conda-forge -c bioconda --override-channels --name singlem singlem'>='0.20.3
```
...
After this, you'll also need to procure the reference data (the "metapackage"). See [singlem data](/singlem/tools/data) or [lyrebird data](/singlem/tools/lyrebird_data).
...
```
docker run wwood/singlem:0.20.3 -h
```

If the sequence data to be analyzed is in the current working directory, SingleM `pipe` can be used like so:

```
docker run -v `pwd`:`pwd` wwood/singlem:0.20.3 pipe --sequences \
    `pwd`/my.fastq.gz -p `pwd`/my.profile.csv --threads 4
```

Two things to note:

1. The default SingleM reference data is included in the docker image, so running [singlem data](/singlem/tools/data) is not necessary for this installation method - you can jump straight to using `pipe`.
2. You should not specify `singlem` in the command line, as this is automatic. Simply use e.g. `docker run wwood/singlem:0.20.3 pipe -h`.
...
```
docker pull wwood/lyrebird:0.20.3
docker run wwood/lyrebird:0.20.3 -h
```
...
```
singularity run singlem_0.20.3.sif -h
```

If the sequence data to be analyzed is in the current working directory, SingleM `pipe` can be used like so:

```
singularity run -B `pwd`:`pwd` singlem_0.20.3.sif pipe --sequences \
    `pwd`/my.fastq.gz -p `pwd`/my.profile.csv --threads 4
```
...
1. The default SingleM reference data is included in the docker image, so running [singlem data](/singlem/tools/data) is not necessary for this installation method - you can jump straight to using `pipe`.
2. You should not specify `singlem` in the command line, as this is automatic. Simply use e.g. `singularity run singlem_0.20.3.sif pipe -h`.

A similar procedure is true for Lyrebird, except that the docker image is [different](https://hub.docker.com/r/wwood/lyrebird), so you need to run:

```
singularity pull docker://wwood/lyrebird:0.20.3
...
After this, you'll also need to procure the reference data (the "metapackage"). See [singlem data](/singlem/tools/data).

This procedure also installs lyrebird, but the reference metapackage is different. See [lyrebird data](/singlem/tools/lyrebird_data) for more information.

# Example data

To test the main subcommand of SingleM, [pipe](/singlem/tools/pipe) works, download a minimal dataset and generate a taxonomic profile like so:
...
* [Installing SingleM](#installing-singlem)
* [Installation via DockerHub](#installation-via-dockerhub)
* [Installation via Singularity / Apptainer](#installation-via-singularity-/-apptainer)
* [Installation via PyPI](#installation-via-pypi)
* [Installation via Github, with pixi dependency management](#installation-via-github,-with-pixi-dependency-management)
* [Example data](#example-data)
* [Containerised SingleM installation examples](#containerised-singlem-installation-examples)
