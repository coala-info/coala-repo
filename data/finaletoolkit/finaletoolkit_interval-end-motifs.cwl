...
baseCommand: finaletoolkit-interval-end-motifs
label: finaletoolkit_interval-end-motifs
doc: "Measures frequency of k-mer 5' end motifs in each region specified in a BED
...
inputs:
  ...
  - id: intervals
    type: File
    doc: Path to a BED file containing intervals to retrieve end motif 
      frequencies over.
    inputBinding:
      position: 3
  - id: fraction_high
  ...
  - id: single_strand
    type:
      - 'null'
      - boolean
    doc: Set flag to only consider one strand for end-motifs. By default, the 
      positive strand is calculated, but with the -n flag, the 5' end motifs of 
      the negative strand are considered instead.
    inputBinding:
      position: 104
...
outputs:
  - id: output_file
    type:
      - 'null'
      - File
    doc: Path to TSV or CSV file to write end motif frequencies to.
    outputBinding:
