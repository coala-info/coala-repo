...
doc: |-
  ...
  multi-sample file. Note that only records from different files can be merged, 
  never from the same file.
inputs:
- id: input_files
  type:
    type: array
  ...
  - pattern: .csi
    required: false
  doc: Input VCF/BCF files to merge
  inputBinding:
    position: 1
- id: apply_filters
  type:
...
- id: gvcf
  ...
  - string
  doc: |-
    Merge gVCF blocks, INFO/END tag is expected. Implies -i 
    QS:sum,MinDP:min,MIN_DP:min,I16:sum,IDV:max,IMF:max -M PL:max,AD:0
  inputBinding:
    position: 102
    prefix: --gvcf
- id: info_rules
  ...
  - string
  doc: |-
    Rules for merging INFO fields (method is one of sum,avg,min,max,join) 
...
- id: local_alleles
  ...
  - int
  doc: |-
    If more than INT alt alleles are encountered, drop FMT/PL and output 
...
- id: merge
  ...
  - string
  doc: |-
    Allow multiallelic records for 
    snps,indels,both,snp-ins-del,all,none,id,*,**; see man page for details
  inputBinding:
    position: 102
    prefix: --merge
- id: missing_rules
  ...
  - string
  doc: |-
    Rules for replacing missing values in numeric vectors (.,0,max) when 
