...
label: picard_CheckDuplicateMarking
...
inputs:
  ...
  - id: input
    type: File
    doc: Input BAM or SAM file to check.
    inputBinding:
      position: 101
      prefix: --INPUT
  ...
  - id: mode
    type:
      - 'null'
      - string
    doc: 'Which reads of the same name should be checked to have same duplicate marking.
      Possible values: {ALL, PRIMARY_ONLY, PRIMARY_MAPPED_ONLY, PRIMARY_PROPER_PAIR_ONLY}'
    default: ALL
    inputBinding:
      position: 101
      prefix: --MODE
...
outputs:
  - id: output
    type:
      - 'null'
      - File
    doc: Output file into which bad querynames will be placed (if not null).
    outputBinding:
      glob: $(inputs.output)
hints:
