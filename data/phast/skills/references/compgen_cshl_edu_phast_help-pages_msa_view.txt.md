...
msa\_view myfile.fa --out-format PHYLIP > myfile.ph
msa\_view myfile2.raw --in-format MPM > myfile2.fa
2. Obtain a sub-alignment by position, using the coordinate frame
of the first sequence in the alignment.
msa\_view myfile.fa --start 1234 --end 5678 --refidx 1 > mysub.fa
3. Obtain a sub-alignment by sequence.
msa\_view myfile.fa --seqs 1,4,5 > seqs145.fa
msa\_view myfile.fa --seqs 1,4,5 --exclude > seqs236.fa
(can also specify sequences by name, e.g., --seqs cow,rat,pig)
4. Concatenate alignments.
msa\_view --aggregate human,mouse,rat myf1.fa myf2.fa myf3.fa
> concat.fa
...
msa\_view chr1.maf --refseq chr1.fa
...
msa\_view chr1.maf --features chr22.gff
--catmap "NCATS = 3; CDS 1-3" --out-format SS
> chr22.pos.ss
11. As in (10), but re-orient genes on - strand so that stats
reflect + strand. Assume genes are defined by tag "transcript\_id".
msa\_view chr1.maf --features chr22.gff
--catmap "NCATS = 3; CDS 1-3" --reverse-groups transcript\_id
...
File naming convention is filename\_root.species.fa. If used with
--gap-strip, gap characters will be stripped from all output files.
In this case, '--gap-strip ~~' should NOT be used (ALL or ANY
...
--gap-strip, -G ALL|ANY|~~Strip columns containing all gaps, any gaps, or a gap in the
specified sequence (~~). Indexing starts at one and refers
...
--mark-missing, -K
Convert all gaps of length greater than  to "\*"
...
specified in name\_list. If a name appears in name\_list but
...
--fill-Ns, -N
Fill sequence no.  ~~with Ns, from  **to . Applied before
...
Read the complete text of the reference sequence from
 (FASTA format) and combine it with the contents of
...
cycle\_size == 3). Useful for in-frame coding sequence, or to
...
--reverse-groups, -W
(For use with --features) Group features by  (e.g.,
...
--unordered-stats --tuple-size 3 --reverse-groups transcript\_id.
(Alignment cleaning)
--clean-coding, -L
...
with fewer than  bases are removed; bases adjacent to
indels, and short gapless subsequences, are replaced with Ns.
If used with --tuple-size, then -1 columns of Ns
