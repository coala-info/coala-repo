...
mgug4120aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mgug4120aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mgug4120aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mgug4120aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mgug4120a.db annotation Package. The purpose of this package is to provide
detailed information about the mgug4120a platform. This package is updated biannually.
...
mgug4120aCHR is an R object that provides mappings between a manufacturer identifier and the
...
mgug4120aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mgug4120aCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mgug4120aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(mgug4120aENSEMBL2PROBE)
if(length(xx) > 0){
...
mgug4120aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgug4120aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mgug4120aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mgug4120aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mgug4120aENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mgug4120aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgug4120aENZYME2PROBE to a list to see inside
xx <- as.list(mgug4120aENZYME2PROBE)
if(length(xx) > 0){
...
mgug4120aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mgug4120aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgug4120aGO2PROBE)
...
mgug4120aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mgug4120aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mgug4120aGO2ALLPROBES or mgug4120aGO2PROBE is cast as a list, each GO term maps to
...
# Convert mgug4120aGO2ALLPROBES to a list
xx <- as.list(mgug4120aGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mgug4120a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgug4120aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mgug4120a.db.
...
mgug4120aMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mgug4120a

Description

mgug4120aORGANISM is an R object that contains a single item: a character string that names
the organism for which mgug4120a was built. mgug4120aORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgug4120aORGANISM
provides a simple way to programmatically extract the organism name. mgug4120aORGPKG pro-
...
mgug4120aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

mgug4120aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the mgug4120aPATH2PROBE object to a list
xx <- as.list(mgug4120aPATH2PROBE)
if(length(xx) > 0){
...
mgug4120aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mgug4120aPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mgug4120aPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mgug4120aPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mgug4120aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object mgug4120aPMID2PROBE to a list
...
mgug4120aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgug4120aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mgug4120aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mgug4120aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
mgug4120aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mgug4120a_dbconn()
mgug4120a_dbfile()
mgug4120a_dbschema(file="", show.indices=FALSE)
mgug4120a_dbInfo()
...
mgug4120a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgug4120a_dbconn or you will break all
the AnnDbObj objects defined in this package!
mgug4120a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
mgug4120a_dbschema prints the schema definition of the package annotation DB.
mgug4120a_dbInfo prints other information about the package annotation DB.

Value

mgug4120a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
mgug4120a_dbfile: a character string with the path to the package annotation DB.
mgug4120a_dbschema: none (invisible NULL).
mgug4120a_dbInfo: none (invisible NULL).
...
dbGetQuery(mgug4120a_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgug4120a_dbconn, 20
mgug4120aACCNUM, 1
mgug4120aALIAS2PROBE, 2
mgug4120aCHR, 3
mgug4120aCHRLENGTHS, 4
mgug4120aCHRLOC, 4
mgug4120aENSEMBL, 5
mgug4120aENTREZID, 6
mgug4120aENZYME, 7
mgug4120aGENENAME, 9
mgug4120aGO, 10
mgug4120aMAPCOUNTS, 12
mgug4120aMGI, 12
mgug4120aORGANISM, 13
mgug4120aPATH, 14
mgug4120aPFAM, 15
mgug4120aPMID, 15
mgug4120aPROSITE, 16
...
mgug4120a (mgug4120a.db), 2
mgug4120a.db, 2
mgug4120a_dbconn, 20
mgug4120a_dbfile (mgug4120a_dbconn), 20

22

mgug4120a_dbInfo (mgug4120a_dbconn), 20
mgug4120a_dbschema (mgug4120a_dbconn),
...
mgug4120aCHRLENGTHS, 4
mgug4120aCHRLOC, 4
mgug4120aCHRLOCEND (mgug4120aCHRLOC), 4
mgug4120aENSEMBL, 5
mgug4120aENSEMBL2PROBE

(mgug4120aENSEMBL), 5

mgug4120aENTREZID, 6
mgug4120aENZYME, 7
mgug4120aENZYME2PROBE

(mgug4120aENZYME), 7

mgug4120aGENENAME, 9
mgug4120aGO, 10
mgug4120aGO2ALLPROBES, 11
mgug4120aGO2ALLPROBES (mgug4120aGO), 10
mgug4120aGO2PROBE (mgug4120aGO), 10
mgug4120aLOCUSID (mgug4120aENTREZID), 6
mgug4120aMAPCOUNTS, 12
mgug4120aMGI, 12
mgug4120aMGI2PROBE (mgug4120aMGI), 12
mgug4120aORGANISM, 13
mgug4120aORGPKG (mgug4120aORGANISM), 13
mgug4120aPATH, 14
mgug4120aPATH2PROBE (mgug4120aPATH), 14
mgug4120aPFAM, 15
mgug4120aPMID, 15
mgug4120aPMID2PROBE (mgug4120aPMID), 15
mgug4120aPROSITE, 16
mgug4120aREFSEQ, 17
mgug4120aSYMBOL, 18
mgug4120aUNIGENE, 19
mgug4120aUNIPROT, 20
mgug4120aUNIPROT2PROBE
