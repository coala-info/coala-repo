...
mgug4121aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mgug4121aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mgug4121aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mgug4121aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mgug4121a.db annotation Package. The purpose of this package is to provide
detailed information about the mgug4121a platform. This package is updated biannually.
...
mgug4121aCHR is an R object that provides mappings between a manufacturer identifier and the
...
mgug4121aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mgug4121aCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mgug4121aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(mgug4121aENSEMBL2PROBE)
if(length(xx) > 0){
...
mgug4121aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgug4121aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mgug4121aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mgug4121aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mgug4121aENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mgug4121aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgug4121aENZYME2PROBE to a list to see inside
xx <- as.list(mgug4121aENZYME2PROBE)
if(length(xx) > 0){
...
mgug4121aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mgug4121aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgug4121aGO2PROBE)
...
mgug4121aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mgug4121aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mgug4121aGO2ALLPROBES or mgug4121aGO2PROBE is cast as a list, each GO term maps to
...
# Convert mgug4121aGO2ALLPROBES to a list
xx <- as.list(mgug4121aGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mgug4121a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgug4121aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mgug4121a.db.
...
mgug4121aMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mgug4121a

Description

mgug4121aORGANISM is an R object that contains a single item: a character string that names
the organism for which mgug4121a was built. mgug4121aORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgug4121aORGANISM
provides a simple way to programmatically extract the organism name. mgug4121aORGPKG pro-
...
mgug4121aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

mgug4121aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the mgug4121aPATH2PROBE object to a list
xx <- as.list(mgug4121aPATH2PROBE)
if(length(xx) > 0){
...
mgug4121aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mgug4121aPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mgug4121aPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mgug4121aPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mgug4121aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object mgug4121aPMID2PROBE to a list
xx <- as.list(mgug4121aPMID2PROBE)
if(length(xx) > 0){
...
mgug4121aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgug4121aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mgug4121aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mgug4121aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
mgug4121aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mgug4121a_dbconn()
mgug4121a_dbfile()
mgug4121a_dbschema(file="", show.indices=FALSE)
mgug4121a_dbInfo()
...
mgug4121a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgug4121a_dbconn or you will break all
the AnnDbObj objects defined in this package!
mgug4121a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
mgug4121a_dbschema prints the schema definition of the package annotation DB.
mgug4121a_dbInfo prints other information about the package annotation DB.

Value

mgug4121a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
mgug4121a_dbfile: a character string with the path to the package annotation DB.
mgug4121a_dbschema: none (invisible NULL).
mgug4121a_dbInfo: none (invisible NULL).
...
dbGetQuery(mgug4121a_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgug4121a_dbconn, 20
mgug4121aACCNUM, 1
mgug4121aALIAS2PROBE, 2
mgug4121aCHR, 3
mgug4121aCHRLENGTHS, 4
mgug4121aCHRLOC, 4
mgug4121aENSEMBL, 5
mgug4121aENTREZID, 6
mgug4121aENZYME, 7
mgug4121aGENENAME, 9
mgug4121aGO, 10
mgug4121aMAPCOUNTS, 12
mgug4121aMGI, 12
mgug4121aORGANISM, 13
mgug4121aPATH, 14
mgug4121aPFAM, 15
mgug4121aPMID, 15
mgug4121aPROSITE, 16
...
mgug4121a (mgug4121a.db), 2
mgug4121a.db, 2
mgug4121a_dbconn, 20
mgug4121a_dbfile (mgug4121a_dbconn), 20

22

mgug4121a_dbInfo (mgug4121a_dbconn), 20
mgug4121a_dbschema (mgug4121a_dbconn),
...
mgug4121aCHRLENGTHS, 4
mgug4121aCHRLOC, 4
mgug4121aCHRLOCEND (mgug4121aCHRLOC), 4
mgug4121aENSEMBL, 5
mgug4121aENSEMBL2PROBE

(mgug4121aENSEMBL), 5

mgug4121aENTREZID, 6
mgug4121aENZYME, 7
mgug4121aENZYME2PROBE

(mgug4121aENZYME), 7

mgug4121aGENENAME, 9
mgug4121aGO, 10
mgug4121aGO2ALLPROBES, 11
mgug4121aGO2ALLPROBES (mgug4121aGO), 10
mgug4121aGO2PROBE (mgug4121aGO), 10
mgug4121aLOCUSID (mgug4121aENTREZID), 6
mgug4121aMAPCOUNTS, 12
mgug4121aMGI, 12
mgug4121aMGI2PROBE (mgug4121aMGI), 12
mgug4121aORGANISM, 13
mgug4121aORGPKG (mgug4121aORGANISM), 13
mgug4121aPATH, 14
mgug4121aPATH2PROBE (mgug4121aPATH), 14
mgug4121aPFAM, 15
mgug4121aPMID, 15
mgug4121aPMID2PROBE (mgug4121aPMID), 15
mgug4121aPROSITE, 16
mgug4121aREFSEQ, 17
mgug4121aSYMBOL, 18
mgug4121aUNIGENE, 19
mgug4121aUNIPROT, 20
mgug4121aUNIPROT2PROBE
