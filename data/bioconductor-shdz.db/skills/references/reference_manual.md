...
SHDZACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
SHDZALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the SHDZ.db annotation Package. The purpose of this package is to provide detailed
information about the SHDZ platform. This package is updated biannually.
...
SHDZCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
SHDZCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
SHDZCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
SHDZENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
SHDZENTREZID is an R object that provides mappings between manufacturer identifiers and
...
SHDZENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. SHDZENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the SHDZENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the SHDZENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In SHDZENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert SHDZENZYME2PROBE to a list to see inside
xx <- as.list(SHDZENZYME2PROBE)
if(length(xx) > 0){
...
SHDZGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
SHDZGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (SHDZGO2PROBE)
...
SHDZGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than SHDZGO2PROBE.

Details

If SHDZGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If SHDZGO2ALLPROBES or SHDZGO2PROBE is cast as a list, each GO term maps to a named
...
SHDZMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package SHDZ.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

SHDZMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package SHDZ.db.
...
SHDZOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for SHDZ

Description

SHDZORGANISM is an R object that contains a single item: a character string that names the
organism for which SHDZ was built. SHDZORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, SHDZORGANISM
provides a simple way to programmatically extract the organism name. SHDZORGPKG provides
...
SHDZPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

SHDZPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
# Now convert the SHDZPATH2PROBE object to a list
xx <- as.list(SHDZPATH2PROBE)
if(length(xx) > 0){
...
SHDZPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
SHDZPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. SHDZPMID2PROBE is an R object that provides mappings between PubMed identifiers
and manufacturer identifiers.

Details

When SHDZPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When SHDZPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
# Now convert the reverse map object SHDZPMID2PROBE to a list
xx <- as.list(SHDZPMID2PROBE)
if(length(xx) > 0){
...
SHDZPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
SHDZREFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
SHDZSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
SHDZUNIGENE is an R object that provides mappings between manufacturer identifiers and Uni-
Gene identifiers.
...
SHDZUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
SHDZ_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by SHDZ_dbconn or you will break all the
AnnDbObj objects defined in this package!
SHDZ_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
SHDZ_dbschema prints the schema definition of the package annotation DB.
SHDZ_dbInfo prints other information about the package annotation DB.

Value

SHDZ_dbconn: a DBIConnection object representing an open connection to the package annotation
DB.
SHDZ_dbfile: a character string with the path to the package annotation DB.
SHDZ_dbschema: none (invisible NULL).
SHDZ_dbInfo: none (invisible NULL).
...
SHDZ_dbfile (SHDZ_dbconn), 21
SHDZ_dbInfo (SHDZ_dbconn), 21
SHDZ_dbschema (SHDZ_dbconn), 21
SHDZACCNUM, 1
SHDZALIAS2PROBE, 2
SHDZCHR, 3
SHDZCHRLENGTHS, 4
SHDZCHRLOC, 4
SHDZCHRLOCEND (SHDZCHRLOC), 4
SHDZENSEMBL, 5
SHDZENSEMBL2PROBE (SHDZENSEMBL), 5
SHDZENTREZID, 6
SHDZENZYME, 7
SHDZENZYME2PROBE (SHDZENZYME), 7
SHDZGENENAME, 9
SHDZGO, 10
SHDZGO2ALLPROBES, 11
SHDZGO2ALLPROBES (SHDZGO), 10
SHDZGO2PROBE (SHDZGO), 10
SHDZLOCUSID (SHDZENTREZID), 6
SHDZMAP, 12
SHDZMAPCOUNTS, 13
SHDZOMIM, 13
SHDZORGANISM, 14
SHDZORGPKG (SHDZORGANISM), 14
SHDZPATH, 15
SHDZPATH2PROBE (SHDZPATH), 15
SHDZPFAM, 16
SHDZPMID, 16
SHDZPMID2PROBE (SHDZPMID), 16
SHDZPROSITE, 17
SHDZREFSEQ, 18
SHDZSYMBOL, 19
SHDZUNIGENE, 20
SHDZUNIPROT, 21
SHDZUNIPROT2PROBE (SHDZUNIPROT), 21
