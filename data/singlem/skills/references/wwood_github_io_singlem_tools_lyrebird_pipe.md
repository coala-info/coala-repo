...
To further convert the generated taxonomic profile to other formats that might be more convenient, see SingleM [`summarise`](/singlem/tools/summarise).

## Algorithm details

**Details**: In its most common usage, the Lyrebird `pipe` subcommand takes as input raw metagenomic reads and outputs a taxonomic profile. It can also take as input whole genomes (or contigs), and can output a table of OTUs. Note that taxonomic profiles are generated from OTU tables, they are [not the same thing](/singlem/#glossary).
...
**$ lyrebird pipe -1 <fastq\_or\_fasta1> -2 <fastq\_or\_fasta2> -p
<output.profile.tsv>**

Get a taxonomic profile Krona diagram for dsDNA phages from single read input:

**$ lyrebird pipe -i <fastq\_or\_fasta> --taxonomic-profile-krona
<output.profile.html>**

Gather an OTU table (per marker sequence groupings) from paired reads:

**$ lyrebird pipe -1 <fastq\_or\_fasta1> -2 <fastq\_or\_fasta2>
--otu-table <output.otu\_table.tsv>**

On this page

* [lyrebird pipe](#lyrebird-pipe)
