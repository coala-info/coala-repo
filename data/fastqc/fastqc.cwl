...
inputs:
  ...
  - id: dup_length
    type:
      - 'null'
      - int
    doc: Sets a length to which the sequences will be truncated when defining 
      them to be duplicates, affecting the duplication and overrepresented 
      sequences plot. This can be useful if you have long reads with higher 
      levels of miscalls, or contamination with adapter dimers containing UMI 
      sequences.
    inputBinding:
      position: 102
      prefix: --dup_length
  - id: extract
    ...
    doc: If set then the zipped output file will be uncompressed in the same 
      directory after it has been created. If --delete is also specified then 
      the zip file will be removed after the contents are unzipped.
    inputBinding:
      position: 102
      prefix: --extract
  - id: format
  ...
  - id: quiet
    type:
      - 'null'
      - boolean
    doc: Suppress all progress messages on stdout and only report errors.
    inputBinding:
      position: 102
      prefix: --quiet
  - id: svg
    type:
      - 'null'
      - boolean
    doc: Save the graphs in the report in SVG format.
    inputBinding:
      position: 102
      prefix: --svg
  - id: threads
