...
| Command | Description |
|---------|-------------|
| bayesprism | BayesPrism is a tool for deconvolution of bulk RNA-seq data using single-cell RNA-seq data as a reference. |
| calculate_sig_score | Calculate signature scores from an expression matrix. |
| cibersort | CIBERSORT analysis tool |
| fastq_qc | Perform quality control on FASTQ files using fastp. |
...
| mcpcounter | MCPcounter is a tool for estimating the abundance of immune cells in transcriptomic data. |
| nmf | Non-negative Matrix Factorization (NMF) for dimensionality reduction and deconvolution. |
| quantiseq | Performs deconvolution of immune cell proportions from gene expression data. |
| tme_cluster | Perform TME clustering on input data. |
| tme_profile | TPM matrix (genes x samples). CSV/TSV/.gz supported. |
