...
use_ompBAM(pkg_path)
#> ✔ Creating '/tmp/RtmpulgC12/MyPkg/'.
#> ✔ Setting active project to "/tmp/RtmpulgC12/MyPkg".
#> ✔ Creating 'R/'.
#> ✔ Writing 'DESCRIPTION'.
#> Package: MyPkg
...
#> ✔ Created /tmp/RtmpulgC12/MyPkg/R/ompBAM_imports.R with roxygen tags
...
#> ✔ MyPkg successfully created in /tmp/RtmpulgC12/MyPkg
...
#> Writing 'NAMESPACE'
#> ℹ Loading MyPkg
#> Exports from /tmp/RtmpulgC12/MyPkg/src/ompBAM_example.cpp:
#>    int idxstats_pbam(std::string bam_file, int n_threads_to_use = 1)
#>
#> /tmp/RtmpulgC12/MyPkg/src/RcppExports.cpp updated.
#> /tmp/RtmpulgC12/MyPkg/R/RcppExports.R updated.
...
#> g++ -std=gnu++17 -I"/home/biocbuild/bbs-3.22-bioc/R/include" -DNDEBUG  -I'/tmp/RtmpcUZy6J/Rinst1f61913cc70957/ompBAM/include' -I'/home/biocbuild/bbs-3.22-bioc/R/site-library/Rcpp/include' -I/usr/local/include   -fopenmp  -fpic  -g -O2  -Wall -Werror=format-security -UNDEBUG -Wall -pedantic -g -O0  -c RcppExports.cpp -o RcppExports.o
#> g++ -std=gnu++17 -I"/home/biocbuild/bbs-3.22-bioc/R/include" -DNDEBUG  -I'/tmp/RtmpcUZy6J/Rinst1f61913cc70957/ompBAM/include' -I'/home/biocbuild/bbs-3.22-bioc/R/site-library/Rcpp/include' -I/usr/local/include   -fopenmp  -fpic  -g -O2  -Wall -Werror=format-security -UNDEBUG -Wall -pedantic -g -O0  -c ompBAM_example.cpp -o ompBAM_example.o
#> g++ -std=gnu++17 -shared -L/home/biocbuild/bbs-3.22-bioc/R/lib -L/usr/local/lib -o MyPkg.so RcppExports.o ompBAM_example.o -fopenmp -L/home/biocbuild/bbs-3.22-bioc/R/lib -lR
#> installing to /tmp/RtmpulgC12/devtools_install_1f628417958aff/00LOCK-MyPkg/00new/MyPkg/libs
...
#> /tmp/RtmpcUZy6J/Rinst1f61913cc70957/ompBAM/extdata/THP1_ND_1.bam summary:
...
#> /tmp/RtmpcUZy6J/Rinst1f61913cc70957/ompBAM/extdata/MGE_E35_1.bam summary:
#> Name Length  Number of reads
#> 1    195471971   50000
#> 10   130694993   0
#> 11   122082543   0
...
use_ompBAM(pkg_path)
#> ✔ Creating '/tmp/RtmpulgC12/myPkgName/'.
#> ✔ Setting active project to "/tmp/RtmpulgC12/myPkgName".
#> ✔ Creating 'R/'.
#> ✔ Writing 'DESCRIPTION'.
#> Package: myPkgName
#> Title: What the Package Does (One Line, Title Case)
...
#> ✔ Creating 'src/'.
#> ✔ Adding "*.o", "*.so", and "*.dll" to 'src/.gitignore'.
#> ✔ Created /tmp/RtmpulgC12/myPkgName/R/ompBAM_imports.R with roxygen tags
#> ✔ Created src/Makevars.in and src/Makevars.win
#> ✔ Created configure scripts
#> ✔ Created src/ompBAM_example.cpp with idxstats_pbam() function
#> ✔ Adding Rcpp to Imports field in DESCRIPTION
#> ✔ Adding ompBAM to LinkingTo field in DESCRIPTION
#> ✔ Adding Rcpp to LinkingTo field in DESCRIPTION
#> ✔ myPkgName successfully created in /tmp/RtmpulgC12/myPkgName
#> Please run roxygen2 using devtools::document() before building the package.
```
...
NB: prior versions of ompBAM suggested linking to zlibbioc. This is now removed as zlibbioc will be deprecated as of Bioconductor 3.20. Instead, simply make sure `Makevars.win` contains the line `PKG_LIBS = -lz $(SHLIB_OPENMP_CXXFLAGS)`
...
use\_ompBAM() has created two `make` files that instructs the compiler to link with OpenMP and the zlib library. You can view these files from within the `src/` directory.
...
For Windows installations, a second `make` file called “Makevars.win” is required. In windows systems, “Makevars.win” is used to build your package, and contains the following:
...
Windows and Linux systems should support OpenMP natively. MacOS, however, does not. In order to set up OpenMP for MacOS, we recommend following this [guide](https://mac.r-project.org/openmp/). In short, to utilise OpenMP on MacOS users must first install the correct OpenMP libraries. Secondly, specific compiler and linker flags must be set. This has already been done using the `configure` script and template `Makevars.in` file as described in section (1c).
...
For other “getter” functions of pbam1\_t, refer to Section (4e-h).
...
# (3) pbam\_in function documentation
...
* `std::vector<uint32_t> & u32_chr_lens` A reference to an uint32\_t vector to contain chromosome lengths
...
When a BAM file is opened via `openFile()` or `SetInputHandle()`, pbam\_in will automatically read and store the header data, namely chromosome names and lengths. `obtainChrs()` can be called to retrieve this data.
...
# (4) pbam1\_t function documentation
...
* whether the block\_size as given by the first 4 bytes of the buffer matches the internally stored value of the read block\_size calculated at its construction
* whether the size of the individual components (cigar, sequence, quality scores, and size of additional tag data) totals the given block\_size.
...
Checks whether the pbam1\_t is persistent (real) or points to the pbam\_in memory (virtual)
...
See comments in the “Usage” section above. Also, it is helpful to refer to [SAMv1.pdf](https://samtools.github.io/hts-specs/SAMv1.pdf) - section 4.2 for further details regarding the BAM format.
...
Refer to [SAMv1.pdf](https://samtools.github.io/hts-specs/SAMv1.pdf) - section 1.4 for further details regarding the cigar string (in SAM format).
...
`uint8_t * seq()` and `char * qual()` returns the pointer to the raw data containing the sequence (in `uint8_t`) and quality scores (in `char`). Advanced users will have to convert the 4-bit sequence to nucleotides as described in the SAM documentation: [SAMv1.pdf](https://samtools.github.io/hts-specs/SAMv1.pdf).
...
It is helpful to refer to [SAMv1.pdf](https://samtools.github.io/hts-specs/SAMv1.pdf) - section 4.2.3 for further details regarding SEQ and QUAL encoding.
...
For more details, refer to [SAMv1.pdf](https://samtools.github.io/hts-specs/SAMv1.pdf), section 4.2.4 for more information about how tags are stored in BAM format.

Also, refer to [SAMtags.pdf](https://samtools.github.io/hts-specs/SAMtags.pdf) for a list of the commonly-used BAM/SAM tags annotated with their SAM-types and the type of information they store.
...
#> [1] MyPkg_0.0.0.9000 ompBAM_1.14.0
#>
#> loaded via a namespace (and not attached):
#>  [1] sass_0.4.10       xml2_1.4.1        stringi_1.8.7     digest_0.6.37
#>  [5] magrittr_2.0.4    evaluate_1.0.5    pkgload_1.4.1     fastmap_1.2.0
#>  [9] rprojroot_2.1.1   jsonlite_2.0.0    processx_3.8.6    pkgbuild_1.4.8
#> [13] sessioninfo_1.2.3 brio_1.1.5        ps_1.9.1          purrr_1.1.0
#> [17] jquerylib_0.1.4   cli_3.6.5         rlang_1.1.6       crayon_1.5.3
#> [21] ellipsis_0.3.2    remotes_2.5.0     withr_3.0.2       cachem_1.1.0
#> [25] yaml_2.3.10       devtools_2.4.6    tools_4.5.1       memoise_2.0.1
#> [29] credentials_2.0.3 vctrs_0.6.5       R6_2.6.1          lifecycle_1.0.4
#> [33] stringr_1.5.2     fs_1.6.6          usethis_3.2.1     pkgconfig_2.0.3
#> [37] desc_1.4.3        callr_3.7.6       pillar_1.11.1     bslib_0.9.0
#> [41] glue_1.8.0        Rcpp_1.1.0        gert_2.1.5        xfun_0.53
#> [45] tibble_3.3.0      rstudioapi_0.17.1 sys_3.4.3         knitr_1.50
#> [49] htmltools_0.5.8.1 rmarkdown_2.30    testthat_3.2.3    compiler_4.5.1
#> [53] roxygen2_7.3.3    askpass_1.2.1     openssl_2.3.4
```