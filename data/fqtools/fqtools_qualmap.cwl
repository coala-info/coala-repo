cwlVersion: v1.2
class: CommandLineTool
baseCommand: fqtools_qualmap
label: fqtools_qualmap
doc: "Translate quality values using a mapping file.\n\nTool homepage: https://github.com/alastair-droop/fqtools"
inputs:
  - id: mapping_file
    type: File
    doc: The mapping file to use (see below)
    inputBinding:
      position: 1
  - id: input_files
    type:
      - 'null'
      - type: array
        items: File
    doc: The fastq file(s) to view.
    inputBinding:
      position: 2
  - id: output_stem
    type:
      - 'null'
      - string
    doc: Output file stem (default "output%").
    default: output%
    inputBinding:
      position: 103
      prefix: -o
  - id: preserve_secondary_headers
    type:
      - 'null'
      - boolean
    doc: Preserve secondary headers (if present).
    inputBinding:
      position: 103
      prefix: -k
outputs:
  - id: stdout
    type: stdout
    doc: Standard output
hints:
  - class: DockerRequirement
    dockerPull: quay.io/biocontainers/fqtools:2.0--h577a1d6_15
stdout: fqtools_qualmap.out
