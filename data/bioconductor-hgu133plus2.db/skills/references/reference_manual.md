...
hgu133plus2ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgu133plus2ALIAS is an R object that provides mappings between common gene symbol identi-
...
Welcome to the hgu133plus2.db annotation Package. The purpose of this package is to provide
detailed information about the hgu133plus2 platform. This package is updated biannually.
...
hgu133plus2CHR is an R object that provides mappings between a manufacturer identifier and the
...
hgu133plus2CHRLENGTHS A named vector for the length of each of the chromosomes

Description

hgu133plus2CHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgu133plus2CHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
hgu133plus2ENSEMBL is an R object that contains mappings between manufacturer identifiers
...
hgu133plus2ENTREZID is an R object that provides mappings between manufacturer identifiers
...
hgu133plus2ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgu133plus2ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgu133plus2ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgu133plus2ENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In hgu133plus2ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu133plus2ENZYME2PROBE to a list to see inside

hgu133plus2GENENAME

9

x <- hgu133plus2ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu133plus2GENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgu133plus2GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgu133plus2GO2PROBE)
...
hgu133plus2GO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgu133plus2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgu133plus2GO2ALLPROBES or hgu133plus2GO2PROBE is cast as a list, each GO term maps
...
x <- hgu133plus2GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu133plus2GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hgu133plus2MAP is an R object that provides mappings between manufacturer identifiers and cy-
toband locations.
...
Number of mapped keys for the maps in package hgu133plus2.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu133plus2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgu133plus2.db.
...
hgu133plus2OMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgu133plus2

Description

hgu133plus2ORGANISM is an R object that contains a single item: a character string that names
the organism for which hgu133plus2 was built. hgu133plus2ORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu133plus2ORGANISM
provides a simple way to programmatically extract the organism name. hgu133plus2ORGPKG
...
hgu133plus2PATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

hgu133plus2PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hgu133plus2PATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu133plus2PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu133plus2PFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgu133plus2PMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hgu133plus2PMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hgu133plus2PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hgu133plus2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hgu133plus2PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu133plus2PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu133plus2PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgu133plus2REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgu133plus2SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgu133plus2UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgu133plus2_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by hgu133plus2_dbconn or you will
break all the AnnDbObj objects defined in this package!

hgu133plus2_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hgu133plus2_dbschema prints the schema definition of the package annotation DB.

hgu133plus2_dbInfo prints other information about the package annotation DB.

Value

hgu133plus2_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

hgu133plus2_dbfile: a character string with the path to the package annotation DB.

hgu133plus2_dbschema: none (invisible NULL).

hgu133plus2_dbInfo: none (invisible NULL).
...
hgu133plus2.db, 2
hgu133plus2_dbconn, 21
hgu133plus2ACCNUM, 1
hgu133plus2ALIAS2PROBE, 2
hgu133plus2CHR, 3
hgu133plus2CHRLENGTHS, 4
hgu133plus2CHRLOC, 4
hgu133plus2ENSEMBL, 5
hgu133plus2ENTREZID, 6
hgu133plus2ENZYME, 7
hgu133plus2GENENAME, 9
hgu133plus2GO, 10
hgu133plus2MAP, 12
hgu133plus2MAPCOUNTS, 13
hgu133plus2OMIM, 13
hgu133plus2ORGANISM, 14
hgu133plus2PATH, 15
hgu133plus2PFAM, 16
hgu133plus2PMID, 16
hgu133plus2PROSITE, 17
hgu133plus2REFSEQ, 18
hgu133plus2SYMBOL, 19
hgu133plus2UNIPROT, 20
...
hgu133plus2CHRLENGTHS, 4
hgu133plus2CHRLOC, 4
hgu133plus2CHRLOCEND

(hgu133plus2CHRLOC), 4

hgu133plus2ENSEMBL, 5
hgu133plus2ENSEMBL2PROBE

(hgu133plus2ENSEMBL), 5

hgu133plus2ENTREZID, 6
hgu133plus2ENZYME, 7
hgu133plus2ENZYME2PROBE

(hgu133plus2ENZYME), 7

hgu133plus2GENENAME, 9
hgu133plus2GO, 10
hgu133plus2GO2ALLPROBES, 11
hgu133plus2GO2ALLPROBES

(hgu133plus2GO), 10

hgu133plus2GO2PROBE (hgu133plus2GO), 10
hgu133plus2LOCUSID
...
hgu133plus2PATH, 15
hgu133plus2PATH2PROBE

(hgu133plus2PATH), 15

hgu133plus2PFAM, 16
hgu133plus2PMID, 16
hgu133plus2PMID2PROBE
