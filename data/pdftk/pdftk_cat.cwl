...
baseCommand:
  - pdftk
label: pdftk_cat
doc: "Assembles (catenates) pages from input PDFs to create a new PDF. Use cat to
  merge PDF pages or to split PDF pages from documents. You can also use it to rotate
...
inputs:
  - id: input_files
    type:
      type: array
      items: File
    doc: A list of the input PDF files. Can be associated with handles (e.g., 
      A=input1.pdf).
    inputBinding:
      position: 1
  - id: operation
  ...
  - id: page_ranges
    type:
      ...
      - type: array
        items: string
    doc: Page ranges to assemble, e.g., A1-12 B6-1even. If omitted, all pages 
      from input PDFs are used.
    inputBinding:
      position: 3
  - id: allow
    type:
  ...
  - id: input_pw
    type:
      ...
      - type: array
        items: string
    doc: Input PDF owner passwords associated with files by handles or order.
    inputBinding:
      position: 104
      prefix: input_pw
  - id: keep_final_id
