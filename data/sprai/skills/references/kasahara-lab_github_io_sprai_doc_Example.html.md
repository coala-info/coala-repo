...
* [next](Download.html "Download The Latest Version") |
* [previous](README.html "README") |
* [sprai 0.9.x documentation](index.html) »
* Example: assemble phage genome

# Example: assemble phage genome[¶](#example-assemble-phage-genome "Link to this heading")

After installing Sprai, assemble a small genome.

For example, we showed how to assemble the phage genome.

## Prepare data[¶](#prepare-data "Link to this heading")

Go to [pacbiotoca wiki](http://sourceforge.net/apps/mediawiki/wgs-assembler/index.php?title=PacBioToCA) and download phage data ( http://www.cbcb.umd.edu/software/PBcR/data/sampleData.tar.gz ).
...
## Prepare Sprai[¶](#prepare-sprai "Link to this heading")
...
*input\_fastq* is your input file name.

*estimated\_genome\_size* is the number of nucleotides of your target.
If you do not know it, set large number. For example, set 1e+12.

*estimated\_depth* is the depth of coverage of input\_fastq of your target.
...
*ca\_path* is the path to your wgs-assembler (Celera Assembler) installed.

*word\_size* is used by blastn.

## Correct errors & assemble[¶](#correct-errors-assemble "Link to this heading")

```
ezez_vx1.pl ec.spec pbasm.spec > log.txt 2>&1 &
```

ezez\_vx1.pl outputs into a *result\_yyyymmdd\_hhmmss* directory. The improved reads will be in a *c01.fin.idfq.gz* file and contigs will be in a *CA/9-terminator/asm.ctg.fasta* file.

We explain temporary files in a *tmp* directory.
Sprai gives ID to each read in *all.fq* and outputs *c00.idfq.gz*.
Sprai partitions *all.fq* to *c00\_00xx.fa*.
*c00.nin*, *c00.nhr* and *c00.nsq* are output by makeblastdb.
Sprai aligns *c00\_00xx.fa* to *c00.nxx* database by using blastn, correct errors and output *c00\_00xx.dfq.gz*.
...
Sprai extracts longest 20X reads of the *estimated\_genome\_size* from *c01.fin.idfq.gz*.
...
## Find contigs[¶](#find-contigs "Link to this heading")
...
Read [the wgs-assembler document](http://sourceforge.net/apps/mediawiki/wgs-assembler/index.php?title=Main_Page) for details.

## Get N50 value[¶](#get-n50-value "Link to this heading")

If you install Statistics::Descriptive module to a directory Celera Assembler can see, Celera Assembler outputs N50 value and so on in a CA/do\_\*\_c01.fin.top20x.log file.
...
If you would like to use more than about 1000 processors, we recommend to use a *pre\_partition* parameter in the ec.spec file like:
...
* Example: assemble phage genome
  ...
  + [Prepare Sprai](#prepare-sprai)
  + [Correct errors & assemble](#correct-errors-assemble)
  + [Find contigs](#find-contigs)
  + [Get N50 value](#get-n50-value)
  + [Notes](#notes)

#### Previous topic

[README](README.html "previous chapter")

#### Next topic

[Download The Latest Version](Download.html "next chapter")
