...
hgug4845aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
a date stamp from the source of: 2012-Mar7
...
hgug4845aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4845aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4845aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4845a.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4845a platform. This package is updated biannually.
...
hgug4845aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4845aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4845aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgug4845aENSEMBL is an R object that contains mappings between Entrez Gene identifiers and
...
# Convert to a list
xx <- as.list(hgug4845aENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4845aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4845aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers.
...
numbers. In hgug4845aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
hgug4845aENZYME2PROBE Map between Enzyme Commission Numbers and Manufacturer Iden-

tifiers

Description

hgug4845aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers to
...
# Convert to a list
xx <- as.list(hgug4845aENZYME2PROBE)
if(length(xx) > 0){
...
hgug4845aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4845aGO is an R object that provides mappings between manufacturer identifiers and the GO
...
lite/ With a date stamp from the source of: 20120303

10

Examples

hgug4845aGO2ALLPROBES
...
hgug4845aGO2ALLPROBES Map between Gene Ontology (GO) Identifiers and all Manufacturer

Identifiers in the subtree

Description

hgug4845aGO2ALLPROBES is an R object that provides mappings between a given GO identifier
...
The name associated with each manufacturer id corresponds to the evidence code for that GO
identifier. This object hgug4845aGO2ALLPROBES maps between a given GO identifier and all
...
from the source of: 20120303 and Entrez Gene ftp://ftp.ncbi.nlm.nih.gov/gene/DATA With a date
stamp from the source of: 2012-Mar7
...
# Convert to a list
xx <- as.list(hgug4845aGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4845aGO2PROBE is an R object that provides mappings between GO identifiers and manu-
...
Entrez Gene ftp://ftp.ncbi.nlm.nih.gov/gene/DATA With a date stamp from the source of: 2012-
Mar7
...
hgug4845aMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4845a.db

Description

hgug4845aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4845a.db.
...
hgug4845aMAPCOUNTS
mapnames <- names(hgug4845aMAPCOUNTS)
hgug4845aMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package hgug4845a.db
checkMAPCOUNTS("hgug4845a.db")
...
hgug4845aOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4845a

Description

hgug4845aORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4845a was built. hgug4845aORGPKG is an R object that contains a
...
Although the package name is suggestive of the organism for which it was built, hgug4845aORGANISM
provides a simple way to programmatically extract the organism name. hgug4845aORGPKG pro-
...
isms. hgug4845aPATH maps probe identifiers to the identifiers used by KEGG for pathways in
...
hgug4845aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
manufacturer identifiers.
...
# Convert the object to a list
xx <- as.list(hgug4845aPATH2PROBE)
if(length(xx) > 0){
...
hgug4845aPFAM is an R object that provides mappings between a manufacturer identifier and the
...
With a date stamp from the source of: 2011-Sep27
...
hgug4845aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers.
...
hgug4845aPMID2PROBE is an R object that provides mappings between PubMed identifiers and
manufacturer identifiers.
...
# Convert the object to a list
xx <- as.list(hgug4845aPMID2PROBE)
if(length(xx) > 0){
...
hgug4845aPROSITE is an R object that provides mappings between a manufacturer identifier and
...
hgug4845aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4845aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4845aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4845aUNIPROT is an R object that contains mappings between Entrez Gene identifiers and
...
hgug4845a_dbconn()
hgug4845a_dbfile()
hgug4845a_dbschema(file="", show.indices=FALSE)
hgug4845a_dbInfo()
...
hgug4845a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4845a_dbconn or you will break all
the AnnDbObj objects defined in this package!

hgug4845a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hgug4845a_dbschema prints the schema definition of the package annotation DB.

hgug4845a_dbInfo prints other information about the package annotation DB.

Value

hgug4845a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

hgug4845a_dbfile: a character string with the path to the package annotation DB.

hgug4845a_dbschema: none (invisible NULL).

hgug4845a_dbInfo: none (invisible NULL).
...
dbGetQuery(hgug4845a_dbconn(), "SELECT COUNT(*) FROM probes")

## The connection object returned by hgug4845a_dbconn() was
## created with:
dbConnect(SQLite(), dbname=hgug4845a_dbfile(), cache_size=64000,
synchronous=0)
...
hgug4845a_dbconn, 22
hgug4845aACCNUM, 1
hgug4845aALIAS2PROBE, 2
hgug4845aCHR, 3
hgug4845aCHRLENGTHS, 3
hgug4845aCHRLOC, 4
hgug4845aENSEMBL, 4
hgug4845aENTREZID, 5
hgug4845aENZYME, 6
hgug4845aENZYME2PROBE, 7
hgug4845aGENENAME, 8
hgug4845aGO, 9
hgug4845aGO2ALLPROBES, 10
hgug4845aGO2PROBE, 11
hgug4845aMAP, 12
hgug4845aMAPCOUNTS, 13
hgug4845aOMIM, 14
hgug4845aORGANISM, 14
hgug4845aPATH, 15
hgug4845aPATH2PROBE, 16
hgug4845aPFAM, 16
hgug4845aPMID, 17
hgug4845aPMID2PROBE, 18
hgug4845aPROSITE, 19
hgug4845aREFSEQ, 19
hgug4845aSYMBOL, 20
hgug4845aUNIGENE, 21
...
hgug4845a (hgug4845a.db), 2
hgug4845a.db, 2
hgug4845a_dbconn, 22
hgug4845a_dbfile (hgug4845a_dbconn), 22
hgug4845a_dbInfo (hgug4845a_dbconn), 22
hgug4845a_dbschema (hgug4845a_dbconn),
...
hgug4845aCHRLENGTHS, 3
hgug4845aCHRLOC, 4
hgug4845aCHRLOCEND (hgug4845aCHRLOC), 4
hgug4845aENSEMBL, 4
hgug4845aENSEMBL2PROBE
...
hgug4845aGO2ALLPROBES, 10
hgug4845aGO2PROBE, 11
hgug4845aLOCUSID (hgug4845aENTREZID), 5
hgug4845aMAP, 12
hgug4845aMAPCOUNTS, 13
hgug4845aOMIM, 14
hgug4845aORGANISM, 14
hgug4845aORGPKG (hgug4845aORGANISM), 14
hgug4845aPATH, 15
hgug4845aPATH2PROBE, 16
hgug4845aPFAM, 16
hgug4845aPMID, 17
