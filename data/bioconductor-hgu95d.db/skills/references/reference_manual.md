...
hgu95dACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu95dALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgu95d.db annotation Package. The purpose of this package is to provide detailed
information about the hgu95d platform. This package is updated biannually.
...
hgu95dCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgu95dCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu95dCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu95dENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu95dENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu95dENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu95dENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu95dENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu95dENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu95dENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu95dENZYME2PROBE to a list to see inside

hgu95dGENENAME

9

x <- hgu95dENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu95dGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu95dGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hgu95dGO2PROBE)
...
hgu95dGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu95dGO2PROBE.

Details

If hgu95dGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu95dGO2ALLPROBES or hgu95dGO2PROBE is cast as a list, each GO term maps to a named
...
x <- hgu95dGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu95dGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hgu95dMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu95d.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu95dMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu95d.db.
...
hgu95dOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu95d

Description

hgu95dORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu95d was built. hgu95dORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu95dORGANISM
provides a simple way to programmatically extract the organism name. hgu95dORGPKG provides
...
hgu95dPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu95dPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- hgu95dPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu95dPATH2PROBE object to a list
...
hgu95dPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu95dPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu95dPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu95dPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgu95dPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- hgu95dPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu95dPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu95dPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hgu95dREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu95dSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu95dUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu95d_dbconn()
hgu95d_dbfile()
hgu95d_dbschema(file="", show.indices=FALSE)
hgu95d_dbInfo()
...
hgu95d_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu95d_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgu95d_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgu95d_dbschema prints the schema definition of the package annotation DB.

hgu95d_dbInfo prints other information about the package annotation DB.

Value

hgu95d_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

hgu95d_dbfile: a character string with the path to the package annotation DB.

hgu95d_dbschema: none (invisible NULL).

hgu95d_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu95d_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu95d_dbInfo (hgu95d_dbconn), 21
hgu95d_dbschema (hgu95d_dbconn), 21
hgu95dACCNUM, 1
hgu95dALIAS2PROBE, 2
hgu95dCHR, 3
hgu95dCHRLENGTHS, 4
hgu95dCHRLOC, 4
hgu95dCHRLOCEND (hgu95dCHRLOC), 4
hgu95dENSEMBL, 5
hgu95dENSEMBL2PROBE (hgu95dENSEMBL), 5
hgu95dENTREZID, 6
hgu95dENZYME, 7
hgu95dENZYME2PROBE (hgu95dENZYME), 7
hgu95dGENENAME, 9
hgu95dGO, 10
hgu95dGO2ALLPROBES, 11
hgu95dGO2ALLPROBES (hgu95dGO), 10
hgu95dGO2PROBE (hgu95dGO), 10
hgu95dLOCUSID (hgu95dENTREZID), 6
hgu95dMAP, 12
hgu95dMAPCOUNTS, 13
hgu95dOMIM, 13
hgu95dORGANISM, 14
hgu95dORGPKG (hgu95dORGANISM), 14
hgu95dPATH, 15
hgu95dPATH2PROBE (hgu95dPATH), 15
hgu95dPFAM, 16
hgu95dPMID, 16
hgu95dPMID2PROBE (hgu95dPMID), 16
hgu95dPROSITE, 17
hgu95dREFSEQ, 18
hgu95dSYMBOL, 19
hgu95dUNIPROT, 20
hgu95dUNIPROT2PROBE (hgu95dUNIPROT), 20
...
hgu95d.db, 2
hgu95d_dbconn, 21
hgu95d_dbfile (hgu95d_dbconn), 21
