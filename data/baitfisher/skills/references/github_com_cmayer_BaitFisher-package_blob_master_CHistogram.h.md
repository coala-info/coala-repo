...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fcmayer%2FBaitFisher-package%2Fblob%2Fmaster%2FCHistogram.h)
...
[History](/cmayer/BaitFisher-package/commits/master/CHistogram.h)

History

668 lines (543 loc) · 15.8 KB
...
[Raw](https://github.com/cmayer/BaitFisher-package/raw/refs/heads/master/CHistogram.h)
...
#ifndef CHISTOGRAM\_H

#define CHISTOGRAM\_H
...
#include "statistic\_functions.h"

#define EPSS 0.00000000001;

inline bool greater\_than\_pair(const std::pair<double, unsigned> &a, const std::pair<double, unsigned> &b )
...
inline void add\_or\_count(std::map<double, unsigned> &m, double &x)
...
bool discrete\_data; // is determined in initialisation. Switched on with -10.

std::map<double, unsigned> discrete\_hist\_data;

private:

bool add\_intern\_non\_discrete(double x)
...
std::cerr << "Warning: CHistrogram::add\_intern\_non\_discrete data.siz\n"
...
bool add\_intern\_discrete\_no\_update(double x)

{

++entries;

add\_or\_count(discrete\_hist\_data, x);

return true;

}

void update\_a\_b\_bins\_discrete\_data()
...
a = discrete\_hist\_data.begin()->first;

b = discrete\_hist\_data.rbegin()->first;

bins = discrete\_hist\_data.size();

}

public:

void init\_with\_vector(const std::vector<double> &vals, int bins\_param=0)

{

init\_with\_range(vals.begin(), vals.end(), vals.size(), bins\_param);
