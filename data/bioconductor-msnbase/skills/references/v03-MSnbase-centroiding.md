...
* [2 Centroiding of profile-mode MS data](#centroiding-of-profile-mode-ms-data)
* [3 Improving the signal quality](#improving-the-signal-quality)
  + [3.1 Data smoothing](#sec:smoothing)
  + [3.2 Refinement of the centroid’s m/z values](#sec:refine)
...
actual m/z value (Smith et al. [2014](#ref-Smith:2014di)). The accuracy of that signal depends
...
*centWave* method in the *[xcms](https://bioconductor.org/packages/3.22/xcms)* package for
...
(Chambers et al. [2012](#ref-Chambers:2012)) allow to apply various data centroiding algorithms,
...
![MS profile data for serine. Upper panel shows the base peak chromatogram (BPC), lower panel the individual signals in the retention time - m/z space. The horizontal dashed red line indicates the theoretical m/z of the [M+H]+ adduct of serine.](data:image/png;base64...)
...
Below we plot the signal for one of of the 43 spectra
...
![On of the spectra for serine in profile mode.](data:image/png;base64...)

Figure 2: On of the spectra for serine in profile mode
...
![Centroided data for serine.](data:image/png;base64...)

Figure 3: Centroided data for serine
...
![Centroided data for proline.](data:image/png;base64...)

Figure 4: Centroided data for proline
...
# 3 Improving the signal quality
...
e.g. a Savitzky-Golay filter (Savitzky and Golay [1964](#ref-Savitzky:1964bn)) which reduces the noise and
...
![Centroided data for serine after smoothing with a Savitzky-Golay filter.](data:image/png;base64...)

Figure 5: Centroided data for serine after smoothing with a Savitzky-Golay filter
...
![Centroided data for proline after smoothing with a Savitzky-Golay filter.](data:image/png;base64...)

Figure 6: Centroided data for proline after smoothing with a Savitzky-Golay filter
...
![Centroided data for proline after smoothing in retention time dimension.](data:image/png;base64...)

Figure 7: Centroided data for proline after smoothing in retention time dimension
...
## 3.2 Refinement of the centroid’s m/z values
...
and *kNeighbors* that can be selected by passing either `refineMz = "descendPeak"` or `refineMz = "kNeighbors"` to the `pickPeaks`
...
![Smoothed and centroided data for serine without (left) and with m/z refinement (right). The horizontal red dashed line indicates the theoretical m/z for the [M+H]+ ion and the vertical red dotted line the position of the largest signal for serine.](data:image/png;base64...)

Figure 8: Smoothed and centroided data for serine without (left) and with m/z refinement (right)
The horizontal red dashed line indicates the theoretical m/z for the [M+H]+ ion and the vertical red dotted line the position of the largest signal for serine.
...
![Smoothed and centroided data for proline without (left) and with m/z refinement (right). The horizontal red dashed line indicates the theoretical m/z for the [M+H]+ ion and the vertical red dotted line the position of the maximum signal.](data:image/png;base64...)

Figure 9: Smoothed and centroided data for proline without (left) and with m/z refinement (right)
The horizontal red dashed line indicates the theoretical m/z for the [M+H]+ ion and the vertical red dotted line the position of the maximum signal.
...
Savitzky, Abraham, and M J E Golay. 1964. “Smoothing and Differentiation of Data by Simplified Least Squares Procedures.” *Analytical Chemistry* 36 (8): 1627–39.
