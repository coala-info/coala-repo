# [Frequently Asked Questions (FAQ)](#id1)[¶](#frequently-asked-questions-faq "Link to this heading")

Contents

* [Frequently Asked Questions (FAQ)](#frequently-asked-questions-faq)

  + [How is sourmash different from mash?](#how-is-sourmash-different-from-mash)
  + [What are the drawbacks to FracMinHash and sourmash?](#what-are-the-drawbacks-to-fracminhash-and-sourmash)
  + [How can I better understand FracMinHash and sourmash intuitively?](#how-can-i-better-understand-fracminhash-and-sourmash-intuitively)
  + [What papers should I read to better understand the FracMinHash approach used by sourmash?](#what-papers-should-i-read-to-better-understand-the-fracminhash-approach-used-by-sourmash)
  + [What k-mer size(s) should I use with sourmash?](#what-k-mer-size-s-should-i-use-with-sourmash)
  + [What scaled values should I use with sourmash?](#what-scaled-values-should-i-use-with-sourmash)
  + [What threshold-bp value should I use with `sourmash prefetch` and `sourmash gather`?](#what-threshold-bp-value-should-i-use-with-sourmash-prefetch-and-sourmash-gather)
  + [How do k-mer-based analyses compare with read mapping?](#how-do-k-mer-based-analyses-compare-with-read-mapping)

    - [Mapping reads to reference vs k-mer “detection” or containment](#mapping-reads-to-reference-vs-k-mer-detection-or-containment)
    - [Why do k-mers underestimate read mapping?](#why-do-k-mers-underestimate-read-mapping)
    - [How do read mapping rates for metagenomes compare with k-mer statistics?](#how-do-read-mapping-rates-for-metagenomes-compare-with-k-mer-statistics)
    - [Further reading and links on k-mers and mapping:](#further-reading-and-links-on-k-mers-and-mapping)
  + [Can I use sourmash to determine the best reference genome for mapping my reads?](#can-i-use-sourmash-to-determine-the-best-reference-genome-for-mapping-my-reads)
  + [How do I get the sequences for a particular reference genome from a metagenome, using sourmash?](#how-do-i-get-the-sequences-for-a-particular-reference-genome-from-a-metagenome-using-sourmash)
  + [How does memory usage for sourmash change with k-mer size?](#how-does-memory-usage-for-sourmash-change-with-k-mer-size)
  + [Can sourmash run with multiple theads?](#can-sourmash-run-with-multiple-theads)

## [How is sourmash different from mash?](#id2)[¶](#how-is-sourmash-different-from-mash "Link to this heading")
...
But… MinHash sketching doesn’t work when comparing *metagenomes* to
genomes, because metagenomes are usually *much* larger than genomes.
...
## [What are the drawbacks to FracMinHash and sourmash?](#id3)[¶](#what-are-the-drawbacks-to-fracminhash-and-sourmash "Link to this heading")

There are two drawbacks to FracMinHash relative to MinHash. Neither
one is a showstopper, but you should know about them!

One drawback is that FracMinHash sketches can be a *lot* bigger than
...
How large depends on the *cardinality* of the metagenome k-mers - if a
metagenome has 1 billion unique k-mers, a FracMinHash sketch with scaled
parameter of 1000 will contain a million hashes.

The other drawback is that FracMinHash sketches *don’t work well* for
...
## [How can I better understand FracMinHash and sourmash intuitively?](#id4)[¶](#how-can-i-better-understand-fracminhash-and-sourmash-intuitively "Link to this heading")

Please see [the k-mers and minhash tutorial](kmers-and-minhash.html).

## [What papers should I read to better understand the FracMinHash approach used by sourmash?](#id5)[¶](#what-papers-should-i-read-to-better-understand-the-fracminhash-approach-used-by-sourmash "Link to this heading")
...
good contrasting point to take into account is that *MinHash cannot do
overlap or containment estimation*, which nicely motivates the
...
## [What k-mer size(s) should I use with sourmash?](#id6)[¶](#what-k-mer-size-s-should-i-use-with-sourmash "Link to this heading")
...
We also provide [Genbank and GTDB databases](databases.html) for k=21,
...
## [What scaled values should I use with sourmash?](#id7)[¶](#what-scaled-values-should-i-use-with-sourmash "Link to this heading")
...
you use [the precomputed databases](databases.html), you will always end up
using your query sketches at a minimum scaled of 1000, even if you created
them with a lower scaled value.

Please also see [What resolution should my signatures be?](using-sourmash-a-guide.html#what-resolution-should-my-signatures-be-and-how-should-i-create-them).

## [What threshold-bp value should I use with `sourmash prefetch` and `sourmash gather`?](#id8)[¶](#what-threshold-bp-value-should-i-use-with-sourmash-prefetch-and-sourmash-gather "Link to this heading")
...
## [How do k-mer-based analyses compare with read mapping?](#id9)[¶](#how-do-k-mer-based-analyses-compare-with-read-mapping "Link to this heading")
...
### [Mapping reads to reference vs k-mer “detection” or containment](#id10)[¶](#mapping-reads-to-reference-vs-k-mer-detection-or-containment "Link to this heading")
...
sequence to be 65%, using e.g. `sourmash search --containment ref.sig.gz isolate.sig.gz`. You then map the reads from the isolate
...
### [Why do k-mers underestimate read mapping?](#id11)[¶](#why-do-k-mers-underestimate-read-mapping "Link to this heading")
...
### [How do read mapping rates for metagenomes compare with k-mer statistics?](#id12)[¶](#how-do-read-mapping-rates-for-metagenomes-compare-with-k-mer-statistics "Link to this heading")
...
in terms of SNPs *and* accessory elements from what’s in the reference
