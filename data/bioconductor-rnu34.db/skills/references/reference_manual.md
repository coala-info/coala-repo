...
rnu34ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
rnu34ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the rnu34.db annotation Package. The purpose of this package is to provide detailed
information about the rnu34 platform. This package is updated biannually.
...
rnu34CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
rnu34CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rnu34CHRLOC is an R object that maps manufacturer identifiers to the starting position of the gene.
...
rnu34ENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
rnu34ENTREZID is an R object that provides mappings between manufacturer identifiers and En-
...
rnu34ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. rnu34ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers to
manufacturer identifiers.

Details

When the rnu34ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the rnu34ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In rnu34ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert rnu34ENZYME2PROBE to a list to see inside
...
rnu34GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rnu34GO is an R object that provides mappings between manufacturer identifiers and the GO identi-
fiers that they are directly associated with. This mapping and its reverse mapping (rnu34GO2PROBE)
...
rnu34GO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than rnu34GO2PROBE.

Details

If rnu34GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If rnu34GO2ALLPROBES or rnu34GO2PROBE is cast as a list, each GO term maps to a named
...
x <- rnu34GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert rnu34GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package rnu34.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rnu34MAPCOUNTS provides the "map count" (i.e.
package rnu34.db.
...
The Organism information for rnu34

Description

rnu34ORGANISM is an R object that contains a single item: a character string that names the
rnu34ORGPKG is an R object that contains a chararcter
organism for which rnu34 was built.
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, rnu34ORGANISM
provides a simple way to programmatically extract the organism name. rnu34ORGPKG provides a
...
rnu34PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the genes
represented by the probe identifiers are involved

rnu34PATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- rnu34PATH
mapped_probes <- mappedkeys(x)
# Now convert the rnu34PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rnu34PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rnu34PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rnu34PMID2PROBE is an R object that provides mappings between PubMed identifiers
and manufacturer identifiers.

Details

When rnu34PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When rnu34PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- rnu34PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object rnu34PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rnu34PROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
rnu34REFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
rnu34SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
rnu34UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
rnu34_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by rnu34_dbconn or you will break all the
AnnDbObj objects defined in this package!

rnu34_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
...
rnu34_dbconn: a DBIConnection object representing an open connection to the package annotation
DB.
...
rnu34CHRLENGTHS, 4
rnu34CHRLOC, 4
rnu34CHRLOCEND (rnu34CHRLOC), 4
rnu34ENSEMBL, 5
rnu34ENSEMBL2PROBE (rnu34ENSEMBL), 5
rnu34ENTREZID, 6
rnu34ENZYME, 7
rnu34ENZYME2PROBE (rnu34ENZYME), 7
rnu34GENENAME, 9
rnu34GO, 10
rnu34GO2ALLPROBES, 11
rnu34GO2ALLPROBES (rnu34GO), 10
rnu34GO2PROBE (rnu34GO), 10
rnu34LOCUSID (rnu34ENTREZID), 6
rnu34MAPCOUNTS, 12
rnu34ORGANISM, 12
rnu34ORGPKG (rnu34ORGANISM), 12
rnu34PATH, 13
rnu34PATH2PROBE (rnu34PATH), 13
rnu34PFAM, 14
rnu34PMID, 14
rnu34PMID2PROBE (rnu34PMID), 14
rnu34PROSITE, 16
rnu34REFSEQ, 16
rnu34SYMBOL, 17
rnu34UNIPROT, 18
rnu34UNIPROT2PROBE (rnu34UNIPROT), 18
...
rnu34.db, 2
rnu34_dbconn, 19
rnu34_dbfile (rnu34_dbconn), 19
rnu34_dbInfo (rnu34_dbconn), 19
rnu34_dbschema (rnu34_dbconn), 19
