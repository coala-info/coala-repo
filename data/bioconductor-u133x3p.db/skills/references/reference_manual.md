...
u133x3pACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
u133x3pALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the u133x3p.db annotation Package. The purpose of this package is to provide detailed
information about the u133x3p platform. This package is updated biannually.
...
u133x3pCHR is an R object that provides mappings between a manufacturer identifier and the
...
u133x3pCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
u133x3pCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
u133x3pENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(u133x3pENSEMBL2PROBE)
if(length(xx) > 0){
...
u133x3pENTREZID is an R object that provides mappings between manufacturer identifiers and
...
u133x3pENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. u133x3pENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the u133x3pENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the u133x3pENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In u133x3pENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert u133x3pENZYME2PROBE to a list to see inside
xx <- as.list(u133x3pENZYME2PROBE)
if(length(xx) > 0){
...
u133x3pGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
u133x3pGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (u133x3pGO2PROBE)
...
u133x3pGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than u133x3pGO2PROBE.

Details

If u133x3pGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If u133x3pGO2ALLPROBES or u133x3pGO2PROBE is cast as a list, each GO term maps to a
...
u133x3pMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package u133x3p.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

u133x3pMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package u133x3p.db.
...
u133x3pOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for u133x3p

Description

u133x3pORGANISM is an R object that contains a single item: a character string that names the
organism for which u133x3p was built. u133x3pORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, u133x3pORGANISM
provides a simple way to programmatically extract the organism name. u133x3pORGPKG provides
...
u133x3pPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

u133x3pPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the u133x3pPATH2PROBE object to a list
xx <- as.list(u133x3pPATH2PROBE)
if(length(xx) > 0){
...
u133x3pPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
u133x3pPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. u133x3pPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When u133x3pPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When u133x3pPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object u133x3pPMID2PROBE to a list
xx <- as.list(u133x3pPMID2PROBE)
if(length(xx) > 0){
...
u133x3pPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
u133x3pREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
u133x3pSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
u133x3pUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
u133x3pUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
u133x3p_dbconn()
u133x3p_dbfile()
u133x3p_dbschema(file="", show.indices=FALSE)
u133x3p_dbInfo()
...
u133x3p_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by u133x3p_dbconn or you will break all the
AnnDbObj objects defined in this package!
u133x3p_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
u133x3p_dbschema prints the schema definition of the package annotation DB.
u133x3p_dbInfo prints other information about the package annotation DB.

Value

u133x3p_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
u133x3p_dbfile: a character string with the path to the package annotation DB.
u133x3p_dbschema: none (invisible NULL).
u133x3p_dbInfo: none (invisible NULL).
...
dbGetQuery(u133x3p_dbconn(), "SELECT COUNT(*) FROM probes")
...
u133x3p (u133x3p.db), 2
u133x3p.db, 2
u133x3p_dbconn, 21

u133x3p_dbfile (u133x3p_dbconn), 21
u133x3p_dbInfo (u133x3p_dbconn), 21
u133x3p_dbschema (u133x3p_dbconn), 21
u133x3pACCNUM, 1
u133x3pALIAS2PROBE, 2
u133x3pCHR, 3
u133x3pCHRLENGTHS, 4
u133x3pCHRLOC, 4
u133x3pCHRLOCEND (u133x3pCHRLOC), 4
u133x3pENSEMBL, 5
u133x3pENSEMBL2PROBE (u133x3pENSEMBL), 5
u133x3pENTREZID, 6
u133x3pENZYME, 7
u133x3pENZYME2PROBE (u133x3pENZYME), 7
u133x3pGENENAME, 9
u133x3pGO, 10
u133x3pGO2ALLPROBES, 11
u133x3pGO2ALLPROBES (u133x3pGO), 10
u133x3pGO2PROBE (u133x3pGO), 10
u133x3pLOCUSID (u133x3pENTREZID), 6
u133x3pMAP, 12
u133x3pMAPCOUNTS, 13
u133x3pOMIM, 13
u133x3pORGANISM, 14
u133x3pORGPKG (u133x3pORGANISM), 14
u133x3pPATH, 15
u133x3pPATH2PROBE (u133x3pPATH), 15
u133x3pPFAM, 16
u133x3pPMID, 16
u133x3pPMID2PROBE (u133x3pPMID), 16
u133x3pPROSITE, 17
u133x3pREFSEQ, 18
u133x3pSYMBOL, 19
u133x3pUNIGENE, 20
u133x3pUNIPROT, 21
u133x3pUNIPROT2PROBE (u133x3pUNIPROT),
