...
baseCommand:
  ...
  - enmo
label: rbpbench_enmo
doc: "Performs motif enrichment analysis on specified regions.\n\nTool homepage: https://github.com/michauhl/RBPBench"
inputs:
  - id: bed_sc_thr
    type:
      - 'null'
  ...
  - id: enmo_pval_thr
    ...
    doc: 'P-value threshold for reporting significantly enriched motifs. NOTE that
      if --enmo-pval-mode Bonferroni is selected, this threshold gets further adjusted
  ...
  - id: input_regions
    type: File
    doc: Genomic or transcript regions file in BED format. Use these regions 
      together with a generated set of background regions to check for enriched 
      motifs
    inputBinding:
      position: 101
      prefix: --in
  ...
  - id: tr_list
    ...
    doc: Supply file with transcript IDs (one ID per row) to define which 
      transcripts to use from --gtf to extract background sites
    inputBinding:
      position: 101
      prefix: --tr-list
  - id: user_cm
