...
DNA sequences or paste in actual [FASTA formatted](../doc/fasta-format.html) DNA sequences to analyse motif spacings.

For ChIP-seq peaks, the best approach is 500bp sequences centered
on the peak summit (or on the peak center if the peak-caller does not
report the peak summit.)

The sequences you input should all be the same length.
The sequences should be at least 400bp long, and 500bp is recommended.
See the [example DNA sequences](https://meme-suite.org/meme/doc/examples/example-datasets/Klf1.fna) which were used to create the [sample output](../doc/examples/spamo_example_output_files/spamo.html)
to get an idea of input that works well for SpaMo.

Sometimes it is helpful if you mask repeats in your DNA sequences
...
[repeat-masker program](http://www.repeatmasker.org) or you can
request repeat-masked sequences when you download genomic sequences using the
[UCSC Genome Table Browser](http://genome.ucsc.edu) or
[Galaxy](https://main.g2.bx.psu.edu).

[ close ]

SpaMo expects a single DNA motif as the primary motif.
...
Using the menus below, input motifs to analyze for spacing relative to the primary motif.
...
SpaMo can create an additional text file containing one line for
each significant primary-secondary motif spacing found in each input sequence.
The text file contains fields that from left to right contain:

1. A portion of the sequence in lower case, with the motif
   matches in upper case.
2. The position (offset) of the secondary match within the sequence
   portion.
3. The fragment of the sequence that matched the primary
   motif.
4. The strand of the primary motif match.
5. The fragment of the sequence that matched the secondary
   motif.
6. The strand of the secondary motif match.
7. Secondary site is on the same (s) or opposite (o) strand
   to the primary site.
8. Secondary site is upstream (u) or downstream (d) of the
   primary site.
9. The gap between the primary and secondary site.
10. The name of the sequence.

[ close ]

SpaMo scans both strands of sequences that have complements (e.g., DNA-like sequences)
by default. Selecting this option will cause DNA sequences to be treated as single-stranded
and SpaMo will only scan the given strand of each sequence for motif matches.
