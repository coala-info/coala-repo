...
m20kcodACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
m20kcodALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the m20kcod.db annotation Package. The purpose of this package is to provide detailed
information about the m20kcod platform. This package is updated biannually.
...
m20kcodCHR is an R object that provides mappings between a manufacturer identifier and the
...
m20kcodCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
m20kcodCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
m20kcodENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(m20kcodENSEMBL2PROBE)
if(length(xx) > 0){
...
m20kcodENTREZID is an R object that provides mappings between manufacturer identifiers and
...
m20kcodENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. m20kcodENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the m20kcodENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the m20kcodENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In m20kcodENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert m20kcodENZYME2PROBE to a list to see inside
...
m20kcodGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
m20kcodGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (m20kcodGO2PROBE)
...
m20kcodGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If m20kcodGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If m20kcodGO2ALLPROBES or m20kcodGO2PROBE is cast as a list, each GO term maps to a
...
# Convert m20kcodGO2ALLPROBES to a list
xx <- as.list(m20kcodGO2ALLPROBES)
if(length(xx) > 0){
...
m20kcodMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package m20kcod.db.
...
m20kcodMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for m20kcod

Description

m20kcodORGANISM is an R object that contains a single item: a character string that names the
organism for which m20kcod was built. m20kcodORGPKG is an R object that contains a chararcter
...
Although the package name is suggestive of the organism for which it was built, m20kcodORGANISM
provides a simple way to programmatically extract the organism name. m20kcodORGPKG pro-
...
m20kcodPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

m20kcodPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the m20kcodPATH2PROBE object to a list
xx <- as.list(m20kcodPATH2PROBE)
if(length(xx) > 0){
...
m20kcodPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. m20kcodPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When m20kcodPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When m20kcodPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object m20kcodPMID2PROBE to a list
xx <- as.list(m20kcodPMID2PROBE)
if(length(xx) > 0){
...
m20kcodREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
m20kcodSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
m20kcodUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
m20kcodUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
m20kcod_dbconn()
m20kcod_dbfile()
m20kcod_dbschema(file="", show.indices=FALSE)
m20kcod_dbInfo()
...
m20kcod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by m20kcod_dbconn or you will break all the
AnnDbObj objects defined in this package!

m20kcod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

m20kcod_dbschema prints the schema definition of the package annotation DB.

m20kcod_dbInfo prints other information about the package annotation DB.

Value

m20kcod_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

m20kcod_dbfile: a character string with the path to the package annotation DB.

m20kcod_dbschema: none (invisible NULL).

m20kcod_dbInfo: none (invisible NULL).
...
dbGetQuery(m20kcod_dbconn(), "SELECT COUNT(*) FROM probes")

m20kcod_dbschema()

m20kcod_dbInfo()

m20kcod_dbschema (m20kcod_dbconn), 17
m20kcodACCNUM, 1
m20kcodALIAS2PROBE, 2
m20kcodCHR, 3
m20kcodCHRLENGTHS, 3
m20kcodCHRLOC, 4
m20kcodCHRLOCEND (m20kcodCHRLOC), 4
m20kcodENSEMBL, 4
m20kcodENSEMBL2PROBE (m20kcodENSEMBL), 4
m20kcodENTREZID, 5
m20kcodENZYME, 6
m20kcodENZYME2PROBE (m20kcodENZYME), 6
m20kcodGENENAME, 7
m20kcodGO, 8
m20kcodGO2ALLPROBES, 9
m20kcodGO2ALLPROBES (m20kcodGO), 8
m20kcodGO2PROBE (m20kcodGO), 8
m20kcodLOCUSID (m20kcodENTREZID), 5
m20kcodMAPCOUNTS, 10
m20kcodMGI, 11
m20kcodMGI2PROBE (m20kcodMGI), 11
m20kcodORGANISM, 11
m20kcodORGPKG (m20kcodORGANISM), 11
m20kcodPATH, 12
m20kcodPATH2PROBE (m20kcodPATH), 12
m20kcodPMID, 13
m20kcodPMID2PROBE (m20kcodPMID), 13
m20kcodREFSEQ, 14
m20kcodSYMBOL, 15
m20kcodUNIGENE, 16
m20kcodUNIPROT, 17
m20kcodUNIPROT2PROBE (m20kcodUNIPROT),
...
m20kcod (m20kcod.db), 2
m20kcod.db, 2
m20kcod_dbconn, 17
m20kcod_dbfile (m20kcod_dbconn), 17
m20kcod_dbInfo (m20kcod_dbconn), 17
