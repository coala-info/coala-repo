...
ragene10stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

ragene10stprobesetACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
ragene10stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(ragene10stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
ragene10stprobeset.db Bioconductor annotation data package

Description

Welcome to the ragene10stprobeset.db annotation Package. The purpose of this package is to pro-
vide detailed information about the ragene10stprobeset platform. This package is updated biannu-
ally.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

ragene10stprobesetCHR
...
## use of select().
ls("package:ragene10stprobeset.db")

ragene10stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

ragene10stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
ragene10stprobesetCHRLOC

ragene10stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

ragene10stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- ragene10stprobesetCHRLENGTHS
...
ragene10stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
ragene10stprobesetENSEMBL is an R object that contains mappings between manufacturer identi-
...
x <- ragene10stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
ragene10stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

ragene10stprobesetENZYME
...
ragene10stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. ragene10stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the ragene10stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the ragene10stprobesetENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In ragene10stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert ragene10stprobesetENZYME2PROBE to a list to see inside
x <- ragene10stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
ragene10stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- ragene10stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

ragene10stprobesetGO
...
ragene10stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

ragene10stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(ragene10stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

ragene10stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
ragene10stprobesetGO2PROBE.

Details

If ragene10stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If ragene10stprobesetGO2ALLPROBES or ragene10stprobesetGO2PROBE is cast as a list, each
...
• ragene10stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
x <- ragene10stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert ragene10stprobesetGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
ragene10stprobesetMAPCOUNTS provides the "map count" (i.e.
each map in package ragene10stprobeset.db.
...
The Organism information for ragene10stprobeset

Description

ragene10stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which ragene10stprobeset was built. ragene10stprobesetORGPKG is an R
...
ragene10stprobesetPATH

Details

13

Although the package name is suggestive of the organism for which it was built, ragene10stprobesetORGANISM
provides a simple way to programmatically extract the organism name. ragene10stprobesetORGPKG
...
ragene10stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

ragene10stprobesetPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
x <- ragene10stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the ragene10stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
ragene10stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
ragene10stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. ragene10stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When ragene10stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When ragene10stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- ragene10stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object ragene10stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
ragene10stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

ragene10stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
ragene10stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
ragene10stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
...
ragene10stprobesetUNIPROT is an R object that contains mappings between the manufacturer iden-
...
ragene10stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by ragene10stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
ragene10stprobeset_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).
ragene10stprobeset_dbschema prints the schema definition of the package annotation DB.
ragene10stprobeset_dbInfo prints other information about the package annotation DB.

Value

ragene10stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
ragene10stprobeset_dbfile: a character string with the path to the package annotation DB.
...
ragene10stprobeset.db, 2
ragene10stprobeset_dbconn, 19
ragene10stprobesetACCNUM, 1
ragene10stprobesetALIAS2PROBE, 2
ragene10stprobesetCHR, 3
ragene10stprobesetCHRLENGTHS, 4
ragene10stprobesetCHRLOC, 4
ragene10stprobesetENSEMBL, 5
ragene10stprobesetENTREZID, 6
ragene10stprobesetENZYME, 7
ragene10stprobesetGENENAME, 9
ragene10stprobesetGO, 10
ragene10stprobesetMAPCOUNTS, 12
ragene10stprobesetORGANISM, 12
ragene10stprobesetPATH, 13
ragene10stprobesetPFAM, 14
ragene10stprobesetPMID, 15
ragene10stprobesetPROSITE, 16
ragene10stprobesetREFSEQ, 16
ragene10stprobesetSYMBOL, 18
ragene10stprobesetUNIPROT, 19
...
ragene10stprobesetCHRLENGTHS, 4
ragene10stprobesetCHRLOC, 4
ragene10stprobesetCHRLOCEND

(ragene10stprobesetCHRLOC), 4

ragene10stprobesetENSEMBL, 5
ragene10stprobesetENSEMBL2PROBE

(ragene10stprobesetENSEMBL), 5

ragene10stprobesetENTREZID, 6
ragene10stprobesetENZYME, 7
ragene10stprobesetENZYME2PROBE

(ragene10stprobesetENZYME), 7

ragene10stprobesetGENENAME, 9
ragene10stprobesetGO, 10
ragene10stprobesetGO2ALLPROBES, 11
ragene10stprobesetGO2ALLPROBES
...
ragene10stprobesetPATH, 13
ragene10stprobesetPATH2PROBE

(ragene10stprobesetPATH), 13

ragene10stprobesetPFAM, 14
ragene10stprobesetPMID, 15
ragene10stprobesetPMID2PROBE
...
ragene10stprobesetUNIPROT2PROBE
