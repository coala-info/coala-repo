...
* [4 Visualizing multiple arrays](#visualizing-multiple-arrays)
* [5 Probe-level linear models](#probe-level-linear-models)
* [6 Transformations](#transformations)
* [References](#references)

# 1 Background

The aim of *mimager* is to simplify the process of imaging microarrays and inspecting them for spatial artifacts by providing a single visualization function (`mimage()`) that works consistently with many of Bioconductor’s microarray object classes. Currently, *mimager* supports `AffyBatch` objects from the *[affy](https://bioconductor.org/packages/3.22/affy)* package, `PLMset` objects from the *[affyPLM](https://bioconductor.org/packages/3.22/affyPLM)* package, `ExpressionFeatureSet`, `ExonFeatureSet`, `GeneFeatureSet` and `SnpFeatureSet` classes from the *[oligoClasses](https://bioconductor.org/packages/3.22/oligoClasses)* package, and the `oligoPLM` class from the *[oligo](https://bioconductor.org/packages/3.22/oligo)* package.
...
In this vignette we’ll work with the `Dilution` data provided by the *[affydata](https://bioconductor.org/packages/3.22/affydata)* package.
