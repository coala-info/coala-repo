...
hthgu133pluspmACCNUM Map Manufacturer identifiers to Accession Numbers

Description

hthgu133pluspmACCNUM is an R object that contains mappings between a manufacturer’s identi-
...
hthgu133pluspmALIAS is an R object that provides mappings between common gene symbol iden-
...
xx <- as.list(hthgu133pluspmALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hthgu133pluspm.db annotation Package. The purpose of this package is to provide
detailed information about the hthgu133pluspm platform. This package is updated biannually.
...
hthgu133pluspmCHR is an R object that provides mappings between a manufacturer identifier and
...
hthgu133pluspmCHRLOC

hthgu133pluspmCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hthgu133pluspmCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- hthgu133pluspmCHRLENGTHS
# Length of chromosome 1
tt["1"]

hthgu133pluspmCHRLOC Map Manufacturer IDs to Chromosomal Location

Description

hthgu133pluspmCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
hthgu133pluspmENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

hthgu133pluspmENSEMBL is an R object that contains mappings between manufacturer identifiers
...
hthgu133pluspmENTREZID is an R object that provides mappings between manufacturer identi-
fiers and Entrez Gene identifiers.

hthgu133pluspmENZYME
...
hthgu133pluspmENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

hthgu133pluspmENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. hthgu133pluspmENZYME2PROBE is an R object that maps Enzyme Commis-
sion (EC) numbers to manufacturer identifiers.

Details

When the hthgu133pluspmENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the hthgu133pluspmENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In hthgu133pluspmENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
hthgu133pluspmGENENAME

9

# Now convert hthgu133pluspmENZYME2PROBE to a list to see inside
x <- hthgu133pluspmENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hthgu133pluspmGENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
hthgu133pluspmGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hthgu133pluspmGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

hthgu133pluspmGO2ALLPROBES is an R object that provides mappings between a given GO
...
hthgu133pluspmGO2PROBE.

Details

If hthgu133pluspmGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hthgu133pluspmGO2ALLPROBES or hthgu133pluspmGO2PROBE is cast as a list, each GO
...
# Convert hthgu133pluspmGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hthgu133pluspmMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package hthgu133pluspm.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hthgu133pluspmMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package hthgu133pluspm.db.
...
hthgu133pluspmOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hthgu133pluspm

Description

hthgu133pluspmORGANISM is an R object that contains a single item: a character string that
names the organism for which hthgu133pluspm was built. hthgu133pluspmORGPKG is an R object
...
Although the package name is suggestive of the organism for which it was built, hthgu133pluspmORGANISM
provides a simple way to programmatically extract the organism name. hthgu133pluspmORGPKG
...
hthgu133pluspmPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

hthgu133pluspmPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
x <- hthgu133pluspmPATH
mapped_probes <- mappedkeys(x)
# Now convert the hthgu133pluspmPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133pluspmPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hthgu133pluspmPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hthgu133pluspmPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hthgu133pluspmPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hthgu133pluspmPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hthgu133pluspmPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hthgu133pluspmPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133pluspmPROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers

18

Description

hthgu133pluspmREFSEQ

hthgu133pluspmPROSITE is an R object that provides mappings between manufacturer identifiers
...
hthgu133pluspmREFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

hthgu133pluspmREFSEQ is an R object that provides mappings between manufacturer identifiers
...
hthgu133pluspmSYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

hthgu133pluspmSYMBOL is an R object that provides mappings between manufacturer identifiers
...
## Bimap interface:
x <- hthgu133pluspmSYMBOL

20

hthgu133pluspmUNIPROT
...
hthgu133pluspmUNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

hthgu133pluspmUNIPROT is an R object that contains mappings between the manufacturer identi-
...
hthgu133pluspm_dbconn

21

hthgu133pluspm_dbconn Collect information about the package annotation DB
...
hthgu133pluspm_dbconn()
hthgu133pluspm_dbfile()
hthgu133pluspm_dbschema(file="", show.indices=FALSE)
...
hthgu133pluspm_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by hthgu133pluspm_dbconn or you
will break all the AnnDbObj objects defined in this package!

hthgu133pluspm_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).

hthgu133pluspm_dbschema prints the schema definition of the package annotation DB.

hthgu133pluspm_dbInfo prints other information about the package annotation DB.

Value

hthgu133pluspm_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

hthgu133pluspm_dbfile: a character string with the path to the package annotation DB.

hthgu133pluspm_dbschema: none (invisible NULL).

hthgu133pluspm_dbInfo: none (invisible NULL).
...
dbGetQuery(hthgu133pluspm_dbconn(), "SELECT COUNT(*) FROM probes")
...
hthgu133pluspm.db, 2
hthgu133pluspm_dbconn, 21
hthgu133pluspmACCNUM, 1
hthgu133pluspmALIAS2PROBE, 2
hthgu133pluspmCHR, 3
hthgu133pluspmCHRLENGTHS, 4
hthgu133pluspmCHRLOC, 4
hthgu133pluspmENSEMBL, 5
hthgu133pluspmENTREZID, 6
hthgu133pluspmENZYME, 7
hthgu133pluspmGENENAME, 9
hthgu133pluspmGO, 10
hthgu133pluspmMAP, 12
hthgu133pluspmMAPCOUNTS, 13
hthgu133pluspmOMIM, 13
hthgu133pluspmORGANISM, 14
hthgu133pluspmPATH, 15
hthgu133pluspmPFAM, 16
hthgu133pluspmPMID, 16
hthgu133pluspmPROSITE, 17
hthgu133pluspmREFSEQ, 18
hthgu133pluspmSYMBOL, 19
hthgu133pluspmUNIPROT, 20
...
hthgu133pluspm (hthgu133pluspm.db), 2
hthgu133pluspm.db, 2
hthgu133pluspm_dbconn, 21
hthgu133pluspm_dbfile
...
hthgu133pluspmCHRLENGTHS, 4
hthgu133pluspmCHRLOC, 4
hthgu133pluspmCHRLOCEND

(hthgu133pluspmCHRLOC), 4

hthgu133pluspmENSEMBL, 5
hthgu133pluspmENSEMBL2PROBE

(hthgu133pluspmENSEMBL), 5

hthgu133pluspmENTREZID, 6
hthgu133pluspmENZYME, 7
hthgu133pluspmENZYME2PROBE

(hthgu133pluspmENZYME), 7

hthgu133pluspmGENENAME, 9
hthgu133pluspmGO, 10
hthgu133pluspmGO2ALLPROBES, 11
hthgu133pluspmGO2ALLPROBES
...
hthgu133pluspmPATH, 15
hthgu133pluspmPATH2PROBE

(hthgu133pluspmPATH), 15

hthgu133pluspmPFAM, 16
hthgu133pluspmPMID, 16
hthgu133pluspmPMID2PROBE
