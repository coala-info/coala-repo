...
baseCommand: seqkit concat
label: seqkit_concat
doc: "concatenate sequences with same ID from multiple files\n\nTool homepage: https://github.com/shenwei356/seqkit"
inputs:
  ...
  - id: fill
    type:
      - 'null'
      - string
    doc: fill with N bases/residues for IDs missing in some files when using 
      -f/--full
    inputBinding:
      position: 101
      prefix: --fill
  - id: full
    type:
      - 'null'
      - boolean
    doc: keep all sequences, like full/outer join
    inputBinding:
      position: 101
      prefix: --full
  - id: id_ncbi
  ...
  - id: separator
    type:
      - 'null'
      - string
    doc: separator for descriptions of records with the same ID
    default: '|'
    inputBinding:
      position: 101
      prefix: --separator
