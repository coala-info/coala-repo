...
hwgcodACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hwgcodALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hwgcod.db annotation Package. The purpose of this package is to provide detailed
information about the hwgcod platform. This package is updated biannually.
...
hwgcodCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hwgcodCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hwgcodCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hwgcodENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hwgcodENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hwgcodENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hwgcodENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hwgcodENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hwgcodENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hwgcodENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hwgcodENZYME2PROBE to a list to see inside
xx <- as.list(hwgcodENZYME2PROBE)
if(length(xx) > 0){
...
hwgcodGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hwgcodGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hwg-
...
hwgcodGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than hwg-
codGO2PROBE.

Details

If hwgcodGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hwgcodGO2ALLPROBES or hwgcodGO2PROBE is cast as a list, each GO term maps to a
...
# Convert hwgcodGO2ALLPROBES to a list
xx <- as.list(hwgcodGO2ALLPROBES)
if(length(xx) > 0){
...
hwgcodMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hwgcod.db

Description

hwgcodMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hwgcod.db.
...
hwgcodMAPCOUNTS
mapnames <- names(hwgcodMAPCOUNTS)
hwgcodMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package hwgcod.db
checkMAPCOUNTS("hwgcod.db")
...
hwgcodOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hwgcod

Description

hwgcodORGANISM is an R object that contains a single item: a character string that names the
organism for which hwgcod was built. hwgcodORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hwgcodORGAN-
ISM provides a simple way to programmatically extract the organism name. hwgcodORGPKG
...
hwgcodPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hwgcodPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the hwgcodPATH2PROBE object to a list
xx <- as.list(hwgcodPATH2PROBE)
if(length(xx) > 0){
...
hwgcodPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hwgcodPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hwgcodPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hwgcodPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
# Now convert the reverse map object hwgcodPMID2PROBE to a list

hwgcodREFSEQ

15

xx <- as.list(hwgcodPMID2PROBE)
if(length(xx) > 0){
...
hwgcodREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hwgcodSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hwgcodUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hwgcodUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hwgcod_dbconn()
hwgcod_dbfile()
hwgcod_dbschema(file="", show.indices=FALSE)
hwgcod_dbInfo()
...
hwgcod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hwgcod_dbconn or you will break all the
AnnDbObj objects defined in this package!
hwgcod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
hwgcod_dbschema prints the schema definition of the package annotation DB.
hwgcod_dbInfo prints other information about the package annotation DB.

Value

hwgcod_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.
hwgcod_dbfile: a character string with the path to the package annotation DB.
hwgcod_dbschema: none (invisible NULL).
hwgcod_dbInfo: none (invisible NULL).
...
dbGetQuery(hwgcod_dbconn(), "SELECT COUNT(*) FROM probes")
...
hwgcod.db, 2
hwgcod_dbconn, 18
hwgcod_dbfile (hwgcod_dbconn), 18

20

hwgcod_dbInfo (hwgcod_dbconn), 18
hwgcod_dbschema (hwgcod_dbconn), 18
hwgcodACCNUM, 1
hwgcodALIAS2PROBE, 2
hwgcodCHR, 3
hwgcodCHRLENGTHS, 3
hwgcodCHRLOC, 4
hwgcodCHRLOCEND (hwgcodCHRLOC), 4
hwgcodENSEMBL, 4
hwgcodENSEMBL2PROBE (hwgcodENSEMBL), 4
hwgcodENTREZID, 5
hwgcodENZYME, 6
hwgcodENZYME2PROBE (hwgcodENZYME), 6
hwgcodGENENAME, 7
hwgcodGO, 8
hwgcodGO2ALLPROBES, 9
hwgcodGO2ALLPROBES (hwgcodGO), 8
hwgcodGO2PROBE (hwgcodGO), 8
hwgcodLOCUSID (hwgcodENTREZID), 5
hwgcodMAP, 10
hwgcodMAPCOUNTS, 11
hwgcodOMIM, 11
hwgcodORGANISM, 12
hwgcodORGPKG (hwgcodORGANISM), 12
hwgcodPATH, 13
hwgcodPATH2PROBE (hwgcodPATH), 13
hwgcodPMID, 14
hwgcodPMID2PROBE (hwgcodPMID), 14
hwgcodREFSEQ, 15
hwgcodSYMBOL, 16
hwgcodUNIGENE, 17
hwgcodUNIPROT, 17
hwgcodUNIPROT2PROBE (hwgcodUNIPROT), 17
