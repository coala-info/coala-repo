#+TITLE: LoRDEC 0.6 - README file
#+AUTHOR: Leena Salmela (leena.salmela@cs.helsinki), Eric Rivals (rivals@lirmm.fr)
#+Language:en
#+OPTIONS: H:2 num:2 toc:nil \*:t @:t ::t |:t ^:t f:t TeX:t \n:t
#+LaTeX\_CLASS\_OPTIONS: [11pt, a4paper, french, nolongtable]
#+LATEX\_HEADER: \usepackage[a4paper,hmargin=20mm,vmargin=20mm]{geometry}
#+LATEX\_HEADER: \usepackage[english]{babel} \usepackage{pslatex}
#+LATEX\_HEADER: \usepackage{graphicx}
#+LATEX\_HEADER: \usepackage[figuresleft]{rotating}
...
- for linux systems type: make install\_dep
...
-i, --long\_reads=
-k, --kmer\_len=
-o, --corrected\_read\_file=
-s, --solid\_threshold=
Optional parameters:
-t, --trials= Default: 5
-b, --branch= Default: 200
-e, --errorrate= Default: 0.4
-T, --threads= Default: use all cores
-S, --stat\_file= Default: not generated
-c, --complete\_search If not given a weak region is corrected even if all paths have not been explored. If this flag is set, a weak region is only corrrected if all alternative paths have been explored.
...
\*\* Statistics on paths
LoRDEC can generate statistics on the explored paths while correcting reads. To turn on the path statistics run LoRDEC with an additional parameter, -s, --statfile=.
...
\*\* Build and save the de Bruijn Graph
To correct long reads or to generate k-mer statistics, LoRDEC builds a de Bruijn Graph from the short reads file. This program allows to build and save the graph in a file before doing such analyses, and then to load the graph file instead of computing it from the short read file. This saves time if you reuse the graph several times. The graph is saved in [[http://fr.wikipedia.org/wiki/Hierarchical\_Data\_Format][Hierarchical Data Format]] (HDF5: version 5).
lordec-build-SR-graph [-T ] -2  -k  -s  -g  of short reads, then builds and save their de Bruijn graph for k-mers of length  and occurring at least  time
\* Examples
...
\* Changes
\*\* Version 0.6
1. Added -c, --complete\_search parameter.
\*\* Version 0.5.1
1. Fixed integer overflow in computing statistics.
\*\* Version 0.5
...
3. A target named "install\_dep" was added to the Makefile to install the last GATB-core version before compiling LoRDEC.
\*\* Version 0.4.1
...
By default GATB 1.0.5 is used. If you wish to link against older GATB use the compiler flag -DOLD\_GATB.
\*\* Version 0.3
