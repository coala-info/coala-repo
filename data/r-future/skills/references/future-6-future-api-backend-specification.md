...
*WARNING: Starting with **future** 1.40.0 (2025-04-10), we are
...
in the **[future](https://future.futureverse.org)** package for R that available on CRAN. The Future
...
**[future.apply](https://future.apply.futureverse.org)** package on CRAN that provides `future_lapply()`,
...
is documented and implemented in the **[future.tests](https://future.tests.futureverse.org)** package available
on CRAN.
...
* An S3 method of `result()` for `MyParallelFuture` that returns a
  `FutureResult` object (as defined by the **[future](https://future.futureverse.org)** package) when the
...
functions mentioned below are from the **[future](https://future.futureverse.org)** package.

### Constructor function creating a Future

The place-holder function `myparallel()` that is used by `plan()` must
inherits from class `future` such that `inherits(myparallel, "future")` is true. It must also have attribute `factory` set to the
