...
* [Phyluce in Daily Use](index.html)
  ...
  + Assembly
    - [Setup](#setup)
    - [Running the assembly](#running-the-assembly)
...
* [Phyluce in Daily Use](index.html)
* Assembly
* [View page source](../_sources/daily-use/daily-use-2-assembly.rst.txt)
...
[velvet](http://www.ebi.ac.uk/~zerbino/velvet/), [ABySS](http://www.bcgsc.ca/platform/bioinfo/software/abyss), and [spades](https://cab.spbu.ru/software/spades/) for assembly.

Most of the assembly process is automated using code within [phyluce](https://github.com/faircloth-lab/phyluce),
specifically the following 3 scripts:

* phyluce\_assembly\_assemblo\_abyss
* phyluce\_assembly\_assemblo\_spades
* phyluce\_assembly\_assemblo\_velvet

The code of each of the above programs **always** expects your input directories
to have the following structure (from the [Quality Control](daily-use-1-quality-control.html#quality-control) section):
...
[phyluce](https://github.com/faircloth-lab/phyluce) should screen your taxon name to ensure they do not contain
restricted characters. This includes `.+:"'-?!*@%^&#=/\` or names that **begin**
with a number. It’s probably best to get that all squared away now.

## Running the assembly[](#running-the-assembly "Link to this heading")
...
### General process[](#general-process "Link to this heading")
...
3. For each taxon create `$ASSEMBLY/genus-species` directory, based on config
   file entries
...
9. Link all assembly files with normalized names in $ASSEMBLY/genus-species/ into
   $ASSEMBLY/contigs/genus-species.contigs.fasta, so that all assemblies are linked
   in the same output directory.

### velvet[](#id2 "Link to this heading")
...
The directory structure created for [velvet](http://www.ebi.ac.uk/~zerbino/velvet/)-based assemblies looks like:
...
### ABySS[](#id3 "Link to this heading")
...
Following assembly, phyluce\_assembly\_assemblo\_abyss modifies
the assemblies by replacing degenerate base codes with standard nucleotide
encodings. We do this because [lastz](http://www.bx.psu.edu/~rsharris/lastz/), which we use to match contigs to
targeted UCE loci, is not compatible with degenerate IUPAC codes.

The phyluce\_assembly\_assemblo\_abyss code makes these substitutions for every site having a
degenerate code by selecting the appropriate nucleotide encoding randomly.
The code also renames the ABySS assemblies using the [velvet](http://www.ebi.ac.uk/~zerbino/velvet/) naming
...
The directory structure created for [ABySS](http://www.bcgsc.ca/platform/bioinfo/software/abyss)-based assemblies looks like:
...
### Spades[](#id5 "Link to this heading")
...
#### Results[](#id6 "Link to this heading")

The directory structure created for [spades](https://cab.spbu.ru/software/spades/)-based assemblies looks like:
...
### Common questions[](#common-questions "Link to this heading")

Question: Which assembly program do I pick?

Generally, I would suggest that you use [spades](https://cab.spbu.ru/software/spades/). It produces
reasonable contig assemblies that are longer than the assemblies built by
[velvet](http://www.ebi.ac.uk/~zerbino/velvet/), [ABySS](http://www.bcgsc.ca/platform/bioinfo/software/abyss), or [Trinity](http://trinityrnaseq.sourceforge.net/) (now removed from [phyluce](https://github.com/faircloth-lab/phyluce)). It arguable
...
[Previous](daily-use-1-quality-control.html "Quality Control")
[Next](daily-use-3-uce-processing.html "UCE Processing for Phylogenomics")
