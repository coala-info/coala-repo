...
mogene10stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

mogene10stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
mogene10stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(mogene10stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
mogene10stprobeset.db Bioconductor annotation data package

Description

Welcome to the mogene10stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the mogene10stprobeset platform. This package is updated
biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

mogene10stprobesetCHR
...
mogene10stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

mogene10stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
mogene10stprobesetCHRLOC

mogene10stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mogene10stprobesetCHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
## Bimap interface:
tt <- mogene10stprobesetCHRLENGTHS
...
mogene10stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting po-
...
mogene10stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- mogene10stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
mogene10stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

mogene10stprobesetENZYME
...
mogene10stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. mogene10stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the mogene10stprobesetENZYME maping viewed as a list, each manufacturer identifier
...
For the mogene10stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In mogene10stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert mogene10stprobesetENZYME2PROBE to a list to see inside
x <- mogene10stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mogene10stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- mogene10stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

mogene10stprobesetGO
...
mogene10stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

mogene10stprobesetGO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(mogene10stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

mogene10stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
mogene10stprobesetGO2PROBE.

Details

If mogene10stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If mogene10stprobesetGO2ALLPROBES or mogene10stprobesetGO2PROBE is cast as a list, each
...
• mogene10stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
mogene10stprobesetMGI

x <- mogene10stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mogene10stprobesetGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
mogene10stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package mogene10stprobeset.db.
...
mogene10stprobesetMGI Map MGI gene accession numbers with manufacturer identifiers

Description

mogene10stprobesetMGI is an R object that contains mappings between manufacturer identifiers
...
The Organism information for mogene10stprobeset

Description

mogene10stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which mogene10stprobeset was built. mogene10stprobesetORGPKG is an
...
Although the package name is suggestive of the organism for which it was built, mogene10stprobesetORGANISM
provides a simple way to programmatically extract the organism name. mogene10stprobesetORGPKG
...
mogene10stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

mogene10stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- mogene10stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the mogene10stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mogene10stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
mogene10stprobesetPMID

Maps between Manufacturer Identifiers and PubMed Identifiers

Description

mogene10stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. mogene10stprobesetPMID2PROBE is an R object that provides mappings
...
When mogene10stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When mogene10stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- mogene10stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mogene10stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])

17

mogene10stprobesetPROSITE
...
mogene10stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
mogene10stprobesetREFSEQ

Map between Manufacturer Identifiers and RefSeq Identifiers

Description

mogene10stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
mogene10stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
fiers and gene abbreviations.

mogene10stprobesetUNIPROT
...
mogene10stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
mogene10stprobeset_dbconn()
mogene10stprobeset_dbfile()
mogene10stprobeset_dbschema(file="", show.indices=FALSE)
...
mogene10stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by mogene10stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
mogene10stprobeset_dbfile returns the path (character string) to the package annotation DB
...
mogene10stprobeset_dbschema prints the schema definition of the package annotation DB.
mogene10stprobeset_dbInfo prints other information about the package annotation DB.

Value

mogene10stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
mogene10stprobeset_dbfile: a character string with the path to the package annotation DB.
mogene10stprobeset_dbschema: none (invisible NULL).
mogene10stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(mogene10stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")
...
mogene10stprobeset.db, 2
mogene10stprobeset_dbconn, 20
mogene10stprobesetACCNUM, 1
mogene10stprobesetALIAS2PROBE, 2
mogene10stprobesetCHR, 3
mogene10stprobesetCHRLENGTHS, 4
mogene10stprobesetCHRLOC, 4
mogene10stprobesetENSEMBL, 5
mogene10stprobesetENTREZID, 6
mogene10stprobesetENZYME, 7
mogene10stprobesetGENENAME, 9
mogene10stprobesetGO, 10
mogene10stprobesetMAPCOUNTS, 12
mogene10stprobesetMGI, 12
mogene10stprobesetORGANISM, 13
mogene10stprobesetPATH, 14
mogene10stprobesetPFAM, 15
mogene10stprobesetPMID, 15
mogene10stprobesetPROSITE, 17
mogene10stprobesetREFSEQ, 17
mogene10stprobesetSYMBOL, 18
mogene10stprobesetUNIPROT, 19
...
mogene10stprobesetCHRLENGTHS, 4
mogene10stprobesetCHRLOC, 4
mogene10stprobesetCHRLOCEND

(mogene10stprobesetCHRLOC), 4

mogene10stprobesetENSEMBL, 5
mogene10stprobesetENSEMBL2PROBE

(mogene10stprobesetENSEMBL), 5

mogene10stprobesetENTREZID, 6
mogene10stprobesetENZYME, 7
mogene10stprobesetENZYME2PROBE

(mogene10stprobesetENZYME), 7

mogene10stprobesetGENENAME, 9
mogene10stprobesetGO, 10
mogene10stprobesetGO2ALLPROBES, 11
mogene10stprobesetGO2ALLPROBES
...
mogene10stprobesetLOCUSID

(mogene10stprobesetENTREZID), 6

mogene10stprobesetMAPCOUNTS, 12
mogene10stprobesetMGI, 12
mogene10stprobesetMGI2PROBE
...
mogene10stprobesetPATH, 14
mogene10stprobesetPATH2PROBE

(mogene10stprobesetPATH), 14

mogene10stprobesetPFAM, 15
mogene10stprobesetPMID, 15
mogene10stprobesetPMID2PROBE
...
mogene10stprobesetSYMBOL, 18
mogene10stprobesetUNIPROT, 19
mogene10stprobesetUNIPROT2PROBE
