...
### Tool Description
Bash completion support for the `eider` command
...
#
# 2. Open a new bash console, and type `eider [TAB][TAB]`
#
...
# 'eider (..)'. By reading entered command line parameters,
...
# _complete_eider finds which commands and subcommands have been specified
# on the command line and delegates to the appropriate function
# to generate possible options and subcommands for the last specified subcommand.
function _complete_eider() {
  ...
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} help" ];    then _picocli_eider; return $?; fi
  if [ "${COMP_LINE}" = "${COMP_WORDS[0]} generate-completion" ];    then _picocli_eider; return $?; fi

  # Find the longest sequence of subcommands and call the bash function for that subcommand.
  local cmds0=(help)
  local cmds1=(generate-completion)

  if CompWordsContainsArray "${cmds1[@]}"; then _picocli_eider_generatecompletion; return $?; fi
  if CompWordsContainsArray "${cmds0[@]}"; then _picocli_eider_help; return $?; fi

  # No subcommands were specified; generate completions for the top-level command.
  _picocli_eider; return $?;
}

# Generates completions for the options and subcommands of the `eider` command.
function _picocli_eider() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
  local prev_word=${COMP_WORDS[COMP_CWORD-1]}

  local commands="help generate-completion"
  local flag_opts="'--preserve-whitespace' '--skip-history' '--verbose' '-h' '--help' '-V' '--version'"
  local arg_opts="'-u' '--url' '-q' '--query' '-i' '--query-path' '-p' '--parameters'"
...
function _picocli_eider_generatecompletion() {
  # Get completion data
  local curr_word=${COMP_WORDS[COMP_CWORD]}
...
# `eider`, `eider.sh`, and `eider.bash` commands.
# Uses the bash `complete` builtin (see [6]) to specify that shell function
# `_complete_eider` is responsible for generating possible completions for the
