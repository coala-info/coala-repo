Just for development, GitInfo: <nil>

[![](https://bioinformatics.mdanderson.org/img/MDAnderson-Master-Logo_Texas_V_NoTag_2CRGB.png)](https://bioinformatics.mdanderson.org/%20)

[Faculty](https://www.mdanderson.org/research/departments-labs-institutes/departments-divisions/bioinformatics-and-computational-biology/faculty-staff.html "Faculty")
[Research](https://www.mdanderson.org/research/departments-labs-institutes/departments-divisions/bioinformatics-and-computational-biology/research.html "Research")
[Public Software](https://bioinformatics.mdanderson.org/public-software "Public Software")
[Public Datasets](https://bioinformatics.mdanderson.org/public-datasets "Public Datasets")
[Supplements](https://bioinformatics.mdanderson.org/public-datasets/supplements "Supplements")
[Program Support](https://bioinformatics.mdanderson.org/program-support "Program Support")
...
[Home](../../)
> [Public Software](https://bioinformatics.mdanderson.org/public-software)
...
| GitHub | [wwylab/MuSE](https://github.com/wwylab/MuSE) |
| Language | C/C++ |
| Current version | 2.0 |
...
| Status | Active |
| References | |
| Citation | Fan, Y., Xi, L., Hughes, D. S. T., Zhang, J., Zhang, J., Futreal, P. A., Wheeler, D. A., and Wang, W. *Accounting for inter-tumor heterogeneity using a sample-specific error model improves sensitivity and specificity in mutation calling for sequencing data.* **Genome Biology**. 2016. [https://dx.doi.org/10.1186%2Fs13059-016-1029-6](https://dx.doi.org/10.1186/s13059-016-1029-6) |
| Help and Support | |
| Contact | Wenyi Wang |
| Discussion | [On GitHub](https://github.com/wwylab/MuSE) |

## MuSE

The detection of somatic point mutations is a key component of cancer genomic research, which has been rapidly developing since next-generation sequencing (NGS) technology revealed its potential for describing genetic alterations in cancer. We previously launched MuSE 1.0 [1](#fn:1), a statistical approach for mutation calling based on a Markov substitution model for molecular evolution. It has been used as a major contributing caller in a consensus calling strategy by the TCGA PanCanAtlas project [2](#fn:2) and the ICGC Pan-Cancer Analysis of Whole Genomes (PCAWG) initiative [3](#fn:3).
...
The latest version MuSE 2.0 only supports Linux system with `gcc=7.0` and `git=2.0` or above. MuSE 1.0 supports both Linux system and MacOS.
Please type the following commands sequentially in the terminal to generate the executable file (`MuSE`) of MuSE 2.0:
...
The first step, `MuSE call`, takes as input (1) and (2). The BAM files require aligning all the sequence reads against the reference genome using the Burrows-Wheeler alignment tool (BWA), with either the backtrack or the maximal exact matches (MEM) algorithm [2](#fn:2). In addition, the BAM files need to be processed by following the Genome Analysis Toolkit (GATK) Best Practices [3](#fn:3),[4](#fn:4),[5](#fn:5) that include marking duplicates, realigning the paired tumor-normal BAMs jointly and recalibrating base quality scores.
...
1. Fan, Y. et al. MuSE: accounting for tumor heterogeneity using a sample-specific error model improves sensitivity and specificity in mutation calling from sequencing data. Genome biology. 178 (2016).
   [[return]](#fnref:1)
