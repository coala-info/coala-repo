...
ragene11stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

ragene11stprobesetACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
ragene11stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(ragene11stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
ragene11stprobeset.db Bioconductor annotation data package

Description

Welcome to the ragene11stprobeset.db annotation Package. The purpose of this package is to pro-
vide detailed information about the ragene11stprobeset platform. This package is updated biannu-
ally.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

ragene11stprobesetCHR
...
## use of select().
ls("package:ragene11stprobeset.db")

ragene11stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

ragene11stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
ragene11stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- ragene11stprobesetCHRLENGTHS
...
ragene11stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
ragene11stprobesetENSEMBL is an R object that contains mappings between manufacturer identi-
...
x <- ragene11stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
ragene11stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

ragene11stprobesetENZYME
...
ragene11stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. ragene11stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the ragene11stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the ragene11stprobesetENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In ragene11stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert ragene11stprobesetENZYME2PROBE to a list to see inside
x <- ragene11stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
ragene11stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- ragene11stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

ragene11stprobesetGO
...
ragene11stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

ragene11stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(ragene11stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

ragene11stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
ragene11stprobesetGO2PROBE.

Details

If ragene11stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If ragene11stprobesetGO2ALLPROBES or ragene11stprobesetGO2PROBE is cast as a list, each
...
• ragene11stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
ragene11stprobesetORGANISM

x <- ragene11stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
ragene11stprobesetMAPCOUNTS provides the "map count" (i.e.
each map in package ragene11stprobeset.db.
...
The Organism information for ragene11stprobeset

Description

ragene11stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which ragene11stprobeset was built. ragene11stprobesetORGPKG is an R
...
ragene11stprobesetPATH

Details

13

Although the package name is suggestive of the organism for which it was built, ragene11stprobesetORGANISM
provides a simple way to programmatically extract the organism name. ragene11stprobesetORGPKG
...
ragene11stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

ragene11stprobesetPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
x <- ragene11stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the ragene11stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
ragene11stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
ragene11stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. ragene11stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When ragene11stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When ragene11stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- ragene11stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object ragene11stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
ragene11stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

ragene11stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
ragene11stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
ragene11stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
...
ragene11stprobesetUNIPROT is an R object that contains mappings between the manufacturer iden-
...
ragene11stprobeset_dbconn()
ragene11stprobeset_dbfile()
ragene11stprobeset_dbschema(file="", show.indices=FALSE)
ragene11stprobeset_dbInfo()
...
ragene11stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by ragene11stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
ragene11stprobeset_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).
ragene11stprobeset_dbschema prints the schema definition of the package annotation DB.
ragene11stprobeset_dbInfo prints other information about the package annotation DB.

Value

ragene11stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
ragene11stprobeset_dbfile: a character string with the path to the package annotation DB.
ragene11stprobeset_dbschema: none (invisible NULL).
ragene11stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(ragene11stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")

ragene11stprobeset_dbschema()
...
ragene11stprobeset.db, 2
ragene11stprobeset_dbconn, 19
ragene11stprobesetACCNUM, 1
ragene11stprobesetALIAS2PROBE, 2
ragene11stprobesetCHR, 3
ragene11stprobesetCHRLENGTHS, 4
ragene11stprobesetCHRLOC, 4
ragene11stprobesetENSEMBL, 5
ragene11stprobesetENTREZID, 6
ragene11stprobesetENZYME, 7
ragene11stprobesetGENENAME, 9
ragene11stprobesetGO, 10
ragene11stprobesetMAPCOUNTS, 12
ragene11stprobesetORGANISM, 12
ragene11stprobesetPATH, 13
ragene11stprobesetPFAM, 14
ragene11stprobesetPMID, 15
ragene11stprobesetPROSITE, 16
ragene11stprobesetREFSEQ, 16
ragene11stprobesetSYMBOL, 18
ragene11stprobesetUNIPROT, 19
...
ragene11stprobesetCHRLENGTHS, 4
ragene11stprobesetCHRLOC, 4
ragene11stprobesetCHRLOCEND

(ragene11stprobesetCHRLOC), 4

ragene11stprobesetENSEMBL, 5
ragene11stprobesetENSEMBL2PROBE

(ragene11stprobesetENSEMBL), 5

ragene11stprobesetENTREZID, 6
ragene11stprobesetENZYME, 7
ragene11stprobesetENZYME2PROBE

(ragene11stprobesetENZYME), 7

ragene11stprobesetGENENAME, 9
ragene11stprobesetGO, 10
ragene11stprobesetGO2ALLPROBES, 11
ragene11stprobesetGO2ALLPROBES
...
ragene11stprobesetPATH, 13
ragene11stprobesetPATH2PROBE

(ragene11stprobesetPATH), 13

ragene11stprobesetPFAM, 14
ragene11stprobesetPMID, 15
ragene11stprobesetPMID2PROBE
...
ragene11stprobesetUNIPROT2PROBE
