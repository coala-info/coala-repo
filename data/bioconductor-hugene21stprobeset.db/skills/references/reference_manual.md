...
hugene21stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

hugene21stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
hugene21stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(hugene21stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
hugene21stprobeset.db Bioconductor annotation data package

Description

Welcome to the hugene21stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the hugene21stprobeset platform. This package is updated bian-
nually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

hugene21stprobesetCHR
...
## use of select().
ls("package:hugene21stprobeset.db")

hugene21stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

hugene21stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
hugene21stprobesetCHRLOC

hugene21stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hugene21stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- hugene21stprobesetCHRLENGTHS
...
hugene21stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
hugene21stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- hugene21stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene21stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

hugene21stprobesetENZYME
...
hugene21stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. hugene21stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the hugene21stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the hugene21stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In hugene21stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert hugene21stprobesetENZYME2PROBE to a list to see inside
x <- hugene21stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hugene21stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- hugene21stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

hugene21stprobesetGO
...
hugene21stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

hugene21stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hugene21stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

hugene21stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
hugene21stprobesetGO2PROBE.

Details

If hugene21stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If hugene21stprobesetGO2ALLPROBES or hugene21stprobesetGO2PROBE is cast as a list, each
...
• hugene21stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
hugene21stprobesetMAP

x <- hugene21stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
hugene21stprobesetMAP Map between Manufacturer Identifiers and cytogenetic maps/bands

Description

hugene21stprobesetMAP is an R object that provides mappings between manufacturer identifiers
...
hugene21stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package hugene21stprobeset.db.
...
hugene21stprobesetOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for hugene21stprobeset

Description

hugene21stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which hugene21stprobeset was built. hugene21stprobesetORGPKG is an R
...
hugene21stprobesetPATH

Details

15

Although the package name is suggestive of the organism for which it was built, hugene21stprobesetORGANISM
provides a simple way to programmatically extract the organism name. hugene21stprobesetORGPKG
...
hugene21stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

hugene21stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- hugene21stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the hugene21stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene21stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
hugene21stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. hugene21stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When hugene21stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When hugene21stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hugene21stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hugene21stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene21stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

hugene21stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
hugene21stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
hugene21stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
...
hugene21stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
hugene21stprobeset_dbconn()
hugene21stprobeset_dbfile()
hugene21stprobeset_dbschema(file="", show.indices=FALSE)
...
hugene21stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by hugene21stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
hugene21stprobeset_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).
hugene21stprobeset_dbschema prints the schema definition of the package annotation DB.
hugene21stprobeset_dbInfo prints other information about the package annotation DB.

Value

hugene21stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
hugene21stprobeset_dbfile: a character string with the path to the package annotation DB.
hugene21stprobeset_dbschema: none (invisible NULL).
hugene21stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(hugene21stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")

hugene21stprobeset_dbschema()
...
hugene21stprobeset.db, 2
hugene21stprobeset_dbconn, 21
hugene21stprobesetACCNUM, 1
hugene21stprobesetALIAS2PROBE, 2
hugene21stprobesetCHR, 3
hugene21stprobesetCHRLENGTHS, 4
hugene21stprobesetCHRLOC, 4
hugene21stprobesetENSEMBL, 5
hugene21stprobesetENTREZID, 6
hugene21stprobesetENZYME, 7
hugene21stprobesetGENENAME, 9
hugene21stprobesetGO, 10
hugene21stprobesetMAP, 12
hugene21stprobesetMAPCOUNTS, 13
hugene21stprobesetOMIM, 13
hugene21stprobesetORGANISM, 14
hugene21stprobesetPATH, 15
hugene21stprobesetPFAM, 16
hugene21stprobesetPMID, 17
hugene21stprobesetPROSITE, 18
hugene21stprobesetREFSEQ, 18
hugene21stprobesetSYMBOL, 20
hugene21stprobesetUNIPROT, 21
...
hugene21stprobesetCHRLENGTHS, 4
hugene21stprobesetCHRLOC, 4
hugene21stprobesetCHRLOCEND

(hugene21stprobesetCHRLOC), 4

hugene21stprobesetENSEMBL, 5
hugene21stprobesetENSEMBL2PROBE

(hugene21stprobesetENSEMBL), 5

hugene21stprobesetENTREZID, 6
hugene21stprobesetENZYME, 7
hugene21stprobesetENZYME2PROBE

(hugene21stprobesetENZYME), 7

hugene21stprobesetGENENAME, 9
hugene21stprobesetGO, 10
hugene21stprobesetGO2ALLPROBES, 11
hugene21stprobesetGO2ALLPROBES
...
hugene21stprobesetPATH, 15
hugene21stprobesetPATH2PROBE

(hugene21stprobesetPATH), 15

hugene21stprobesetPFAM, 16
hugene21stprobesetPMID, 17
hugene21stprobesetPMID2PROBE
...
hugene21stprobesetSYMBOL, 20
hugene21stprobesetUNIPROT, 21
hugene21stprobesetUNIPROT2PROBE
