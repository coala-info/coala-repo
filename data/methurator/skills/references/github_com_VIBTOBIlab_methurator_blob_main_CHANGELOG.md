...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2FVIBTOBIlab%2Fmethurator%2Fblob%2Fmain%2FCHANGELOG.md)
...
[History](/VIBTOBIlab/methurator/commits/main/CHANGELOG.md)

History

76 lines (48 loc) · 4.31 KB
...
[Raw](https://github.com/VIBTOBIlab/methurator/raw/refs/heads/main/CHANGELOG.md)
...
* **New output format** — `methurator_summary.yml` with Chao estimator results
  ...
  + Includes extrapolation factor (t) values from 0 to t\_max
...
* Thanks to @ewels for adding a new YAML output file which contains all data in a single file. It also contains run metadata: date, tool version, options etc. for reproducibility and future MultiQC support. An example [here](/VIBTOBIlab/methurator/blob/main/tests/data/methurator_summary.yml).
...
* Due to the restructuring above, now `methurator plot` command takes in input the methurator\_summary.yml file, to avoid calculating again the sequencing saturation curve.
...
**methurator** — a Python package for estimating sequencing saturation in bisulfite sequencing data. Read the [README file](/VIBTOBIlab/methurator/blob/main/README.md) for more information.
