...
doc: |
  Performs band calculation on Sentinel-2 L2A product bands
...
inputs:
  product_url:
    label: STAC Item URL
    doc: The STAC Item should contain Assets with Sentinel-2 product bands.
    type: string

outputs:
  chlorophyll_a:
    doc: Raw data of chlorophyll-a calculation.
    type: File
    format: "ogc:geotiff"
  ...
  cyanobacteria:
    doc: Raw data of cyanobacteria calculation.
    type: File
    format: "ogc:geotiff"
  ...
  turbidity:
    doc: Raw data of turbidity calculation.
    type: File
    format: "ogc:geotiff"
...
steps:
  download_b02_10m:
    run: download-band-sentinel2-stac-item.cwl
    in:
  ...
  reproject_b03_60m:
    run: reproject-image.cwl
    in:
      input_image: download_b03_10m/product
      output_resolution:
        valueFrom: "${ return [60, 60] }"
      output_name:
        valueFrom: B03_60m  # needed for turbidity since B01 only has 60m available
    out: [result]
  ...
  download_b01_60m:
    ...
    in:
      ...
      band:
        valueFrom: B01
      resolution:
        valueFrom: 60m  # STAC Item only provides 60m, not corresponding 10m
    out: [product]
  ...
  calculate_chlorophyll_a:
    ...
    in:
      ...
      band_c: reproject_b03_60m/result
      calc:
  ...
  calculate_cyanobacteria:
    ...
    in:
      ...
      calc:
        valueFrom: '115530*(((B.astype(float)*C.astype(float))/A.astype(float))**2.38)'
      # must chain workflow input to step input for 'name' evaluation
      product_url: product_url
      name:
