...
***Tip:*** Bash completion is very helpful when using commands like `tsv-summarize` that have many options. See [Enable bash-completion](/tsv-utils/docs/TipsAndTricks.html#enable-bash-completion) for details.