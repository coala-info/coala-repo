...
#### *Eric J. Kort (eric.kort@vai.org)*

#### *2019-02-28*

Abstract

The L1000 data set from the NIH LINCS program holds the promise to deconvolute a wide range of biological questions in transcriptional space. However, using this large and decentralized data set presents its own challenge. Here we demonstrate how to use the slinky package to simplify filtering and accessing this data in an efficient analysis pipeline.
...
* [Data selection](#data-selection)
  + [From the info file](#from-the-info-file)
  + [By querying the API](#by-querying-the-api)
...
The The Library of Integrated Network-Based Cellular Signatures (LINCS) program has created over 1 million transcriptional profiles (Subramanian et al. 2017, Keenan et al. (2018)) that detail the effects of thousands of drugs and genetic perturbagens in gene expression space (known as the “L1000” dataset as it is based on the measured expression of approximately 1000 highly variable genes).
...
* Filters out instances that have not been designated by the LINCS L1000 project as “gold” (by virtue of their low inter-replicate variability).
* Identifies the appropriate control for these samples (in this case, DMSO)
* Finds the corresponding control instances comign from the same plates as the perturbed samples.
* Loads the corresponing data from the local GCTX file (giving a warning if not all the identified samples are available in the local GCTX file)
* Assembles the expression data and metadata into a SummarizedExperiment object and returns it.
...
Alternatively, you can use the slinky package itself to fetch this file. Again, this will take a while and may fail if your connection is not rock solid.
...
sl <- Slinky(user_key, system.file("extdata", "demo.gctx", package = "slinky"),
...
```
col.ix <- which(metadata(sl)$pert_iname == "amoxicillin" & metadata(sl)$cell_id ==
"MCF7")

data <- readGCTX(sl[, col.ix])

## This would be slower: data <- readGCTX(sl)[,col.ix]
```
...
```
amox_gold_sumex <- loadL1K(sl, where_clause = list(pert_type = "trt_cp",
pert_iname = "amoxicillin", cell_id = "MCF7", is_gold = TRUE))
```
...
amox_gold_sumex <- loadL1K(sl[1:978, ], where_clause = list(pert_type = "trt_cp",
pert_iname = "amoxicillin", cell_id = "MCF7", is_gold = TRUE),
inferred = FALSE)
...
```
## [1] "CPC020_MCF7_24H_X1_F1B4_DUO52HI53LO:P17"
## [2] "CPC020_MCF7_24H_X2_F1B4_DUO52HI53LO:P17"
## [3] "CPC020_MCF7_24H_X3_F1B4_DUO52HI53LO:P17"
## [4] "CPC020_MCF7_24H_X4_F2B4_DUO52HI53LO:P17"
## [5] "CPC020_MCF7_24H_X5_B5_DUO52HI53LO:P17"
```

```
# note subsetting first will be faster as it avoids loading
# in the entire set of names from the gctx file:
...
```
# sanity check
all.equal(as.character(colnames(sl)), as.character(metadata(sl)$distil_id))
```
...
```
fda <- clue(sl, "rep_drugs", where_clause = list(status_source = list(like = "FDA Orange"),
final_status = "Launched", animal_only = "0", in_cmap = TRUE),
verbose = FALSE)

fda_pert <- clueInstances(sl, poscon = "omit", where_clause = list(pert_type = "trt_cp",
is_gold = TRUE, pert_iname = list(inq = fda$pert_iname)),
verbose = FALSE)
```

Note the `inq` syntax used above to provide an array of possible values to match. This is a vernacular of the `loopback` framework on which the clue.io service is built. Further details can be obtained from the [loopback website](https://loopback.io/doc/en/lb3/Querying-data.html), or the numerous examples at the [clue.io API](https://clue.io/api).
...
amox_and_control <- loadL1K(sl, ids = c(amox_gold, metadata(sl[,
ix])$inst_id))
```
...
amox_and_control <- loadL1K(sl, ids = c(amox_gold, ids.ctrl))
```
...
The slinky package facilitates this analysis by identifying and loading suitable data subsets necessary to either define or apply gene signatures. The package takes this one step further by formatting the data for characteristic direction analysis (Clark et al. 2014) as implemented in the `GeoDE` package.
...
Alternatively, you can simply specify a perturbagen of interest as the “treat” argument. (You can also provide additional parameters to the “where\_clause” argument to further narrow your query). Then `diffexp` will load the corresponding data, and identify and load the corresponding same-plate controls.
...
```
cd_vecs <- diffexp(sl, treat = "E2F3", where_clause = list(pert_type = "trt_sh",
cell_id = "MCF7"), split_by_plate = TRUE, verbose = FALSE)
cd_vecs[1:5, 1:3]
```

```
##      KDA001_MCF7_144H_X2 KDA001_MCF7_144H_X3 KDA001_MCF7_96H_X1
## 5720        -0.011311670       -1.042297e-02       -0.009905573
## 466         -0.006983589       -7.000988e-03        0.008619106
## 6009         0.000515203       -2.359553e-02       -0.020492520
## 2309        -0.005588067        8.400016e-03        0.008891955
## 387         -0.004151143        8.411729e-16        0.006958661
```

Here, `diffexp` returns a matrix of scores, one row per gene and one column per plate. The plates could then be summarized in some fashion. One nice approach is to take the rank product (Breitling et al. 2004) of the scores which identifies those genes that are most consistently up regulated (the smaller the rank product, the more likely the gene is to be differentially expressed.) This approach can also identify down regulated genes, of course, provided the values are sorted in the opposite direction.
...
tsne_plot <- data.frame(x = ts$Y[, 1], y = ts$Y[, 2], treatment = sumex$pert_iname,
plate = sumex$rna_plate)

ggplot(tsne_plot) + geom_point(aes(x = x, y = y, color = plate)) +
labs(x = "TSNE X", y = "TSNE Y") + theme(axis.title = element_text(face = "bold",
color = "gray"))
```
...
```
ggplot(tsne_plot) + geom_point(aes(x = x, y = y, color = treatment)) +
labs(x = "TSNE X", y = "TSNE Y") + theme(axis.title = element_text(face = "bold",
color = "gray"))
```
...
Breitling, R., P. Armengaud, A. Amtmann, and P. Herzyk. 2004. “Rank products: a simple, yet powerful, new method to detect differentially regulated genes in replicated microarray experiments.” *FEBS Lett.* 573 (1-3):83–92.

Clark, N. R., K. S. Hu, A. S. Feldmann, Y. Kou, E. Y. Chen, Q. Duan, and A. Ma’ayan. 2014. “The characteristic direction: a geometrical approach to identify differentially expressed genes.” *BMC Bioinformatics* 15 (March):79.

Keenan, A. B., S. L. Jenkins, K. M. Jagodnik, S. Koplev, E. He, D. Torre, Z. Wang, et al. 2018. “The Library of Integrated Network-Based Cellular Signatures NIH Program: System-Level Cataloging of Human Cells Response to Perturbations.” *Cell Syst* 6 (1):13–24.

Subramanian, A., R. Narayan, S. M. Corsello, D. D. Peck, T. E. Natoli, X. Lu, J. Gould, et al. 2017. “A Next Generation Connectivity Map: L1000 Platform and the First 1,000,000 Profiles.” *Cell* 171 (6):1437–52.