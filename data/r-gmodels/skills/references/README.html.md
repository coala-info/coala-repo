...
* `coefFrame` - Return model parameters in a data
  frame
* `CrossTable` - Cross Tabulation with Tests for Factor
  Independence
* `estimable` - Compute contrasts and estimable linear
  functions
* `fast.prcomp` - Efficient computation of principal
...
* `glh.test` - Test a General Linear Hypothesis for a
  Regression Model
* `make.contrasts` - Construct a User-Specified Contrast
  Matrix
