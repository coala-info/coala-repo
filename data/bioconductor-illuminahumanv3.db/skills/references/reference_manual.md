...
illuminaHumanv3ACCNUM Map Manufacturer identifiers to Accession Numbers

Description

illuminaHumanv3ACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
illuminaHumanv3ALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(illuminaHumanv3ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the illuminaHumanv3.db annotation Package. The purpose of this package is to provide
detailed information about the illuminaHumanv3 platform. This package is updated biannually.
...
illuminaHumanv3CHR is an R object that provides mappings between a manufacturer identifier and
...
illuminaHumanv3CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

illuminaHumanv3CHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
tt <- illuminaHumanv3CHRLENGTHS
# Length of chromosome 1
tt["1"]

4

illuminaHumanv3ENSEMBL

illuminaHumanv3CHRLOC Map Manufacturer IDs to Chromosomal Location

Description

illuminaHumanv3CHRLOC is an R object that maps manufacturer identifiers to the starting position
...
illuminaHumanv3ENSEMBL is an R object that contains mappings between manufacturer identi-
fiers and Ensembl gene accession numbers.

illuminaHumanv3ENTREZID
...
# Convert to a list
xx <- as.list(illuminaHumanv3ENSEMBL2PROBE)
if(length(xx) > 0){
...
illuminaHumanv3ENTREZID is an R object that provides mappings between manufacturer identi-
...
illuminaHumanv3ENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

illuminaHumanv3ENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers.
illuminaHumanv3ENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the illuminaHumanv3ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the illuminaHumanv3ENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In illuminaHumanv3ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
illuminaHumanv3GENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
illuminaHumanv3GO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(illuminaHumanv3GO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

illuminaHumanv3GO2ALLPROBES is an R object that provides mappings between a given GO
...
illuminaHumanv3GO2PROBE.

Details

If illuminaHumanv3GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If illuminaHumanv3GO2ALLPROBES or illuminaHumanv3GO2PROBE is cast as a list, each GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(illuminaHumanv3GO2PROBE)
if(length(xx) > 0){
...
# Convert illuminaHumanv3GO2ALLPROBES to a list
xx <- as.list(illuminaHumanv3GO2ALLPROBES)
if(length(xx) > 0){
...
illuminaHumanv3MAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package illuminaHumanv3.db

Description

illuminaHumanv3MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package illuminaHumanv3.db.
...
illuminaHumanv3MAPCOUNTS
mapnames <- names(illuminaHumanv3MAPCOUNTS)
illuminaHumanv3MAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package illuminaHumanv3.db
checkMAPCOUNTS("illuminaHumanv3.db")
...
We have used an extensive re-annotation of the illuminaHumanv3 probe sequences to provide ad-
...
illuminaHumanv3listNewMappings List all the custom re-annotation mappings provided by the

package

illuminaHumanv3fullReannotation Return all the re-annotation information as a matrix
illuminaHumanv3ARRAYADDRESS Array Address code used to identify the probe at the bead-

level

illuminaHumanv3NUID Lumi’s nuID (universal naming scheme for oligonucleotides) Reference:

Du et al. (2007), Biol Direct 2:16

illuminaHumanv3PROBESEQUENCE The 50 base sequence for the probe
illuminaHumanv3PROBEQUALITY Quality grade assigned to the probe: “Perfect” if it per-
...
illuminaHumanv3CODINGZONE Coding status of target sequence: intergenic / intronic / Tran-
scriptomic (“Transcriptomic” when the target transcript is non-coding or there is no informa-
tion on the coding sequence)

illuminaHumanv3GENOMICLOCATION Probe’s genomic coordinates (hg19 for human, mm9

for mouse or rn4 for rat)

illuminaHumanv3GENOMICMATCHSIMILARITY Percentage of similarity between the probe

and its best genomic match in the alignable region, taking the probe as reference

illuminaHumanv3SECONDMATCHES Genomic coordinates of second best matches between

the probe and the genome

illuminaHumanv3SECONDMATCHSIMILARITY Percentage of similarity between the probe
and its second best genomic match in the alignable region, taking the probe as reference
illuminaHumanv3TRANSCRIPTOMICMATCHSIMILARITY Percentage of similarity between

the probe and its target transcript in the alignable region, taking the probe as reference

illuminaHumanv3OTHERGENOMICMATCHES Genomic coordinates of sequences as alignable

with the probe (in terms of number of matching nucleotides) as its main target

illuminaHumanv3REPEATMASK Overlapping RepeatMasked sequences, with number of bases

overlapped by the repeat

illuminaHumanv3OVERLAPPINGSNP Overlapping annotated SNPs
illuminaHumanv3ENTREZREANNOTATED Entrez IDs
illuminaHumanv3ENSEMBLREANNOTATED Ensembl IDs
illuminaHumanv3SYMBOLREANNOTATED Gene symbol derived by re-annotation
illuminaHumanv3REPORTERGROUPID For probes marked as controls in Illuminas annota-

tion file, these gives the type of control

illuminaHumanv3REPORTERGROUPNAME Usually a more informative name for the control
...
y<- revmap(illuminaHumanv3ARRAYADDRESS)
...
x <- illuminaHumanv3PROBESEQUENCE
...
illuminaHumanv3OMIM is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv3ORGANISM

15

Examples

x <- illuminaHumanv3OMIM
...
The Organism information for illuminaHumanv3

Description

illuminaHumanv3ORGANISM is an R object that contains a single item: a character string that
names the organism for which illuminaHumanv3 was built.
illuminaHumanv3ORGPKG is an R
...
manv3ORGANISM provides a simple way to programmatically extract the organism name. illumi-
naHumanv3ORGPKG provides a simple way to programmatically extract the name of the parent
...
illuminaHumanv3PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

illuminaHumanv3PATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the illuminaHumanv3PATH2PROBE object to a list
xx <- as.list(illuminaHumanv3PATH2PROBE)
if(length(xx) > 0){
...
illuminaHumanv3PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. illuminaHumanv3PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When illuminaHumanv3PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When illuminaHumanv3PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object illuminaHumanv3PMID2PROBE to a list
xx <- as.list(illuminaHumanv3PMID2PROBE)
if(length(xx) > 0){
...
illuminaHumanv3REFSEQ

illuminaHumanv3REFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

illuminaHumanv3REFSEQ is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv3SYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

illuminaHumanv3SYMBOL is an R object that provides mappings between manufacturer identifiers
...
illuminaHumanv3UNIGENE

Map between Manufacturer Identifiers and UniGene cluster identifiers

Description

illuminaHumanv3UNIGENE is an R object that provides mappings between manufacturer identi-
...
illuminaHumanv3UNIPROT is an R object that contains mappings between the manufacturer iden-
...
illuminaHumanv3_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by illuminaHumanv3_dbconn
or you will break all the AnnDbObj objects defined in this package!

illuminaHumanv3_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).

illuminaHumanv3_dbschema prints the schema definition of the package annotation DB.

illuminaHumanv3_dbInfo prints other information about the package annotation DB.

Value

illuminaHumanv3_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.

illuminaHumanv3_dbfile: a character string with the path to the package annotation DB.

illuminaHumanv3_dbschema: none (invisible NULL).

illuminaHumanv3_dbInfo: none (invisible NULL).
...
illuminaHumanv3.db, 2
illuminaHumanv3_dbconn, 21
illuminaHumanv3ACCNUM, 1
illuminaHumanv3ALIAS2PROBE, 2
illuminaHumanv3CHR, 3
illuminaHumanv3CHRLENGTHS, 3
illuminaHumanv3CHRLOC, 4
illuminaHumanv3ENSEMBL, 4
illuminaHumanv3ENTREZID, 5
illuminaHumanv3ENZYME, 6
illuminaHumanv3GENENAME, 7
illuminaHumanv3GO, 8
illuminaHumanv3listNewMappings, 11
illuminaHumanv3MAP, 10
illuminaHumanv3MAPCOUNTS, 11
illuminaHumanv3OMIM, 14
illuminaHumanv3ORGANISM, 15
illuminaHumanv3PATH, 15
illuminaHumanv3PMID, 16
illuminaHumanv3REFSEQ, 18
illuminaHumanv3SYMBOL, 19
illuminaHumanv3UNIGENE, 19
illuminaHumanv3UNIPROT, 20
...
illuminaHumanv3 (illuminaHumanv3.db), 2
illuminaHumanv3.db, 2
illuminaHumanv3_dbconn, 21
...
illuminaHumanv3ENZYME, 6
illuminaHumanv3ENZYME2PROBE

(illuminaHumanv3ENZYME), 6

illuminaHumanv3fullReannotation
...
illuminaHumanv3GENOMICMATCHSIMILARITY
...
illuminaHumanv3TRANSCRIPTOMICMATCHSIMILARITY
...
illuminaHumanv3UNIGENE, 19
illuminaHumanv3UNIPROT, 20
illuminaHumanv3UNIPROT2PROBE

(illuminaHumanv3UNIPROT), 20

mappedkeys, 11

illuminaHumanv3GO2ALLPROBES, 9
illuminaHumanv3GO2ALLPROBES
...
illuminaHumanv3PATH, 15
illuminaHumanv3PATH2PROBE

(illuminaHumanv3PATH), 15

illuminaHumanv3PMID, 16
illuminaHumanv3PMID2PROBE
...
illuminaHumanv3SECONDMATCHSIMILARITY
