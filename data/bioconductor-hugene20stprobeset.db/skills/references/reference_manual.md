...
hugene20stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

hugene20stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
hugene20stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(hugene20stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
hugene20stprobeset.db Bioconductor annotation data package

Description

Welcome to the hugene20stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the hugene20stprobeset platform. This package is updated bian-
nually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

hugene20stprobesetCHR
...
hugene20stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

hugene20stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
hugene20stprobesetCHRLOC

hugene20stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hugene20stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- hugene20stprobesetCHRLENGTHS
...
hugene20stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
hugene20stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- hugene20stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene20stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

hugene20stprobesetENZYME
...
hugene20stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. hugene20stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the hugene20stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the hugene20stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In hugene20stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert hugene20stprobesetENZYME2PROBE to a list to see inside
x <- hugene20stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hugene20stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- hugene20stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

hugene20stprobesetGO
...
hugene20stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

hugene20stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hugene20stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

hugene20stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
hugene20stprobesetGO2PROBE.

Details

If hugene20stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If hugene20stprobesetGO2ALLPROBES or hugene20stprobesetGO2PROBE is cast as a list, each
...
• hugene20stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
hugene20stprobesetMAP

x <- hugene20stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hugene20stprobesetGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene20stprobesetMAP Map between Manufacturer Identifiers and cytogenetic maps/bands

Description

hugene20stprobesetMAP is an R object that provides mappings between manufacturer identifiers
and cytoband locations.
...
hugene20stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package hugene20stprobeset.db.
...
hugene20stprobesetOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for hugene20stprobeset

Description

hugene20stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which hugene20stprobeset was built. hugene20stprobesetORGPKG is an R
...
hugene20stprobesetPATH

Details

15

Although the package name is suggestive of the organism for which it was built, hugene20stprobesetORGANISM
provides a simple way to programmatically extract the organism name. hugene20stprobesetORGPKG
...
hugene20stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

hugene20stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- hugene20stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the hugene20stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene20stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
hugene20stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. hugene20stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When hugene20stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When hugene20stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hugene20stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hugene20stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene20stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

hugene20stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
hugene20stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
hugene20stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
...
hugene20stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
hugene20stprobeset_dbconn()
hugene20stprobeset_dbfile()
hugene20stprobeset_dbschema(file="", show.indices=FALSE)
...
hugene20stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by hugene20stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
hugene20stprobeset_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).
hugene20stprobeset_dbschema prints the schema definition of the package annotation DB.
hugene20stprobeset_dbInfo prints other information about the package annotation DB.

Value

hugene20stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
hugene20stprobeset_dbfile: a character string with the path to the package annotation DB.
hugene20stprobeset_dbschema: none (invisible NULL).
hugene20stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(hugene20stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")
...
hugene20stprobeset.db, 2
hugene20stprobeset_dbconn, 21
hugene20stprobesetACCNUM, 1
hugene20stprobesetALIAS2PROBE, 2
hugene20stprobesetCHR, 3
hugene20stprobesetCHRLENGTHS, 4
hugene20stprobesetCHRLOC, 4
hugene20stprobesetENSEMBL, 5
hugene20stprobesetENTREZID, 6
hugene20stprobesetENZYME, 7
hugene20stprobesetGENENAME, 9
hugene20stprobesetGO, 10
hugene20stprobesetMAP, 12
hugene20stprobesetMAPCOUNTS, 13
hugene20stprobesetOMIM, 13
hugene20stprobesetORGANISM, 14
hugene20stprobesetPATH, 15
hugene20stprobesetPFAM, 16
hugene20stprobesetPMID, 17
hugene20stprobesetPROSITE, 18
hugene20stprobesetREFSEQ, 18
hugene20stprobesetSYMBOL, 20
hugene20stprobesetUNIPROT, 21
...
hugene20stprobesetCHRLENGTHS, 4
hugene20stprobesetCHRLOC, 4
hugene20stprobesetCHRLOCEND

(hugene20stprobesetCHRLOC), 4

hugene20stprobesetENSEMBL, 5
hugene20stprobesetENSEMBL2PROBE

(hugene20stprobesetENSEMBL), 5

hugene20stprobesetENTREZID, 6
hugene20stprobesetENZYME, 7
hugene20stprobesetENZYME2PROBE

(hugene20stprobesetENZYME), 7

hugene20stprobesetGENENAME, 9
hugene20stprobesetGO, 10
hugene20stprobesetGO2ALLPROBES, 11
hugene20stprobesetGO2ALLPROBES
...
hugene20stprobesetPATH, 15
hugene20stprobesetPATH2PROBE

(hugene20stprobesetPATH), 15

hugene20stprobesetPFAM, 16
hugene20stprobesetPMID, 17
hugene20stprobesetPMID2PROBE
...
hugene20stprobesetSYMBOL, 20
hugene20stprobesetUNIPROT, 21
hugene20stprobesetUNIPROT2PROBE
