...
baseCommand:
  - gatk
  - TagGermlineEvents
label: gatk_TagGermlineEvents
doc: A tool for tagging possible germline events in a tumor segment file. The 
  algorithm used is very simplistic. Segments called as amplified or deleted in 
  the normal are matched by breakpoints (+/- some padding) or reciprocal 
  overlap.
inputs:
  - id: called_matched_normal_seg_file
    type: File
    doc: Matched normal called segment file.
    inputBinding:
      position: 101
      prefix: --called-matched-normal-seg-file
  - id: output
    type: string
    doc: Output TSV file identical to the tumor segment file, but with 
      additional germline tag column (POSSIBLE_GERMLINE).
    inputBinding:
      position: 101
      prefix: --output
  - id: reference
  ...
  - id: reciprocal_threshold
    type:
      - 'null'
      - float
    doc: Reciprocal threshold to match a segment (when endpoints do not match). 
      Over this threshold will cause a tumor segment to be tagged.
    inputBinding:
      position: 101
      prefix: --reciprocal-threshold
