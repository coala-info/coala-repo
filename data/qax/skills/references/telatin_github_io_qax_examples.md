...
If you know that an artifact has a specific UUID but do not know the exact location, you can find it out combining `qax` with `find`.
...
If we have a list of artifacts (*e. g.*, `*.qz?`) we can extract the full bibliography and save it to file:
...
The *view* subprogram will extract the main file and print it to the standard output, so we can combine it with [grep](https://linux.die.net/man/1/grep) to count how many lines have a ">" sign.t

```
qax view input/rep-seqs.qza | grep -c '>'
```

[Previous](/qax/usage "General usage")
