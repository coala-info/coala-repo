...
inputs:
  ...
  - id: input_file
    type: File
    doc: Input FASTA file with DNA/RNA sequences used for motif search
    inputBinding:
      position: 101
      prefix: --in
  - id: make_uniq_headers
    type:
      - 'null'
      - boolean
    doc: Make FASTA headers unique. By default, an error is thrown if identical 
      FASTA headers are encountered
    inputBinding:
      position: 101
  ...
  - id: max_seq_len
    type:
      - 'null'
      - int
    doc: Maximum sequence length required for input sequences to be included in 
      search
    inputBinding:
      position: 101
      prefix: --max-seq-len
  ...
  - id: min_seq_len
    type:
      - 'null'
      - int
    doc: Minimum sequence length required for input sequences to be included in 
      search
    inputBinding:
      position: 101
      prefix: --min-seq-len
  ...
  - id: plot_motifs
    ...
    doc: Visualize selected sequence motifs, by outputting sequence logos and 
      motif hit statistics into .html file
    inputBinding:
      position: 101
      prefix: --plot-motifs
  - id: plot_pdf
  ...
  - id: profiles
    type:
      - 'null'
      - boolean
    doc: Generate motif hit + k-mer profiles for each input sequence, and 
      compare profiles between sequences in PCA plot. Generates an HTML report
    inputBinding:
      position: 101
      prefix: --profiles
  - id: profiles_k
  ...
  - id: profiles_seq_id
    type:
      - 'null'
      - string
    doc: Specify sequence ID to highlight in profile PCA plots and to report top
      --profiles-top-n similar other sequences for, based on motif hit and k-mer
      profiles
    inputBinding:
      position: 101
      prefix: --profiles-seq-id
  - id: profiles_top_n
    type:
      - 'null'
      - int
