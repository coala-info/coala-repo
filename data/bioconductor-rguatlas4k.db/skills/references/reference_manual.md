...
rguatlas4kACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
rguatlas4kALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

rguatlas4kALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(rguatlas4kALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the rguatlas4k.db annotation Package. The purpose of this package is to provide de-
tailed information about the rguatlas4k platform. This package is updated biannually.
...
rguatlas4kCHR is an R object that provides mappings between a manufacturer identifier and the
...
rguatlas4kCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

rguatlas4kCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rguatlas4kCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
rguatlas4kENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(rguatlas4kENSEMBL2PROBE)
if(length(xx) > 0){
...
rguatlas4kENTREZID is an R object that provides mappings between manufacturer identifiers and
...
rguatlas4kENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. rguatlas4kENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the rguatlas4kENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the rguatlas4kENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In rguatlas4kENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert rguatlas4kENZYME2PROBE to a list to see inside
xx <- as.list(rguatlas4kENZYME2PROBE)
if(length(xx) > 0){
...
rguatlas4kGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rguatlas4kGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (rguat-
las4kGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

rguatlas4kGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than rguat-
las4kGO2PROBE.

Details

If rguatlas4kGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If rguatlas4kGO2ALLPROBES or rguatlas4kGO2PROBE is cast as a list, each GO term maps to a
...
# Convert rguatlas4kGO2ALLPROBES to a list
xx <- as.list(rguatlas4kGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package rguatlas4k.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rguatlas4kMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package rguatlas4k.db.
...
The Organism information for rguatlas4k

Description

rguatlas4kORGANISM is an R object that contains a single item: a character string that names
the organism for which rguatlas4k was built. rguatlas4kORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, rguatlas4kORGANISM
provides a simple way to programmatically extract the organism name. rguatlas4kORGPKG pro-
...
rguatlas4kPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

rguatlas4kPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the rguatlas4kPATH2PROBE object to a list
xx <- as.list(rguatlas4kPATH2PROBE)
if(length(xx) > 0){
...
rguatlas4kPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rguatlas4kPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rguatlas4kPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When rguatlas4kPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When rguatlas4kPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object rguatlas4kPMID2PROBE to a list
xx <- as.list(rguatlas4kPMID2PROBE)
if(length(xx) > 0){
...
rguatlas4kPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
rguatlas4kREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
rguatlas4kSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
rguatlas4kUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
rguatlas4kUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
rguatlas4k_dbconn()
rguatlas4k_dbfile()
rguatlas4k_dbschema(file="", show.indices=FALSE)
rguatlas4k_dbInfo()
...
rguatlas4k_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by rguatlas4k_dbconn or you will
break all the AnnDbObj objects defined in this package!

rguatlas4k_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

rguatlas4k_dbschema prints the schema definition of the package annotation DB.

rguatlas4k_dbInfo prints other information about the package annotation DB.

Value

rguatlas4k_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

rguatlas4k_dbfile: a character string with the path to the package annotation DB.

rguatlas4k_dbschema: none (invisible NULL).

rguatlas4k_dbInfo: none (invisible NULL).
...
dbGetQuery(rguatlas4k_dbconn(), "SELECT COUNT(*) FROM probes")
...
rguatlas4k.db, 2
rguatlas4k_dbconn, 20
rguatlas4kACCNUM, 1
rguatlas4kALIAS2PROBE, 2
rguatlas4kCHR, 3
rguatlas4kCHRLENGTHS, 4
rguatlas4kCHRLOC, 4
rguatlas4kENSEMBL, 5
rguatlas4kENTREZID, 6
rguatlas4kENZYME, 7
rguatlas4kGENENAME, 9
rguatlas4kGO, 10
rguatlas4kMAPCOUNTS, 12
rguatlas4kORGANISM, 12
rguatlas4kPATH, 13
rguatlas4kPFAM, 14
rguatlas4kPMID, 14
rguatlas4kPROSITE, 16
rguatlas4kREFSEQ, 16
rguatlas4kSYMBOL, 17
rguatlas4kUNIGENE, 18
rguatlas4kUNIPROT, 19
...
rguatlas4k (rguatlas4k.db), 2
rguatlas4k.db, 2
rguatlas4k_dbconn, 20
rguatlas4k_dbfile (rguatlas4k_dbconn),

20

rguatlas4k_dbInfo (rguatlas4k_dbconn),
...
rguatlas4kCHRLENGTHS, 4
rguatlas4kCHRLOC, 4
rguatlas4kCHRLOCEND (rguatlas4kCHRLOC),

4
rguatlas4kENSEMBL, 5
rguatlas4kENSEMBL2PROBE

(rguatlas4kENSEMBL), 5

rguatlas4kENTREZID, 6
rguatlas4kENZYME, 7
rguatlas4kENZYME2PROBE

(rguatlas4kENZYME), 7

rguatlas4kGENENAME, 9
rguatlas4kGO, 10
rguatlas4kGO2ALLPROBES, 11
rguatlas4kGO2ALLPROBES (rguatlas4kGO),

10

rguatlas4kGO2PROBE (rguatlas4kGO), 10
rguatlas4kLOCUSID (rguatlas4kENTREZID),

6

rguatlas4kMAPCOUNTS, 12
rguatlas4kORGANISM, 12
rguatlas4kORGPKG (rguatlas4kORGANISM),

12
rguatlas4kPATH, 13
rguatlas4kPATH2PROBE (rguatlas4kPATH),

13
rguatlas4kPFAM, 14
rguatlas4kPMID, 14
rguatlas4kPMID2PROBE (rguatlas4kPMID),
