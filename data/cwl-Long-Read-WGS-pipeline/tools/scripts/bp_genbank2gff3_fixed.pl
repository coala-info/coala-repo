...
for my $file ( @files ) {
    ...
    while ( my $seq = $in->next_seq() ) {
        reset_gene_variables();
        my $seq_name = $seq->accession_number;
        my $version = $seq->seq_version;;
        my $accession = $seq->accession_number;
        if (defined $version) { # Rewrite the seq_name WITH a version number so as not to get problems downstream in the GFF3 output
            $seq_name = "$accession.$version";
        } else {
            $seq_name = $accession;
        }
        my $end = $seq->length;
...
sub reset_gene_variables {
    ...
    %exonpar = ();
    %seen = ();
    warn "# Reset gene variables for new sequence record\n" if $DEBUG;
}
