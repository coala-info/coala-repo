...
rgu34aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
rgu34aALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the rgu34a.db annotation Package. The purpose of this package is to provide detailed
information about the rgu34a platform. This package is updated biannually.
...
rgu34aCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
rgu34aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rgu34aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
rgu34aENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
rgu34aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
rgu34aENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. rgu34aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the rgu34aENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the rgu34aENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In rgu34aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
rgu34aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rgu34aGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (rgu34aGO2PROBE)
...
rgu34aGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than rgu34aGO2PROBE.

Details

If rgu34aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If rgu34aGO2ALLPROBES or rgu34aGO2PROBE is cast as a list, each GO term maps to a named
...
x <- rgu34aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert rgu34aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package rgu34a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rgu34aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package rgu34a.db.
...
The Organism information for rgu34a

Description

rgu34aORGANISM is an R object that contains a single item: a character string that names the
organism for which rgu34a was built. rgu34aORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, rgu34aORGANISM
provides a simple way to programmatically extract the organism name. rgu34aORGPKG provides
...
rgu34aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

rgu34aPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- rgu34aPATH
mapped_probes <- mappedkeys(x)
# Now convert the rgu34aPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rgu34aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rgu34aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rgu34aPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When rgu34aPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When rgu34aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- rgu34aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object rgu34aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rgu34aPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
rgu34aREFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
rgu34aSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
rgu34aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
rgu34a_dbconn()
rgu34a_dbfile()
rgu34a_dbschema(file="", show.indices=FALSE)
rgu34a_dbInfo()
...
rgu34a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by rgu34a_dbconn or you will break all the
AnnDbObj objects defined in this package!

rgu34a_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

rgu34a_dbschema prints the schema definition of the package annotation DB.

rgu34a_dbInfo prints other information about the package annotation DB.

Value

rgu34a_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

rgu34a_dbfile: a character string with the path to the package annotation DB.

rgu34a_dbschema: none (invisible NULL).

rgu34a_dbInfo: none (invisible NULL).
...
rgu34aCHRLENGTHS, 4
rgu34aCHRLOC, 4
rgu34aCHRLOCEND (rgu34aCHRLOC), 4
rgu34aENSEMBL, 5
rgu34aENSEMBL2PROBE (rgu34aENSEMBL), 5
rgu34aENTREZID, 6
rgu34aENZYME, 7
rgu34aENZYME2PROBE (rgu34aENZYME), 7
rgu34aGENENAME, 9
rgu34aGO, 10
rgu34aGO2ALLPROBES, 11
rgu34aGO2ALLPROBES (rgu34aGO), 10
rgu34aGO2PROBE (rgu34aGO), 10
rgu34aLOCUSID (rgu34aENTREZID), 6
rgu34aMAPCOUNTS, 12
rgu34aORGANISM, 12
rgu34aORGPKG (rgu34aORGANISM), 12
rgu34aPATH, 13
rgu34aPATH2PROBE (rgu34aPATH), 13
rgu34aPFAM, 14
rgu34aPMID, 14
rgu34aPMID2PROBE (rgu34aPMID), 14
rgu34aPROSITE, 16
rgu34aREFSEQ, 16
rgu34aSYMBOL, 17
rgu34aUNIPROT, 18
rgu34aUNIPROT2PROBE (rgu34aUNIPROT), 18
...
rgu34a.db, 2
rgu34a_dbconn, 19
rgu34a_dbfile (rgu34a_dbconn), 19
rgu34a_dbInfo (rgu34a_dbconn), 19
rgu34a_dbschema (rgu34a_dbconn), 19
