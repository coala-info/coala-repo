...
Note, do *not* use `library()` or `loadNamespace()` to resolve these problems. It is always better to use the above `packages` approach.
...
Another example where the future assignment operator `%<-%` requires curly brackets is when using the **[magrittr](https://cran.r-project.org/package%3Dmagrittr)** infix operator `%>%`, e.g.
