...
The duplicate chain is also used when tagging duplicates with the original read. The `-t` option will add an do:Z:read\_name tag to the duplicate to show which read it is a duplicate of. This option is intended for debugging and monitoring the behaviour of samtools markdup.
