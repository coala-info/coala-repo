...
illuminaHumanWGDASLv4ACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

illuminaHumanWGDASLv4ACCNUM is an R object that contains mappings between a manufac-
...
illuminaHumanWGDASLv4ALIAS2PROBE
...
illuminaHumanWGDASLv4ALIAS is an R object that provides mappings between common gene
...
xx <- as.list(illuminaHumanWGDASLv4ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the illuminaHumanWGDASLv4.db annotation Package. The purpose of this package
is to provide detailed information about the illuminaHumanWGDASLv4 platform. This package is
updated biannually.
...
illuminaHumanWGDASLv4CHR is an R object that provides mappings between a manufacturer
...
illuminaHumanWGDASLv4CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

illuminaHumanWGDASLv4CHRLENGTHS provides the length measured in base pairs for each
of the chromosomes.
...
tt <- illuminaHumanWGDASLv4CHRLENGTHS
# Length of chromosome 1
tt["1"]

illuminaHumanWGDASLv4CHRLOC
...
illuminaHumanWGDASLv4CHRLOC is an R object that maps manufacturer identifiers to the start-
...
illuminaHumanWGDASLv4ENSEMBL is an R object that contains mappings between manufac-
turer identifiers and Ensembl gene accession numbers.
...
x <- illuminaHumanWGDASLv4ENSEMBL
...
# Convert to a list
xx <- as.list(illuminaHumanWGDASLv4ENSEMBL2PROBE)
if(length(xx) > 0){
...
illuminaHumanWGDASLv4ENZYME

illuminaHumanWGDASLv4ENTREZID

Map between Manufacturer Identifiers and Entrez Gene

Description

illuminaHumanWGDASLv4ENTREZID is an R object that provides mappings between manufac-
...
x <- illuminaHumanWGDASLv4ENTREZID
...
illuminaHumanWGDASLv4ENZYME is an R object that provides mappings between manufac-
turer identifiers and EC numbers. illuminaHumanWGDASLv4ENZYME2PROBE is an R object
...
When the illuminaHumanWGDASLv4ENZYME maping viewed as a list, each manufacturer iden-
tifier maps to a named vector containing the EC number that corresponds to the enzyme produced by
...
For the illuminaHumanWGDASLv4ENZYME2PROBE, each EC number maps to a named vector
...
numbers.
In illuminaHumanWGDASLv4ENZYME2PROBE, EC is dropped from the Enzyme
Commission numbers.
...
# Now convert illuminaHumanWGDASLv4ENZYME2PROBE to a list to see inside
xx <- as.list(illuminaHumanWGDASLv4ENZYME2PROBE)
if(length(xx) > 0){

8

illuminaHumanWGDASLv4GO
...
illuminaHumanWGDASLv4GENENAME

Map between Manufacturer IDs and Genes

Description

illuminaHumanWGDASLv4GENENAME is an R object that maps manufacturer identifiers to the
...
x <- illuminaHumanWGDASLv4GENENAME
...
illuminaHumanWGDASLv4GO is an R object that provides mappings between manufacturer iden-
tifiers and the GO identifiers that they are directly associated with. This mapping and its reverse
mapping (illuminaHumanWGDASLv4GO2PROBE) do NOT associate the child terms from the GO
ontology with the gene. Only the directly evidenced terms are represented here.

illuminaHumanWGDASLv4GO2ALLPROBES is an R object that provides mappings between a
...
than illuminaHumanWGDASLv4GO2PROBE.

Details

If illuminaHumanWGDASLv4GO is cast as a list, each manufacturer identifier is mapped to a list
...
If illuminaHumanWGDASLv4GO2ALLPROBES or illuminaHumanWGDASLv4GO2PROBE is
cast as a list, each GO term maps to a named vector of manufacturer identifiers and evidence codes.
...
illuminaHumanWGDASLv4GO2ALLPROBES.

Examples

x <- illuminaHumanWGDASLv4GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(illuminaHumanWGDASLv4GO2PROBE)
if(length(xx) > 0){
...
# Convert illuminaHumanWGDASLv4GO2ALLPROBES to a list
xx <- as.list(illuminaHumanWGDASLv4GO2ALLPROBES)
if(length(xx) > 0){
...
illuminaHumanWGDASLv4MAP is an R object that provides mappings between manufacturer
identifiers and cytoband locations.

illuminaHumanWGDASLv4MAPCOUNTS
...
Number of mapped keys for the maps in package illuminaHumanWG-
DASLv4.db

Description

illuminaHumanWGDASLv4MAPCOUNTS provides the "map count" (i.e.
keys) for each map in package illuminaHumanWGDASLv4.db.
...
illuminaHumanWGDASLv4MAPCOUNTS
mapnames <- names(illuminaHumanWGDASLv4MAPCOUNTS)
illuminaHumanWGDASLv4MAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package illuminaHumanWGDASLv4.db
checkMAPCOUNTS("illuminaHumanWGDASLv4.db")
...
We have used an extensive re-annotation of the illuminaHumanWGDASLv4 probe sequences to
provide additional information that is not captured in the standard Bioconductor packages. Whereas
Bioconductor annotations are based on the RefSeq ID that each probe maps to, our additional map-
pings provide data specific to each probe on the platform. See below for details. We recommend
using the probe quality as a form of filtering, and retaining only perfect or good probes for an
analysis.

Details of custom mappings

illuminaHumanWGDASLv4listNewMappings List all the custom re-annotation mappings pro-

vided by the package

illuminaHumanWGDASLv4fullReannotation Return all the re-annotation information as a ma-

trix

illuminaHumanWGDASLv4ARRAYADDRESS Array Address code used to identify the probe

at the bead-level

illuminaHumanWGDASLv4NUID Lumi’s nuID (universal naming scheme for oligonucleotides)

Reference: Du et al. (2007), Biol Direct 2:16

illuminaHumanWGDASLv4PROBESEQUENCE The 50 base sequence for the probe

illuminaHumanWGDASLv4PROBEQUALITY Quality grade assigned to the probe: “Perfect”
if it perfectly and uniquely matches the target transcript; “Good” if the probe, although imper-
fectly matching the target transcript, is still likely to provide considerably sensitive signal (up
to two mismatches are allowed, based on empirical evidence that the signal intensity for 50-
mer probes with less than 95% identity to the respective targets is less than 50% of the signal
associated with perfect matches *); “Bad” if the probe matches repeat sequences, intergenic
or intronic regions, or is unlikely to provide specific signal for any transcript; “No match” if it
does not match any genomic region or transcript.

illuminaHumanWGDASLv4CODINGZONE Coding status of target sequence: intergenic / in-
tronic / Transcriptomic (“Transcriptomic” when the target transcript is non-coding or there is
no information on the coding sequence)

illuminaHumanWGDASLv4GENOMICLOCATION Probe’s genomic coordinates (hg19 for hu-

man, mm9 for mouse or rn4 for rat)

illuminaHumanWGDASLv4listNewMappings

13

illuminaHumanWGDASLv4GENOMICMATCHSIMILARITY Percentage of similarity between
the probe and its best genomic match in the alignable region, taking the probe as reference

illuminaHumanWGDASLv4SECONDMATCHES Genomic coordinates of second best matches

between the probe and the genome

illuminaHumanWGDASLv4SECONDMATCHSIMILARITY Percentage of similarity between
the probe and its second best genomic match in the alignable region, taking the probe as ref-
erence

illuminaHumanWGDASLv4TRANSCRIPTOMICMATCHSIMILARITY Percentage of sim-
ilarity between the probe and its target transcript in the alignable region, taking the probe as
reference

illuminaHumanWGDASLv4OTHERGENOMICMATCHES Genomic coordinates of sequences
as alignable with the probe (in terms of number of matching nucleotides) as its main target

illuminaHumanWGDASLv4REPEATMASK Overlapping RepeatMasked sequences, with num-

ber of bases overlapped by the repeat

illuminaHumanWGDASLv4OVERLAPPINGSNP Overlapping annotated SNPs

illuminaHumanWGDASLv4ENTREZREANNOTATED Entrez IDs

illuminaHumanWGDASLv4ENSEMBLREANNOTATED Ensembl IDs

illuminaHumanWGDASLv4SYMBOLREANNOTATED Gene symbol derived by re-annotation

illuminaHumanWGDASLv4REPORTERGROUPID For probes marked as controls in Illumi-

nas annotation file, these gives the type of control

illuminaHumanWGDASLv4REPORTERGROUPNAME Usually a more informative name for
...
illuminaHumanWGDASLv4OMIM is an R object that provides mappings between manufacturer
...
illuminaHumanWGDASLv4ORGANISM

The Organism information for illuminaHumanWGDASLv4

Description

illuminaHumanWGDASLv4ORGANISM is an R object that contains a single item: a character
string that names the organism for which illuminaHumanWGDASLv4 was built. illuminaHuman-
WGDASLv4ORGPKG is an R object that contains a chararcter vector with the name of the organ-
ism package that a chip package depends on for its gene-centric annotation.

16

Details

illuminaHumanWGDASLv4PATH

Although the package name is suggestive of the organism for which it was built, illuminaHuman-
WGDASLv4ORGANISM provides a simple way to programmatically extract the organism name.
illuminaHumanWGDASLv4ORGPKG provides a simple way to programmatically extract the name
...
illuminaHumanWGDASLv4PATH maps probe identifiers to the identifiers used by KEGG for path-
ways in which the genes represented by the probe identifiers are involved

illuminaHumanWGDASLv4PATH2PROBE is an R object that provides mappings between KEGG
...
# Now convert the illuminaHumanWGDASLv4PATH2PROBE object to a list
xx <- as.list(illuminaHumanWGDASLv4PATH2PROBE)
if(length(xx) > 0){
...
illuminaHumanWGDASLv4PMID is an R object that provides mappings between manufacturer
identifiers and PubMed identifiers. illuminaHumanWGDASLv4PMID2PROBE is an R object that
provides mappings between PubMed identifiers and manufacturer identifiers.

Details

When illuminaHumanWGDASLv4PMID is viewed as a list each manufacturer identifier is mapped
...
When illuminaHumanWGDASLv4PMID2PROBE is viewed as a list each PubMed identifier is
...
# Now convert the reverse map object illuminaHumanWGDASLv4PMID2PROBE to a list
xx <- as.list(illuminaHumanWGDASLv4PMID2PROBE)
if(length(xx) > 0){
...
illuminaHumanWGDASLv4REFSEQ is an R object that provides mappings between manufacturer
...
illuminaHumanWGDASLv4SYMBOL is an R object that provides mappings between manufac-
...
illuminaHumanWGDASLv4UNIGENE is an R object that provides mappings between manufac-
...
x <- illuminaHumanWGDASLv4UNIGENE
...
illuminaHumanWGDASLv4UNIPROT is an R object that contains mappings between the manu-
facturer identifiers and Uniprot accession numbers.
...
x <- illuminaHumanWGDASLv4UNIPROT
...
illuminaHumanWGDASLv4_dbconn()
illuminaHumanWGDASLv4_dbfile()
illuminaHumanWGDASLv4_dbschema(file="", show.indices=FALSE)
illuminaHumanWGDASLv4_dbInfo()
...
illuminaHumanWGDASLv4_dbconn returns a connection object to the package annotation DB. IM-
PORTANT: Don’t call dbDisconnect on the connection object returned by illuminaHumanWGDASLv4_dbconn
or you will break all the AnnDbObj objects defined in this package!

illuminaHumanWGDASLv4_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).

illuminaHumanWGDASLv4_dbschema prints the schema definition of the package annotation DB.

illuminaHumanWGDASLv4_dbInfo prints other information about the package annotation DB.

Value

illuminaHumanWGDASLv4_dbconn: a DBIConnection object representing an open connection to
the package annotation DB.

illuminaHumanWGDASLv4_dbfile: a character string with the path to the package annotation DB.

illuminaHumanWGDASLv4_dbschema: none (invisible NULL).

illuminaHumanWGDASLv4_dbInfo: none (invisible NULL).
...
dbGetQuery(illuminaHumanWGDASLv4_dbconn(), "SELECT COUNT(*) FROM probes")

illuminaHumanWGDASLv4_dbschema()
...
illuminaHumanWGDASLv4.db, 2
illuminaHumanWGDASLv4_dbconn, 21
illuminaHumanWGDASLv4ACCNUM, 1
illuminaHumanWGDASLv4ALIAS2PROBE,

2

illuminaHumanWGDASLv4CHR, 3
illuminaHumanWGDASLv4CHRLENGTHS, 3
illuminaHumanWGDASLv4CHRLOC, 4
illuminaHumanWGDASLv4ENSEMBL, 5
illuminaHumanWGDASLv4ENTREZID, 6
illuminaHumanWGDASLv4ENZYME, 6
illuminaHumanWGDASLv4GENENAME, 8
illuminaHumanWGDASLv4GO, 8
illuminaHumanWGDASLv4listNewMappings,

12

illuminaHumanWGDASLv4MAP, 10
illuminaHumanWGDASLv4MAPCOUNTS, 11
illuminaHumanWGDASLv4OMIM, 15
illuminaHumanWGDASLv4ORGANISM, 15
illuminaHumanWGDASLv4PATH, 16
illuminaHumanWGDASLv4PMID, 17
illuminaHumanWGDASLv4REFSEQ, 18
illuminaHumanWGDASLv4SYMBOL, 19
illuminaHumanWGDASLv4UNIGENE, 20
illuminaHumanWGDASLv4UNIPROT, 21
...
illuminaHumanWGDASLv4ACCNUM, 1
illuminaHumanWGDASLv4ALIAS2PROBE, 2
illuminaHumanWGDASLv4ARRAYADDRESS
...
illuminaHumanWGDASLv4CHRLENGTHS, 3
illuminaHumanWGDASLv4CHRLOC, 4
illuminaHumanWGDASLv4CHRLOCEND

(illuminaHumanWGDASLv4CHRLOC),
4

illuminaHumanWGDASLv4CODINGZONE

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4ENSEMBL, 5
illuminaHumanWGDASLv4ENSEMBL2PROBE

(illuminaHumanWGDASLv4ENSEMBL),
5

illuminaHumanWGDASLv4ENSEMBLREANNOTATED

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4ENTREZID, 6
illuminaHumanWGDASLv4ENTREZREANNOTATED

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4ENZYME, 6
illuminaHumanWGDASLv4ENZYME2PROBE

(illuminaHumanWGDASLv4ENZYME),
6

illuminaHumanWGDASLv4fullReannotation
...
illuminaHumanWGDASLv4GENENAME, 8
illuminaHumanWGDASLv4GENOMICLOCATION

12

illuminaHumanWGDASLv4REPORTERGROUPNAME
...
illuminaHumanWGDASLv4GENOMICMATCHSIMILARITY

illuminaHumanWGDASLv4SECONDMATCHES
...
illuminaHumanWGDASLv4GO, 8
illuminaHumanWGDASLv4GO2ALLPROBES, 10
illuminaHumanWGDASLv4GO2ALLPROBES

(illuminaHumanWGDASLv4GO), 8

illuminaHumanWGDASLv4GO2PROBE
...
illuminaHumanWGDASLv4MAP, 10
illuminaHumanWGDASLv4MAPCOUNTS, 11
illuminaHumanWGDASLv4NUID

illuminaHumanWGDASLv4SECONDMATCHSIMILARITY

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4SYMBOL, 19
illuminaHumanWGDASLv4SYMBOLREANNOTATED

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4TRANSCRIPTOMICMATCHSIMILARITY
...
illuminaHumanWGDASLv4UNIGENE, 20
illuminaHumanWGDASLv4UNIPROT, 21
illuminaHumanWGDASLv4UNIPROT2PROBE
...
illuminaHumanWGDASLv4OTHERGENOMICMATCHES

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4OVERLAPPINGSNP

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4PATH, 16
illuminaHumanWGDASLv4PATH2PROBE

(illuminaHumanWGDASLv4PATH), 16

illuminaHumanWGDASLv4PMID, 17
illuminaHumanWGDASLv4PMID2PROBE

(illuminaHumanWGDASLv4PMID), 17

illuminaHumanWGDASLv4PROBEQUALITY

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4PROBESEQUENCE

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4REFSEQ, 18
illuminaHumanWGDASLv4REPEATMASK

(illuminaHumanWGDASLv4listNewMappings),
12

illuminaHumanWGDASLv4REPORTERGROUPID
