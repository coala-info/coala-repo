...
mogene11stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

mogene11stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
mogene11stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(mogene11stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
mogene11stprobeset.db Bioconductor annotation data package

Description

Welcome to the mogene11stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the mogene11stprobeset platform. This package is updated
biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

mogene11stprobesetCHR
...
## use of select().
ls("package:mogene11stprobeset.db")

mogene11stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

mogene11stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
mogene11stprobesetCHRLOC

mogene11stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mogene11stprobesetCHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
## Bimap interface:
tt <- mogene11stprobesetCHRLENGTHS
...
mogene11stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting po-
...
mogene11stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- mogene11stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
mogene11stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

mogene11stprobesetENZYME
...
mogene11stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. mogene11stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the mogene11stprobesetENZYME maping viewed as a list, each manufacturer identifier
maps to a named vector containing the EC number that corresponds to the enzyme produced by
...
For the mogene11stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In mogene11stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert mogene11stprobesetENZYME2PROBE to a list to see inside
x <- mogene11stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mogene11stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- mogene11stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

mogene11stprobesetGO
...
mogene11stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

mogene11stprobesetGO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(mogene11stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

mogene11stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
mogene11stprobesetGO2PROBE.

Details

If mogene11stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If mogene11stprobesetGO2ALLPROBES or mogene11stprobesetGO2PROBE is cast as a list, each
...
• mogene11stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
mogene11stprobesetMGI

x <- mogene11stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mogene11stprobesetGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
mogene11stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package mogene11stprobeset.db.
...
mogene11stprobesetMGI Map MGI gene accession numbers with manufacturer identifiers

Description

mogene11stprobesetMGI is an R object that contains mappings between manufacturer identifiers
...
The Organism information for mogene11stprobeset

Description

mogene11stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which mogene11stprobeset was built. mogene11stprobesetORGPKG is an
R object that contains a chararcter vector with the name of the organism package that a chip package
depends on for its gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mogene11stprobesetORGANISM
provides a simple way to programmatically extract the organism name. mogene11stprobesetORGPKG
...
mogene11stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

mogene11stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- mogene11stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the mogene11stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mogene11stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
mogene11stprobesetPMID

Maps between Manufacturer Identifiers and PubMed Identifiers

Description

mogene11stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. mogene11stprobesetPMID2PROBE is an R object that provides mappings
...
When mogene11stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When mogene11stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- mogene11stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mogene11stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])

17

mogene11stprobesetPROSITE
...
mogene11stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
mogene11stprobesetREFSEQ

Map between Manufacturer Identifiers and RefSeq Identifiers

Description

mogene11stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
mogene11stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
fiers and gene abbreviations.

mogene11stprobesetUNIPROT
...
mogene11stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
mogene11stprobeset_dbconn()
mogene11stprobeset_dbfile()
mogene11stprobeset_dbschema(file="", show.indices=FALSE)
...
mogene11stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by mogene11stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
mogene11stprobeset_dbfile returns the path (character string) to the package annotation DB
...
mogene11stprobeset_dbschema prints the schema definition of the package annotation DB.
mogene11stprobeset_dbInfo prints other information about the package annotation DB.

Value

mogene11stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
mogene11stprobeset_dbfile: a character string with the path to the package annotation DB.
mogene11stprobeset_dbschema: none (invisible NULL).
mogene11stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(mogene11stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")
...
mogene11stprobeset.db, 2
mogene11stprobeset_dbconn, 20
mogene11stprobesetACCNUM, 1
mogene11stprobesetALIAS2PROBE, 2
mogene11stprobesetCHR, 3
mogene11stprobesetCHRLENGTHS, 4
mogene11stprobesetCHRLOC, 4
mogene11stprobesetENSEMBL, 5
mogene11stprobesetENTREZID, 6
mogene11stprobesetENZYME, 7
mogene11stprobesetGENENAME, 9
mogene11stprobesetGO, 10
mogene11stprobesetMAPCOUNTS, 12
mogene11stprobesetMGI, 12
mogene11stprobesetORGANISM, 13
mogene11stprobesetPATH, 14
mogene11stprobesetPFAM, 15
mogene11stprobesetPMID, 15
mogene11stprobesetPROSITE, 17
mogene11stprobesetREFSEQ, 17
mogene11stprobesetSYMBOL, 18
mogene11stprobesetUNIPROT, 19
...
mogene11stprobesetCHRLENGTHS, 4
mogene11stprobesetCHRLOC, 4
mogene11stprobesetCHRLOCEND

(mogene11stprobesetCHRLOC), 4

mogene11stprobesetENSEMBL, 5
mogene11stprobesetENSEMBL2PROBE

(mogene11stprobesetENSEMBL), 5

mogene11stprobesetENTREZID, 6
mogene11stprobesetENZYME, 7
mogene11stprobesetENZYME2PROBE

(mogene11stprobesetENZYME), 7

mogene11stprobesetGENENAME, 9
mogene11stprobesetGO, 10
mogene11stprobesetGO2ALLPROBES, 11
mogene11stprobesetGO2ALLPROBES
...
mogene11stprobesetLOCUSID

(mogene11stprobesetENTREZID), 6

mogene11stprobesetMAPCOUNTS, 12
mogene11stprobesetMGI, 12
mogene11stprobesetMGI2PROBE
...
mogene11stprobesetPATH, 14
mogene11stprobesetPATH2PROBE

(mogene11stprobesetPATH), 14

mogene11stprobesetPFAM, 15
mogene11stprobesetPMID, 15
mogene11stprobesetPMID2PROBE
...
mogene11stprobesetSYMBOL, 18
mogene11stprobesetUNIPROT, 19
mogene11stprobesetUNIPROT2PROBE
