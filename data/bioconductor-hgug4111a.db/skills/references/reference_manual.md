...
hgug4111aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgug4111aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4111aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4111aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4111a.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4111a platform. This package is updated biannually.
...
hgug4111aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4111aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4111aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgug4111aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgug4111aENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4111aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4111aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgug4111aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgug4111aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgug4111aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hgug4111aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgug4111aENZYME2PROBE to a list to see inside
xx <- as.list(hgug4111aENZYME2PROBE)
if(length(xx) > 0){
...
hgug4111aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4111aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgug4111aGO2PROBE)
...
hgug4111aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgug4111aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgug4111aGO2ALLPROBES or hgug4111aGO2PROBE is cast as a list, each GO term maps to
...
# Convert hgug4111aGO2ALLPROBES to a list
xx <- as.list(hgug4111aGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4111aMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4111a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgug4111aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4111a.db.
...
hgug4111aOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4111a

Description

hgug4111aORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4111a was built. hgug4111aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgug4111aORGANISM
provides a simple way to programmatically extract the organism name. hgug4111aORGPKG pro-
...
hgug4111aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgug4111aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hgug4111aPATH2PROBE object to a list
xx <- as.list(hgug4111aPATH2PROBE)
if(length(xx) > 0){
...
hgug4111aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgug4111aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgug4111aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgug4111aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgug4111aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgug4111aPMID2PROBE to a list
xx <- as.list(hgug4111aPMID2PROBE)
if(length(xx) > 0){
...
hgug4111aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgug4111aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4111aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4111aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4111aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgug4111a_dbconn()
hgug4111a_dbfile()
hgug4111a_dbschema(file="", show.indices=FALSE)
hgug4111a_dbInfo()
...
hgug4111a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4111a_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgug4111a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgug4111a_dbschema prints the schema definition of the package annotation DB.
hgug4111a_dbInfo prints other information about the package annotation DB.

Value

hgug4111a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
hgug4111a_dbfile: a character string with the path to the package annotation DB.
hgug4111a_dbschema: none (invisible NULL).
hgug4111a_dbInfo: none (invisible NULL).
...
dbGetQuery(hgug4111a_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgug4111a_dbconn, 21
hgug4111aACCNUM, 1
hgug4111aALIAS2PROBE, 2
hgug4111aCHR, 3
hgug4111aCHRLENGTHS, 4
hgug4111aCHRLOC, 4
hgug4111aENSEMBL, 5
hgug4111aENTREZID, 6
hgug4111aENZYME, 7
...
hgug4111aGO, 10
hgug4111aMAP, 12
hgug4111aMAPCOUNTS, 13
hgug4111aOMIM, 13
hgug4111aORGANISM, 14
hgug4111aPATH, 15
hgug4111aPFAM, 16
hgug4111aPMID, 16
hgug4111aPROSITE, 17
...
hgug4111a (hgug4111a.db), 2
hgug4111a.db, 2
hgug4111a_dbconn, 21

hgug4111a_dbfile (hgug4111a_dbconn), 21
hgug4111a_dbInfo (hgug4111a_dbconn), 21
hgug4111a_dbschema (hgug4111a_dbconn),
...
hgug4111aCHRLENGTHS, 4
hgug4111aCHRLOC, 4
hgug4111aCHRLOCEND (hgug4111aCHRLOC), 4
hgug4111aENSEMBL, 5
hgug4111aENSEMBL2PROBE

(hgug4111aENSEMBL), 5

hgug4111aENTREZID, 6
hgug4111aENZYME, 7
hgug4111aENZYME2PROBE

(hgug4111aENZYME), 7

hgug4111aGENENAME, 9
hgug4111aGO, 10
hgug4111aGO2ALLPROBES, 11
hgug4111aGO2ALLPROBES (hgug4111aGO), 10
hgug4111aGO2PROBE (hgug4111aGO), 10
hgug4111aLOCUSID (hgug4111aENTREZID), 6
hgug4111aMAP, 12
hgug4111aMAPCOUNTS, 13
hgug4111aOMIM, 13
hgug4111aORGANISM, 14
hgug4111aORGPKG (hgug4111aORGANISM), 14
hgug4111aPATH, 15
hgug4111aPATH2PROBE (hgug4111aPATH), 15
hgug4111aPFAM, 16
hgug4111aPMID, 16
hgug4111aPMID2PROBE (hgug4111aPMID), 16
hgug4111aPROSITE, 17
hgug4111aREFSEQ, 18
hgug4111aSYMBOL, 19
hgug4111aUNIGENE, 20
hgug4111aUNIPROT, 21
hgug4111aUNIPROT2PROBE
