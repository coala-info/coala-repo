...
m10kcodACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
m10kcodALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the m10kcod.db annotation Package. The purpose of this package is to provide detailed
information about the m10kcod platform. This package is updated biannually.
...
m10kcodCHR is an R object that provides mappings between a manufacturer identifier and the
...
m10kcodCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
m10kcodCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
m10kcodENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(m10kcodENSEMBL2PROBE)
if(length(xx) > 0){
...
m10kcodENTREZID is an R object that provides mappings between manufacturer identifiers and
...
m10kcodENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. m10kcodENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the m10kcodENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the m10kcodENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In m10kcodENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert m10kcodENZYME2PROBE to a list to see inside
xx <- as.list(m10kcodENZYME2PROBE)
if(length(xx) > 0){
...
m10kcodGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
m10kcodGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (m10kcodGO2PROBE)
...
m10kcodGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If m10kcodGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If m10kcodGO2ALLPROBES or m10kcodGO2PROBE is cast as a list, each GO term maps to a
...
m10kcodMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package m10kcod.db.
...
m10kcodMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for m10kcod

Description

m10kcodORGANISM is an R object that contains a single item: a character string that names the
organism for which m10kcod was built. m10kcodORGPKG is an R object that contains a chararcter
...
Although the package name is suggestive of the organism for which it was built, m10kcodORGANISM
provides a simple way to programmatically extract the organism name. m10kcodORGPKG pro-
...
m10kcodPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

m10kcodPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the m10kcodPATH2PROBE object to a list
xx <- as.list(m10kcodPATH2PROBE)
if(length(xx) > 0){
...
m10kcodPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. m10kcodPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When m10kcodPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When m10kcodPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object m10kcodPMID2PROBE to a list
xx <- as.list(m10kcodPMID2PROBE)
if(length(xx) > 0){
...
m10kcodREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
m10kcodSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
m10kcodUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
m10kcodUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
m10kcod_dbconn()
m10kcod_dbfile()
m10kcod_dbschema(file="", show.indices=FALSE)
m10kcod_dbInfo()
...
m10kcod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by m10kcod_dbconn or you will break all the
AnnDbObj objects defined in this package!

m10kcod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

m10kcod_dbschema prints the schema definition of the package annotation DB.

m10kcod_dbInfo prints other information about the package annotation DB.

Value

m10kcod_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

m10kcod_dbfile: a character string with the path to the package annotation DB.

m10kcod_dbschema: none (invisible NULL).

m10kcod_dbInfo: none (invisible NULL).
...
dbGetQuery(m10kcod_dbconn(), "SELECT COUNT(*) FROM probes")

m10kcod_dbschema()

m10kcod_dbInfo()

m10kcod_dbschema (m10kcod_dbconn), 17
m10kcodACCNUM, 1
m10kcodALIAS2PROBE, 2
m10kcodCHR, 3
m10kcodCHRLENGTHS, 3
m10kcodCHRLOC, 4
m10kcodCHRLOCEND (m10kcodCHRLOC), 4
m10kcodENSEMBL, 4
m10kcodENSEMBL2PROBE (m10kcodENSEMBL), 4
m10kcodENTREZID, 5
m10kcodENZYME, 6
m10kcodENZYME2PROBE (m10kcodENZYME), 6
m10kcodGENENAME, 7
m10kcodGO, 8
m10kcodGO2ALLPROBES, 9
m10kcodGO2ALLPROBES (m10kcodGO), 8
m10kcodGO2PROBE (m10kcodGO), 8
m10kcodLOCUSID (m10kcodENTREZID), 5
m10kcodMAPCOUNTS, 10
m10kcodMGI, 11
m10kcodMGI2PROBE (m10kcodMGI), 11
m10kcodORGANISM, 11
m10kcodORGPKG (m10kcodORGANISM), 11
m10kcodPATH, 12
m10kcodPATH2PROBE (m10kcodPATH), 12
m10kcodPMID, 13
m10kcodPMID2PROBE (m10kcodPMID), 13
m10kcodREFSEQ, 14
m10kcodSYMBOL, 15
m10kcodUNIGENE, 16
m10kcodUNIPROT, 17
m10kcodUNIPROT2PROBE (m10kcodUNIPROT),
...
m10kcod (m10kcod.db), 2
m10kcod.db, 2
m10kcod_dbconn, 17
m10kcod_dbfile (m10kcod_dbconn), 17
m10kcod_dbInfo (m10kcod_dbconn), 17
