...
mgug4104aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mgug4104aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mgug4104aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mgug4104aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mgug4104a.db annotation Package. The purpose of this package is to provide
detailed information about the mgug4104a platform. This package is updated biannually.
...
mgug4104aCHR is an R object that provides mappings between a manufacturer identifier and the
...
mgug4104aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mgug4104aCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mgug4104aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(mgug4104aENSEMBL2PROBE)
if(length(xx) > 0){
...
mgug4104aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgug4104aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mgug4104aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mgug4104aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mgug4104aENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mgug4104aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgug4104aENZYME2PROBE to a list to see inside
xx <- as.list(mgug4104aENZYME2PROBE)
if(length(xx) > 0){
...
mgug4104aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mgug4104aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgug4104aGO2PROBE)
...
mgug4104aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mgug4104aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mgug4104aGO2ALLPROBES or mgug4104aGO2PROBE is cast as a list, each GO term maps to
...
# Convert mgug4104aGO2ALLPROBES to a list
xx <- as.list(mgug4104aGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mgug4104a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgug4104aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mgug4104a.db.
...
mgug4104aMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mgug4104a

Description

mgug4104aORGANISM is an R object that contains a single item: a character string that names
the organism for which mgug4104a was built. mgug4104aORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgug4104aORGANISM
provides a simple way to programmatically extract the organism name. mgug4104aORGPKG pro-
...
mgug4104aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

mgug4104aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the mgug4104aPATH2PROBE object to a list
xx <- as.list(mgug4104aPATH2PROBE)
if(length(xx) > 0){
...
mgug4104aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mgug4104aPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mgug4104aPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mgug4104aPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mgug4104aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object mgug4104aPMID2PROBE to a list
xx <- as.list(mgug4104aPMID2PROBE)
if(length(xx) > 0){
...
mgug4104aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgug4104aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mgug4104aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mgug4104aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
mgug4104aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mgug4104a_dbconn()
mgug4104a_dbfile()
mgug4104a_dbschema(file="", show.indices=FALSE)
mgug4104a_dbInfo()
...
mgug4104a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgug4104a_dbconn or you will break all
the AnnDbObj objects defined in this package!
mgug4104a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
mgug4104a_dbschema prints the schema definition of the package annotation DB.
mgug4104a_dbInfo prints other information about the package annotation DB.

Value

mgug4104a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
mgug4104a_dbfile: a character string with the path to the package annotation DB.
mgug4104a_dbschema: none (invisible NULL).
mgug4104a_dbInfo: none (invisible NULL).
...
dbGetQuery(mgug4104a_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgug4104a_dbconn, 20
mgug4104aACCNUM, 1
mgug4104aALIAS2PROBE, 2
mgug4104aCHR, 3
mgug4104aCHRLENGTHS, 4
mgug4104aCHRLOC, 4
mgug4104aENSEMBL, 5
mgug4104aENTREZID, 6
mgug4104aENZYME, 7
mgug4104aGENENAME, 9
mgug4104aGO, 10
mgug4104aMAPCOUNTS, 12
mgug4104aMGI, 12
mgug4104aORGANISM, 13
mgug4104aPATH, 14
mgug4104aPFAM, 15
mgug4104aPMID, 15
mgug4104aPROSITE, 16
...
mgug4104a (mgug4104a.db), 2
mgug4104a.db, 2
mgug4104a_dbconn, 20
mgug4104a_dbfile (mgug4104a_dbconn), 20

22

mgug4104a_dbInfo (mgug4104a_dbconn), 20
mgug4104a_dbschema (mgug4104a_dbconn),
...
mgug4104aCHRLENGTHS, 4
mgug4104aCHRLOC, 4
mgug4104aCHRLOCEND (mgug4104aCHRLOC), 4
mgug4104aENSEMBL, 5
mgug4104aENSEMBL2PROBE

(mgug4104aENSEMBL), 5

mgug4104aENTREZID, 6
mgug4104aENZYME, 7
mgug4104aENZYME2PROBE

(mgug4104aENZYME), 7

mgug4104aGENENAME, 9
mgug4104aGO, 10
mgug4104aGO2ALLPROBES, 11
mgug4104aGO2ALLPROBES (mgug4104aGO), 10
mgug4104aGO2PROBE (mgug4104aGO), 10
mgug4104aLOCUSID (mgug4104aENTREZID), 6
mgug4104aMAPCOUNTS, 12
mgug4104aMGI, 12
mgug4104aMGI2PROBE (mgug4104aMGI), 12
mgug4104aORGANISM, 13
mgug4104aORGPKG (mgug4104aORGANISM), 13
mgug4104aPATH, 14
mgug4104aPATH2PROBE (mgug4104aPATH), 14
mgug4104aPFAM, 15
mgug4104aPMID, 15
mgug4104aPMID2PROBE (mgug4104aPMID), 15
mgug4104aPROSITE, 16
mgug4104aREFSEQ, 17
mgug4104aSYMBOL, 18
mgug4104aUNIGENE, 19
mgug4104aUNIPROT, 20
mgug4104aUNIPROT2PROBE
