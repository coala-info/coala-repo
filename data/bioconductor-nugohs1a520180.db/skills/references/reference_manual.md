...
nugohs1a520180ACCNUM Map Manufacturer identifiers to Accession Numbers

Description

nugohs1a520180ACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
nugohs1a520180ALIAS is an R object that provides mappings between common gene symbol iden-
...
xx <- as.list(nugohs1a520180ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the nugohs1a520180.db annotation Package. The purpose of this package is to provide
detailed information about the nugohs1a520180 platform. This package is updated biannually.
...
nugohs1a520180CHR is an R object that provides mappings between a manufacturer identifier and
...
nugohs1a520180CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

nugohs1a520180CHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
tt <- nugohs1a520180CHRLENGTHS
# Length of chromosome 1
tt["1"]

4

nugohs1a520180ENSEMBL

nugohs1a520180CHRLOC Map Manufacturer IDs to Chromosomal Location

Description

nugohs1a520180CHRLOC is an R object that maps manufacturer identifiers to the starting position
...
nugohs1a520180ENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

nugohs1a520180ENSEMBL is an R object that contains mappings between manufacturer identifiers
and Ensembl gene accession numbers.

nugohs1a520180ENTREZID
...
# Convert to a list
xx <- as.list(nugohs1a520180ENSEMBL2PROBE)
if(length(xx) > 0){
...
nugohs1a520180ENTREZID is an R object that provides mappings between manufacturer identi-
...
nugohs1a520180ENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

nugohs1a520180ENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. nugohs1a520180ENZYME2PROBE is an R object that maps Enzyme Commis-
sion (EC) numbers to manufacturer identifiers.

Details

When the nugohs1a520180ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the nugohs1a520180ENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In nugohs1a520180ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert nugohs1a520180ENZYME2PROBE to a list to see inside
xx <- as.list(nugohs1a520180ENZYME2PROBE)
if(length(xx) > 0){
...
nugohs1a520180GENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
nugohs1a520180GO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(nugohs1a520180GO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

nugohs1a520180GO2ALLPROBES is an R object that provides mappings between a given GO
...
nugohs1a520180GO2PROBE.

Details

If nugohs1a520180GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If nugohs1a520180GO2ALLPROBES or nugohs1a520180GO2PROBE is cast as a list, each GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(nugohs1a520180GO2PROBE)
if(length(xx) > 0){
...
# Convert nugohs1a520180GO2ALLPROBES to a list
xx <- as.list(nugohs1a520180GO2ALLPROBES)
if(length(xx) > 0){
...
nugohs1a520180MAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package nugohs1a520180.db

Description

nugohs1a520180MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package nugohs1a520180.db.
...
nugohs1a520180MAPCOUNTS
mapnames <- names(nugohs1a520180MAPCOUNTS)
nugohs1a520180MAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package nugohs1a520180.db
checkMAPCOUNTS("nugohs1a520180.db")
...
nugohs1a520180OMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for nugohs1a520180

Description

nugohs1a520180ORGANISM is an R object that contains a single item: a character string that
names the organism for which nugohs1a520180 was built. nugohs1a520180ORGPKG is an R ob-
ject that contains a chararcter vector with the name of the organism package that a chip package
depends on for its gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, nugohs1a520180ORGANISM
provides a simple way to programmatically extract the organism name. nugohs1a520180ORGPKG
...
nugohs1a520180PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

nugohs1a520180PATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the nugohs1a520180PATH2PROBE object to a list
xx <- as.list(nugohs1a520180PATH2PROBE)
if(length(xx) > 0){
...
nugohs1a520180PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. nugohs1a520180PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When nugohs1a520180PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When nugohs1a520180PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object nugohs1a520180PMID2PROBE to a list

nugohs1a520180REFSEQ

15

xx <- as.list(nugohs1a520180PMID2PROBE)
if(length(xx) > 0){
...
nugohs1a520180REFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

nugohs1a520180REFSEQ is an R object that provides mappings between manufacturer identifiers
...
nugohs1a520180SYMBOL

x <- nugohs1a520180REFSEQ
...
nugohs1a520180SYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

nugohs1a520180SYMBOL is an R object that provides mappings between manufacturer identifiers
...
nugohs1a520180UNIGENE

17

nugohs1a520180UNIGENE Map between Manufacturer Identifiers and UniGene cluster identifiers

Description

nugohs1a520180UNIGENE is an R object that provides mappings between manufacturer identifiers
...
nugohs1a520180UNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

nugohs1a520180UNIPROT is an R object that contains mappings between the manufacturer iden-
...
nugohs1a520180_dbconn

x <- nugohs1a520180UNIPROT
...
nugohs1a520180_dbconn Collect information about the package annotation DB
...
nugohs1a520180_dbconn()
nugohs1a520180_dbfile()
nugohs1a520180_dbschema(file="", show.indices=FALSE)
nugohs1a520180_dbInfo()
...
nugohs1a520180_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by nugohs1a520180_dbconn or you
will break all the AnnDbObj objects defined in this package!
nugohs1a520180_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).
nugohs1a520180_dbschema prints the schema definition of the package annotation DB.
nugohs1a520180_dbInfo prints other information about the package annotation DB.

Value

nugohs1a520180_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
nugohs1a520180_dbfile: a character string with the path to the package annotation DB.
nugohs1a520180_dbschema: none (invisible NULL).
nugohs1a520180_dbInfo: none (invisible NULL).
...
dbGetQuery(nugohs1a520180_dbconn(), "SELECT COUNT(*) FROM probes")

nugohs1a520180_dbschema()
...
nugohs1a520180.db, 2
nugohs1a520180_dbconn, 18
nugohs1a520180ACCNUM, 1
nugohs1a520180ALIAS2PROBE, 2
nugohs1a520180CHR, 3
nugohs1a520180CHRLENGTHS, 3
nugohs1a520180CHRLOC, 4
nugohs1a520180ENSEMBL, 4
nugohs1a520180ENTREZID, 5
nugohs1a520180ENZYME, 6
nugohs1a520180GENENAME, 7
nugohs1a520180GO, 8
nugohs1a520180MAP, 10
nugohs1a520180MAPCOUNTS, 11
nugohs1a520180OMIM, 11
nugohs1a520180ORGANISM, 12
nugohs1a520180PATH, 13
nugohs1a520180PMID, 14
nugohs1a520180REFSEQ, 15
nugohs1a520180SYMBOL, 16
nugohs1a520180UNIGENE, 17
nugohs1a520180UNIPROT, 17
...
nugohs1a520180 (nugohs1a520180.db), 2
nugohs1a520180.db, 2
nugohs1a520180_dbconn, 18
...
nugohs1a520180CHRLENGTHS, 3
nugohs1a520180CHRLOC, 4
nugohs1a520180CHRLOCEND

(nugohs1a520180CHRLOC), 4

nugohs1a520180ENSEMBL, 4
nugohs1a520180ENSEMBL2PROBE

(nugohs1a520180ENSEMBL), 4

nugohs1a520180ENTREZID, 5
nugohs1a520180ENZYME, 6
nugohs1a520180ENZYME2PROBE

(nugohs1a520180ENZYME), 6

nugohs1a520180GENENAME, 7
nugohs1a520180GO, 8
nugohs1a520180GO2ALLPROBES, 9
nugohs1a520180GO2ALLPROBES
...
nugohs1a520180PATH, 13
nugohs1a520180PATH2PROBE

(nugohs1a520180PATH), 13

nugohs1a520180PMID, 14
nugohs1a520180PMID2PROBE
