# argparse2tool CWL Generation Report

## argparse2tool

### Tool Description
A tool to convert argparse help text to CWL CommandLineTool.

### Metadata
- **Docker Image**: quay.io/biocontainers/argparse2tool:0.5.2--pyhdfd78af_0
- **Homepage**: https://github.com/erasche/argparse2tool
- **Package**: https://anaconda.org/channels/bioconda/packages/argparse2tool/overview
- **Validation**: PASS

- **Conda**: https://anaconda.org/channels/bioconda/packages/argparse2tool/overview
- **Total Downloads**: 3.0K
- **Last updated**: 2025-04-22
- **GitHub**: https://github.com/erasche/argparse2tool
- **Stars**: N/A
### Original Help Text
```text
/usr/local/lib/python3.13/site-packages/argparse2tool/dropins
```

