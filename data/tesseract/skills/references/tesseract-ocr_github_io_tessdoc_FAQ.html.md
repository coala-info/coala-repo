...
This is a collection of frequently asked questions and the answers, or pointers to them for **Tesseract 4**. Also see [Common errors and information for their resolution](/tessdoc/Common-Errors-and-Resolutions.html).

For the older version of the FAQ pertaining to Tesseract 2.0x, 3.0x and 4.00.00alpha, please see [FAQ Old](/tessdoc/tess3/FAQ-Old.html).
\*\*\*
...
See [Tesseract Wiki Home](/tessdoc/Home.html) page for details.
...
See the [Tesseract Wiki Data Files](/tessdoc/Data-Files.html#updated-data-files-for-version-400-september-15-2017) page for information regarding the three different types of language models available for Tesseract 4.0.0.

User contributed language models are linked from [Data Files Contributions](/tessdoc/Data-Files-Contributions.html).
...
With the configfile option set to `hocr`, tesseract will produce XHTML output compliant with the [hOCR specification](https://docs.google.com/document/preview?id=1QQnIQtvdAC_8n92-LhwPcjtAUFwBlzE8EWnKAxlgVf0&pli=1) (the input image name must be ASCII if the operating system use something other than UTF-8 encoding for filenames - see [issue 809](https://web.archive.org/web/%2A/http%3A//code.google.com/p/tesseract-ocr/issues/detail?id=809) for some details).
...
See [Tesseract Wiki Command Line Usage](/tessdoc/Command-Line-Usage.html) page for information on how to run Tesseract from the command line.
...
See [issue 579](https://web.archive.org/web/%2A/http%3A//code.google.com/p/tesseract-ocr/issues/detail?id=579). On Linux you can redirect stderr and stdout output to `/dev/null`. E.g.:
...
See [Tesseract Wiki API examples](/tessdoc/APIExample.html) page for sample programs for using the API.
...
You should note that in many cases, in order to get better OCR results, you’ll need to [improve the quality](/tessdoc/ImproveQuality.html) of the input image you are giving Tesseract.
...
Tesseract can be trained to recognize other languages or finetune existing language models. See [Tesseract Wiki Training Tesseract 4.00](/tessdoc/tess4/TrainingTesseract-4.00.html) page for information on training the LSTM engine.
...
[tessdoc](https://github.com/tesseract-ocr/tessdoc) is maintained by [tesseract-ocr](https://github.com/tesseract-ocr).
