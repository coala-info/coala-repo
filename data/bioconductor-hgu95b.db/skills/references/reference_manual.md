...
hgu95bACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu95bALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgu95b.db annotation Package. The purpose of this package is to provide detailed
information about the hgu95b platform. This package is updated biannually.
...
hgu95bCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgu95bCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu95bCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu95bENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu95bENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu95bENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu95bENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu95bENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu95bENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu95bENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu95bENZYME2PROBE to a list to see inside

hgu95bGENENAME

9

x <- hgu95bENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu95bGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu95bGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hgu95bGO2PROBE)
...
hgu95bGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu95bGO2PROBE.

Details

If hgu95bGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu95bGO2ALLPROBES or hgu95bGO2PROBE is cast as a list, each GO term maps to a named
...
x <- hgu95bGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu95bGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hgu95bMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu95b.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu95bMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu95b.db.
...
hgu95bOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu95b

Description

hgu95bORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu95b was built. hgu95bORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu95bORGANISM
provides a simple way to programmatically extract the organism name. hgu95bORGPKG provides
...
hgu95bPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu95bPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- hgu95bPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu95bPATH2PROBE object to a list
...
hgu95bPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu95bPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu95bPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu95bPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgu95bPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- hgu95bPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu95bPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu95bPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hgu95bREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu95bSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu95bUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu95b_dbconn()
hgu95b_dbfile()
hgu95b_dbschema(file="", show.indices=FALSE)
hgu95b_dbInfo()
...
hgu95b_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu95b_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgu95b_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgu95b_dbschema prints the schema definition of the package annotation DB.

hgu95b_dbInfo prints other information about the package annotation DB.

Value

hgu95b_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

hgu95b_dbfile: a character string with the path to the package annotation DB.

hgu95b_dbschema: none (invisible NULL).

hgu95b_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu95b_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu95b_dbInfo (hgu95b_dbconn), 21
hgu95b_dbschema (hgu95b_dbconn), 21
hgu95bACCNUM, 1
hgu95bALIAS2PROBE, 2
hgu95bCHR, 3
hgu95bCHRLENGTHS, 4
hgu95bCHRLOC, 4
hgu95bCHRLOCEND (hgu95bCHRLOC), 4
hgu95bENSEMBL, 5
hgu95bENSEMBL2PROBE (hgu95bENSEMBL), 5
hgu95bENTREZID, 6
hgu95bENZYME, 7
hgu95bENZYME2PROBE (hgu95bENZYME), 7
hgu95bGENENAME, 9
hgu95bGO, 10
hgu95bGO2ALLPROBES, 11
hgu95bGO2ALLPROBES (hgu95bGO), 10
hgu95bGO2PROBE (hgu95bGO), 10
hgu95bLOCUSID (hgu95bENTREZID), 6
hgu95bMAP, 12
hgu95bMAPCOUNTS, 13
hgu95bOMIM, 13
hgu95bORGANISM, 14
hgu95bORGPKG (hgu95bORGANISM), 14
hgu95bPATH, 15
hgu95bPATH2PROBE (hgu95bPATH), 15
hgu95bPFAM, 16
hgu95bPMID, 16
hgu95bPMID2PROBE (hgu95bPMID), 16
hgu95bPROSITE, 17
hgu95bREFSEQ, 18
hgu95bSYMBOL, 19
hgu95bUNIPROT, 20
hgu95bUNIPROT2PROBE (hgu95bUNIPROT), 20
...
hgu95b.db, 2
hgu95b_dbconn, 21
hgu95b_dbfile (hgu95b_dbconn), 21
