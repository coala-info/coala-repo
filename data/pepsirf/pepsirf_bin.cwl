...
inputs:
  - id: bin_size
    type:
      - 'null'
      - int
    doc: The minimum number of peptides that a bin must contain. If a bin would 
      be created with fewer than bin_size peptides, it will be combined with the
      next bin until at least bin_size peptides are found.
    inputBinding:
      position: 101
      prefix: --bin_size
  - id: round_to
    type:
      - 'null'
      - int
    doc: The 'rounding factor' for the scores parsed from the score matrix prior
      to binning. Scores found in the matrix will be rounded to the nearest 
      1/10^x for a rounding factor x.
    inputBinding:
      position: 101
      prefix: --round_to
  - id: scores
    type: File
    doc: Input tab-delimited normalized score matrix file to use for peptide 
      binning. This matrix should only contain info for the negative control 
      samples that should be used to generate bins. Peptides with similar 
      scores, summed across the negative controls, will be binned together.
    inputBinding:
      position: 101
      prefix: --scores
  - id: logfile_path
