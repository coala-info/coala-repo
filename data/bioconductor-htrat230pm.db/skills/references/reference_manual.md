...
htrat230pmACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
htrat230pmALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

htrat230pmALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(htrat230pmALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the htrat230pm.db annotation Package. The purpose of this package is to provide
detailed information about the htrat230pm platform. This package is updated biannually.
...
htrat230pmCHR is an R object that provides mappings between a manufacturer identifier and the
...
htrat230pmCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

htrat230pmCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
htrat230pmCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
htrat230pmENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
htrat230pmENTREZID is an R object that provides mappings between manufacturer identifiers and
...
htrat230pmENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. htrat230pmENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the htrat230pmENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the htrat230pmENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In htrat230pmENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert htrat230pmENZYME2PROBE to a list to see inside

htrat230pmGENENAME

9

x <- htrat230pmENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
htrat230pmGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
htrat230pmGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (htrat230pmGO2PROBE)
...
htrat230pmGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If htrat230pmGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If htrat230pmGO2ALLPROBES or htrat230pmGO2PROBE is cast as a list, each GO term maps to
...
x <- htrat230pmGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert htrat230pmGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package htrat230pm.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

htrat230pmMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package htrat230pm.db.
...
The Organism information for htrat230pm

Description

htrat230pmORGANISM is an R object that contains a single item: a character string that names
the organism for which htrat230pm was built. htrat230pmORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, htrat230pmORGANISM
provides a simple way to programmatically extract the organism name. htrat230pmORGPKG pro-
...
htrat230pmPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

htrat230pmPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- htrat230pmPATH
mapped_probes <- mappedkeys(x)
# Now convert the htrat230pmPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htrat230pmPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
htrat230pmPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. htrat230pmPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When htrat230pmPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When htrat230pmPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- htrat230pmPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object htrat230pmPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htrat230pmPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
htrat230pmREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
htrat230pmSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
htrat230pmUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
htrat230pm_dbconn()
htrat230pm_dbfile()
htrat230pm_dbschema(file="", show.indices=FALSE)
htrat230pm_dbInfo()
...
htrat230pm_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by htrat230pm_dbconn or you will
break all the AnnDbObj objects defined in this package!

htrat230pm_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

htrat230pm_dbschema prints the schema definition of the package annotation DB.

htrat230pm_dbInfo prints other information about the package annotation DB.

Value

htrat230pm_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

htrat230pm_dbfile: a character string with the path to the package annotation DB.

htrat230pm_dbschema: none (invisible NULL).

htrat230pm_dbInfo: none (invisible NULL).
...
dbGetQuery(htrat230pm_dbconn(), "SELECT COUNT(*) FROM probes")
...
htrat230pm.db, 2
htrat230pm_dbconn, 19
htrat230pmACCNUM, 1
htrat230pmALIAS2PROBE, 2
htrat230pmCHR, 3
htrat230pmCHRLENGTHS, 4
htrat230pmCHRLOC, 4
htrat230pmENSEMBL, 5
htrat230pmENTREZID, 6
htrat230pmENZYME, 7
htrat230pmGENENAME, 9
htrat230pmGO, 10
htrat230pmMAPCOUNTS, 12
htrat230pmORGANISM, 12
htrat230pmPATH, 13
htrat230pmPFAM, 14
htrat230pmPMID, 14
htrat230pmPROSITE, 16
htrat230pmREFSEQ, 16
htrat230pmSYMBOL, 17
htrat230pmUNIPROT, 18
...
htrat230pm (htrat230pm.db), 2
htrat230pm.db, 2
htrat230pm_dbconn, 19
htrat230pm_dbfile (htrat230pm_dbconn),

19

htrat230pm_dbInfo (htrat230pm_dbconn),
...
htrat230pmCHRLENGTHS, 4
htrat230pmCHRLOC, 4
htrat230pmCHRLOCEND (htrat230pmCHRLOC),

4
htrat230pmENSEMBL, 5
htrat230pmENSEMBL2PROBE

(htrat230pmENSEMBL), 5

htrat230pmENTREZID, 6
htrat230pmENZYME, 7
htrat230pmENZYME2PROBE

(htrat230pmENZYME), 7

htrat230pmGENENAME, 9
htrat230pmGO, 10
htrat230pmGO2ALLPROBES, 11
htrat230pmGO2ALLPROBES (htrat230pmGO),

10

htrat230pmGO2PROBE (htrat230pmGO), 10
htrat230pmLOCUSID (htrat230pmENTREZID),

6

htrat230pmMAPCOUNTS, 12
htrat230pmORGANISM, 12
htrat230pmORGPKG (htrat230pmORGANISM),

12
htrat230pmPATH, 13
htrat230pmPATH2PROBE (htrat230pmPATH),

13
htrat230pmPFAM, 14
htrat230pmPMID, 14
htrat230pmPMID2PROBE (htrat230pmPMID),
