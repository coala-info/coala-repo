...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2FAstrobioMike%2FGToTree%2Fwiki%2FSCG-sets)
...
Mike Lee edited this page Sep 26, 2024
·
[28 revisions](/AstrobioMike/GToTree/wiki/SCG-sets/_history)

There are 15 single copy gene (SCG)-set HMMs included with GToTree spanning multiple taxonomic ranks. 14 of these are newly created as described below, and one set is included that covers all 3 domains (from [Hug et al. 2016](https://www.nature.com/articles/nmicrobiol201648)).
...
More information on each is provided in the "hmm-sources-and-info.tsv" file found [here](https://github.com/AstrobioMike/GToTree/blob/master/hmm_sets/hmm-sources-and-info.tsv), and in the GToTree installed location (can see by running `gtt-hmms`).
...
> Note that there is a helper program packaged with GToTree now that enables doing this with the latest PFams and any input group of target organisms, `gtt-gen-SCG-HMMs`.
...
# cutting down to just pfam ID and coverage (in later versions column 36 became column 34
...
Note that one of those also needs a script that's available from [my toolset here](https://github.com/AstrobioMike/bit#bioinformatics-tools-bit) (`bit-rename-fasta-headers`). Sorry :( this is just here for reproducibility's/posterity's sake, not for mass use (now the way to do this would be with `gtt-gen-SCG-HMMs`). The bit toolset [can be installed with conda](https://github.com/AstrobioMike/bioinf_tools#conda-install), or you can grab just the script from [here](https://raw.githubusercontent.com/AstrobioMike/bit/master/bit/bit-rename-fasta-headers) and it should work fine if you have biopython installed.
...
bash unzip_rename_cat_genes.sh all_