...
inputs:
  ...
  - id: no_theta_pi
    type:
      - 'null'
      - boolean
    doc: Do not compute or output Theta Pi. Note that if Tajima's D is computed,
      we also need to compute the two thetas, but will then not print them in 
      the output.
    inputBinding:
      position: 101
      prefix: --no-theta-pi
  - id: no_theta_watterson
    type:
      - 'null'
      - boolean
    doc: Do not compute or output Theta Watterson. Note that if Tajima's D is 
      computed, we also need to compute the two thetas, but will then not print 
      them in the output.
    inputBinding:
      position: 101
      prefix: --no-theta-watterson
  - id: out_dir
    type:
      - 'null'
      - Directory
  ...
  - id: pool_sizes
    ...
    inputBinding:
      position: 101
      prefix: --pool-sizes
  - id: popoolation_format
    type:
      - 'null'
      - boolean
    doc: If set, instead of writing one output table for all measures and all 
      samples, write the results in separate files for each sample and for each 
      measure of Theta Pi, Theta Watterson, and Tajima's D, following the format
      of PoPoolation.
    inputBinding:
      position: 101
      prefix: --popoolation-format
  ...
  - id: sync_path
    ...
    inputBinding:
      position: 101
      prefix: --sync-path
  - id: tajima_d_denominator_policy
    type:
      - 'null'
      - string
    doc: "Estimator for the expected number of distinct individuals sequenced in the
      denominator of the Achaz (2008) correction of Tajima's D, following its adaptation
      by PoPoolation. With pool seq data, there is no simple way to obtain a statistic
      that is numerically comparable to the classic Tajima's D with individual data.
      Hence, all of the below are simplicications that introduce some bias.\n(1) `empirical-min-read-depth`:
      Use the lowest empirical read depth found in each window, and the pool size,
      to compute the expected number of individuals sequenced. This is a conservative
      estimator that we recommend by default.\n(2) `provided-min-read-depth`: Same
      as (1), but use the user-provided `--filter-sample-min-read-depth` instead of
      the empirical minum read depth. This is what PoPoolation uses.\n(3) `popoolation-bugs`:
      Same as (2), but additionally re-introduce their bugs. We offer this for comparability
      with PoPoolation.\n(4) `pool-size`: Directly use the pool size as an estimate
      of the number of individuals, instead of computing the expected value. This
      assumes the number of individuals sequenced to be equal to the pool size, and
      is good under high read depths.\n(5) `uncorrected`: The Achaz correction is
...
stdout: grenedalf_diversity.out
