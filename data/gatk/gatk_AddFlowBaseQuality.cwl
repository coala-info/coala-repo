...
baseCommand:
  - gatk
  - AddFlowBaseQuality
label: gatk_AddFlowBaseQuality
doc: Prints reads from the input SAM/BAM/CRAM file to the SAM/BAM/CRAM file 
  while adding a base quality attribute.
inputs:
  - id: input
    type:
      type: array
  ...
  - id: maximal_quality_score
    type:
      - 'null'
      - int
    doc: clip quality score to the given value
    inputBinding:
      position: 101
      prefix: --maximal-quality-score
  - id: minimal_error_rate
    type:
      - 'null'
      - float
    doc: lower floor for flow error rate values
    inputBinding:
      position: 101
      prefix: --minimal-error-rate
  - id: quiet
    type:
      - 'null'
      - boolean
  ...
  - id: reference
    ...
    inputBinding:
      position: 101
      prefix: --reference
  - id: replace_quality_mode
    type:
      - 'null'
      - boolean
    doc: replace existing base qualities while saving previous qualities to OQ 
      (when true) or simply write to BQ (when false)
    inputBinding:
      position: 101
      prefix: --replace-quality-mode
