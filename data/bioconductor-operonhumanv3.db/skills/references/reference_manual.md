...
OperonHumanV3ACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
OperonHumanV3ALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(OperonHumanV3ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the OperonHumanV3.db annotation Package. The purpose of this package is to provide
detailed information about the OperonHumanV3 platform. This package is updated biannually.
...
OperonHumanV3CHR is an R object that provides mappings between a manufacturer identifier and
...
OperonHumanV3CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

OperonHumanV3CHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
OperonHumanV3CHRLOC is an R object that maps manufacturer identifiers to the starting position
...
OperonHumanV3ENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

OperonHumanV3ENSEMBL is an R object that contains mappings between manufacturer identi-
...
# Convert to a list
xx <- as.list(OperonHumanV3ENSEMBL2PROBE)
if(length(xx) > 0){
...
OperonHumanV3ENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

OperonHumanV3ENTREZID is an R object that provides mappings between manufacturer identi-
...
OperonHumanV3ENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. OperonHumanV3ENZYME2PROBE is an R object that maps Enzyme Commis-
sion (EC) numbers to manufacturer identifiers.

Details

When the OperonHumanV3ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the OperonHumanV3ENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In OperonHumanV3ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert OperonHumanV3ENZYME2PROBE to a list to see inside
xx <- as.list(OperonHumanV3ENZYME2PROBE)
if(length(xx) > 0){

# Get the probe identifiers for the first five enzyme

OperonHumanV3GENENAME
...
OperonHumanV3GENENAME Map between Manufacturer IDs and Genes

Description

OperonHumanV3GENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
OperonHumanV3GO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(OperonHumanV3GO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

OperonHumanV3GO2ALLPROBES is an R object that provides mappings between a given GO
...
OperonHumanV3GO2PROBE.

Details

If OperonHumanV3GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If OperonHumanV3GO2ALLPROBES or OperonHumanV3GO2PROBE is cast as a list, each GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(OperonHumanV3GO2PROBE)
if(length(xx) > 0){
...
# Convert OperonHumanV3GO2ALLPROBES to a list
xx <- as.list(OperonHumanV3GO2ALLPROBES)
if(length(xx) > 0){
...
OperonHumanV3MAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
OperonHumanV3MAPCOUNTS

13

OperonHumanV3MAPCOUNTS

Number of mapped keys for the maps in package OperonHumanV3.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

OperonHumanV3MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package OperonHumanV3.db.
...
OperonHumanV3OMIM is an R object that provides mappings between manufacturer identifiers
...
OperonHumanV3ORGANISM The Organism information for OperonHumanV3

Description

OperonHumanV3ORGANISM is an R object that contains a single item: a character string that
names the organism for which OperonHumanV3 was built. OperonHumanV3ORGPKG is an R
...
Although the package name is suggestive of the organism for which it was built, OperonHu-
manV3ORGANISM provides a simple way to programmatically extract the organism name. Oper-
onHumanV3ORGPKG provides a simple way to programmatically extract the name of the parent
...
OperonHumanV3PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

OperonHumanV3PATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the OperonHumanV3PATH2PROBE object to a list
xx <- as.list(OperonHumanV3PATH2PROBE)
if(length(xx) > 0){
...
OperonHumanV3PFAM is an R object that provides mappings between manufacturer identifiers
...
OperonHumanV3PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. OperonHumanV3PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When OperonHumanV3PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When OperonHumanV3PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object OperonHumanV3PMID2PROBE to a list
xx <- as.list(OperonHumanV3PMID2PROBE)
if(length(xx) > 0){
...
OperonHumanV3PROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

OperonHumanV3PROSITE is an R object that provides mappings between manufacturer identifiers
...
OperonHumanV3REFSEQ is an R object that provides mappings between manufacturer identifiers
...
OperonHumanV3SYMBOL is an R object that provides mappings between manufacturer identifiers
...
## Bimap interface:
x <- OperonHumanV3SYMBOL

20

OperonHumanV3UNIGENE
...
OperonHumanV3UNIGENE Map between Manufacturer Identifiers and UniGene cluster identifiers

Description

OperonHumanV3UNIGENE is an R object that provides mappings between manufacturer identi-
...
OperonHumanV3UNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

OperonHumanV3UNIPROT is an R object that contains mappings between the manufacturer iden-
...
OperonHumanV3_dbconn()
OperonHumanV3_dbfile()
OperonHumanV3_dbschema(file="", show.indices=FALSE)
...
OperonHumanV3_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by OperonHumanV3_dbconn or you will
break all the AnnDbObj objects defined in this package!
OperonHumanV3_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
OperonHumanV3_dbschema prints the schema definition of the package annotation DB.
OperonHumanV3_dbInfo prints other information about the package annotation DB.

Value

OperonHumanV3_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
OperonHumanV3_dbfile: a character string with the path to the package annotation DB.
OperonHumanV3_dbschema: none (invisible NULL).
OperonHumanV3_dbInfo: none (invisible NULL).
...
dbGetQuery(OperonHumanV3_dbconn(), "SELECT COUNT(*) FROM probes")
...
OperonHumanV3.db, 2
OperonHumanV3_dbconn, 21
OperonHumanV3ACCNUM, 1
OperonHumanV3ALIAS2PROBE, 2
OperonHumanV3CHR, 3
OperonHumanV3CHRLENGTHS, 4
OperonHumanV3CHRLOC, 4
OperonHumanV3ENSEMBL, 5
OperonHumanV3ENTREZID, 6
OperonHumanV3ENZYME, 7
OperonHumanV3GENENAME, 9
OperonHumanV3GO, 10
OperonHumanV3MAP, 12
OperonHumanV3MAPCOUNTS, 13
OperonHumanV3OMIM, 13
OperonHumanV3ORGANISM, 14
OperonHumanV3PATH, 15
OperonHumanV3PFAM, 16
OperonHumanV3PMID, 16
OperonHumanV3PROSITE, 17
OperonHumanV3REFSEQ, 18
OperonHumanV3SYMBOL, 19
OperonHumanV3UNIGENE, 20
OperonHumanV3UNIPROT, 21
...
OperonHumanV3 (OperonHumanV3.db), 2
OperonHumanV3.db, 2
OperonHumanV3_dbconn, 21
...
OperonHumanV3CHRLENGTHS, 4
OperonHumanV3CHRLOC, 4
OperonHumanV3CHRLOCEND

(OperonHumanV3CHRLOC), 4

OperonHumanV3ENSEMBL, 5
OperonHumanV3ENSEMBL2PROBE

(OperonHumanV3ENSEMBL), 5

OperonHumanV3ENTREZID, 6
OperonHumanV3ENZYME, 7
OperonHumanV3ENZYME2PROBE

(OperonHumanV3ENZYME), 7

OperonHumanV3GENENAME, 9
OperonHumanV3GO, 10
OperonHumanV3GO2ALLPROBES, 11
OperonHumanV3GO2ALLPROBES
...
OperonHumanV3PATH, 15
OperonHumanV3PATH2PROBE

(OperonHumanV3PATH), 15

OperonHumanV3PFAM, 16
OperonHumanV3PMID, 16
OperonHumanV3PMID2PROBE
