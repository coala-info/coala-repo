...
Source: [`vignettes/Using-FLORAL-for-Microbiome-Analysis.Rmd`](https://github.com/vdblab/FLORAL/blob/master/vignettes/Using-FLORAL-for-Microbiome-Analysis.Rmd)
...
“best” solutions for
λ\lambda
available: one minimizing the mean squared error
(λmin\lambda\_\min),
and one maximizing the value of
λ\lambda
withing 1 standard error of the minimum mean squared error
(λ1se\lambda\_{\text{1se}}).
These are referred to as the `min` and `1se`
...
and
λ1se\lambda\_{\text{1se}},
respectively. In the MSE plot, the bands represent plus minus one
standard error of the MSE. In the coefficient plot, the colored lines
represent individual taxa, where taxa with non-zero values at
λmin\lambda\_\min
and
λ1se\lambda\_{\text{1se}}
...
To further interpret the positive or negative associations between
the outcome, please refer to the output `step` regression
tables, where the effect sizes of the ratios can be found.

While the corresponding p-values are also available, we recommend
only using the p-values as a criterion to rank the strength of the
...
and
λ1se\lambda\_{\text{1se}}.
Interpretable plots can be created if `plot = TRUE` is
specified.
