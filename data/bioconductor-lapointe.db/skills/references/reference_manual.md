...
LAPOINTEACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
LAPOINTEALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(LAPOINTEALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the LAPOINTE.db annotation Package. The purpose of this package is to provide
detailed information about the LAPOINTE platform. This package is updated biannually.
...
LAPOINTECHR is an R object that provides mappings between a manufacturer identifier and the
...
LAPOINTECHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
LAPOINTECHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
LAPOINTEENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(LAPOINTEENSEMBL2PROBE)
if(length(xx) > 0){
...
LAPOINTEENTREZID is an R object that provides mappings between manufacturer identifiers
...
LAPOINTEENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. LAPOINTEENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the LAPOINTEENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the LAPOINTEENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In LAPOINTEENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert LAPOINTEENZYME2PROBE to a list to see inside
xx <- as.list(LAPOINTEENZYME2PROBE)
if(length(xx) > 0){
...
LAPOINTEGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
LAPOINTEGO is an R object that provides mappings between manufacturer identifiers and the
GO identifiers that they are directly associated with. This mapping and its reverse mapping (LA-
POINTEGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

LAPOINTEGO2ALLPROBES is an R object that provides mappings between a given GO identi-
fier and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than LA-
POINTEGO2PROBE.

Details

If LAPOINTEGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If LAPOINTEGO2ALLPROBES or LAPOINTEGO2PROBE is cast as a list, each GO term maps to
...
# Convert LAPOINTEGO2ALLPROBES to a list
xx <- as.list(LAPOINTEGO2ALLPROBES)
if(length(xx) > 0){
...
LAPOINTEMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package LAPOINTE.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

LAPOINTEMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package LAPOINTE.db.
...
LAPOINTEOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for LAPOINTE

Description

LAPOINTEORGANISM is an R object that contains a single item: a character string that names
the organism for which LAPOINTE was built. LAPOINTEORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, LAPOINTEOR-
GANISM provides a simple way to programmatically extract the organism name. LAPOINTE-
...
LAPOINTEPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

LAPOINTEPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the LAPOINTEPATH2PROBE object to a list
xx <- as.list(LAPOINTEPATH2PROBE)
if(length(xx) > 0){
...
LAPOINTEPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
LAPOINTEPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. LAPOINTEPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When LAPOINTEPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When LAPOINTEPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object LAPOINTEPMID2PROBE to a list
xx <- as.list(LAPOINTEPMID2PROBE)
if(length(xx) > 0){
...
LAPOINTEPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
LAPOINTEREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
LAPOINTESYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
LAPOINTEUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
LAPOINTEUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
LAPOINTE_dbconn()
LAPOINTE_dbfile()
LAPOINTE_dbschema(file="", show.indices=FALSE)
LAPOINTE_dbInfo()
...
LAPOINTE_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by LAPOINTE_dbconn or you will break all
the AnnDbObj objects defined in this package!
LAPOINTE_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
LAPOINTE_dbschema prints the schema definition of the package annotation DB.
LAPOINTE_dbInfo prints other information about the package annotation DB.

Value

LAPOINTE_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
LAPOINTE_dbfile: a character string with the path to the package annotation DB.
LAPOINTE_dbschema: none (invisible NULL).
LAPOINTE_dbInfo: none (invisible NULL).
...
dbGetQuery(LAPOINTE_dbconn(), "SELECT COUNT(*) FROM probes")
...
LAPOINTE_dbconn, 21
LAPOINTEACCNUM, 1
LAPOINTEALIAS2PROBE, 2
LAPOINTECHR, 3
LAPOINTECHRLENGTHS, 4
LAPOINTECHRLOC, 4
LAPOINTEENSEMBL, 5
LAPOINTEENTREZID, 6
LAPOINTEENZYME, 7
...
LAPOINTE (LAPOINTE.db), 2
LAPOINTE.db, 2
LAPOINTE_dbconn, 21

LAPOINTE_dbfile (LAPOINTE_dbconn), 21
LAPOINTE_dbInfo (LAPOINTE_dbconn), 21
LAPOINTE_dbschema (LAPOINTE_dbconn), 21
LAPOINTEACCNUM, 1
LAPOINTEALIAS2PROBE, 2
LAPOINTECHR, 3
LAPOINTECHRLENGTHS, 4
LAPOINTECHRLOC, 4
LAPOINTECHRLOCEND (LAPOINTECHRLOC), 4
LAPOINTEENSEMBL, 5
LAPOINTEENSEMBL2PROBE

(LAPOINTEENSEMBL), 5

LAPOINTEENTREZID, 6
LAPOINTEENZYME, 7
LAPOINTEENZYME2PROBE (LAPOINTEENZYME), 7
LAPOINTEGENENAME, 9
LAPOINTEGO, 10
LAPOINTEGO2ALLPROBES, 11
LAPOINTEGO2ALLPROBES (LAPOINTEGO), 10
LAPOINTEGO2PROBE (LAPOINTEGO), 10
LAPOINTELOCUSID (LAPOINTEENTREZID), 6
LAPOINTEMAP, 12
LAPOINTEMAPCOUNTS, 13
LAPOINTEOMIM, 13
LAPOINTEORGANISM, 14
LAPOINTEORGPKG (LAPOINTEORGANISM), 14
LAPOINTEPATH, 15
LAPOINTEPATH2PROBE (LAPOINTEPATH), 15
LAPOINTEPFAM, 16
LAPOINTEPMID, 16
LAPOINTEPMID2PROBE (LAPOINTEPMID), 16
LAPOINTEPROSITE, 17
LAPOINTEREFSEQ, 18
LAPOINTESYMBOL, 19
LAPOINTEUNIGENE, 20
LAPOINTEUNIPROT, 21
LAPOINTEUNIPROT2PROBE
