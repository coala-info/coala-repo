...
* [Installation](./ "Installation")
  ...
  + [MP and MPI versions](#mp-and-mpi-versions "MP and MPI versions")
  + [Installing with Conda](#installing-with-conda "Installing with Conda")
...
### MP and MPI versions[#](#mp-and-mpi-versions "Permanent link")
...
### Installing with Conda[#](#installing-with-conda "Permanent link")

`conda install -c bioconda hyphy`

*you may need to add the bioconda channel if you have not already done so:*
...
### Building from source[#](#building-from-source "Permanent link")
...
2. Configure HyPhy using *CMake*
   ...
   * If would like to specify a particular C/C++ compiler (please note that it must support the C++-14 language standard) for your HyPhy build, use this command:
     ...
     ```
3. If you are building HyPhy under MacOS X, you can specify which version of SDK to use, for example:

   ```
       cmake -DCMAKE_OSX_SYSROOT=/Developer/SDKs/MacOSX10.9.sdk/ .
   ```
4. Build HyPhy by running `make` with one of the following build targets given below. Note that including the `-j` flag for make (i.e. `make -j <target>`) will dramatically speed this step up by taking advantage of multiple processors.
   ...
   * `MPI` - build a HyPhy executable (`HYPHYMPI`) using the message passing interface [MPI](http://mpi-forum.org) to support parallel execution on distributed systems (clusters). An MPI library (e.g., OpenMPI) must be installed and available in your path. You can check if this is the case by examining the output from running `cmake`. If `openMPI` is installed, you should see something similar to the following in the output:
     ...
     ```
5. Finally, install HyPhy on your system with the command
...
### Environment Options[#](#environment-options "Permanent link")
...
Benchmarks for common HyPhy methods using mpirun and escalating number of processors can be found [here](https://observablehq.com/%40stevenweaver/hyphy-benchmarks-and-profiling)
