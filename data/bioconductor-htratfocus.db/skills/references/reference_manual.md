...
htratfocusACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
htratfocusALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

htratfocusALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(htratfocusALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the htratfocus.db annotation Package. The purpose of this package is to provide detailed
information about the htratfocus platform. This package is updated biannually.
...
htratfocusCHR is an R object that provides mappings between a manufacturer identifier and the
...
htratfocusCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

htratfocusCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
htratfocusCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
htratfocusENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
htratfocusENTREZID is an R object that provides mappings between manufacturer identifiers and
...
htratfocusENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. htratfocusENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the htratfocusENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the htratfocusENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In htratfocusENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert htratfocusENZYME2PROBE to a list to see inside

htratfocusGENENAME

9

x <- htratfocusENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
htratfocusGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
htratfocusGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (htratfo-
cusGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only the
directly evidenced terms are represented here.

htratfocusGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than htratfo-
cusGO2PROBE.

Details

If htratfocusGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If htratfocusGO2ALLPROBES or htratfocusGO2PROBE is cast as a list, each GO term maps to a
...
x <- htratfocusGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert htratfocusGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package htratfocus.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

htratfocusMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package htratfocus.db.
...
The Organism information for htratfocus

Description

htratfocusORGANISM is an R object that contains a single item: a character string that names
the organism for which htratfocus was built. htratfocusORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, htratfocusORGAN-
ISM provides a simple way to programmatically extract the organism name. htratfocusORGPKG
...
htratfocusPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

htratfocusPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- htratfocusPATH
mapped_probes <- mappedkeys(x)
# Now convert the htratfocusPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htratfocusPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
htratfocusPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. htratfocusPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When htratfocusPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When htratfocusPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- htratfocusPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object htratfocusPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htratfocusPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
htratfocusREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
htratfocusSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
htratfocusUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
htratfocus_dbconn()
htratfocus_dbfile()
htratfocus_dbschema(file="", show.indices=FALSE)
htratfocus_dbInfo()
...
htratfocus_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by htratfocus_dbconn or you will
break all the AnnDbObj objects defined in this package!

htratfocus_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

htratfocus_dbschema prints the schema definition of the package annotation DB.

htratfocus_dbInfo prints other information about the package annotation DB.

Value

htratfocus_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

htratfocus_dbfile: a character string with the path to the package annotation DB.

htratfocus_dbschema: none (invisible NULL).

htratfocus_dbInfo: none (invisible NULL).
...
dbGetQuery(htratfocus_dbconn(), "SELECT COUNT(*) FROM probes")
...
htratfocus.db, 2
htratfocus_dbconn, 19
htratfocusACCNUM, 1
htratfocusALIAS2PROBE, 2
htratfocusCHR, 3
htratfocusCHRLENGTHS, 4
htratfocusCHRLOC, 4
htratfocusENSEMBL, 5
htratfocusENTREZID, 6
htratfocusENZYME, 7
htratfocusGENENAME, 9
htratfocusGO, 10
htratfocusMAPCOUNTS, 12
htratfocusORGANISM, 12
htratfocusPATH, 13
htratfocusPFAM, 14
htratfocusPMID, 14
htratfocusPROSITE, 16
htratfocusREFSEQ, 16
htratfocusSYMBOL, 17
htratfocusUNIPROT, 18
...
htratfocus (htratfocus.db), 2
htratfocus.db, 2
htratfocus_dbconn, 19
htratfocus_dbfile (htratfocus_dbconn),

19

htratfocus_dbInfo (htratfocus_dbconn),
...
htratfocusCHRLENGTHS, 4
htratfocusCHRLOC, 4
htratfocusCHRLOCEND (htratfocusCHRLOC),

4
htratfocusENSEMBL, 5
htratfocusENSEMBL2PROBE

(htratfocusENSEMBL), 5

htratfocusENTREZID, 6
htratfocusENZYME, 7
htratfocusENZYME2PROBE

(htratfocusENZYME), 7

htratfocusGENENAME, 9
htratfocusGO, 10
htratfocusGO2ALLPROBES, 11
htratfocusGO2ALLPROBES (htratfocusGO),

10

htratfocusGO2PROBE (htratfocusGO), 10
htratfocusLOCUSID (htratfocusENTREZID),

6

htratfocusMAPCOUNTS, 12
htratfocusORGANISM, 12
htratfocusORGPKG (htratfocusORGANISM),

12
htratfocusPATH, 13
htratfocusPATH2PROBE (htratfocusPATH),

13
htratfocusPFAM, 14
htratfocusPMID, 14
htratfocusPMID2PROBE (htratfocusPMID),
