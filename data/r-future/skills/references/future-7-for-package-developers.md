...
The reason for having to manually limit the number workers in
vignettes, while it is automatically limited for us in package
examples and package tests, is that `R CMD check --as-cran` does *not*
set environment variable `_R_CHECK_LIMIT_CORES_` when re-building
vignettes, resulting in the default `workers = availableCores()` to
use whatever number of cores that the operating system has given our R
process.
...
*remember to stop the cluster at the end of all examples(\*), vignettes, and
unit tests*. This is required in order to not leave behind stray parallel
