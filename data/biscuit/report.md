...
### Tool Description
Index a reference genome for BISCUIT alignment
...
- **Conda**: https://anaconda.org/channels/bioconda/packages/biscuit/overview
...
### Tool Description
Align bisulfite-treated sequencing reads to a reference genome
...
### Tool Description
Text-based alignment viewer for BISCUIT
...
### Tool Description
Correct or append bisulfite strand information in BAM files
...
### Tool Description
Filter and convert bisulfite sequencing reads based on CpH retention and other criteria.
...
### Tool Description
Extract cytosine information from a BAM file based on a reference genome.
...
### Tool Description
Pileup tool for DNA methylation and genetic variant calling from bisulfite sequencing data.
...
### Tool Description
Extract methylation or SNP information from a VCF file into BED format.
...
### Tool Description
Merge CpG sites from a position-sorted BED file with beta values and coverages. Typically used with output from biscuit vcf2bed.
...
### Tool Description
Extract epiread information from a BAM file using a reference genome.
...
### Tool Description
Process epiread files against a reference genome to generate a rectangle format output.
...
### Tool Description
BISCUIT assembly subcommand for processing epiread files
...
### Tool Description
Produces a subset of QC metrics for BISCUIT alignments.
...
### Tool Description
BISCUIT QC coverage tool for calculating coverage statistics from BAM files.
...
Usage: coverage [options] <ref.fa> <cpgs.bed.gz> <in.bam>
...
[main] Version: 1.7.1
[main] CMD: /usr/local/bin/biscuit qc_coverage -help
[main] Real time: 0.001 sec; CPU: 0.050 sec
```
...
### Tool Description
Extract barcodes from FASTQ files and append to read names. Adds an artificial UMI (AAAAAAAA) for compatibility.
...
[main] Version: 1.7.1
[main] CMD: /usr/local/bin/biscuit bc --help
[main] Real time: 0.001 sec; CPU: 0.050 sec
```
...
### Tool Description
BISCUIT quality control script for aligned BAM files
...
INFO:    Environment variable SINGULARITY_CACHEDIR is set, but APPTAINER_CACHEDIR is preferred
INFO:    Using cached SIF image
/usr/local/bin/QC.sh: Missing inputs

Usage: QC.sh [options] assets_directory genome sample_name in_bam

Required inputs:
	assets_directory    : Path to assets directory
	genome              : Path to reference FASTA file used in alignment
	sample_name         : Prefix of output files
	input_bam           : Aligned BAM from BISCUIT

Optional inputs:
	-h,--help           : Print help message and exit
	-s,--single-end     : Input BAM is from single end data [DEFAULT: Assumes paired-end]
	-v,--vcf            : Path to VCF output from BISCUIT [DEFAULT: <unused>]
	-o,--outdir         : Output directory [DEFAULT: BISCUITqc]
	-k,--keep-tmp-files : Flag to keep temporary files for debugging [DEFAULT: Delete files]
	-n,--no-cov-qc      : Do not perform coverage or coverage uniformity QC [DEFAULT: Runs coverage QC]
	-t,--n-threads      : Number of threads to use for coverage [DEFAULT: 1]
```

## biscuit_build_biscuit_QC_assets.pl

### Tool Description
Build biscuit QC assets from a reference genome.
...
## biscuit_flip_pbat_strands.sh

### Tool Description
Flip PBAT strands in a BAM file, optionally for a specific region.
...
INFO:    Environment variable SINGULARITY_CACHEDIR is set, but APPTAINER_CACHEDIR is preferred
INFO:    Using cached SIF image
/usr/local/bin/flip_pbat_strands.sh: Missing inputs

Usage: flip_pbat_strands.sh [-h,--help] [-r,--region] in_bam out_bam

Required inputs:
	in_bam      : input BAM to flip strands
	out_bam     : output BAM to write flipped reads to

Optional inputs:
	-r,--region : region to flip, give as chr:start-end [default: ]
	-h,--help   : print help message and exit
```
