...
HuO22ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
HuO22ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the HuO22.db annotation Package. The purpose of this package is to provide detailed
information about the HuO22 platform. This package is updated biannually.
...
HuO22CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
HuO22CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
HuO22CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
HuO22ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
HuO22ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
HuO22ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. HuO22ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the HuO22ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the HuO22ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In HuO22ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert HuO22ENZYME2PROBE to a list to see inside
xx <- as.list(HuO22ENZYME2PROBE)
if(length(xx) > 0){
...
HuO22GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
HuO22GO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (HuO22GO2PROBE)
...
HuO22GO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than HuO22GO2PROBE.

Details

If HuO22GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If HuO22GO2ALLPROBES or HuO22GO2PROBE is cast as a list, each GO term maps to a named
...
# Convert HuO22GO2ALLPROBES to a list
xx <- as.list(HuO22GO2ALLPROBES)
if(length(xx) > 0){
...
HuO22MAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package HuO22.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

HuO22MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package HuO22.db.
...
HuO22OMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for HuO22

Description

HuO22ORGANISM is an R object that contains a single item: a character string that names the
organism for which HuO22 was built. HuO22ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, HuO22ORGANISM
provides a simple way to programmatically extract the organism name. HuO22ORGPKG provides
...
HuO22PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

HuO22PATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
# Now convert the HuO22PATH2PROBE object to a list
xx <- as.list(HuO22PATH2PROBE)
if(length(xx) > 0){
...
HuO22PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
HuO22PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. HuO22PMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When HuO22PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When HuO22PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
HuO22PROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
HuO22REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
HuO22SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
HuO22UNIGENE is an R object that provides mappings between manufacturer identifiers and Uni-
Gene identifiers.
...
HuO22UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
HuO22_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by HuO22_dbconn or you will break all the
AnnDbObj objects defined in this package!
HuO22_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
HuO22_dbschema prints the schema definition of the package annotation DB.
HuO22_dbInfo prints other information about the package annotation DB.

Value

HuO22_dbconn: a DBIConnection object representing an open connection to the package annotation
DB.
HuO22_dbfile: a character string with the path to the package annotation DB.
HuO22_dbschema: none (invisible NULL).
HuO22_dbInfo: none (invisible NULL).
...
HuO22_dbfile (HuO22_dbconn), 21
HuO22_dbInfo (HuO22_dbconn), 21
HuO22_dbschema (HuO22_dbconn), 21
HuO22ACCNUM, 1
HuO22ALIAS2PROBE, 2
HuO22CHR, 3
HuO22CHRLENGTHS, 4
HuO22CHRLOC, 4
HuO22CHRLOCEND (HuO22CHRLOC), 4
HuO22ENSEMBL, 5
HuO22ENSEMBL2PROBE (HuO22ENSEMBL), 5
HuO22ENTREZID, 6
HuO22ENZYME, 7
HuO22ENZYME2PROBE (HuO22ENZYME), 7
HuO22GENENAME, 9
HuO22GO, 10
HuO22GO2ALLPROBES, 11
HuO22GO2ALLPROBES (HuO22GO), 10
HuO22GO2PROBE (HuO22GO), 10
HuO22LOCUSID (HuO22ENTREZID), 6
HuO22MAP, 12
HuO22MAPCOUNTS, 13
HuO22OMIM, 13
HuO22ORGANISM, 14
HuO22ORGPKG (HuO22ORGANISM), 14
HuO22PATH, 15
HuO22PATH2PROBE (HuO22PATH), 15
HuO22PFAM, 16
HuO22PMID, 16
HuO22PMID2PROBE (HuO22PMID), 16
HuO22PROSITE, 17
HuO22REFSEQ, 18
HuO22SYMBOL, 19
HuO22UNIGENE, 20
HuO22UNIPROT, 21
HuO22UNIPROT2PROBE (HuO22UNIPROT), 21
