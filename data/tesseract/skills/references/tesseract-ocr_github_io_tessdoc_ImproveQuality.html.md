...
You can see how Tesseract has processed the image by setting the [configuration variable `tessedit_write_images`](/tessdoc/tess3/ControlParams.html) to `true` (or using configfile `get.images`) when running Tesseract. If the resulting `tessinput.tif` file looks problematic, try some of these image processing operations before passing the image to Tesseract.
...
Tesseract works best on images which have a DPI of at least 300 dpi, so it may be beneficial to resize images. For more information see [the FAQ](/tessdoc/tess3/FAQ-Old.html#is-there-a-minimum--maximum-text-size-it-wont-read-screen-text).
...
![binarisation.png](/tessdoc/images/binarisation.png)
...
![noise.png](/tessdoc/images/noise.png)
...
![Erosion_original.png](/tessdoc/images/Erosion_original.png)

Erosion applied:

![Erosion_applied.png](/tessdoc/images/Erosion_applied.png)

### Rotation / Deskewing

![skew-linedetection.png](/tessdoc/images/skew-linedetection.png)
...
![borders.png](/tessdoc/images/borders.png)
...
* [page\_dewarp](https://github.com/mzucker/page_dewarp) - python example for Text page dewarping using a “cubic sheet” model. Details in blog [Page dewarping](https://mzucker.github.io/2016/08/15/page-dewarping.html).
...
Disabling the dictionaries Tesseract uses should increase recognition if most of your text isn’t dictionary words. They can be disabled by setting both of the [configuration variables `load_system_dawg` and `load_freq_dawg`](/tessdoc/tess3/ControlParams.html) to `false`.
...
If you know you will only encounter a subset of the characters available in the language, such as only digits, you can use the [`tessedit_char_whitelist` configuration variable](/tessdoc/tess3/ControlParams.html). See the [FAQ for an example](/tessdoc/tess3/FAQ-Old.html#how-do-i-recognize-only-digits).
