...
hgug4100aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgug4100aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4100aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4100aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4100a.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4100a platform. This package is updated biannually.
...
hgug4100aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4100aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4100aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgug4100aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgug4100aENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4100aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4100aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgug4100aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgug4100aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgug4100aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hgug4100aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgug4100aENZYME2PROBE to a list to see inside
xx <- as.list(hgug4100aENZYME2PROBE)
if(length(xx) > 0){
...
hgug4100aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4100aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgug4100aGO2PROBE)
...
hgug4100aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgug4100aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgug4100aGO2ALLPROBES or hgug4100aGO2PROBE is cast as a list, each GO term maps to
...
# Convert hgug4100aGO2ALLPROBES to a list
xx <- as.list(hgug4100aGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4100aMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4100a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgug4100aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4100a.db.
...
hgug4100aOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4100a

Description

hgug4100aORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4100a was built. hgug4100aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgug4100aORGANISM
provides a simple way to programmatically extract the organism name. hgug4100aORGPKG pro-
...
hgug4100aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgug4100aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hgug4100aPATH2PROBE object to a list
xx <- as.list(hgug4100aPATH2PROBE)
if(length(xx) > 0){
...
hgug4100aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgug4100aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgug4100aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgug4100aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgug4100aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgug4100aPMID2PROBE to a list
xx <- as.list(hgug4100aPMID2PROBE)
if(length(xx) > 0){
...
hgug4100aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgug4100aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4100aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4100aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4100aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgug4100a_dbconn()
hgug4100a_dbfile()
hgug4100a_dbschema(file="", show.indices=FALSE)
hgug4100a_dbInfo()
...
hgug4100a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4100a_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgug4100a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgug4100a_dbschema prints the schema definition of the package annotation DB.
hgug4100a_dbInfo prints other information about the package annotation DB.

Value

hgug4100a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
hgug4100a_dbfile: a character string with the path to the package annotation DB.
hgug4100a_dbschema: none (invisible NULL).
hgug4100a_dbInfo: none (invisible NULL).
...
dbGetQuery(hgug4100a_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgug4100a_dbconn, 21
hgug4100aACCNUM, 1
hgug4100aALIAS2PROBE, 2
hgug4100aCHR, 3
hgug4100aCHRLENGTHS, 4
hgug4100aCHRLOC, 4
hgug4100aENSEMBL, 5
hgug4100aENTREZID, 6
hgug4100aENZYME, 7
...
hgug4100aGO, 10
hgug4100aMAP, 12
hgug4100aMAPCOUNTS, 13
hgug4100aOMIM, 13
hgug4100aORGANISM, 14
hgug4100aPATH, 15
hgug4100aPFAM, 16
hgug4100aPMID, 16
hgug4100aPROSITE, 17
...
hgug4100a (hgug4100a.db), 2
hgug4100a.db, 2
hgug4100a_dbconn, 21

hgug4100a_dbfile (hgug4100a_dbconn), 21
hgug4100a_dbInfo (hgug4100a_dbconn), 21
hgug4100a_dbschema (hgug4100a_dbconn),
...
hgug4100aCHRLENGTHS, 4
hgug4100aCHRLOC, 4
hgug4100aCHRLOCEND (hgug4100aCHRLOC), 4
hgug4100aENSEMBL, 5
hgug4100aENSEMBL2PROBE

(hgug4100aENSEMBL), 5

hgug4100aENTREZID, 6
hgug4100aENZYME, 7
hgug4100aENZYME2PROBE

(hgug4100aENZYME), 7

hgug4100aGENENAME, 9
hgug4100aGO, 10
hgug4100aGO2ALLPROBES, 11
hgug4100aGO2ALLPROBES (hgug4100aGO), 10
hgug4100aGO2PROBE (hgug4100aGO), 10
hgug4100aLOCUSID (hgug4100aENTREZID), 6
hgug4100aMAP, 12
hgug4100aMAPCOUNTS, 13
hgug4100aOMIM, 13
hgug4100aORGANISM, 14
hgug4100aORGPKG (hgug4100aORGANISM), 14
hgug4100aPATH, 15
hgug4100aPATH2PROBE (hgug4100aPATH), 15
hgug4100aPFAM, 16
hgug4100aPMID, 16
hgug4100aPMID2PROBE (hgug4100aPMID), 16
hgug4100aPROSITE, 17
hgug4100aREFSEQ, 18
hgug4100aSYMBOL, 19
hgug4100aUNIGENE, 20
hgug4100aUNIPROT, 21
hgug4100aUNIPROT2PROBE
