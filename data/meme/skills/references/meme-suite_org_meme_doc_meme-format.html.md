...
[Programs for converting](motif_conversion.html) many other motif formats
into Minimal MEME Motif Format are provided by the downloadable version of MEME Suite.

### MEME, STREME and DREME Output Formats

The HTML (`.html`) and text (`.txt`) output files
...
The MEME Minimal Motif Format is a simple text format for motifs that is accepted
by the programs in the MEME Suite that require MEME Motif Format.

The format is a **plain text** (ASCII) format
...
[conversion script](motif_conversion.html) that
can do the job for you.

A text file in MEME minimal motif format can contain more than one motif,
...
alphabets like DNA), as illustrated in the example files below.

* [Example DNA minimal motif format file](examples/sample-dna-motif.meme)
* [Example PROTEIN minimal motif format file](examples/sample-protein-motif.meme)

### Format Specification

The minimal MEME format contains following sections:

1. [Version (required)](#min_ver)
2. [Alphabet (recommended)](#min_alph)
3. [Strands (optional)](#min_strands)
4. [Background frequencies (recommended)](#min_bg)
5. [Motifs (required)](#min_motif_name)

For each motif in the motifs section there are the sub-sections:

1. [Motif name (required)](#min_motif_name)
2. [Motif letter-probability matrix (required)](#min_motif_pspm)
3. [Motif URL (optional)](#min_motif_url)

As well as the documentation you can also refer to these examples.

* [minimal DNA motif](examples/sample-dna-motif.meme)
* [minimal Protein motif](examples/sample-protein-motif.meme)

#### MEME version line (required)

The MEME Suite requires this line to be certain that
...
The version number should be the MEME
Suite version you are targeting.
...
The alphabet line tells the MEME Suite what alphabet to expect the
motifs to be in. If this line is not present then the MEME Suite can
attempt to detect this from the background or the motifs themselves.

ALPHABET= alphabet

The alphabet can be ACGT for DNA,
ACGU for RNA or ACDEFGHIKLMNPQRSTVWY for protein.

For example using a DNA alphabet:

ALPHABET= ACGT

If you are using a custom alphabet instead of the alphabet line you
can also supply the complete
[alphabet definition](alphabet-format.html) followed by a
line containing just "END ALPHABET" (without the quotes).
...
The strands line only has meaning for motifs of complementable
...
of complementable alphabets were created from both strands.

strands: which strands

The which strands can be replaced with +
to indicate only the given strand and + - to indicate both strands.
...
The background frequencies tell the MEME Suite how prevalent each
...
uses this background to automatically create the log-odds matrices for MAST.

Background letter frequencies (from source):
letter 1 frequency 1
letter 2 frequency 2
... (repeated) ...
letter n-1 frequency n-1
letter n frequency n

The source is not required and if you
...
[custom alphabet ordering](alphabet-format.html#ordering))
and the frequencies should sum to 1.
...
The motif name line indicates the start of a new motif and designates
...
nor the alternate name may contain spaces or equal signs (=).
...
The letter probability matrix is a table of probabilities where the
...
probabilities in the row must sum to 1.

letter-probability matrix:
alength= alphabet length
w= motif length
nsites= source sites
E= source *E*-value
... (letter-probability matrix goes here) ...

All the "key= value" pairs after the "letter-probability
matrix:" text are optional. The "alength=
alphabet length" and
"w= motif length" can be derived
...
source sites" will default to 20 if it
is not provided and the "E=
source *E*-value" will default to zero.
(Note: "S= source Score"
may replace "E= source *E*-value".)
The source sites is used to apply
pseudocounts to the motif and the
source *E*-value
(or source *P*-value or source Score)
...
The URL line specifies a web-page to link to when mentioning the motif
in results.
...
URL http://jaspar.genereg.net/cgi-bin/jaspar\_db.pl?ID=MA0002.1&rm=present&collection=CORE

## Other MEME Motif Formats

The motif discovery tools in the MEME Suite
...
* the [Example original MEME text output](examples/meme_example_output_files/meme.txt),
* the [Example MEME HTML output](examples/meme_example_output_files/meme.html),
* the [Example MEME XML output](examples/meme_example_output_files/meme.xml),
* the [Example STREME text output](examples/streme_example_output_files/streme.txt),* the [Example STREME HTML output](examples/streme_example_output_files/streme.html) and* the [Example STREME XML output](examples/streme_example_output_files/streme.xml).
    * the [Example DREME text output](examples/dreme_example_output_files/dreme.txt),* the [Example DREME HTML output](examples/dreme_example_output_files/dreme.html) and* the [Example DREME XML output](examples/dreme_example_output_files/dreme.xml).