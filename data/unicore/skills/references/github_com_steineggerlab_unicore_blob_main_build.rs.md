...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fsteineggerlab%2Funicore%2Fblob%2Fmain%2Fbuild.rs)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=steineggerlab%2Funicore)
...
[History](/steineggerlab/unicore/commits/main/build.rs)
...
[Raw](https://github.com/steineggerlab/unicore/raw/refs/heads/main/build.rs)
...
let pwd = current\_dir()

.unwrap\_or\_else(|\_| error("Could not get current directory"))

.to\_str()

.unwrap\_or\_else(|| error("Could not convert path to string"))

.to\_string();

let script\_rls = format!("#!/bin/sh\n{pwd}/target/release/unicore $@\n");

let script\_dbg = format!("#!/bin/sh\n{pwd}/target/debug/unicore $@\n");

let mut file = File::create("bin/unicore").unwrap();

file.write\_all(script\_rls.as\_bytes()).unwrap();

let mut file = File::create("bin/unicore-debug").unwrap();

file.write\_all(script\_dbg.as\_bytes()).unwrap();
