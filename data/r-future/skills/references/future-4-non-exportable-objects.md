...
*If you identify other cases, please consider [reporting](https://github.com/futureverse/future/issues/) them so they can be documented here and possibly even be fixed.*
...
| **keras** | keras.engine.sequential.Sequential (`externalptr`), keras.engine.base\_layer.Layer (`externalptr`) |
...
However, there are objects with *external pointer*:s that can be exported, e.g. `data.table` objects of the **[data.table](https://cran.r-project.org/package%3Ddata.table)** package is one such example. In other words, the existence of a *external pointer* is just a suggestion for an object being non-exportable - it is not a sufficient condition.

Below are some examples of packages who produce non-exportable objects with *external pointer*:s.

#### Package: arrow

The **[arrow](https://cran.r-project.org/package%3Darrow)** package provides efficient in-memory storage of arrays
...
The **[bigmemory](https://cran.r-project.org/package%3Dbigmemory)** package provides mechanisms for working with very
...
Another example is **[cpp11](https://cran.r-project.org/package%3Dcpp11)**, which allows us to easily create R functions that are implemented in C++, e.g.
...
**[DBI](https://cran.r-project.org/package%3DDBI)** provides a unified database interface for communication between R and various database engines. Analogously to regular connections in R, DBIConnection objects can *not* safely be exported to another R process, e.g.
...
Another example is **[inline](https://cran.r-project.org/package%3Dinline)**, which allows us to easily create R functions that are implemented in C and C++, e.g.
...
The **[keras](https://cran.r-project.org/package%3DDBI)** package provides an R interface to
...
implementation accesses the Keras Python API via **[reticulate](https://cran.r-project.org/package%3Dreticulate)**.
...
The **[magick](https://cran.r-project.org/package%3Dmagick)** package provides an R-level API for [ImageMagick](https://imagemagick.org/) to work with images. When working with this API, the images are represented internally as external pointers of class ‘magick\_image’ that cannot be be exported to another R process, e.g.
...
The **[polars](https://rpolars.github.io/)** package provides objects for performant processing on
...
The **[raster](https://cran.r-project.org/package%3Draster)** package provides methods for working with spatial data, which are held in ‘RasterLayer’ objects. Not all but some of these objects use an external pointer. For example,
...
Similarly to **cpp11**, **[Rcpp](https://cran.r-project.org/package%3DRcpp)** can be use to create R functions that are implemented in C++, e.g.
...
The **[reticulate](https://cran.r-project.org/package%3Dreticulate)** package provides methods for creating and calling Python code from within R. If one attempt to use Python-binding objects from this package, we get errors like:
...
Here is an example that shows how **[rJava](https://cran.r-project.org/package%3DrJava)** objects cannot be exported to external R processes.
...
Now, it is indeed possible to parallelize **[udpipe](https://cran.r-project.org/package%3Dudpipe)** calls. For details on how to do this, see the ‘UDPipe Natural Language Processing - Parallel’ vignette that comes with the **udpipe** package.

#### Package: xgboost

The **[xgboost](https://cran.r-project.org/package%3Dxgboost)** package provides fast gradient-boosting methods. Some of its data structures use external pointers. For example,
...
Another example is XML objects of the **[XML](https://cran.r-project.org/package%3DXML)** package, which may
...
Yet another example is XML objects of the **[xml2](https://cran.r-project.org/package%3Dxml2)** package, which
...
Package **[ncdf4](https://cran.r-project.org/package%3Dncdf4)** provides an R API to work with data that live in netCDF files. For example, we can create a simple netCDF file that holds a variable ‘x’:
...
The **[data.table](https://cran.r-project.org/package%3Ddata.table)** package creates objects comprising external pointers. Contrary to above non-exportable examples, such objects can be saved to file and used in another R session, or exported to a parallel worker. This is because **data.table** is capable of restoring these objects to a valid state. Consider the following example:
...
The **[rstan](https://cran.r-project.org/package%3Drstan)** package creates objects comprising external pointers. Contrary to above non-exportable examples, such objects can be saved to file and used in another R session, or exported to a parallel worker. This is because **rstan** is capable of restoring these objects to a valid state. Consider the following example from `example("rstan", package = "rstan")`:
