...
* [next](../Installation/ "2. Installation") |
* [previous](../ "Mikado: pick your transcript") |
* [Mikado](../) »

# 1. Introduction[¶](#introduction "Permalink to this headline")

Numerous algorithms have been proposed to analyse RNA-Seq data, both in terms of aligning the reads to a reference genome ([[TopHat2]](../References/#tophat2), [[STAR]](../References/#star), [[Hisat]](../References/#hisat)) or to assemble them to infer the sequence and structure of the original molecules present in the sample. The latter phase can be performed either by using alignment data against the reference genome ([[Cufflinks]](../References/#cufflinks), [[StringTie]](../References/#stringtie), [[Class2]](../References/#class2), [[Trinity]](../References/#trinity)) or in the absence of such information ([[Trinity]](../References/#trinity), [[Oases]](../References/#oases), [[Bridger]](../References/#bridger), ). Each of these methods has to contend with numerous sources of variability in the RNA-Seq data:
...
Multiple assessments of these methods on real and simulated data. In particular, the RGASP consortium promoted a competition among some of the most popular methods in 2012 [[RGASP]](../References/#rgasp); the results showed that the accuracy of the methods was dependent on the input data, the expression levels of the genes, and on the species under analysis. A different concern regards the completeness of each of the assembly methods, ie whether methods that are not necessarily the best across all gene loci might nonetheless be capable of outperforming other competitors for specific contigs. Recently, approaches have been proposed to integrate multiple RNA-Seq assemblies by inferring their protein-coding content and collapsing them accordingly [[EviGeneTobacco]](../References/#evigenetobacco) or to determine the best proposed assembly using multiple measures related to how the transcript compares to the input RNA-Seq reads ([[Transrate]](../References/#transrate), [[RSEMeval]](../References/#rsemeval)).

A different but related line of research has focused on how to integrate data coming from multiple samples. While some researchers advocate for merging together the input data and assembling it [[Trinity]](../References/#trinity), others have developed methods to integrate multiple assemblies into a single coherent annotation ([[CuffMerge]](../References/#cuffmerge), Stringtie-merge). Another common approach relies on a third party tool to integrate multiple assemblies together with other data - typically protein alignments and *ab initio* predictions - to select for the best model. The latter approach has been chosen by some of the most popular pipelines for genome annotation in the last years ([[EVM]](../References/#evm), [[Maker]](../References/#maker)).

Our tool, Mikado, contributes to this area of research by proposing a novel algorithm to integrate multiple transcript assemblies, leveraging additional data regarding the position of ORFs (generally derived with Transdecoder [[Trinity]](../References/#trinity)), sequence similarity (derived with BLAST [[Blastplus]](../References/#blastplus)) and reliable splicing junctions (generally derived using Portcullis [[Portcullis]](../References/#portcullis)). Through the combination of these input data sources, we are capable of identifying artefacts - especially gene fusions and small fragments - and retrieve the original transcripts in each locus with high accuracy and sensitivity.

An important caveat is that our approach does not look for the *best* candidate in terms of the input data, as Transrate [[Transrate]](../References/#transrate) or RSEM-Eval [[RSEMeval]](../References/#rsemeval) do.
Rather, our approach is more similar to EvidentialGene [[EviGeneTobacco]](../References/#evigenetobacco) as Mikado will try to find the candidates most likely to represent the isoforms
...
## 1.1. Overview[¶](#overview "Permalink to this headline")
...
1. [In the first step](../Usage/Prepare/#prepare), `mikado prepare` will combine assemblies from multiple sources into a single coherent, filtered dataset.
2. [In the second step](../Usage/Serialise/#serialise), `mikado serialise` will collect data from multiple sources ([Portcullis](https://github.com/maplesond/portcullis), [Transdecoder](http://transdecoder.github.io/), BLASTX) and load it into a database for fast integration with the input data.
3. [In the final step](../Usage/Pick/#pick), `mikado pick` will integrate the prepared transcripts with the serialised additional data to choose the best transcripts.

By far, the most important source of data for Mikado is the ORF calling, which can be executed with [Prodigal](https://github.com/hyattpd/Prodigal) or [Transdecoder](http://transdecoder.github.io/).
...
Moreover, the ORF features are a very important part of the scoring mechanism. Please refer to the [Algorithms](../Algorithms/#algorithms) section for more details on how Mikado operates.
...
*ab initio* annotation with Augustus [[Augustus]](../References/#augustus) or Maker2 [[Maker]](../References/#maker).

Mikado in action: recovering fused gene loci

[![../_images/locus_example.jpeg](../_images/locus_example.jpeg)](../_images/locus_example.jpeg)
...
## 1.2. The command-line interface[¶](#the-command-line-interface "Permalink to this headline")
...
Each of these subcommands is explained in detail in the [Usage](../Usage/#usage) section.

`daijin` instead provides the interface to the [Daijin pipeline manager](../Usage/Daijin/#daijin), which manages the task of going from a dataset of multiple reads to the Mikado final picking. This is its interface:
...
* 1. Introduction
  ...
  + [1.2. The command-line interface](#the-command-line-interface)
...
[Mikado: pick your transcript](../ "previous chapter")

#### Next topic

[2. Installation](../Installation/ "next chapter")
