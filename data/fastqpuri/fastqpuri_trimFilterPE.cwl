...
baseCommand: trimFilterPE
label: fastqpuri_trimFilterPE
doc: "Reads in paired end fq files (gz, bz2, z formats also accepted) and removes:
  low quality reads, reads containing N base callings, reads representing contaminations,
  belonging to sequences in INPUT.fa. Outputs 8 [O_PREFIX][1|2]_fq.gz files containing:
  \"good\" reads, discarded low Q reads, discarded reads containing N's, discarded
...
inputs:
  - id: adapter_input
    type:
      - 'null'
      - string
    doc: 'adapter input. Four fields separated by colons: <AD1.fa>: fasta file containing
      adapters, <AD2.fa>: fasta file containing adapters, <mismatches>: maximum mismatch
      count allowed, <score>: score threshold for the aligner.'
    inputBinding:
      position: 101
      prefix: --adapter
  ...
  - id: index_file_input
    type:
      - 'null'
      - string
    doc: 'index input file. To be included with any methods to remove. contaminations
      (TREE, BLOOM). 3 fields separated by colons: <INDEX_FILE>: output of makeTree,
      makeBloom, <score>: score threshold to accept a match [0,1], [lmer_len]: the
      length of the lmers to be looked for in the reads [1,READ_LENGTH].'
    inputBinding:
      position: 101
      prefix: --idx
