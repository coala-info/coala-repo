...
bovineACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
bovineALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the bovine.db annotation Package. The purpose of this package is to provide detailed
information about the bovine platform. This package is updated biannually.
...
bovineCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
bovineCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
bovineCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
Mappings were based on data provided by: UCSC Genome Bioinformatics (Bos taurus)
...
bovineENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
bovineENTREZID is an R object that provides mappings between manufacturer identifiers and
...
bovineENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. bovineENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the bovineENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the bovineENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In bovineENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert bovineENZYME2PROBE to a list to see inside

bovineGENENAME

9

x <- bovineENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
bovineGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
bovineGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (bovineGO2PROBE)
...
bovineGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than bovineGO2PROBE.

Details

If bovineGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If bovineGO2ALLPROBES or bovineGO2PROBE is cast as a list, each GO term maps to a named
...
x <- bovineGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert bovineGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package bovine.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

bovineMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package bovine.db.
...
The Organism information for bovine

Description

bovineORGANISM is an R object that contains a single item: a character string that names the
organism for which bovine was built. bovineORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, bovineORGAN-
ISM provides a simple way to programmatically extract the organism name. bovineORGPKG pro-
...
bovinePATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

bovinePATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- bovinePATH
mapped_probes <- mappedkeys(x)
# Now convert the bovinePATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
bovinePFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
bovinePMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. bovinePMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When bovinePMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When bovinePMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- bovinePMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object bovinePMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
bovinePROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
bovineREFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
bovineSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
bovineUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
bovine_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by bovine_dbconn or you will break all the
AnnDbObj objects defined in this package!

bovine_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

bovine_dbschema prints the schema definition of the package annotation DB.

bovine_dbInfo prints other information about the package annotation DB.

Value

bovine_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

bovine_dbfile: a character string with the path to the package annotation DB.

bovine_dbschema: none (invisible NULL).

bovine_dbInfo: none (invisible NULL).
...
bovineGENENAME, 9
bovineGO, 10
bovineGO2ALLPROBES, 11
bovineGO2ALLPROBES (bovineGO), 10
bovineGO2PROBE (bovineGO), 10
bovineLOCUSID (bovineENTREZID), 6
bovineMAPCOUNTS, 12
bovineORGANISM, 12
bovineORGPKG (bovineORGANISM), 12
bovinePATH, 13
bovinePATH2PROBE (bovinePATH), 13
bovinePFAM, 14
bovinePMID, 14
bovinePMID2PROBE (bovinePMID), 14
bovinePROSITE, 16
bovineREFSEQ, 16
bovineSYMBOL, 17
bovineUNIPROT, 18
bovineUNIPROT2PROBE (bovineUNIPROT), 18
...
bovine.db, 2
bovine_dbconn, 19
bovine_dbfile (bovine_dbconn), 19
bovine_dbInfo (bovine_dbconn), 19
bovine_dbschema (bovine_dbconn), 19
bovineACCNUM, 1
bovineALIAS2PROBE, 2
bovineCHR, 3
bovineCHRLENGTHS, 4
bovineCHRLOC, 4
bovineCHRLOCEND (bovineCHRLOC), 4
bovineENSEMBL, 5
bovineENSEMBL2PROBE (bovineENSEMBL), 5
bovineENTREZID, 6
bovineENZYME, 7
bovineENZYME2PROBE (bovineENZYME), 7
