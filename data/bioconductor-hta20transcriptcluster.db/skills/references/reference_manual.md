...
hta20transcriptclusterACCNUM is an R object that contains mappings between a manufacturer’s
...
hta20transcriptclusterALIAS is an R object that provides mappings between common gene symbol
...
Welcome to the hta20transcriptcluster.db annotation Package. The purpose of this package is to
provide detailed information about the hta20transcriptcluster platform. This package is updated
biannually.
...
hta20transcriptclusterCHR is an R object that provides mappings between a manufacturer identifier
...
hta20transcriptclusterCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hta20transcriptclusterCHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
## Bimap interface:
tt <- hta20transcriptclusterCHRLENGTHS
...
hta20transcriptclusterCHRLOC is an R object that maps manufacturer identifiers to the starting
...
hta20transcriptclusterENSEMBL is an R object that contains mappings between manufacturer iden-
...
hta20transcriptclusterENTREZID is an R object that provides mappings between manufacturer
...
hta20transcriptclusterENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. hta20transcriptclusterENZYME2PROBE is an R object that maps Enzyme
...
When the hta20transcriptclusterENZYME maping viewed as a list, each manufacturer identifier
...
vector will contain an NA.
For the hta20transcriptclusterENZYME2PROBE, each EC number maps to a named vector con-
...
numbers. In hta20transcriptclusterENZYME2PROBE, EC is dropped from the Enzyme Commis-
sion numbers.
...
hta20transcriptclusterGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
hta20transcriptclusterGO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hta20transcriptclusterGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

hta20transcriptclusterGO2ALLPROBES is an R object that provides mappings between a given GO
...
If hta20transcriptclusterGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If hta20transcriptclusterGO2ALLPROBES or hta20transcriptclusterGO2PROBE is cast as a list,
...
hta20transcriptclusterMAP is an R object that provides mappings between manufacturer identifiers
...
hta20transcriptclusterMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package hta20transcriptcluster.db.
...
hta20transcriptclusterOMIM is an R object that provides mappings between manufacturer identi-
fiers and OMIM identifiers.
...
hta20transcriptclusterORGANISM is an R object that contains a single item: a character string that
names the organism for which hta20transcriptcluster was built. hta20transcriptclusterORGPKG is
...
Although the package name is suggestive of the organism for which it was built, hta20transcriptclusterORGANISM
provides a simple way to programmatically extract the organism name. hta20transcriptclusterORGPKG
...
hta20transcriptclusterPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

hta20transcriptclusterPATH2PROBE is an R object that provides mappings between KEGG identi-
...
hta20transcriptclusterPFAM is an R object that provides mappings between manufacturer identifiers
...
hta20transcriptclusterPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. hta20transcriptclusterPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When hta20transcriptclusterPMID is viewed as a list each manufacturer identifier is mapped to a
...
When hta20transcriptclusterPMID2PROBE is viewed as a list each PubMed identifier is mapped
...
hta20transcriptclusterPROSITE is an R object that provides mappings between manufacturer iden-
...
hta20transcriptclusterREFSEQ is an R object that provides mappings between manufacturer identi-
...
hta20transcriptclusterSYMBOL is an R object that provides mappings between manufacturer iden-
...
hta20transcriptclusterUNIPROT is an R object that contains mappings between the manufacturer
...
hta20transcriptcluster_dbconn returns a connection object to the package annotation DB. IM-
PORTANT: Don’t call dbDisconnect on the connection object returned by hta20transcriptcluster_dbconn
or you will break all the AnnDbObj objects defined in this package!
hta20transcriptcluster_dbfile returns the path (character string) to the package annotation
...
hta20transcriptcluster_dbconn: a DBIConnection object representing an open connection to
...
hta20transcriptclusterCHR, 3
hta20transcriptclusterCHRLENGTHS, 4
hta20transcriptclusterCHRLOC, 4
