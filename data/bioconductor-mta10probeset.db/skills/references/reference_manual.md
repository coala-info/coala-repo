...
mta10probesetACCNUM is an R object that contains mappings between a manufacturer’s identi-
...
mta10probesetALIAS is an R object that provides mappings between common gene symbol identi-
...
xx <- as.list(mta10probesetALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mta10probeset.db annotation Package. The purpose of this package is to provide
detailed information about the mta10probeset platform. This package is updated biannually.
...
mta10probesetCHR is an R object that provides mappings between a manufacturer identifier and
...
mta10probesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mta10probesetCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mta10probesetCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
mta10probesetENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

mta10probesetENSEMBL is an R object that contains mappings between manufacturer identifiers
...
mta10probesetENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

mta10probesetENTREZID is an R object that provides mappings between manufacturer identifiers
...
mta10probesetENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. mta10probesetENZYME2PROBE is an R object that maps Enzyme Commission
(EC) numbers to manufacturer identifiers.

Details

When the mta10probesetENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the mta10probesetENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In mta10probesetENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mta10probesetENZYME2PROBE to a list to see inside

mta10probesetGENENAME

9

x <- mta10probesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mta10probesetGENENAME Map between Manufacturer IDs and Genes

Description

mta10probesetGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mta10probesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(mta10probesetGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

mta10probesetGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
mta10probesetGO2PROBE.

Details

If mta10probesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mta10probesetGO2ALLPROBES or mta10probesetGO2PROBE is cast as a list, each GO term
...
mta10probesetMAPCOUNTS

Number of mapped keys for the maps in package mta10probeset.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mta10probesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package mta10probeset.db.
...
mta10probesetMGI is an R object that contains mappings between manufacturer identifiers and
...
mta10probesetORGANISM The Organism information for mta10probeset

Description

mta10probesetORGANISM is an R object that contains a single item: a character string that names
the organism for which mta10probeset was built. mta10probesetORGPKG is an R object that con-
...
Although the package name is suggestive of the organism for which it was built, mta10probesetORGANISM
provides a simple way to programmatically extract the organism name. mta10probesetORGPKG
...
mta10probesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

mta10probesetPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- mta10probesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the mta10probesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mta10probesetPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mta10probesetPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mta10probesetPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mta10probesetPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When mta10probesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- mta10probesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mta10probesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mta10probesetPROSITE

}

17

mta10probesetPROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

mta10probesetPROSITE is an R object that provides mappings between manufacturer identifiers
...
mta10probesetREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mta10probesetSYMBOL is an R object that provides mappings between manufacturer identifiers
...
mta10probesetUNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

mta10probesetUNIPROT is an R object that contains mappings between the manufacturer identi-
...
mta10probeset_dbconn()
mta10probeset_dbfile()
mta10probeset_dbschema(file="", show.indices=FALSE)
mta10probeset_dbInfo()
...
mta10probeset_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by mta10probeset_dbconn or you will
break all the AnnDbObj objects defined in this package!

mta10probeset_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

mta10probeset_dbschema prints the schema definition of the package annotation DB.

mta10probeset_dbInfo prints other information about the package annotation DB.

Value

mta10probeset_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

mta10probeset_dbfile: a character string with the path to the package annotation DB.

mta10probeset_dbschema: none (invisible NULL).

mta10probeset_dbInfo: none (invisible NULL).
...
dbGetQuery(mta10probeset_dbconn(), "SELECT COUNT(*) FROM probes")

mta10probeset_dbschema()
...
mta10probeset.db, 2
mta10probeset_dbconn, 20
mta10probesetACCNUM, 1
mta10probesetALIAS2PROBE, 2
mta10probesetCHR, 3
mta10probesetCHRLENGTHS, 4
mta10probesetCHRLOC, 4
mta10probesetENSEMBL, 5
mta10probesetENTREZID, 6
mta10probesetENZYME, 7
mta10probesetGENENAME, 9
mta10probesetGO, 10
mta10probesetMAPCOUNTS, 12
mta10probesetMGI, 12
mta10probesetORGANISM, 13
mta10probesetPATH, 14
mta10probesetPFAM, 15
mta10probesetPMID, 15
mta10probesetPROSITE, 17
mta10probesetREFSEQ, 17
mta10probesetSYMBOL, 18
mta10probesetUNIPROT, 19
...
mta10probeset (mta10probeset.db), 2
mta10probeset.db, 2
mta10probeset_dbconn, 20
mta10probeset_dbfile
...
mta10probesetCHRLENGTHS, 4
mta10probesetCHRLOC, 4
mta10probesetCHRLOCEND

(mta10probesetCHRLOC), 4

mta10probesetENSEMBL, 5
mta10probesetENSEMBL2PROBE

(mta10probesetENSEMBL), 5

mta10probesetENTREZID, 6
mta10probesetENZYME, 7
mta10probesetENZYME2PROBE

(mta10probesetENZYME), 7

mta10probesetGENENAME, 9
mta10probesetGO, 10
mta10probesetGO2ALLPROBES, 11
mta10probesetGO2ALLPROBES
...
mta10probesetLOCUSID

(mta10probesetENTREZID), 6

mta10probesetMAPCOUNTS, 12
mta10probesetMGI, 12
mta10probesetMGI2PROBE
...
mta10probesetPATH, 14
mta10probesetPATH2PROBE

(mta10probesetPATH), 14

mta10probesetPFAM, 15
mta10probesetPMID, 15
mta10probesetPMID2PROBE
