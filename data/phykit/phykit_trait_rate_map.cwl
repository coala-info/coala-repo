...
baseCommand:
  - phykit
  - trait_rate_map
label: phykit_trait_rate_map
doc: Estimate per-branch evolutionary rates for a continuous trait and display 
  them as a branch-colored phylogram.
inputs:
  - id: tree
    type: File
    doc: a tree file
  ...
  - id: trait_data
    type:
      - 'null'
      - File
    doc: 'tab-delimited trait file (two-column: taxon<tab>value, no header; or multi-column
      with header when --trait is used)'
    inputBinding:
      position: 101
      prefix: --trait_data
  - id: output
  ...
  - id: trait
    type:
      - 'null'
      - string
    doc: column name to use from a multi-column trait file (if omitted, 
      two-column format is expected)
    inputBinding:
      position: 101
      prefix: --trait
  - id: fig_width
