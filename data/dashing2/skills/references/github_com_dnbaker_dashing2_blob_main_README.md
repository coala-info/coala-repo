...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fdnbaker%2Fdashing2%2Fblob%2Fmain%2FREADME.md)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=dnbaker%2Fdashing2)
...
1. [dashing2](/dnbaker/dashing2/tree/main)
...
[History](/dnbaker/dashing2/commits/main/README.md)

History

309 lines (206 loc) · 15.5 KB
...
[Raw](https://github.com/dnbaker/dashing2/raw/refs/heads/main/README.md)
...
6. Compilation in 45s in parallel, 2 min serial

Input Formats -- See [inputs](#inputs) below for details.

1. Fastq/Fasta -- enhanced alphabet support
2. Default -- ACGT
3. Protein encoding - 20 characters (`--protein`)
4. Reduced alphabets
   1. Protein - 14, 8, and 6-character alphabets for long-distance homology.
5. Optional -- generating 128-bit k-mers (`--long-kmers`)
6. All k-mer parsing can be winnowed by setting `--window-size` to be > k
7. Seeds can be spaced by providing a `--spacing` option, which provides the number of ignored characters in between used characters for seeds.
8. Exact multiset comparisons (`--countdict`)
9. Weight-aware sketching -- multiset and probability distribution
   1. We use BagMinHash for weighted sets (`--bagminhash` or `--multiset`), and ProbMinHash for discrete probability distributions (`--prob`)
10. Minimizer sequence transduction
...
11. Splicing data
12. LeafCutter splicing output files `--leafcutter`
13. Interval Sets
14. BigWig files (`--bigwig`) [Weighted Interval Set]
15. BED files (`--bed`)
16. Binary dumps
17. Binary files made from 64-bit identifiers
18. Optional: paired weights for each feature
19. Can be sketched as sets, weighted sets, and discrete probability distributions.
