...
label: gatk_CollectInsertSizeMetricsSpark
doc: Collects insert size distribution information in SAM/BAM/CRAM file(s). The 
  tool leverages the Spark framework for faster operation.
inputs:
  - id: histogram_plot_file
    type: string
    doc: File to write insert size histogram chart to.
    inputBinding:
      position: 101
      prefix: --histogram-plot-file
  - id: input
    type:
      - 'null'
      - type: array
  ...
  - id: histogram_plot_deviations_tolerance
    type:
      - 'null'
      - float
    doc: Generate mean, sd and plots by trimming the data down to MEDIAN + 
      maxMADTolerance*MEDIAN_ABSOLUTE_DEVIATION.
    inputBinding:
      position: 101
      prefix: --histogram-plot-deviations-tolerance
  ...
  - id: min_category_reads_percentage
    type:
      - 'null'
      - float
    ...
    inputBinding:
      position: 101
      prefix: --min-category-reads-percentage
  - id: num_reducers
    type:
      - 'null'
      - int
  ...
  - id: show_hidden
    ...
    inputBinding:
      ...
      prefix: --showHidden
outputs:
  - id: output_histogram_plot_file
    type: File
    doc: File to write insert size histogram chart to.
    outputBinding:
      glob: $(inputs.histogram_plot_file)
  - id: output_output
    type: File
    doc: File to write the output to.
    outputBinding:
