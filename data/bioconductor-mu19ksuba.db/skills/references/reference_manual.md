...
mu19ksubaACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mu19ksubaALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mu19ksubaALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mu19ksubaALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mu19ksuba.db annotation Package. The purpose of this package is to provide
detailed information about the mu19ksuba platform. This package is updated biannually.
...
mu19ksubaCHR is an R object that provides mappings between a manufacturer identifier and the
...
mu19ksubaCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mu19ksubaCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mu19ksubaENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mu19ksubaENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mu19ksubaENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mu19ksubaENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mu19ksubaENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mu19ksubaENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mu19ksubaENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mu19ksubaENZYME2PROBE to a list to see inside

mu19ksubaGENENAME

9

x <- mu19ksubaENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mu19ksubaGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mu19ksubaGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mu19ksubaGO2PROBE)
...
mu19ksubaGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mu19ksubaGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mu19ksubaGO2ALLPROBES or mu19ksubaGO2PROBE is cast as a list, each GO term maps to
...
x <- mu19ksubaGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mu19ksubaGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package mu19ksuba.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mu19ksubaMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mu19ksuba.db.
...
mu19ksubaMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mu19ksuba

Description

mu19ksubaORGANISM is an R object that contains a single item: a character string that names
the organism for which mu19ksuba was built. mu19ksubaORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mu19ksubaORGANISM
provides a simple way to programmatically extract the organism name. mu19ksubaORGPKG pro-
...
mu19ksubaPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mu19ksubaPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- mu19ksubaPATH
mapped_probes <- mappedkeys(x)
# Now convert the mu19ksubaPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mu19ksubaPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mu19ksubaPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mu19ksubaPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mu19ksubaPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mu19ksubaPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- mu19ksubaPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mu19ksubaPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mu19ksubaPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mu19ksubaREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mu19ksubaSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mu19ksubaUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mu19ksuba_dbconn()
mu19ksuba_dbfile()
mu19ksuba_dbschema(file="", show.indices=FALSE)
mu19ksuba_dbInfo()
...
mu19ksuba_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mu19ksuba_dbconn or you will break all
the AnnDbObj objects defined in this package!

mu19ksuba_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

mu19ksuba_dbschema prints the schema definition of the package annotation DB.

mu19ksuba_dbInfo prints other information about the package annotation DB.

Value

mu19ksuba_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

mu19ksuba_dbfile: a character string with the path to the package annotation DB.

mu19ksuba_dbschema: none (invisible NULL).

mu19ksuba_dbInfo: none (invisible NULL).
...
dbGetQuery(mu19ksuba_dbconn(), "SELECT COUNT(*) FROM probes")
...
mu19ksuba_dbconn, 20
mu19ksubaACCNUM, 1
mu19ksubaALIAS2PROBE, 2
mu19ksubaCHR, 3
mu19ksubaCHRLENGTHS, 4
mu19ksubaCHRLOC, 4
mu19ksubaENSEMBL, 5
mu19ksubaENTREZID, 6
mu19ksubaENZYME, 7
mu19ksubaGENENAME, 9
mu19ksubaGO, 10
mu19ksubaMAPCOUNTS, 12
mu19ksubaMGI, 12
mu19ksubaORGANISM, 13
mu19ksubaPATH, 14
mu19ksubaPFAM, 15
mu19ksubaPMID, 15
mu19ksubaPROSITE, 17
...
mu19ksuba (mu19ksuba.db), 2
mu19ksuba.db, 2
mu19ksuba_dbconn, 20
mu19ksuba_dbfile (mu19ksuba_dbconn), 20
mu19ksuba_dbInfo (mu19ksuba_dbconn), 20

22

mu19ksuba_dbschema (mu19ksuba_dbconn),
...
mu19ksubaCHRLENGTHS, 4
mu19ksubaCHRLOC, 4
mu19ksubaCHRLOCEND (mu19ksubaCHRLOC), 4
mu19ksubaENSEMBL, 5
mu19ksubaENSEMBL2PROBE

(mu19ksubaENSEMBL), 5

mu19ksubaENTREZID, 6
mu19ksubaENZYME, 7
mu19ksubaENZYME2PROBE

(mu19ksubaENZYME), 7

mu19ksubaGENENAME, 9
mu19ksubaGO, 10
mu19ksubaGO2ALLPROBES, 11
mu19ksubaGO2ALLPROBES (mu19ksubaGO), 10
mu19ksubaGO2PROBE (mu19ksubaGO), 10
mu19ksubaLOCUSID (mu19ksubaENTREZID), 6
mu19ksubaMAPCOUNTS, 12
mu19ksubaMGI, 12
mu19ksubaMGI2PROBE (mu19ksubaMGI), 12
mu19ksubaORGANISM, 13
mu19ksubaORGPKG (mu19ksubaORGANISM), 13
mu19ksubaPATH, 14
mu19ksubaPATH2PROBE (mu19ksubaPATH), 14
mu19ksubaPFAM, 15
mu19ksubaPMID, 15
mu19ksubaPMID2PROBE (mu19ksubaPMID), 15
mu19ksubaPROSITE, 17
mu19ksubaREFSEQ, 17
mu19ksubaSYMBOL, 18
mu19ksubaUNIPROT, 19
mu19ksubaUNIPROT2PROBE
