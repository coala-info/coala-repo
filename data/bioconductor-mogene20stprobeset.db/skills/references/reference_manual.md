...
mogene20stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

mogene20stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
mogene20stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(mogene20stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
mogene20stprobeset.db Bioconductor annotation data package

Description

Welcome to the mogene20stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the mogene20stprobeset platform. This package is updated
biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

mogene20stprobesetCHR
...
mogene20stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

mogene20stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
mogene20stprobesetCHRLOC

mogene20stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mogene20stprobesetCHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
## Bimap interface:
tt <- mogene20stprobesetCHRLENGTHS
...
mogene20stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting po-
...
mogene20stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- mogene20stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
mogene20stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

mogene20stprobesetENZYME
...
mogene20stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. mogene20stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the mogene20stprobesetENZYME maping viewed as a list, each manufacturer identifier
...
For the mogene20stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In mogene20stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert mogene20stprobesetENZYME2PROBE to a list to see inside
x <- mogene20stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mogene20stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- mogene20stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

mogene20stprobesetGO
...
mogene20stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

mogene20stprobesetGO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(mogene20stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

mogene20stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
mogene20stprobesetGO2PROBE.

Details

If mogene20stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If mogene20stprobesetGO2ALLPROBES or mogene20stprobesetGO2PROBE is cast as a list, each
...
• mogene20stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
mogene20stprobesetMGI

x <- mogene20stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
mogene20stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package mogene20stprobeset.db.
...
mogene20stprobesetMGI Map MGI gene accession numbers with manufacturer identifiers

Description

mogene20stprobesetMGI is an R object that contains mappings between manufacturer identifiers
...
The Organism information for mogene20stprobeset

Description

mogene20stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which mogene20stprobeset was built. mogene20stprobesetORGPKG is an
...
Although the package name is suggestive of the organism for which it was built, mogene20stprobesetORGANISM
provides a simple way to programmatically extract the organism name. mogene20stprobesetORGPKG
...
mogene20stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

mogene20stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- mogene20stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the mogene20stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mogene20stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
mogene20stprobesetPMID

Maps between Manufacturer Identifiers and PubMed Identifiers

Description

mogene20stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. mogene20stprobesetPMID2PROBE is an R object that provides mappings
...
When mogene20stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When mogene20stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- mogene20stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mogene20stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])

17

mogene20stprobesetPROSITE
...
mogene20stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
mogene20stprobesetREFSEQ

Map between Manufacturer Identifiers and RefSeq Identifiers

Description

mogene20stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
mogene20stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
fiers and gene abbreviations.

mogene20stprobesetUNIPROT
...
mogene20stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
mogene20stprobeset_dbconn()
mogene20stprobeset_dbfile()
mogene20stprobeset_dbschema(file="", show.indices=FALSE)
...
mogene20stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by mogene20stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
mogene20stprobeset_dbfile returns the path (character string) to the package annotation DB
...
mogene20stprobeset_dbschema prints the schema definition of the package annotation DB.
mogene20stprobeset_dbInfo prints other information about the package annotation DB.

Value

mogene20stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
mogene20stprobeset_dbfile: a character string with the path to the package annotation DB.
mogene20stprobeset_dbschema: none (invisible NULL).
mogene20stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(mogene20stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")

mogene20stprobeset_dbschema()
...
mogene20stprobeset.db, 2
mogene20stprobeset_dbconn, 20
mogene20stprobesetACCNUM, 1
mogene20stprobesetALIAS2PROBE, 2
mogene20stprobesetCHR, 3
mogene20stprobesetCHRLENGTHS, 4
mogene20stprobesetCHRLOC, 4
mogene20stprobesetENSEMBL, 5
mogene20stprobesetENTREZID, 6
mogene20stprobesetENZYME, 7
mogene20stprobesetGENENAME, 9
mogene20stprobesetGO, 10
mogene20stprobesetMAPCOUNTS, 12
mogene20stprobesetMGI, 12
mogene20stprobesetORGANISM, 13
mogene20stprobesetPATH, 14
mogene20stprobesetPFAM, 15
mogene20stprobesetPMID, 15
mogene20stprobesetPROSITE, 17
mogene20stprobesetREFSEQ, 17
mogene20stprobesetSYMBOL, 18
mogene20stprobesetUNIPROT, 19
...
mogene20stprobesetCHRLENGTHS, 4
mogene20stprobesetCHRLOC, 4
mogene20stprobesetCHRLOCEND

(mogene20stprobesetCHRLOC), 4

mogene20stprobesetENSEMBL, 5
mogene20stprobesetENSEMBL2PROBE

(mogene20stprobesetENSEMBL), 5

mogene20stprobesetENTREZID, 6
mogene20stprobesetENZYME, 7
mogene20stprobesetENZYME2PROBE

(mogene20stprobesetENZYME), 7

mogene20stprobesetGENENAME, 9
mogene20stprobesetGO, 10
mogene20stprobesetGO2ALLPROBES, 11
mogene20stprobesetGO2ALLPROBES
...
mogene20stprobesetLOCUSID

(mogene20stprobesetENTREZID), 6

mogene20stprobesetMAPCOUNTS, 12
mogene20stprobesetMGI, 12
mogene20stprobesetMGI2PROBE
...
mogene20stprobesetPATH, 14
mogene20stprobesetPATH2PROBE

(mogene20stprobesetPATH), 14

mogene20stprobesetPFAM, 15
mogene20stprobesetPMID, 15
mogene20stprobesetPMID2PROBE
...
mogene20stprobesetSYMBOL, 18
mogene20stprobesetUNIPROT, 19
mogene20stprobesetUNIPROT2PROBE
