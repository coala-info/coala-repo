...
hthgu133plusbACCNUM is an R object that contains mappings between a manufacturer’s identi-
...
hthgu133plusbALIAS is an R object that provides mappings between common gene symbol iden-
...
xx <- as.list(hthgu133plusbALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hthgu133plusb.db annotation Package. The purpose of this package is to provide
detailed information about the hthgu133plusb platform. This package is updated biannually.
...
hthgu133plusbCHR is an R object that provides mappings between a manufacturer identifier and
...
hthgu133plusbCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hthgu133plusbCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hthgu133plusbCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
hthgu133plusbENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

hthgu133plusbENSEMBL is an R object that contains mappings between manufacturer identifiers
...
hthgu133plusbENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

hthgu133plusbENTREZID is an R object that provides mappings between manufacturer identifiers
...
hthgu133plusbENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. hthgu133plusbENZYME2PROBE is an R object that maps Enzyme Commission
(EC) numbers to manufacturer identifiers.

Details

When the hthgu133plusbENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the hthgu133plusbENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In hthgu133plusbENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hthgu133plusbENZYME2PROBE to a list to see inside

hthgu133plusbGENENAME

9

x <- hthgu133plusbENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hthgu133plusbGENENAME Map between Manufacturer IDs and Genes

Description

hthgu133plusbGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hthgu133plusbGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hthgu133plusbGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

hthgu133plusbGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
hthgu133plusbGO2PROBE.

Details

If hthgu133plusbGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hthgu133plusbGO2ALLPROBES or hthgu133plusbGO2PROBE is cast as a list, each GO term
...
x <- hthgu133plusbGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hthgu133plusbGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hthgu133plusbMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package hthgu133plusb.db

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hthgu133plusbMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package hthgu133plusb.db.
...
hthgu133plusbOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
hthgu133plusbORGANISM The Organism information for hthgu133plusb

Description

hthgu133plusbORGANISM is an R object that contains a single item: a character string that names
the organism for which hthgu133plusb was built. hthgu133plusbORGPKG is an R object that
...
Although the package name is suggestive of the organism for which it was built, hthgu133plusbORGANISM
provides a simple way to programmatically extract the organism name. hthgu133plusbORGPKG
...
hthgu133plusbPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

hthgu133plusbPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hthgu133plusbPATH
mapped_probes <- mappedkeys(x)
# Now convert the hthgu133plusbPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133plusbPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hthgu133plusbPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hthgu133plusbPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hthgu133plusbPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hthgu133plusbPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hthgu133plusbPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hthgu133plusbPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133plusbPROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers
...
hthgu133plusbPROSITE is an R object that provides mappings between manufacturer identifiers
...
hthgu133plusbREFSEQ is an R object that provides mappings between manufacturer identifiers
...
hthgu133plusbSYMBOL is an R object that provides mappings between manufacturer identifiers
...
## Bimap interface:
x <- hthgu133plusbSYMBOL

20

hthgu133plusbUNIPROT
...
hthgu133plusbUNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

hthgu133plusbUNIPROT is an R object that contains mappings between the manufacturer identi-
...
hthgu133plusb_dbschema(file="", show.indices=FALSE)
...
hthgu133plusb_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by hthgu133plusb_dbconn or you will
break all the AnnDbObj objects defined in this package!

hthgu133plusb_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hthgu133plusb_dbschema prints the schema definition of the package annotation DB.

hthgu133plusb_dbInfo prints other information about the package annotation DB.

Value

hthgu133plusb_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

hthgu133plusb_dbfile: a character string with the path to the package annotation DB.

hthgu133plusb_dbschema: none (invisible NULL).

hthgu133plusb_dbInfo: none (invisible NULL).
...
dbGetQuery(hthgu133plusb_dbconn(), "SELECT COUNT(*) FROM probes")
...
hthgu133plusb.db, 2
hthgu133plusb_dbconn, 21
hthgu133plusbACCNUM, 1
hthgu133plusbALIAS2PROBE, 2
hthgu133plusbCHR, 3
hthgu133plusbCHRLENGTHS, 4
hthgu133plusbCHRLOC, 4
hthgu133plusbENSEMBL, 5
hthgu133plusbENTREZID, 6
hthgu133plusbENZYME, 7
hthgu133plusbGENENAME, 9
hthgu133plusbGO, 10
hthgu133plusbMAP, 12
hthgu133plusbMAPCOUNTS, 13
hthgu133plusbOMIM, 13
hthgu133plusbORGANISM, 14
hthgu133plusbPATH, 15
hthgu133plusbPFAM, 16
hthgu133plusbPMID, 16
hthgu133plusbPROSITE, 17
hthgu133plusbREFSEQ, 18
hthgu133plusbSYMBOL, 19
hthgu133plusbUNIPROT, 20
...
hthgu133plusb (hthgu133plusb.db), 2
hthgu133plusb.db, 2
hthgu133plusb_dbconn, 21
hthgu133plusb_dbfile
...
hthgu133plusbCHRLENGTHS, 4
hthgu133plusbCHRLOC, 4
hthgu133plusbCHRLOCEND

(hthgu133plusbCHRLOC), 4

hthgu133plusbENSEMBL, 5
hthgu133plusbENSEMBL2PROBE

(hthgu133plusbENSEMBL), 5

hthgu133plusbENTREZID, 6
hthgu133plusbENZYME, 7
hthgu133plusbENZYME2PROBE

(hthgu133plusbENZYME), 7

hthgu133plusbGENENAME, 9
hthgu133plusbGO, 10
hthgu133plusbGO2ALLPROBES, 11
hthgu133plusbGO2ALLPROBES
...
hthgu133plusbPATH, 15
hthgu133plusbPATH2PROBE

(hthgu133plusbPATH), 15

hthgu133plusbPFAM, 16
hthgu133plusbPMID, 16
hthgu133plusbPMID2PROBE
