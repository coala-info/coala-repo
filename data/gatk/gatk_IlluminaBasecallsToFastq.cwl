...
label: gatk_IlluminaBasecallsToFastq
...
inputs:
  ...
  - id: multiplex_params
    ...
    doc: Tab-separated file for creating all output FASTQs demultiplexed by 
      barcode for a lane with single IlluminaBasecallsToFastq invocation. 
      Required unless OUTPUT_PREFIX is provided.
    inputBinding:
      position: 101
  ...
  - id: output_prefix
    ...
    doc: The prefix for output FASTQs. Extensions as described above are 
      appended. Use this option for a non-barcoded run, or for a barcoded run in
      which it is not desired to demultiplex reads into separate files by 
      barcode. Required unless MULTIPLEX_PARAMS is provided.
    inputBinding:
      position: 101
  ...
  - id: min_trimmed_length
    type:
      - 'null'
      - int
    doc: The minimum length for a trimmed read. If trimming would create a 
      smaller read, then trim to this length instead
    inputBinding:
      position: 101
