...
hugene10sttranscriptclusterACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

hugene10sttranscriptclusterACCNUM is an R object that contains mappings between a manufac-
...
hugene10sttranscriptclusterALIAS2PROBE
...
hugene10sttranscriptclusterALIAS is an R object that provides mappings between common gene
...
Welcome to the hugene10sttranscriptcluster.db annotation Package. The purpose of this package
is to provide detailed information about the hugene10sttranscriptcluster platform. This package is
updated biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

hugene10sttranscriptclusterCHR
...
hugene10sttranscriptclusterCHR is an R object that provides mappings between a manufacturer
...
## Bimap interface:
x <- hugene10sttranscriptclusterCHR
...
hugene10sttranscriptclusterCHRLOC
...
hugene10sttranscriptclusterCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hugene10sttranscriptclusterCHRLENGTHS provides the length measured in base pairs for each of
the chromosomes.
...
## Bimap interface:
tt <- hugene10sttranscriptclusterCHRLENGTHS
...
hugene10sttranscriptclusterCHRLOC is an R object that maps manufacturer identifiers to the start-
...
hugene10sttranscriptclusterENSEMBL
...
## Bimap interface:
x <- hugene10sttranscriptclusterCHRLOC
...
hugene10sttranscriptclusterENSEMBL is an R object that contains mappings between manufacturer
...
hugene10sttranscriptclusterENTREZID
...
x <- hugene10sttranscriptclusterENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene10sttranscriptclusterENTREZID is an R object that provides mappings between manufac-
turer identifiers and Entrez Gene identifiers.

hugene10sttranscriptclusterENZYME
...
hugene10sttranscriptclusterENZYME is an R object that provides mappings between manufacturer
identifiers and EC numbers. hugene10sttranscriptclusterENZYME2PROBE is an R object that
...
When the hugene10sttranscriptclusterENZYME maping viewed as a list, each manufacturer identi-
...
For the hugene10sttranscriptclusterENZYME2PROBE, each EC number maps to a named vector
...
numbers. In hugene10sttranscriptclusterENZYME2PROBE, EC is dropped from the Enzyme Com-
mission numbers.
...
## Bimap interface:
x <- hugene10sttranscriptclusterENZYME
...
hugene10sttranscriptclusterGENENAME
...
# Now convert hugene10sttranscriptclusterENZYME2PROBE to a list to see inside
x <- hugene10sttranscriptclusterENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hugene10sttranscriptclusterGENENAME is an R object that maps manufacturer identifiers to the
...
hugene10sttranscriptclusterGO is an R object that provides mappings between manufacturer iden-
tifiers and the GO identifiers that they are directly associated with. This mapping and its reverse
mapping (hugene10sttranscriptclusterGO2PROBE) do NOT associate the child terms from the GO
ontology with the gene. Only the directly evidenced terms are represented here.

hugene10sttranscriptclusterGO2ALLPROBES is an R object that provides mappings between a
...
than hugene10sttranscriptclusterGO2PROBE.

Details

If hugene10sttranscriptclusterGO is cast as a list, each manufacturer identifier is mapped to a list
...
If hugene10sttranscriptclusterGO2ALLPROBES or hugene10sttranscriptclusterGO2PROBE is cast
...
• hugene10sttranscriptclusterGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
## Bimap interface:
x <- hugene10sttranscriptclusterGO
...
# For the reverse map:
x <- hugene10sttranscriptclusterGO2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene10sttranscriptclusterMAP

x <- hugene10sttranscriptclusterGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hugene10sttranscriptclusterGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene10sttranscriptclusterMAP is an R object that provides mappings between manufacturer iden-
...
hugene10sttranscriptclusterMAPCOUNTS
...
## Bimap interface:
x <- hugene10sttranscriptclusterMAP
...
hugene10sttranscriptclusterMAPCOUNTS provides the "map count" (i.e.
keys) for each map in package hugene10sttranscriptcluster.db.
...
hugene10sttranscriptclusterOMIM
...
hugene10sttranscriptclusterOMIM is an R object that provides mappings between manufacturer
...
## Bimap interface:
x <- hugene10sttranscriptclusterOMIM
...
The Organism information for hugene10sttranscriptcluster

Description

hugene10sttranscriptclusterORGANISM is an R object that contains a single item: a character
string that names the organism for which hugene10sttranscriptcluster was built. hugene10sttranscriptclusterORGPKG
...
hugene10sttranscriptclusterPATH

15

Details

Although the package name is suggestive of the organism for which it was built, hugene10sttranscriptclusterORGANISM
provides a simple way to programmatically extract the organism name. hugene10sttranscriptclusterORGPKG
...
## Bimap interface:
hugene10sttranscriptclusterORGANISM
hugene10sttranscriptclusterORGPKG
...
hugene10sttranscriptclusterPATH maps probe identifiers to the identifiers used by KEGG for path-
ways in which the genes represented by the probe identifiers are involved

hugene10sttranscriptclusterPATH2PROBE is an R object that provides mappings between KEGG
...
hugene10sttranscriptclusterPFAM
...
## Bimap interface:
x <- hugene10sttranscriptclusterPATH
...
x <- hugene10sttranscriptclusterPATH
mapped_probes <- mappedkeys(x)
# Now convert the hugene10sttranscriptclusterPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene10sttranscriptclusterPFAM is an R object that provides mappings between manufacturer
...
hugene10sttranscriptclusterPMID
...
hugene10sttranscriptclusterPMID is an R object that provides mappings between manufacturer
identifiers and PubMed identifiers. hugene10sttranscriptclusterPMID2PROBE is an R object that
provides mappings between PubMed identifiers and manufacturer identifiers.

Details

When hugene10sttranscriptclusterPMID is viewed as a list each manufacturer identifier is mapped
...
When hugene10sttranscriptclusterPMID2PROBE is viewed as a list each PubMed identifier is mapped
...
## Bimap interface:
x <- hugene10sttranscriptclusterPMID
...
hugene10sttranscriptclusterREFSEQ
...
x <- hugene10sttranscriptclusterPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hugene10sttranscriptclusterPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene10sttranscriptclusterPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

hugene10sttranscriptclusterPROSITE is an R object that provides mappings between manufacturer
...
hugene10sttranscriptclusterREFSEQ is an R object that provides mappings between manufacturer
...
## Bimap interface:
x <- hugene10sttranscriptclusterREFSEQ
...
hugene10sttranscriptclusterSYMBOL is an R object that provides mappings between manufacturer
...
hugene10sttranscriptclusterUNIPROT
...
hugene10sttranscriptclusterUNIPROT is an R object that contains mappings between the manufac-
...
## Bimap interface:
x <- hugene10sttranscriptclusterUNIPROT
...
hugene10sttranscriptcluster_dbschema(file="", show.indices=FALSE)
...
hugene10sttranscriptcluster_dbconn returns a connection object to the package annotation
DB. IMPORTANT: Don’t call dbDisconnect on the connection object returned by hugene10sttranscriptcluster_dbconn
or you will break all the AnnDbObj objects defined in this package!
hugene10sttranscriptcluster_dbfile returns the path (character string) to the package anno-
tation DB (this is an SQLite file).
hugene10sttranscriptcluster_dbschema prints the schema definition of the package annotation
DB.
hugene10sttranscriptcluster_dbInfo prints other information about the package annotation
DB.

Value

hugene10sttranscriptcluster_dbconn: a DBIConnection object representing an open connec-
tion to the package annotation DB.
hugene10sttranscriptcluster_dbfile: a character string with the path to the package annota-
tion DB.
hugene10sttranscriptcluster_dbschema: none (invisible NULL).
hugene10sttranscriptcluster_dbInfo: none (invisible NULL).
...
dbGetQuery(hugene10sttranscriptcluster_dbconn(), "SELECT COUNT(*) FROM probes")
...
hugene10sttranscriptclusterCHR, 3
hugene10sttranscriptclusterCHRLENGTHS,
...
hugene10sttranscriptclusterGO, 10
hugene10sttranscriptclusterMAP, 12
hugene10sttranscriptclusterMAPCOUNTS,
...
hugene10sttranscriptcluster.db, 2
hugene10sttranscriptcluster_dbconn, 21
hugene10sttranscriptcluster_dbfile
...
hugene10sttranscriptclusterACCNUM, 1
hugene10sttranscriptclusterALIAS2PROBE,
...
hugene10sttranscriptclusterCHRLOC, 4
hugene10sttranscriptclusterCHRLOCEND

(hugene10sttranscriptclusterCHRLOC),
4

hugene10sttranscriptclusterENSEMBL, 5
hugene10sttranscriptclusterENSEMBL2PROBE
...
hugene10sttranscriptclusterENTREZID, 6
hugene10sttranscriptclusterENZYME, 7
hugene10sttranscriptclusterENZYME2PROBE

(hugene10sttranscriptclusterENZYME),
7

hugene10sttranscriptclusterGENENAME, 9
hugene10sttranscriptclusterGO, 10
hugene10sttranscriptclusterGO2ALLPROBES,
...
hugene10sttranscriptclusterLOCUSID
...
hugene10sttranscriptclusterOMIM, 13
hugene10sttranscriptclusterORGANISM,
...
hugene10sttranscriptclusterPATH, 15
hugene10sttranscriptclusterPATH2PROBE

(hugene10sttranscriptclusterPATH),
15

hugene10sttranscriptclusterPFAM, 16
hugene10sttranscriptclusterPMID, 17
hugene10sttranscriptclusterPMID2PROBE

(hugene10sttranscriptclusterPMID),
17

hugene10sttranscriptclusterPROSITE, 18
hugene10sttranscriptclusterREFSEQ, 18
hugene10sttranscriptclusterSYMBOL, 20
hugene10sttranscriptclusterUNIPROT, 21
hugene10sttranscriptclusterUNIPROT2PROBE
