# Code example from 'v01-pRoloc-tutorial' vignette. See references/ for full tutorial.
...
## ----visualiseRes, fig.asp=1, eval = TRUE-------------------------------------
plot(dd)

## ----plotClusterings, fig.width=8, fig.height=4, eval = TRUE------------------
getMarkerClasses(hld, "MM")
plot(dd[["Endoplasmic reticulum/Golgi apparatus"]], hld)
plot(dd[["Actin cytoskeleton"]], hld)

## ----visualiseRaw, fig.asp=1, eval = TRUE-------------------------------------
plot(dd, method = "raw")

## ----plot2dnull, echo=TRUE, fig.asp=1, fig.cap="Plain PCA representation of the `tan2009r1` data."----
...
## ----params1, echo=TRUE, fig.cap="Assessing parameter optimisation. Here, we see the respective distributions of the 10 macro F1 scores for the best cost/sigma parameter pairs. See also the output of *f1Count* in relation to this plot."----
plot(params)

## ----params2, echo=TRUE, fig.cap="Assessing parameter optimisation. Visualisation of the averaged macro F1 scores, for the full range of parameter values."----
levelPlot(params)
...
## ----svmParamOptim2, eval = FALSE, message = FALSE, tidy=FALSE----------------
...
## ----medscores3, tidy = FALSE-------------------------------------------------
ts <- orgQuants(hl, fcol = "svm", t = .75)
hl <- getPredictions(hl, fcol = "svm", scol = "svm.scores", t = ts)

## ----plothl, fig.asp=1, echo=FALSE, fig.cap="Classification results following prediction from a class-weight SVM classifier on LOPIT data produced from mouse embryonic stem cells."----
