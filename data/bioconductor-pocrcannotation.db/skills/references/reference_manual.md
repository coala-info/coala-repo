...
POCRCannotationACCNUM Map Manufacturer identifiers to Accession Numbers

Description

POCRCannotationACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
POCRCannotationALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(POCRCannotationALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the POCRCannotation.db annotation Package. The purpose of this package is to pro-
vide detailed information about the POCRCannotation platform. This package is updated biannu-
ally.
...
POCRCannotationCHR is an R object that provides mappings between a manufacturer identifier
...
POCRCannotationCHRLOC

POCRCannotationCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

POCRCannotationCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- POCRCannotationCHRLENGTHS
# Length of chromosome 1
tt["1"]

POCRCannotationCHRLOC Map Manufacturer IDs to Chromosomal Location

Description

POCRCannotationCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
POCRCannotationENSEMBL is an R object that contains mappings between manufacturer identi-
...
# Convert to a list
xx <- as.list(POCRCannotationENSEMBL2PROBE)
if(length(xx) > 0){
...
POCRCannotationENTREZID is an R object that provides mappings between manufacturer identi-
...
POCRCannotationENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

POCRCannotationENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. POCRCannotationENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the POCRCannotationENZYME maping viewed as a list, each manufacturer identifier maps
...
For the POCRCannotationENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In POCRCannotationENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert POCRCannotationENZYME2PROBE to a list to see inside
xx <- as.list(POCRCannotationENZYME2PROBE)
if(length(xx) > 0){

# Get the probe identifiers for the first five enzyme

POCRCannotationGENENAME
...
POCRCannotationGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
POCRCannotationGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(POCRCannotationGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

POCRCannotationGO2ALLPROBES is an R object that provides mappings between a given GO
...
POCRCannotationGO2PROBE.

Details

If POCRCannotationGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If POCRCannotationGO2ALLPROBES or POCRCannotationGO2PROBE is cast as a list, each
...
# For the reverse map:
# Convert to a list
xx <- as.list(POCRCannotationGO2PROBE)
if(length(xx) > 0){
...
# Convert POCRCannotationGO2ALLPROBES to a list
xx <- as.list(POCRCannotationGO2ALLPROBES)
if(length(xx) > 0){
...
POCRCannotationMAP is an R object that provides mappings between manufacturer identifiers
...
POCRCannotationMAPCOUNTS

13

POCRCannotationMAPCOUNTS

Number of mapped keys for the maps in package POCRCannota-
tion.db
...
POCRCannotationMAPCOUNTS provides the "map count" (i.e.
each map in package POCRCannotation.db.
...
POCRCannotationOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for POCRCannotation

Description

POCRCannotationORGANISM is an R object that contains a single item: a character string that
names the organism for which POCRCannotation was built. POCRCannotationORGPKG is an R
...
Although the package name is suggestive of the organism for which it was built, POCRCannota-
tionORGANISM provides a simple way to programmatically extract the organism name. POCR-
CannotationORGPKG provides a simple way to programmatically extract the name of the parent
...
POCRCannotationPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

POCRCannotationPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the POCRCannotationPATH2PROBE object to a list
xx <- as.list(POCRCannotationPATH2PROBE)
if(length(xx) > 0){
...
POCRCannotationPFAM is an R object that provides mappings between manufacturer identifiers
...
POCRCannotationPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. POCRCannotationPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When POCRCannotationPMID is viewed as a list each manufacturer identifier is mapped to a
...
When POCRCannotationPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object POCRCannotationPMID2PROBE to a list
xx <- as.list(POCRCannotationPMID2PROBE)
if(length(xx) > 0){
...
POCRCannotationPROSITE is an R object that provides mappings between manufacturer identi-
...
POCRCannotationREFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

POCRCannotationREFSEQ is an R object that provides mappings between manufacturer identifiers
...
POCRCannotationSYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

POCRCannotationSYMBOL is an R object that provides mappings between manufacturer identi-
...
## Bimap interface:
x <- POCRCannotationSYMBOL

20

POCRCannotationUNIGENE
...
POCRCannotationUNIGENE is an R object that provides mappings between manufacturer identi-
...
POCRCannotationUNIPROT is an R object that contains mappings between the manufacturer iden-
...
POCRCannotation_dbconn()
POCRCannotation_dbfile()
POCRCannotation_dbschema(file="", show.indices=FALSE)
...
POCRCannotation_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by POCRCannotation_dbconn
or you will break all the AnnDbObj objects defined in this package!
POCRCannotation_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).
POCRCannotation_dbschema prints the schema definition of the package annotation DB.
POCRCannotation_dbInfo prints other information about the package annotation DB.

Value

POCRCannotation_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.
POCRCannotation_dbfile: a character string with the path to the package annotation DB.
POCRCannotation_dbschema: none (invisible NULL).
POCRCannotation_dbInfo: none (invisible NULL).
...
dbGetQuery(POCRCannotation_dbconn(), "SELECT COUNT(*) FROM probes")

POCRCannotation_dbschema()
...
POCRCannotation.db, 2
POCRCannotation_dbconn, 22
POCRCannotationACCNUM, 1
POCRCannotationALIAS2PROBE, 2
POCRCannotationCHR, 3
POCRCannotationCHRLENGTHS, 4
POCRCannotationCHRLOC, 4
POCRCannotationENSEMBL, 5
POCRCannotationENTREZID, 6
POCRCannotationENZYME, 7
POCRCannotationGENENAME, 9
POCRCannotationGO, 10
POCRCannotationMAP, 12
POCRCannotationMAPCOUNTS, 13
POCRCannotationOMIM, 13
POCRCannotationORGANISM, 14
POCRCannotationPATH, 15
POCRCannotationPFAM, 16
POCRCannotationPMID, 16
POCRCannotationPROSITE, 17
POCRCannotationREFSEQ, 18
POCRCannotationSYMBOL, 19
POCRCannotationUNIGENE, 20
POCRCannotationUNIPROT, 21
...
POCRCannotation (POCRCannotation.db), 2
POCRCannotation.db, 2
POCRCannotation_dbconn, 22
...
POCRCannotationCHRLENGTHS, 4
POCRCannotationCHRLOC, 4
POCRCannotationCHRLOCEND

(POCRCannotationCHRLOC), 4

POCRCannotationENSEMBL, 5
POCRCannotationENSEMBL2PROBE

(POCRCannotationENSEMBL), 5

POCRCannotationENTREZID, 6
POCRCannotationENZYME, 7
POCRCannotationENZYME2PROBE

(POCRCannotationENZYME), 7

POCRCannotationGENENAME, 9
POCRCannotationGO, 10
POCRCannotationGO2ALLPROBES, 11
POCRCannotationGO2ALLPROBES
...
POCRCannotationPATH, 15
POCRCannotationPATH2PROBE

(POCRCannotationPATH), 15

POCRCannotationPFAM, 16
POCRCannotationPMID, 16
POCRCannotationPMID2PROBE
