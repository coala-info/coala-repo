...
* [2 Setup](#setup)
  ...
  + [2.2 Data](#data)
* [3 Differential Signal Analysis](#differential-signal-analysis)
  + [3.1 Sliding Windows](#sliding-windows)
  + [3.2 Filtering of Sliding Windows](#filtering-of-sliding-windows)
  + [3.3 Initial Visualisation](#initial-visualisation)
  + [3.4 Statistical Testing](#statistical-testing)
  + [3.5 Merging Windows](#merging-windows)
  + [3.6 Alternative Normalisation Approaches](#alternative-normalisation-approaches)
    - [3.6.1 TMM Normalisation](#tmm-normalisation)
* [4 Mapping of Windows](#mapping-of-windows)
  + [4.1 Mapping to Genes](#mapping-to-genes)
  + [4.2 Mapping to Regions](#mapping-to-regions)
* [5 Visualisation of Results](#visualisation-of-results)
  + [5.1 Pie Charts](#pie-charts)
  + [5.2 Split Donut Charts](#split-donut-charts)
  + [5.3 Coverage Plots](#coverage-plots)
    - [5.3.1 Adding Annotations To Coverage](#adding-annotations-to-coverage)
    - [5.3.2 Displaying Genes and Features](#displaying-genes-and-features)
...
![*A) Densities for logCPM values across all samples after discarding windows less likely to contain H3K27ac signal using `dualFilter()` B) PCA plot based on the logCPM assay*](data:image/png;base64...)
...
![*RLE plots (A) across all samples and (B) with values calculated within treatment groups*](data:image/png;base64...)
...
methods, a range-based \(H\_0\) (McCarthy and Smyth 2009) can be specified
...
(Wilson 2019). This approach tests H0: no \(p\)-value in the
...
weights \(w\_i = \frac{1}{p\_i}\). A representative window, corresponding to
...
![*MA Plots using A) all sliding windows (after filtering before merging) and B) the final set of merged windows. The most highly ranked merged window is also labelled in the set of merged windows. Blue lines represent a loess curve through the data.*](data:image/png;base64...)
...
![*MA-plot with the top 20 regions by significance labelled according to the most likely gene influenced by the detected signal. Regions which directly overlap a TSS are shown as empty circles*](data:image/png;base64...)
...
![*Pie chart showing customised labels. Here the regions have been modified to use title case in the labels, but without modifying the underlying data.*](data:image/png;base64...)
...
![*Split-Donut chart showing status with overlapping region*](data:image/png;base64...)
...
![*Split-Donut chart exploding key regions and customising labels.*](data:image/png;base64...)
...
![The addition of an annotation track for the coverage tracks shows which regions were retained during the analysis, as well as those which were considered as showing changed or unchanged signal](data:image/png;base64...)
...
RNA-seq Read Counts.” *Genome Biol.* 15 (2):
R29.
...
RNA-sequence Data Using Quasi-Likelihood
