...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Febi-gene-expression-group%2Fscran-cli%2Fblob%2Fdevelop%2Fscran-model-gene-var-with-spikes.R)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=ebi-gene-expression-group%2Fscran-cli)
...
1. [scran-cli](/ebi-gene-expression-group/scran-cli/tree/develop)
...
[History](/ebi-gene-expression-group/scran-cli/commits/develop/scran-model-gene-var-with-spikes.R)
...
[Raw](https://github.com/ebi-gene-expression-group/scran-cli/raw/refs/heads/develop/scran-model-gene-var-with-spikes.R)
...
opt = wsc\_parse\_args(option\_list, mandatory = c("input\_sce\_object", "output\_geneVarSpikes\_table"))

#read SCE object

if(!file.exists(opt$input\_sce\_object)) stop("Input file does not exist.")

sce <- readRDS(opt$input\_sce\_object)

#Model Variance

suppressPackageStartupMessages(require(scran))

var\_table <- modelGeneVarWithSpikes(sce, block=opt$block, spikes=opt$spikes, size.factors=opt$size\_factors, spike.size.factors=opt$spike\_size\_factors,

assay.type=opt$assay\_type, design=opt$design,

subset.row=opt$subset\_row, subset.fit=opt$subset\_fill, equiweight=opt$equiweight, method=opt$method,

parametric=opt$parametric, min.mean=opt$min\_mean)

#save table containing modelled variance and its biological and technical estimates

write.table(var\_table, opt$output\_geneVarSpikes\_table, sep="\t")
