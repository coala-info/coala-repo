...
hgfocusACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgfocusALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgfocus.db annotation Package. The purpose of this package is to provide detailed
information about the hgfocus platform. This package is updated biannually.
...
hgfocusCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgfocusCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgfocusCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgfocusENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgfocusENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgfocusENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgfocusENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgfocusENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgfocusENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgfocusENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgfocusENZYME2PROBE to a list to see inside

hgfocusGENENAME

9

x <- hgfocusENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgfocusGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgfocusGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgfo-
...
hgfocusGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than hgfo-
cusGO2PROBE.

Details

If hgfocusGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgfocusGO2ALLPROBES or hgfocusGO2PROBE is cast as a list, each GO term maps to a
...
x <- hgfocusGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgfocusGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hgfocusMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgfocus.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgfocusMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgfocus.db.
...
hgfocusOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgfocus

Description

hgfocusORGANISM is an R object that contains a single item: a character string that names the
organism for which hgfocus was built. hgfocusORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgfocusORGAN-
ISM provides a simple way to programmatically extract the organism name. hgfocusORGPKG
...
hgfocusPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgfocusPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- hgfocusPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgfocusPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgfocusPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgfocusPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgfocusPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgfocusPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgfocusPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
x <- hgfocusPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgfocusPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgfocusPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgfocusREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgfocusSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgfocusUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgfocus_dbconn()
hgfocus_dbfile()
hgfocus_dbschema(file="", show.indices=FALSE)
hgfocus_dbInfo()
...
hgfocus_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgfocus_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgfocus_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgfocus_dbschema prints the schema definition of the package annotation DB.

hgfocus_dbInfo prints other information about the package annotation DB.

Value

hgfocus_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

hgfocus_dbfile: a character string with the path to the package annotation DB.

hgfocus_dbschema: none (invisible NULL).

hgfocus_dbInfo: none (invisible NULL).
...
dbGetQuery(hgfocus_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgfocus (hgfocus.db), 2
hgfocus.db, 2
hgfocus_dbconn, 21
hgfocus_dbfile (hgfocus_dbconn), 21

23

hgfocus_dbInfo (hgfocus_dbconn), 21
hgfocus_dbschema (hgfocus_dbconn), 21
hgfocusACCNUM, 1
hgfocusALIAS2PROBE, 2
hgfocusCHR, 3
hgfocusCHRLENGTHS, 4
hgfocusCHRLOC, 4
hgfocusCHRLOCEND (hgfocusCHRLOC), 4
hgfocusENSEMBL, 5
hgfocusENSEMBL2PROBE (hgfocusENSEMBL), 5
hgfocusENTREZID, 6
hgfocusENZYME, 7
hgfocusENZYME2PROBE (hgfocusENZYME), 7
hgfocusGENENAME, 9
hgfocusGO, 10
hgfocusGO2ALLPROBES, 11
hgfocusGO2ALLPROBES (hgfocusGO), 10
hgfocusGO2PROBE (hgfocusGO), 10
hgfocusLOCUSID (hgfocusENTREZID), 6
hgfocusMAP, 12
hgfocusMAPCOUNTS, 13
hgfocusOMIM, 13
hgfocusORGANISM, 14
hgfocusORGPKG (hgfocusORGANISM), 14
hgfocusPATH, 15
hgfocusPATH2PROBE (hgfocusPATH), 15
hgfocusPFAM, 16
hgfocusPMID, 16
hgfocusPMID2PROBE (hgfocusPMID), 16
hgfocusPROSITE, 17
hgfocusREFSEQ, 18
hgfocusSYMBOL, 19
hgfocusUNIPROT, 20
hgfocusUNIPROT2PROBE (hgfocusUNIPROT),
