...
baseCommand:
  - pdftk
label: pdftk_dump_data_annots
doc: "Reads a single input PDF file and reports annotation information (currently
  only link annotations) to the given output filename or stdout.\n\nTool homepage:
...
inputs:
  ...
  - id: compress
    type:
      - 'null'
      - boolean
    doc: Use PDF page stream compression
    inputBinding:
      position: 103
      prefix: compress
  - id: do_ask
  ...
  - id: encrypt_128bit
    type:
      - 'null'
      - boolean
    doc: Use 128-bit encryption on output
    inputBinding:
      position: 103
  ...
  - id: encrypt_40bit
    type:
      - 'null'
      - boolean
    doc: Use 40-bit encryption on output
    inputBinding:
      position: 103
...
outputs:
  - id: output
    type:
      - 'null'
      - File
    doc: Output filename for the annotation data report
    outputBinding:
      glob: $(inputs.output)
