...
hu35ksubdACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hu35ksubdALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hu35ksubdALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the hu35ksubd.db annotation Package. The purpose of this package is to provide de-
tailed information about the hu35ksubd platform. This package is updated biannually.
...
hu35ksubdCHR is an R object that provides mappings between a manufacturer identifier and the
...
hu35ksubdCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hu35ksubdCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hu35ksubdENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hu35ksubdENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hu35ksubdENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hu35ksubdENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hu35ksubdENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hu35ksubdENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hu35ksubdENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hu35ksubdENZYME2PROBE to a list to see inside

hu35ksubdGENENAME

9

x <- hu35ksubdENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hu35ksubdGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hu35ksubdGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hu35ksubdGO2PROBE)
...
hu35ksubdGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hu35ksubdGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hu35ksubdGO2ALLPROBES or hu35ksubdGO2PROBE is cast as a list, each GO term maps to
...
x <- hu35ksubdGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hu35ksubdGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hu35ksubdMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hu35ksubd.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hu35ksubdMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hu35ksubd.db.
...
hu35ksubdOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hu35ksubd

Description

hu35ksubdORGANISM is an R object that contains a single item: a character string that names
the organism for which hu35ksubd was built. hu35ksubdORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hu35ksubdORGANISM
provides a simple way to programmatically extract the organism name. hu35ksubdORGPKG pro-
...
hu35ksubdPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hu35ksubdPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hu35ksubdPATH
mapped_probes <- mappedkeys(x)
# Now convert the hu35ksubdPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubdPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hu35ksubdPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hu35ksubdPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hu35ksubdPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hu35ksubdPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hu35ksubdPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hu35ksubdPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubdPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hu35ksubdREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hu35ksubdSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hu35ksubdUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hu35ksubd_dbconn()
hu35ksubd_dbfile()
hu35ksubd_dbschema(file="", show.indices=FALSE)
hu35ksubd_dbInfo()
...
hu35ksubd_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hu35ksubd_dbconn or you will break all
the AnnDbObj objects defined in this package!

hu35ksubd_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hu35ksubd_dbschema prints the schema definition of the package annotation DB.

hu35ksubd_dbInfo prints other information about the package annotation DB.

Value

hu35ksubd_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

hu35ksubd_dbfile: a character string with the path to the package annotation DB.

hu35ksubd_dbschema: none (invisible NULL).

hu35ksubd_dbInfo: none (invisible NULL).
...
dbGetQuery(hu35ksubd_dbconn(), "SELECT COUNT(*) FROM probes")
...
hu35ksubd_dbconn, 21
hu35ksubdACCNUM, 1
hu35ksubdALIAS2PROBE, 2
hu35ksubdCHR, 3
hu35ksubdCHRLENGTHS, 4
hu35ksubdCHRLOC, 4
hu35ksubdENSEMBL, 5
hu35ksubdENTREZID, 6
hu35ksubdENZYME, 7
...
hu35ksubdGO, 10
hu35ksubdMAP, 12
hu35ksubdMAPCOUNTS, 13
hu35ksubdOMIM, 13
hu35ksubdORGANISM, 14
hu35ksubdPATH, 15
hu35ksubdPFAM, 16
hu35ksubdPMID, 16
hu35ksubdPROSITE, 17
...
hu35ksubd (hu35ksubd.db), 2
hu35ksubd.db, 2
hu35ksubd_dbconn, 21
hu35ksubd_dbfile (hu35ksubd_dbconn), 21

23

hu35ksubd_dbInfo (hu35ksubd_dbconn), 21
hu35ksubd_dbschema (hu35ksubd_dbconn),
...
hu35ksubdCHRLENGTHS, 4
hu35ksubdCHRLOC, 4
hu35ksubdCHRLOCEND (hu35ksubdCHRLOC), 4
hu35ksubdENSEMBL, 5
hu35ksubdENSEMBL2PROBE

(hu35ksubdENSEMBL), 5

hu35ksubdENTREZID, 6
hu35ksubdENZYME, 7
hu35ksubdENZYME2PROBE

(hu35ksubdENZYME), 7

hu35ksubdGENENAME, 9
hu35ksubdGO, 10
hu35ksubdGO2ALLPROBES, 11
hu35ksubdGO2ALLPROBES (hu35ksubdGO), 10
hu35ksubdGO2PROBE (hu35ksubdGO), 10
hu35ksubdLOCUSID (hu35ksubdENTREZID), 6
hu35ksubdMAP, 12
hu35ksubdMAPCOUNTS, 13
hu35ksubdOMIM, 13
hu35ksubdORGANISM, 14
hu35ksubdORGPKG (hu35ksubdORGANISM), 14
hu35ksubdPATH, 15
hu35ksubdPATH2PROBE (hu35ksubdPATH), 15
hu35ksubdPFAM, 16
hu35ksubdPMID, 16
hu35ksubdPMID2PROBE (hu35ksubdPMID), 16
hu35ksubdPROSITE, 17
hu35ksubdREFSEQ, 18
hu35ksubdSYMBOL, 19
hu35ksubdUNIPROT, 20
hu35ksubdUNIPROT2PROBE
