...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fnatir%2Fknot%2Fblob%2Fmaster%2FReadme.md)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=natir%2Fknot)
...
[History](/natir/knot/commits/master/Readme.md)

History

258 lines (198 loc) · 10 KB
...
[Raw](https://github.com/natir/knot/raw/refs/heads/master/Readme.md)
...
* [Usage](#usage)
  + [Run knot](#run-knot)
  + [Generate html report on knot result](/natir/knot/blob/master/generate-html-report-on-knot-result)
...
The AAG is present in the `{output prefix}_AAG.csv`. (Note: the other GFA files produced by KNOT are *not* the AAG)
...
5. nb\_read: nb\_read in path between read1 and read2 (include)
6. nb\_base: nb\_base in path between read2 and read2
7. paths: id of read in path found between read1 and read2, separated by ;
8. nbread\_contig: number of read assign for each contig in format {tig name}:{nb of read in paths assign to contig}/{nb of read in tig} not\_assign used to read not assigned to a contig, separated by ;.
...
You can use corrected long reads in place of raw\_reads with `-m` option.
...
[![Globale pipeline](/natir/knot/raw/master/images/pipeline.png)](/natir/knot/blob/master/images/pipeline.png)
...
├── contigs_filtred.paf     # corresponding paf file made usin