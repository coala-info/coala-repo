...
h10kcodACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
h10kcodALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the h10kcod.db annotation Package. The purpose of this package is to provide detailed
information about the h10kcod platform. This package is updated biannually.
...
h10kcodCHR is an R object that provides mappings between a manufacturer identifier and the
...
h10kcodCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
h10kcodCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
h10kcodENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(h10kcodENSEMBL2PROBE)
if(length(xx) > 0){
...
h10kcodENTREZID is an R object that provides mappings between manufacturer identifiers and
...
h10kcodENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. h10kcodENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the h10kcodENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the h10kcodENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In h10kcodENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert h10kcodENZYME2PROBE to a list to see inside
xx <- as.list(h10kcodENZYME2PROBE)
if(length(xx) > 0){
...
h10kcodGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
h10kcodGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (h10kcodGO2PROBE)
...
h10kcodGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than h10kcodGO2PROBE.

Details

If h10kcodGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If h10kcodGO2ALLPROBES or h10kcodGO2PROBE is cast as a list, each GO term maps to a
...
# Convert h10kcodGO2ALLPROBES to a list
xx <- as.list(h10kcodGO2ALLPROBES)
if(length(xx) > 0){
...
h10kcodMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package h10kcod.db

Description

h10kcodMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package h10kcod.db.
...
h10kcodMAPCOUNTS
mapnames <- names(h10kcodMAPCOUNTS)
h10kcodMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package h10kcod.db
checkMAPCOUNTS("h10kcod.db")
...
h10kcodOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for h10kcod

Description

h10kcodORGANISM is an R object that contains a single item: a character string that names the
organism for which h10kcod was built. h10kcodORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, h10kcodORGANISM
provides a simple way to programmatically extract the organism name. h10kcodORGPKG provides
...
h10kcodPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

h10kcodPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the h10kcodPATH2PROBE object to a list
xx <- as.list(h10kcodPATH2PROBE)
if(length(xx) > 0){
...
h10kcodPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. h10kcodPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When h10kcodPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When h10kcodPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object h10kcodPMID2PROBE to a list

h10kcodREFSEQ

15

xx <- as.list(h10kcodPMID2PROBE)
if(length(xx) > 0){
...
h10kcodREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
h10kcodSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
h10kcodUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
h10kcodUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
h10kcod_dbconn()
h10kcod_dbfile()
h10kcod_dbschema(file="", show.indices=FALSE)
h10kcod_dbInfo()
...
h10kcod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by h10kcod_dbconn or you will break all the
AnnDbObj objects defined in this package!
h10kcod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
h10kcod_dbschema prints the schema definition of the package annotation DB.
h10kcod_dbInfo prints other information about the package annotation DB.

Value

h10kcod_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
h10kcod_dbfile: a character string with the path to the package annotation DB.
h10kcod_dbschema: none (invisible NULL).
h10kcod_dbInfo: none (invisible NULL).
...
dbGetQuery(h10kcod_dbconn(), "SELECT COUNT(*) FROM probes")
...
h10kcod (h10kcod.db), 2
h10kcod.db, 2
h10kcod_dbconn, 18
h10kcod_dbfile (h10kcod_dbconn), 18

20

h10kcod_dbInfo (h10kcod_dbconn), 18
h10kcod_dbschema (h10kcod_dbconn), 18
h10kcodACCNUM, 1
h10kcodALIAS2PROBE, 2
h10kcodCHR, 3
h10kcodCHRLENGTHS, 3
h10kcodCHRLOC, 4
h10kcodCHRLOCEND (h10kcodCHRLOC), 4
h10kcodENSEMBL, 4
h10kcodENSEMBL2PROBE (h10kcodENSEMBL), 4
h10kcodENTREZID, 5
h10kcodENZYME, 6
h10kcodENZYME2PROBE (h10kcodENZYME), 6
h10kcodGENENAME, 7
h10kcodGO, 8
h10kcodGO2ALLPROBES, 9
h10kcodGO2ALLPROBES (h10kcodGO), 8
h10kcodGO2PROBE (h10kcodGO), 8
h10kcodLOCUSID (h10kcodENTREZID), 5
h10kcodMAP, 10
h10kcodMAPCOUNTS, 11
h10kcodOMIM, 11
h10kcodORGANISM, 12
h10kcodORGPKG (h10kcodORGANISM), 12
h10kcodPATH, 13
h10kcodPATH2PROBE (h10kcodPATH), 13
h10kcodPMID, 14
h10kcodPMID2PROBE (h10kcodPMID), 14
h10kcodREFSEQ, 15
h10kcodSYMBOL, 16
h10kcodUNIGENE, 17
h10kcodUNIPROT, 17
h10kcodUNIPROT2PROBE (h10kcodUNIPROT),
