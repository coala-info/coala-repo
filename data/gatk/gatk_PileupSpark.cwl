...
baseCommand:
  - gatk
  - PileupSpark
label: gatk_PileupSpark
doc: Prints read alignments in samtools pileup format. The tool leverages the 
  Spark framework for faster operation. The output comprises one line per 
  genomic position, listing the chromosome name, coordinate, reference base, 
  read bases, and read qualities.
inputs:
  - id: input
    type:
      type: array
  ...
  - id: show_verbose
    type:
      - 'null'
      - boolean
    doc: Add extra informative columns to the pileup output.
    inputBinding:
      position: 101
      prefix: --show-verbose
  - id: shuffle
    type:
      - 'null'
      - boolean
    doc: whether to use the shuffle implementation or overlaps partitioning (the
      default)
    inputBinding:
      position: 101
      prefix: --shuffle
