...
baseCommand:
  - phykit
  - density_map
label: phykit_density_map
doc: Density Map visualization of posterior discrete state probabilities along 
  each branch of a phylogeny. Runs stochastic character mapping internally (N 
  simulations), then for each point along each branch computes the fraction of 
  simulations in each state.
inputs:
  - id: tree
    type: File
    doc: a tree file
