...
illuminaMousev1p1ACCNUM is an R object that contains mappings between a manufacturer’s
...
illuminaMousev1p1ALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(illuminaMousev1p1ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the illuminaMousev1p1.db annotation Package. The purpose of this package is to pro-
vide detailed information about the illuminaMousev1p1 platform. This package is updated biannu-
ally.
...
illuminaMousev1p1CHR

3

illuminaMousev1p1CHR Map Manufacturer IDs to Chromosomes

Description

illuminaMousev1p1CHR is an R object that provides mappings between a manufacturer identifier
...
illuminaMousev1p1CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

illuminaMousev1p1CHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
tt <- illuminaMousev1p1CHRLENGTHS
# Length of chromosome 1
tt["1"]

4

illuminaMousev1p1ENSEMBL

illuminaMousev1p1CHRLOC

Map Manufacturer IDs to Chromosomal Location

Description

illuminaMousev1p1CHRLOC is an R object that maps manufacturer identifiers to the starting po-
sition of the gene. The position of a gene is measured as the number of base pairs.
...
illuminaMousev1p1ENSEMBL is an R object that contains mappings between manufacturer iden-
tifiers and Ensembl gene accession numbers.

illuminaMousev1p1ENTREZID
...
illuminaMousev1p1ENTREZID is an R object that provides mappings between manufacturer iden-
...
illuminaMousev1p1ENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. illuminaMousev1p1ENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the illuminaMousev1p1ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the illuminaMousev1p1ENZYME2PROBE, each EC number maps to a named vector contain-
ing all of the manufacturer identifiers that correspond to the gene that produces that enzyme. The
name of the vector corresponds to the EC number.
...
numbers. In illuminaMousev1p1ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert illuminaMousev1p1ENZYME2PROBE to a list to see inside
xx <- as.list(illuminaMousev1p1ENZYME2PROBE)
if(length(xx) > 0){
...
illuminaMousev1p1GENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
illuminaMousev1p1GO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(illuminaMousev1p1GO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

illuminaMousev1p1GO2ALLPROBES is an R object that provides mappings between a given GO
...
illuminaMousev1p1GO2PROBE.

Details

If illuminaMousev1p1GO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If illuminaMousev1p1GO2ALLPROBES or illuminaMousev1p1GO2PROBE is cast as a list, each
...
illuminaMousev1p1GO2ALLPROBES.

Examples

x <- illuminaMousev1p1GO
...
# For the reverse map:
# Convert to a list
xx <- as.list(illuminaMousev1p1GO2PROBE)
if(length(xx) > 0){
...
# Convert illuminaMousev1p1GO2ALLPROBES to a list
xx <- as.list(illuminaMousev1p1GO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package illuminaMou-
sev1p1.db

Description

illuminaMousev1p1MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package illuminaMousev1p1.db.
...
illuminaMousev1p1MGI Map MGI gene accession numbers with manufacturer identifiers

Description

illuminaMousev1p1MGI is an R object that contains mappings between manufacturer identifiers
...
We have used an extensive re-annotation of the illuminaMousev1p1 probe sequences to provide
additional information that is not captured in the standard Bioconductor packages. Whereas Bio-
conductor annotations are based on the RefSeq ID that each probe maps to, our additional mappings
...
illuminaMousev1p1listNewMappings List all the custom re-annotation mappings provided by

the package

illuminaMousev1p1fullReannotation Return all the re-annotation information as a matrix
illuminaMousev1p1ARRAYADDRESS Array Address code used to identify the probe at the

bead-level

illuminaMousev1p1NUID Lumi’s nuID (universal naming scheme for oligonucleotides) Refer-

ence: Du et al. (2007), Biol Direct 2:16

illuminaMousev1p1PROBESEQUENCE The 50 base sequence for the probe
illuminaMousev1p1PROBEQUALITY Quality grade assigned to the probe: “Perfect” if it per-
...
illuminaMousev1p1CODINGZONE Coding status of target sequence:

intergenic / intronic /
Transcriptomic (“Transcriptomic” when the target transcript is non-coding or there is no in-
formation on the coding sequence)

illuminaMousev1p1GENOMICLOCATION Probe’s genomic coordinates (hg19 for human, mm9

for mouse or rn4 for rat)

illuminaMousev1p1GENOMICMATCHSIMILARITY Percentage of similarity between the probe

and its best genomic match in the alignable region, taking the probe as reference

illuminaMousev1p1SECONDMATCHES Genomic coordinates of second best matches between

the probe and the genome

illuminaMousev1p1SECONDMATCHSIMILARITY Percentage of similarity between the probe
and its second best genomic match in the alignable region, taking the probe as reference
illuminaMousev1p1TRANSCRIPTOMICMATCHSIMILARITY Percentage of similarity be-
tween the probe and its target transcript in the alignable region, taking the probe as reference
illuminaMousev1p1OTHERGENOMICMATCHES Genomic coordinates of sequences as alignable

with the probe (in terms of number of matching nucleotides) as its main target

illuminaMousev1p1REPEATMASK Overlapping RepeatMasked sequences, with number of bases

overlapped by the repeat

illuminaMousev1p1OVERLAPPINGSNP Overlapping annotated SNPs
illuminaMousev1p1ENTREZREANNOTATED Entrez IDs
illuminaMousev1p1ENSEMBLREANNOTATED Ensembl IDs
illuminaMousev1p1SYMBOLREANNOTATED Gene symbol derived by re-annotation
illuminaMousev1p1REPORTERGROUPID For probes marked as controls in Illuminas annota-

tion file, these gives the type of control

illuminaMousev1p1REPORTERGROUPNAME Usually a more informative name for the con-
...
x <- illuminaMousev1p1PROBEQUALITY
...
x <- illuminaMousev1p1ARRAYADDRESS
...
y<- revmap(illuminaMousev1p1ARRAYADDRESS)
...
x <- illuminaMousev1p1PROBESEQUENCE
...
The Organism information for illuminaMousev1p1

Description

illuminaMousev1p1ORGANISM is an R object that contains a single item: a character string that
names the organism for which illuminaMousev1p1 was built. illuminaMousev1p1ORGPKG is an R
...
sev1p1ORGANISM provides a simple way to programmatically extract the organism name. illumi-
naMousev1p1ORGPKG provides a simple way to programmatically extract the name of the parent
...
illuminaMousev1p1ORGANISM
illuminaMousev1p1ORGPKG

illuminaMousev1p1PATH

15

illuminaMousev1p1PATH Mappings between probe identifiers and KEGG pathway identifiers
...
illuminaMousev1p1PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

illuminaMousev1p1PATH2PROBE is an R object that provides mappings between KEGG identi-
...
# Now convert the illuminaMousev1p1PATH2PROBE object to a list
xx <- as.list(illuminaMousev1p1PATH2PROBE)
if(length(xx) > 0){
...
illuminaMousev1p1PMID Maps between Manufacturer Identifiers and PubMed Identifiers

Description

illuminaMousev1p1PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. illuminaMousev1p1PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When illuminaMousev1p1PMID is viewed as a list each manufacturer identifier is mapped to a
...
When illuminaMousev1p1PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
illuminaMousev1p1REFSEQ is an R object that provides mappings between manufacturer identi-
...
illuminaMousev1p1SYMBOL is an R object that provides mappings between manufacturer identi-
...
illuminaMousev1p1UNIGENE is an R object that provides mappings between manufacturer iden-
tifiers and UniGene identifiers.
...
illuminaMousev1p1UNIPROT is an R object that contains mappings between the manufacturer
...
illuminaMousev1p1_dbconn

x <- illuminaMousev1p1UNIPROT
...
illuminaMousev1p1_dbconn()
illuminaMousev1p1_dbfile()
illuminaMousev1p1_dbschema(file="", show.indices=FALSE)
...
illuminaMousev1p1_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by illuminaMousev1p1_dbconn
or you will break all the AnnDbObj objects defined in this package!

illuminaMousev1p1_dbfile returns the path (character string) to the package annotation DB (this
is an SQLite file).

illuminaMousev1p1_dbschema prints the schema definition of the package annotation DB.

illuminaMousev1p1_dbInfo prints other information about the package annotation DB.
...
illuminaMousev1p1_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.

illuminaMousev1p1_dbfile: a character string with the path to the package annotation DB.

illuminaMousev1p1_dbschema: none (invisible NULL).

illuminaMousev1p1_dbInfo: none (invisible NULL).
...
dbGetQuery(illuminaMousev1p1_dbconn(), "SELECT COUNT(*) FROM probes")
...
illuminaMousev1p1.db, 2
illuminaMousev1p1_dbconn, 20
illuminaMousev1p1ACCNUM, 1
illuminaMousev1p1ALIAS2PROBE, 2
illuminaMousev1p1CHR, 3
illuminaMousev1p1CHRLENGTHS, 3
illuminaMousev1p1CHRLOC, 4
illuminaMousev1p1ENSEMBL, 4
illuminaMousev1p1ENTREZID, 5
illuminaMousev1p1ENZYME, 6
illuminaMousev1p1GENENAME, 7
illuminaMousev1p1GO, 8
illuminaMousev1p1listNewMappings,

11

illuminaMousev1p1MAPCOUNTS, 10
illuminaMousev1p1MGI, 11
illuminaMousev1p1ORGANISM, 14
illuminaMousev1p1PATH, 15
illuminaMousev1p1PMID, 16
illuminaMousev1p1REFSEQ, 17
illuminaMousev1p1SYMBOL, 18
illuminaMousev1p1UNIGENE, 19
illuminaMousev1p1UNIPROT, 19
...
illuminaMousev1p1ENSEMBL, 4
illuminaMousev1p1ENSEMBL2PROBE
...
illuminaMousev1p1ENZYME, 6
illuminaMousev1p1ENZYME2PROBE

(illuminaMousev1p1ENZYME), 6

illuminaMousev1p1fullReannotation
...
illuminaMousev1p1GENOMICMATCHSIMILARITY
...
illuminaMousev1p1TRANSCRIPTOMICMATCHSIMILARITY
...
illuminaMousev1p1UNIGENE, 19
illuminaMousev1p1UNIPROT, 19
illuminaMousev1p1UNIPROT2PROBE
...
illuminaMousev1p1GO, 8
illuminaMousev1p1GO2ALLPROBES, 9
illuminaMousev1p1GO2ALLPROBES
...
illuminaMousev1p1listNewMappings, 11
illuminaMousev1p1LOCUSID

(illuminaMousev1p1ENTREZID), 5

illuminaMousev1p1MAPCOUNTS, 10
illuminaMousev1p1MGI, 11
illuminaMousev1p1MGI2PROBE
...
illuminaMousev1p1PATH, 15
illuminaMousev1p1PATH2PROBE

(illuminaMousev1p1PATH), 15

illuminaMousev1p1PMID, 16
illuminaMousev1p1PMID2PROBE
...
illuminaMousev1p1SECONDMATCHSIMILARITY
