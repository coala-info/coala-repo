...
doc: "Command-line tool for compressing and decompressing protein structure files.\n\
...
inputs:
  ...
  - id: input_format_or_file
    type: string
    doc: Input format (pdb|cif) or file/directory for 
      compress/decompress/check/rmsd
    inputBinding:
      position: 2
  - id: output_format_or_file
    type:
      - 'null'
      - string
  ...
  - id: plddt_digits
    ...
    doc: "extract pLDDT score with specified number of digits (only for extraction
      mode)\n                          - 1: single digit (fasta-like format), 2: 2-digit(00-99;
      tsv), 3: 3-digit, 4: 4-digit (max)"
    inputBinding:
      position: 104
      prefix: --plddt-digits
  - id: recursive
...
stdout: foldcomp_decompress.out
