...
mguatlas5kACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mguatlas5kALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mguatlas5kALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mguatlas5kALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mguatlas5k.db annotation Package. The purpose of this package is to provide
detailed information about the mguatlas5k platform. This package is updated biannually.
...
mguatlas5kCHR is an R object that provides mappings between a manufacturer identifier and the
...
mguatlas5kCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mguatlas5kCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mguatlas5kCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mguatlas5kENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(mguatlas5kENSEMBL2PROBE)
if(length(xx) > 0){
...
mguatlas5kENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mguatlas5kENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mguatlas5kENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mguatlas5kENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mguatlas5kENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mguatlas5kENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mguatlas5kENZYME2PROBE to a list to see inside
xx <- as.list(mguatlas5kENZYME2PROBE)
if(length(xx) > 0){
...
mguatlas5kGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mguatlas5kGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mguat-
las5kGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

mguatlas5kGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than mguat-
las5kGO2PROBE.

Details

If mguatlas5kGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mguatlas5kGO2ALLPROBES or mguatlas5kGO2PROBE is cast as a list, each GO term maps to
...
# Convert mguatlas5kGO2ALLPROBES to a list
xx <- as.list(mguatlas5kGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mguatlas5k.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mguatlas5kMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mguatlas5k.db.
...
mguatlas5kMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
# Convert to a list
xx <- as.list(mguatlas5kMGI2PROBE)
if(length(xx) > 0){
...
The Organism information for mguatlas5k

Description

mguatlas5kORGANISM is an R object that contains a single item: a character string that names
the organism for which mguatlas5k was built. mguatlas5kORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mguatlas5kORGANISM
provides a simple way to programmatically extract the organism name. mguatlas5kORGPKG pro-
...
mguatlas5kPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mguatlas5kPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the mguatlas5kPATH2PROBE object to a list
xx <- as.list(mguatlas5kPATH2PROBE)
if(length(xx) > 0){
...
mguatlas5kPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mguatlas5kPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mguatlas5kPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mguatlas5kPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mguatlas5kPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object mguatlas5kPMID2PROBE to a list
xx <- as.list(mguatlas5kPMID2PROBE)
if(length(xx) > 0){
...
mguatlas5kPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mguatlas5kREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mguatlas5kSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mguatlas5kUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
mguatlas5kUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mguatlas5k_dbconn()
mguatlas5k_dbfile()
mguatlas5k_dbschema(file="", show.indices=FALSE)
mguatlas5k_dbInfo()
...
mguatlas5k_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by mguatlas5k_dbconn or you will
break all the AnnDbObj objects defined in this package!
mguatlas5k_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
mguatlas5k_dbschema prints the schema definition of the package annotation DB.
mguatlas5k_dbInfo prints other information about the package annotation DB.

Value

mguatlas5k_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
mguatlas5k_dbfile: a character string with the path to the package annotation DB.
mguatlas5k_dbschema: none (invisible NULL).
mguatlas5k_dbInfo: none (invisible NULL).
...
dbGetQuery(mguatlas5k_dbconn(), "SELECT COUNT(*) FROM probes")
...
mguatlas5k.db, 2
mguatlas5k_dbconn, 20
mguatlas5kACCNUM, 1
mguatlas5kALIAS2PROBE, 2
mguatlas5kCHR, 3
mguatlas5kCHRLENGTHS, 4
mguatlas5kCHRLOC, 4
mguatlas5kENSEMBL, 5
mguatlas5kENTREZID, 6
mguatlas5kENZYME, 7
mguatlas5kGENENAME, 9
mguatlas5kGO, 10
mguatlas5kMAPCOUNTS, 12
mguatlas5kMGI, 12
mguatlas5kORGANISM, 13
mguatlas5kPATH, 14
mguatlas5kPFAM, 15
mguatlas5kPMID, 15
mguatlas5kPROSITE, 16
mguatlas5kREFSEQ, 17
mguatlas5kSYMBOL, 18
mguatlas5kUNIGENE, 19
mguatlas5kUNIPROT, 20
...
mguatlas5k_dbInfo (mguatlas5k_dbconn),
...
mguatlas5kCHRLENGTHS, 4
mguatlas5kCHRLOC, 4
mguatlas5kCHRLOCEND (mguatlas5kCHRLOC),

4
mguatlas5kENSEMBL, 5
mguatlas5kENSEMBL2PROBE

(mguatlas5kENSEMBL), 5

mguatlas5kENTREZID, 6
mguatlas5kENZYME, 7
mguatlas5kENZYME2PROBE

(mguatlas5kENZYME), 7

mguatlas5kGENENAME, 9
mguatlas5kGO, 10
mguatlas5kGO2ALLPROBES, 11
mguatlas5kGO2ALLPROBES (mguatlas5kGO),

10

mguatlas5kGO2PROBE (mguatlas5kGO), 10
mguatlas5kLOCUSID (mguatlas5kENTREZID),

6

mguatlas5kMAPCOUNTS, 12
mguatlas5kMGI, 12
mguatlas5kMGI2PROBE (mguatlas5kMGI), 12
mguatlas5kORGANISM, 13
mguatlas5kORGPKG (mguatlas5kORGANISM),

13
mguatlas5kPATH, 14
mguatlas5kPATH2PROBE (mguatlas5kPATH),

14
mguatlas5kPFAM, 15
mguatlas5kPMID, 15
mguatlas5kPMID2PROBE (mguatlas5kPMID),

mguatlas5k (mguatlas5k.db), 2
mguatlas5k.db, 2
mguatlas5k_dbconn, 20
mguatlas5k_dbfile (mguatlas5k_dbconn),
