...
Checking this box instructs MCAST to mask all lower-case
...
**Note:** Leaving this box checked is STRONGLY advised when scanning
one of the built-in DNA databases as repetitive DNA regions have
been marked using lower-case via the [RepeatMasker](http://repeatmasker.org) tool.
...
This value sets the scale for calculating p-scores of motif hits. Hit
*p*-values below this threshold get positive scores while hit
*p*-values above this threshold get negative scores.
...
This value specifies the longest distance allowed between two hits in
a match. Hits separated by more than this value will be placed in
different matches.

Large values for this field combined with large values for the
hit *p*-value threshold may prevent MCAST fr