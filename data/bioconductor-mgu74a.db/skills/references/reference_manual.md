...
mgu74aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
mgu74aALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the mgu74a.db annotation Package. The purpose of this package is to provide detailed
information about the mgu74a platform. This package is updated biannually.
...
mgu74aCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
mgu74aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
mgu74aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
mgu74aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mgu74aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgu74aENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. mgu74aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the mgu74aENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the mgu74aENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In mgu74aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgu74aENZYME2PROBE to a list to see inside

mgu74aGENENAME

9

x <- mgu74aENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mgu74aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
mgu74aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgu74aGO2PROBE)
...
mgu74aGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than mgu74aGO2PROBE.

Details

If mgu74aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If mgu74aGO2ALLPROBES or mgu74aGO2PROBE is cast as a list, each GO term maps to a
...
x <- mgu74aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mgu74aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mgu74a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgu74aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package mgu74a.db.
...
mgu74aMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for mgu74a

Description

mgu74aORGANISM is an R object that contains a single item: a character string that names the
organism for which mgu74a was built. mgu74aORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgu74aORGANISM
provides a simple way to programmatically extract the organism name. mgu74aORGPKG provides
...
mgu74aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mgu74aPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- mgu74aPATH
mapped_probes <- mappedkeys(x)
# Now convert the mgu74aPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
mgu74aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. mgu74aPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When mgu74aPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When mgu74aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
x <- mgu74aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mgu74aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgu74aREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
mgu74aSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
mgu74aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
mgu74a_dbconn()
mgu74a_dbfile()
mgu74a_dbschema(file="", show.indices=FALSE)
mgu74a_dbInfo()
...
mgu74a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgu74a_dbconn or you will break all the
AnnDbObj objects defined in this package!

mgu74a_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

mgu74a_dbschema prints the schema definition of the package annotation DB.

mgu74a_dbInfo prints other information about the package annotation DB.

Value

mgu74a_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

mgu74a_dbfile: a character string with the path to the package annotation DB.

mgu74a_dbschema: none (invisible NULL).

mgu74a_dbInfo: none (invisible NULL).
...
dbGetQuery(mgu74a_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgu74a_dbschema (mgu74a_dbconn), 20
mgu74aACCNUM, 1
mgu74aALIAS2PROBE, 2
mgu74aCHR, 3
mgu74aCHRLENGTHS, 4
mgu74aCHRLOC, 4
mgu74aCHRLOCEND (mgu74aCHRLOC), 4
mgu74aENSEMBL, 5
mgu74aENSEMBL2PROBE (mgu74aENSEMBL), 5
mgu74aENTREZID, 6
mgu74aENZYME, 7
mgu74aENZYME2PROBE (mgu74aENZYME), 7
mgu74aGENENAME, 9
mgu74aGO, 10
mgu74aGO2ALLPROBES, 11
mgu74aGO2ALLPROBES (mgu74aGO), 10
mgu74aGO2PROBE (mgu74aGO), 10
mgu74aLOCUSID (mgu74aENTREZID), 6
mgu74aMAPCOUNTS, 12
mgu74aMGI, 12
mgu74aMGI2PROBE (mgu74aMGI), 12
mgu74aORGANISM, 13
mgu74aORGPKG (mgu74aORGANISM), 13
mgu74aPATH, 14
mgu74aPATH2PROBE (mgu74aPATH), 14
mgu74aPFAM, 15
mgu74aPMID, 15
mgu74aPMID2PROBE (mgu74aPMID), 15
mgu74aPROSITE, 17
mgu74aREFSEQ, 17
mgu74aSYMBOL, 18
mgu74aUNIPROT, 19
mgu74aUNIPROT2PROBE (mgu74aUNIPROT), 19
...
mgu74a.db, 2
mgu74a_dbconn, 20
mgu74a_dbfile (mgu74a_dbconn), 20
mgu74a_dbInfo (mgu74a_dbconn), 20
