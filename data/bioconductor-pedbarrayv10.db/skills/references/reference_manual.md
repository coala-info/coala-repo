...
pedbarrayv10ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
pedbarrayv10ALIAS is an R object that provides mappings between common gene symbol identi-
...
xx <- as.list(pedbarrayv10ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the pedbarrayv10.db annotation Package. The purpose of this package is to provide
detailed information about the pedbarrayv10 platform. This package is updated biannually.
...
pedbarrayv10CHR is an R object that provides mappings between a manufacturer identifier and the
...
pedbarrayv10CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

pedbarrayv10CHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
pedbarrayv10CHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
pedbarrayv10ENSEMBL is an R object that contains mappings between manufacturer identifiers
...
# Convert to a list
xx <- as.list(pedbarrayv10ENSEMBL2PROBE)
if(length(xx) > 0){
...
pedbarrayv10ENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

pedbarrayv10ENTREZID is an R object that provides mappings between manufacturer identifiers
...
pedbarrayv10ENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. pedbarrayv10ENZYME2PROBE is an R object that maps Enzyme Commission
(EC) numbers to manufacturer identifiers.

Details

When the pedbarrayv10ENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the pedbarrayv10ENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In pedbarrayv10ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert pedbarrayv10ENZYME2PROBE to a list to see inside
xx <- as.list(pedbarrayv10ENZYME2PROBE)
if(length(xx) > 0){

# Get the probe identifiers for the first five enzyme
#commission numbers

pedbarrayv10GENENAME
...
pedbarrayv10GENENAME Map between Manufacturer IDs and Genes

Description

pedbarrayv10GENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
pedbarrayv10GO is an R object that provides mappings between manufacturer identifiers and the
GO identifiers that they are directly associated with. This mapping and its reverse mapping (pedbar-
rayv10GO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

pedbarrayv10GO2ALLPROBES is an R object that provides mappings between a given GO identi-
...
If pedbarrayv10GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If pedbarrayv10GO2ALLPROBES or pedbarrayv10GO2PROBE is cast as a list, each GO term
...
# For the reverse map:
# Convert to a list
xx <- as.list(pedbarrayv10GO2PROBE)
if(length(xx) > 0){
...
# Convert pedbarrayv10GO2ALLPROBES to a list
xx <- as.list(pedbarrayv10GO2ALLPROBES)
if(length(xx) > 0){
...
pedbarrayv10MAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
pedbarrayv10MAPCOUNTS

13

pedbarrayv10MAPCOUNTS Number of mapped keys for the maps in package pedbarrayv10.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

pedbarrayv10MAPCOUNTS provides the "map count" (i.e.
map in package pedbarrayv10.db.
...
pedbarrayv10OMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for pedbarrayv10

Description

pedbarrayv10ORGANISM is an R object that contains a single item: a character string that names
the organism for which pedbarrayv10 was built. pedbarrayv10ORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, pedbarrayv10ORGANISM
provides a simple way to programmatically extract the organism name. pedbarrayv10ORGPKG
...
pedbarrayv10PATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

pedbarrayv10PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the pedbarrayv10PATH2PROBE object to a list
xx <- as.list(pedbarrayv10PATH2PROBE)
if(length(xx) > 0){
...
pedbarrayv10PFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
pedbarrayv10PMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. pedbarrayv10PMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When pedbarrayv10PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When pedbarrayv10PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object pedbarrayv10PMID2PROBE to a list
xx <- as.list(pedbarrayv10PMID2PROBE)
if(length(xx) > 0){
...
pedbarrayv10PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
pedbarrayv10REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
pedbarrayv10SYMBOL is an R object that provides mappings between manufacturer identifiers
...
pedbarrayv10UNIGENE is an R object that provides mappings between manufacturer identifiers
...
pedbarrayv10UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
pedbarrayv10_dbconn()
pedbarrayv10_dbfile()
pedbarrayv10_dbschema(file="", show.indices=FALSE)
pedbarrayv10_dbInfo()
...
pedbarrayv10_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by pedbarrayv10_dbconn or you will
break all the AnnDbObj objects defined in this package!
pedbarrayv10_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
pedbarrayv10_dbschema prints the schema definition of the package annotation DB.
pedbarrayv10_dbInfo prints other information about the package annotation DB.

Value

pedbarrayv10_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
pedbarrayv10_dbfile: a character string with the path to the package annotation DB.
pedbarrayv10_dbschema: none (invisible NULL).
pedbarrayv10_dbInfo: none (invisible NULL).
...
dbGetQuery(pedbarrayv10_dbconn(), "SELECT COUNT(*) FROM probes")
...
pedbarrayv10.db, 2
pedbarrayv10_dbconn, 21
pedbarrayv10ACCNUM, 1
pedbarrayv10ALIAS2PROBE, 2
pedbarrayv10CHR, 3
pedbarrayv10CHRLENGTHS, 4
pedbarrayv10CHRLOC, 4
pedbarrayv10ENSEMBL, 5
pedbarrayv10ENTREZID, 6
pedbarrayv10ENZYME, 7
pedbarrayv10GENENAME, 9
pedbarrayv10GO, 10
pedbarrayv10MAP, 12
pedbarrayv10MAPCOUNTS, 13
pedbarrayv10OMIM, 13
pedbarrayv10ORGANISM, 14
pedbarrayv10PATH, 15
pedbarrayv10PFAM, 16
pedbarrayv10PMID, 16
pedbarrayv10PROSITE, 17
pedbarrayv10REFSEQ, 18
pedbarrayv10SYMBOL, 19
pedbarrayv10UNIGENE, 20
pedbarrayv10UNIPROT, 21
...
pedbarrayv10 (pedbarrayv10.db), 2
pedbarrayv10.db, 2
pedbarrayv10_dbconn, 21
...
pedbarrayv10CHRLENGTHS, 4
pedbarrayv10CHRLOC, 4
pedbarrayv10CHRLOCEND

(pedbarrayv10CHRLOC), 4

pedbarrayv10ENSEMBL, 5
pedbarrayv10ENSEMBL2PROBE

(pedbarrayv10ENSEMBL), 5

pedbarrayv10ENTREZID, 6
pedbarrayv10ENZYME, 7
pedbarrayv10ENZYME2PROBE

(pedbarrayv10ENZYME), 7

pedbarrayv10GENENAME, 9
pedbarrayv10GO, 10
pedbarrayv10GO2ALLPROBES, 11
pedbarrayv10GO2ALLPROBES

(pedbarrayv10GO), 10
pedbarrayv10GO2PROBE (pedbarrayv10GO),
...
pedbarrayv10PATH, 15
pedbarrayv10PATH2PROBE

(pedbarrayv10PATH), 15

pedbarrayv10PFAM, 16
pedbarrayv10PMID, 16
pedbarrayv10PMID2PROBE
