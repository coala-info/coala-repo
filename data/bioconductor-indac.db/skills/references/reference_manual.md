...
indacACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
indacALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the indac.db annotation Package. The purpose of this package is to provide detailed
information about the indac platform. This package is updated biannually.
...
indacCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
indacCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
indacCHRLOC is an R object that maps manufacturer identifiers to the starting position of the gene.
...
ftp://hgdownload.cse.ucsc.edu/goldenPath/dm6 With a date stamp from the source of: 2014-Dec12
...
indacENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
indacENTREZID is an R object that provides mappings between manufacturer identifiers and En-
...
indacENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. indacENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers to
manufacturer identifiers.

Details

When the indacENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the indacENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In indacENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
indacFLYBASE is an R object that contains mappings between manufacturer identifiers and Fly-
...
Map FlyBase CG accession numbers with manufacturer identifiers

Description

indacFLYBASE is an R object that contains mappings between manufacturer identifiers and Fly-
Base CG accession numbers. These accessions are used by Ensembl and supported by FlyBase.
...
# Convert to a list
xx <- as.list(indacFLYBASECG2PROBE)
if(length(xx) > 0){
...
indacGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
indacGO is an R object that provides mappings between manufacturer identifiers and the GO identi-
fiers that they are directly associated with. This mapping and its reverse mapping (indacGO2PROBE)
...
indacGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than indacGO2PROBE.

Details

If indacGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If indacGO2ALLPROBES or indacGO2PROBE is cast as a list, each GO term maps to a named
...
# Convert indacGO2ALLPROBES to a list
xx <- as.list(indacGO2ALLPROBES)
if(length(xx) > 0){
...
indacMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package indac.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

indacMAPCOUNTS provides the "map count" (i.e.
package indac.db.
...
indacORGANISM is an R object that contains a single item: a character string that names the organ-
ism for which indac was built. indacORGPKG is an R object that contains a chararcter vector with
the name of the organism package that a chip package depends on for its gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, indacORGANISM
provides a simple way to programmatically extract the organism name. indacORGPKG provides a
...
indacPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the genes
represented by the probe identifiers are involved

indacPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
# Now convert the indacPATH2PROBE object to a list
xx <- as.list(indacPATH2PROBE)
if(length(xx) > 0){
...
indacPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. indacPMID2PROBE is an R object that provides mappings between PubMed identifiers
...
When indacPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When indacPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
indacREFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
indacSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
indacUNIGENE is an R object that provides mappings between manufacturer identifiers and Uni-
Gene identifiers.
...
indacUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
indac_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by indac_dbconn or you will break all the
AnnDbObj objects defined in this package!
indac_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
...
indac_dbconn: a DBIConnection object representing an open connection to the package annotation
DB.
...
indac.db, 2
indac_dbconn, 21
indac_dbfile (indac_dbconn), 21

23

indac_dbInfo (indac_dbconn), 21
indac_dbschema (indac_dbconn), 21
indacACCNUM, 1
indacALIAS2PROBE, 2
indacCHR, 3
indacCHRLENGTHS, 4
indacCHRLOC, 4
indacCHRLOCEND (indacCHRLOC), 4
indacENSEMBL, 5
indacENSEMBL2PROBE (indacENSEMBL), 5
indacENTREZID, 6
indacENZYME, 7
indacENZYME2PROBE (indacENZYME), 7
indacFLYBASE, 9
indacFLYBASE2PROBE (indacFLYBASE), 9
indacFLYBASECG, 10
indacFLYBASECG2PROBE (indacFLYBASECG),

10
indacGENENAME, 11
indacGO, 11
indacGO2ALLPROBES, 13
indacGO2ALLPROBES (indacGO), 11
indacGO2PROBE (indacGO), 11
indacLOCUSID (indacENTREZID), 6
indacMAP, 13
indacMAPCOUNTS, 14
indacORGANISM, 15
indacORGPKG (indacORGANISM), 15
indacPATH, 15
indacPATH2PROBE (indacPATH), 15
indacPMID, 16
indacPMID2PROBE (indacPMID), 16
indacREFSEQ, 18
indacSYMBOL, 19
indacUNIGENE, 20
indacUNIPROT, 21
indacUNIPROT2PROBE (indacUNIPROT), 21
