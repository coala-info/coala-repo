...
clariomsrathttranscriptclusterACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

clariomsrathttranscriptclusterACCNUM is an R object that contains mappings between a manufac-
...
## Bimap interface:
x <- clariomsrathttranscriptclusterACCNUM
...
clariomsrathttranscriptclusterALIAS2PROBE
...
clariomsrathttranscriptclusterALIAS is an R object that provides mappings between common gene
...
xx <- as.list(clariomsrathttranscriptclusterALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the clariomsrathttranscriptcluster.db annotation Package. The purpose of this package
is to provide detailed information about the clariomsrathttranscriptcluster platform. This package
is updated biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

clariomsrathttranscriptclusterCHR
...
## use of select().
ls("package:clariomsrathttranscriptcluster.db")
...
clariomsrathttranscriptclusterCHR is an R object that provides mappings between a manufacturer
...
## Bimap interface:
x <- clariomsrathttranscriptclusterCHR
...
clariomsrathttranscriptclusterCHRLOC
...
clariomsrathttranscriptclusterCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

clariomsrathttranscriptclusterCHRLENGTHS provides the length measured in base pairs for each
of the chromosomes.
...
## Bimap interface:
tt <- clariomsrathttranscriptclusterCHRLENGTHS
...
clariomsrathttranscriptclusterCHRLOC is an R object that maps manufacturer identifiers to the
...
clariomsrathttranscriptclusterENSEMBL
...
## Bimap interface:
x <- clariomsrathttranscriptclusterCHRLOC
...
clariomsrathttranscriptclusterENSEMBL is an R object that contains mappings between manufac-
...
clariomsrathttranscriptclusterENTREZID
...
## Bimap interface:
x <- clariomsrathttranscriptclusterENSEMBL
...
x <- clariomsrathttranscriptclusterENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterENTREZID is an R object that provides mappings between manufac-
turer identifiers and Entrez Gene identifiers.

clariomsrathttranscriptclusterENZYME
...
## Bimap interface:
x <- clariomsrathttranscriptclusterENTREZID
...
clariomsrathttranscriptclusterENZYME is an R object that provides mappings between manufac-
turer identifiers and EC numbers. clariomsrathttranscriptclusterENZYME2PROBE is an R object
...
When the clariomsrathttranscriptclusterENZYME maping viewed as a list, each manufacturer iden-
...
For the clariomsrathttranscriptclusterENZYME2PROBE, each EC number maps to a named vector
...
numbers.
In clariomsrathttranscriptclusterENZYME2PROBE, EC is dropped from the Enzyme
Commission numbers.
...
## Bimap interface:
x <- clariomsrathttranscriptclusterENZYME
...
clariomsrathttranscriptclusterGENENAME
...
# Now convert clariomsrathttranscriptclusterENZYME2PROBE to a list to see inside
x <- clariomsrathttranscriptclusterENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterGENENAME is an R object that maps manufacturer identifiers to the
...
## Bimap interface:
x <- clariomsrathttranscriptclusterGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

clariomsrathttranscriptclusterGO
...
clariomsrathttranscriptclusterGO is an R object that provides mappings between manufacturer iden-
tifiers and the GO identifiers that they are directly associated with. This mapping and its reverse
mapping (clariomsrathttranscriptclusterGO2PROBE) do NOT associate the child terms from the
GO ontology with the gene. Only the directly evidenced terms are represented here.

clariomsrathttranscriptclusterGO2ALLPROBES is an R object that provides mappings between a
...
than clariomsrathttranscriptclusterGO2PROBE.

Details

If clariomsrathttranscriptclusterGO is cast as a list, each manufacturer identifier is mapped to a list
...
If clariomsrathttranscriptclusterGO2ALLPROBES or clariomsrathttranscriptclusterGO2PROBE is
...
• clariomsrathttranscriptclusterGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
## Bimap interface:
x <- clariomsrathttranscriptclusterGO
...
# For the reverse map:
x <- clariomsrathttranscriptclusterGO2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterORGANISM

x <- clariomsrathttranscriptclusterGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert clariomsrathttranscriptclusterGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterMAPCOUNTS

Number of mapped keys for the maps in package clariomsrathttran-
scriptcluster.db
...
clariomsrathttranscriptclusterMAPCOUNTS provides the "map count" (i.e.
keys) for each map in package clariomsrathttranscriptcluster.db.
...
The Organism information for clariomsrathttranscriptcluster

Description

clariomsrathttranscriptclusterORGANISM is an R object that contains a single item: a character
string that names the organism for which clariomsrathttranscriptcluster was built. clariomsratht-
transcriptclusterORGPKG is an R object that contains a chararcter vector with the name of the
organism package that a chip package depends on for its gene-centric annotation.

clariomsrathttranscriptclusterPATH

13

Details

Although the package name is suggestive of the organism for which it was built, clariomsrathttran-
scriptclusterORGANISM provides a simple way to programmatically extract the organism name.
clariomsrathttranscriptclusterORGPKG provides a simple way to programmatically extract the name
...
## Bimap interface:
clariomsrathttranscriptclusterORGANISM
clariomsrathttranscriptclusterORGPKG
...
clariomsrathttranscriptclusterPATH maps probe identifiers to the identifiers used by KEGG for path-
ways in which the genes represented by the probe identifiers are involved

clariomsrathttranscriptclusterPATH2PROBE is an R object that provides mappings between KEGG
...
clariomsrathttranscriptclusterPFAM
...
## Bimap interface:
x <- clariomsrathttranscriptclusterPATH
...
x <- clariomsrathttranscriptclusterPATH
mapped_probes <- mappedkeys(x)
# Now convert the clariomsrathttranscriptclusterPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterPFAM is an R object that provides mappings between manufacturer
...
clariomsrathttranscriptclusterPMID
...
clariomsrathttranscriptclusterPMID is an R object that provides mappings between manufacturer
identifiers and PubMed identifiers. clariomsrathttranscriptclusterPMID2PROBE is an R object that
provides mappings between PubMed identifiers and manufacturer identifiers.

Details

When clariomsrathttranscriptclusterPMID is viewed as a list each manufacturer identifier is mapped
...
When clariomsrathttranscriptclusterPMID2PROBE is viewed as a list each PubMed identifier is
...
## Bimap interface:
x <- clariomsrathttranscriptclusterPMID
...
clariomsrathttranscriptclusterREFSEQ
...
x <- clariomsrathttranscriptclusterPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object clariomsrathttranscriptclusterPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
clariomsrathttranscriptclusterPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

clariomsrathttranscriptclusterPROSITE is an R object that provides mappings between manufac-
...
clariomsrathttranscriptclusterREFSEQ is an R object that provides mappings between manufacturer
...
## Bimap interface:
x <- clariomsrathttranscriptclusterREFSEQ
...
clariomsrathttranscriptclusterSYMBOL
...
clariomsrathttranscriptclusterSYMBOL is an R object that provides mappings between manufac-
...
clariomsrathttranscriptclusterUNIPROT
...
clariomsrathttranscriptclusterUNIPROT is an R object that contains mappings between the manu-
...
## Bimap interface:
x <- clariomsrathttranscriptclusterUNIPROT
...
clariomsrathttranscriptcluster_dbconn
...
clariomsrathttranscriptcluster_dbconn()
clariomsrathttranscriptcluster_dbfile()
clariomsrathttranscriptcluster_dbschema(file="", show.indices=FALSE)
clariomsrathttranscriptcluster_dbInfo()
...
clariomsrathttranscriptcluster_dbconn returns a connection object to the package annota-
tion DB. IMPORTANT: Don’t call dbDisconnect on the connection object returned by clariomsrathttranscriptcluster_dbconn
or you will break all the AnnDbObj objects defined in this package!
clariomsrathttranscriptcluster_dbfile returns the path (character string) to the package an-
notation DB (this is an SQLite file).
clariomsrathttranscriptcluster_dbschema prints the schema definition of the package anno-
tation DB.
clariomsrathttranscriptcluster_dbInfo prints other information about the package annota-
tion DB.

Value

clariomsrathttranscriptcluster_dbconn: a DBIConnection object representing an open con-
nection to the package annotation DB.
clariomsrathttranscriptcluster_dbfile: a character string with the path to the package an-
notation DB.
clariomsrathttranscriptcluster_dbschema: none (invisible NULL).
clariomsrathttranscriptcluster_dbInfo: none (invisible NULL).
...
dbGetQuery(clariomsrathttranscriptcluster_dbconn(), "SELECT COUNT(*) FROM probes")

clariomsrathttranscriptcluster_dbschema()
...
clariomsrathttranscriptclusterCHR, 3
clariomsrathttranscriptclusterCHRLENGTHS,
...
clariomsrathttranscriptclusterCHRLOCEND
...
clariomsrathttranscriptclusterENSEMBL2PROBE
...
clariomsrathttranscriptclusterENZYME2PROBE
...
clariomsrathttranscriptclusterGO, 10
clariomsrathttranscriptclusterGO2ALLPROBES,
...
clariomsrathttranscriptclusterGO2PROBE

(clariomsrathttranscriptclusterGO),
10

clariomsrathttranscriptclusterLOCUSID
...
clariomsrathttranscriptclusterPATH, 13
clariomsrathttranscriptclusterPATH2PROBE

(clariomsrathttranscriptclusterPATH),
13

clariomsrathttranscriptclusterPFAM, 14
clariomsrathttranscriptclusterPMID, 15
clariomsrathttranscriptclusterPMID2PROBE
...
clariomsrathttranscriptclusterUNIPROT2PROBE
