...
[Skip to content](#capcruncher-tips-and-tricks)
...
* [ ]
  ...
  Table of contents
  + [Interruptions to the pipeline (e.g. system failure, manual interruption)](#interruptions-to-the-pipeline-eg-system-failure-manual-interruption)

    - [Restarting the pipeline after an interruption](#restarting-the-pipeline-after-an-interruption)
    - [Unlocking the working directory](#unlocking-the-working-directory)
  + [Interuptions to the pipeline (e.g. error in pipeline)](#interuptions-to-the-pipeline-eg-error-in-pipeline)

    - [No fastq files found](#no-fastq-files-found)
    - [Indicies not found or specified incorrectly](#indicies-not-found-or-specified-incorrectly)
    - [Incorrect viewpoint coordinates](#incorrect-viewpoint-coordinates)

      * [Viewpoint names contain invalid characters](#viewpoint-names-contain-invalid-characters)
      * [Viewpoint coordinates are incorrect for the supplied reference genome](#viewpoint-coordinates-are-incorrect-for-the-supplied-reference-genome)
  + [Adding additional Snakemake options](#adding-additional-snakemake-options)
...
# CapCruncher Tips and Tricks[¶](#capcruncher-tips-and-tricks "Permanent link")

## Interruptions to the pipeline (e.g. system failure, manual interruption)[¶](#interruptions-to-the-pipeline-eg-system-failure-manual-interruption "Permanent link")

### Restarting the pipeline after an interruption[¶](#restarting-the-pipeline-after-an-interruption "Permanent link")
...
### Unlocking the working directory[¶](#unlocking-the-working-directory "Permanent link")
...
## Interuptions to the pipeline (e.g. error in pipeline)[¶](#interuptions-to-the-pipeline-eg-error-in-pipeline "Permanent link")

Pipeline errors very frequently are found in a few major areas:

### No fastq files found[¶](#no-fastq-files-found "Permanent link")
...
To correct this, ensure that the fastq files are in the current working directory and are named correctly. See the [pipeline guide](../pipeline/#setting-up-the-input-directory) for more information on how to name the fastq files.

### Indicies not found or specified incorrectly[¶](#indicies-not-found-or-specified-incorrectly "Permanent link")

If the pipeline cannot find the reference genome files this will cause an error at the alignment step. Please ensure that the reference genome (`aligner_indicies`) are added to the [config file](../pipeline/#pipeline-configuration).
...
### Incorrect viewpoint coordinates[¶](#incorrect-viewpoint-coordinates "Permanent link")

Viewpoint coordinate errors fall into two categories:

#### Viewpoint names contain invalid characters[¶](#viewpoint-names-contain-invalid-characters "Permanent link")
...
#### Viewpoint coordinates are incorrect for the supplied reference genome[¶](#viewpoint-coordinates-are-incorrect-for-the-supplied-reference-genome "Permanent link")
...
Viewpoint coordinates are supplied in the [config file](../pipeline/#pipeline-configuration) as a [BED](https://genome.ucsc.edu/FAQ/FAQformat.html#format1) file and should be checked against the reference genome.

##### Capture-C and Tri-C experiments[¶](#capture-c-and-tri-c-experiments "Permanent link")
...
##### Tiled-C experiments[¶](#tiled-c-experiments "Permanent link")
...
## Adding additional Snakemake options[¶](#adding-additional-snakemake-options "Permanent link")
