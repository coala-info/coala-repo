...
[View this page](_sources/troubleshooting.rst.txt "View this page")
...
## Most/all of my samples merge when I run a query[¶](#most-all-of-my-samples-merge-when-i-run-a-query "Link to this heading")
...
* Use `--run-qc` (see [Data quality control (--qc-db)](qc.html)) with one or more of:
...
See [issue 194](https://github.com/bacpop/PopPUNK/issues/194) for more discussion.

## I get different cluster assignments for my queries with and without –update-db flag[¶](#i-get-different-cluster-assignments-for-my-queries-with-and-without-update-db-flag "Link to this heading")
...
## Memory/run-time issues[¶](#memory-run-time-issues "Link to this heading")
...
* In `--refine-model` set `--pos-shift 0` to avoid creating huge networks
  with close to \(N^2\) edges. Mixture models normally need to be pruned.
...
## Known bugs[¶](#known-bugs "Link to this heading")

### Calculating query-query distances when unnecessary[¶](#calculating-query-query-distances-when-unnecessary "Link to this heading")
...
### Older HDBSCAN models fail to load[¶](#older-hdbscan-models-fail-to-load "Link to this heading")
...
### When I look at my clusters on a tree, they make no sense[¶](#when-i-look-at-my-clusters-on-a-tree-they-make-no-sense "Link to this heading")
...
### I have updated PopPUNK, and my clusters still seemed scrambled[¶](#i-have-updated-poppunk-and-my-clusters-still-seemed-scrambled "Link to this heading")
...
### Calculating distances using 0 thread(s)[¶](#calculating-distances-using-0-thread-s "Link to this heading")
...
## Error/warning messages[¶](#error-warning-messages "Link to this heading")

### Row name mismatch[¶](#row-name-mismatch "Link to this heading")
...
### Samples are missing from the final network[¶](#samples-are-missing-from-the-final-network "Link to this heading")
...
### Old cluster split across multiple new clusters[¶](#old-cluster-split-across-multiple-new-clusters "Link to this heading")
...
### “No non-zero Jaccard distances” Error[¶](#no-non-zero-jaccard-distances-error "Link to this heading")
...
are the correct species (e.g. using [BLAST](https://blast.ncbi.nlm.nih.gov/Blast.cgi) or [Kraken2](https://github.com/DerrickWood/kraken2)); if not, remove them from your input file and re-run your analysis.
...
* Troubleshooting
  + [Most/all of my samples merge when I run a query](#most-all-of-my-samples-merge-when-i-run-a-query)
  + [I get different cluster assignments for my queries with and without –update-db flag](#i-get-different-cluster-assignments-for-my-queries-with-and-without-update-db-flag)
  + [Memory/run-time issues](#memory-run-time-issues)
  + [Known bugs](#known-bugs)
    - [Calculating query-query distances when unnecessary](#calculating-query-query-distances-when-unnecessary)
    - [Older HDBSCAN models fail to load](#older-hdbscan-models-fail-to-load)
    - [When I look at my clusters on a tree, they make no sense](#when-i-look-at-my-clusters-on-a-tree-they-make-no-sense)
    - [I have updated PopPUNK, and my clusters still seemed scrambled](#i-have-updated-poppunk-and-my-clusters-still-seemed-scrambled)
    - [Calculating distances using 0 thread(s)](#calculating-distances-using-0-thread-s)
  + [Error/warning messages](#error-warning-messages)
    - [Row name mismatch](#row-name-mismatch)
    - [Samples are missing from the final network](#samples-are-missing-from-the-final-network)
    - [Old cluster split across multiple new clusters](#old-cluster-split-across-multiple-new-clusters)
    - [“No non-zero Jaccard distances” Error](#no-non-zero-jaccard-distances-error)