...
covered by reads, with the reads overlapping by at least --min-overlap
bases, when each read is aligned to its true location of origin.
...
according to a policy specified by --alignment-policy and
--min-alignment-prob, and then builds our best guess as to the
“true” assembly based on these alignments.
...
This will output a file, cc\_0.fa, that contains the “true” assembly.
...
--reference arg
:   The prefix of the reference built by
    rsem-prepare-reference. Required.
...
--paired-end
:   If you have paired-end data, and you want to estimate the “true”
...
:   A prefix to write the “true” assembly or sequence of assemblies
    to. The suffix \_*x*.fa will be appended to this prefix,
    where *x* is the minimum overlap size. Required.
...
:   Either:
    ...
    * An integer that specifies how much overlap between two reads is
      ...
      --min-overlap=0, then only reads whose chosen alignments
      are contiguous (or overlap by a positive amount) will be joined
      into contigs.

    Or:

    * A pair of integers, separated by commas, specifying a range of
      overlap sizes, as described above. For example, if
      --min-overlap=2,4 is given, then three assemblies will be
      produced, corresponding to --min-overlap=2,
      --min-overlap=3, and --min-overlap=4 You might
