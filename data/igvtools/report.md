# igvtools CWL Generation Report

## igvtools_version

### Tool Description
Display the version information for IGV tools

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
IGV Version 2.17.3 03/05/2024 10:31 PM
Done
```


## igvtools_sort

### Tool Description
Sorts an alignment file by start position. The input file must be a .sam, .bam, .aligned, or .vcf file.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:47] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:47] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_index

### Tool Description
Creates an index for an alignment or feature file (e.g., BED, GFF, or VCF).

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:47] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:47] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_toTDF

### Tool Description
The toTDF command converts a sorted data file to a binary tiled data file (TDF). Supported input formats are: .wig, .bed, .gff, .map, .psl, .sam, and .bam.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:47] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:47] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_count

### Tool Description
The count command computes alignment coverage of an alignment file. The output is a .tdf file, which can be loaded into IGV for viewing.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:48] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:48] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_formatexp

### Tool Description
Formats an expression data file for use in IGV.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:48] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:48] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_gui

### Tool Description
Launch the IGVTools Graphical User Interface (GUI). Note: This tool requires a display environment and will fail in headless mode.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:48] [IgvTools] null
SEVERE [Feb 24,2026 01:48] [IgvTools] java.awt.HeadlessException
	at java.desktop/java.awt.GraphicsEnvironment.checkHeadless(GraphicsEnvironment.java:166)
	at java.desktop/java.awt.Window.<init>(Window.java:553)
	at java.desktop/java.awt.Frame.<init>(Frame.java:428)
	at java.desktop/java.awt.Frame.<init>(Frame.java:393)
	at java.desktop/javax.swing.SwingUtilities$SharedOwnerFrame.<init>(SwingUtilities.java:1925)
	at java.desktop/javax.swing.SwingUtilities.getSharedOwnerFrame(SwingUtilities.java:2001)
	at java.desktop/javax.swing.JDialog.<init>(JDialog.java:276)
	at java.desktop/javax.swing.JDialog.<init>(JDialog.java:210)
	at java.desktop/javax.swing.JDialog.<init>(JDialog.java:158)
	at org.igv/org.broad.igv.ui.IGVDialog.<init>(IGVDialog.java:11)
	at org.igv/org.broad.igv.tools.IgvToolsGui.<init>(IgvToolsGui.java:75)
	at org.igv/org.broad.igv.tools.IgvToolsGui.launch(IgvToolsGui.java:577)
	at org.igv/org.broad.igv.tools.IgvToolsGui.main(IgvToolsGui.java:571)
	at org.igv/org.broad.igv.tools.IgvTools.launchGUI(IgvTools.java:1272)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:285)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## igvtools_See

### Tool Description
IGV Tools is a set of utilities for preprocessing and manipulating genomic data files.

### Metadata
- **Docker Image**: quay.io/biocontainers/igvtools:2.17.3--hdfd78af_0
- **Homepage**: http://www.broadinstitute.org/igv/
- **Package**: https://anaconda.org/channels/bioconda/packages/igvtools/overview
- **Validation**: PASS

### Original Help Text
```text
/usr/local/bin/igvtools: line 7: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8): No such file or directory
SEVERE [Feb 24,2026 01:49] [IgvTools] Error: No input file provided

SEVERE [Feb 24,2026 01:49] [IgvTools] org.broad.igv.tools.PreprocessingException: Error: No input file provided

	at org.igv/org.broad.igv.tools.IgvTools.validateArgsLength(IgvTools.java:1122)
	at org.igv/org.broad.igv.tools.IgvTools.run(IgvTools.java:320)
	at org.igv/org.broad.igv.tools.IgvTools.main(IgvTools.java:254)
```


## Metadata
- **Skill**: generated
