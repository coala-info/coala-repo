...
* **-p , --pheight**  occupancy (reads density) P value cutoff for defining
  ...
  + individual protein binding peak. Setting to a value 0 will disable this parameter and use the parameter -q as an instead (default: 1e-10)
  + **-q , --height** occupancy (reads density) cutoff for defining individual protein binding peak. Setting to a value 0 will disable this parameter and use the parameter -p as an instead (default: 0)
* **-t , --testcut**  P value cutoff for defining peak based on **difference in protein binding occupancy** between each pair of datasets. Set as 0 will disable this parameter, then peaks will be defined solely based on **occupancy** cutoff (-p or -q), but a P value of difference will still be calculated at each peak for each pair of data sets. (default: 0)
...
* -kd , --peak\_dis minimal tail-to-head distance between neighboring peaks (bp), neighboring peaks closer than -kd will be merged as one single peak (default: 40)
* -kw , --peak\_width minimal width of each peak (default: 40)
* -kf , --peak\_reference A file containing a set of reference peak. When this parameter is specified, Dregion will not call peaks, but simply assign values for the reference peaks provided in this file. (default: None)
...
* **-H , --exclude\_high\_percent** the percent of extremely high occupancy positions to be excluded in determining normalization factors, may be helpful to avoid the influence of some highly clonal reads or repeat peaks on normalization. MUST BE USED WITH CAUTIONS! (default: 0)
