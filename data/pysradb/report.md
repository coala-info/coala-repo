# pysradb CWL Generation Report

## pysradb

### Tool Description
A Python tool to query SRA metadata and download data from SRA/ENA.

### Metadata
- **Docker Image**: quay.io/biocontainers/pysradb:2.5.1--pyhdfd78af_0
- **Homepage**: https://github.com/saketkc/pysradb
- **Package**: https://anaconda.org/channels/bioconda/packages/pysradb/overview
- **Validation**: PASS

- **Conda**: https://anaconda.org/channels/bioconda/packages/pysradb/overview
- **Total Downloads**: 92.5K
- **Last updated**: 2025-10-20
- **GitHub**: https://github.com/saketkc/pysradb
- **Stars**: N/A
### Original Help Text
```text
INFO:    Environment variable SINGULARITY_CACHEDIR is set, but APPTAINER_CACHEDIR is preferred
INFO:    Converting OCI blobs to SIF format
INFO:    Starting build...
INFO:    Fetching OCI image...
FATAL:   Unable to handle docker://quay.io/biocontainers/pysradb:2.5.1--pyhdfd78af_0 uri: while building SIF from layers: conveyor failed to get: invalid character '}' after top-level value
```


## Metadata
- **Skill**: generated
