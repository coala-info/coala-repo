...
scATAC-seq dataset: `--barcodes celltype1\_barcodes.txt --max-count 2`.
### Bugs fixed
1) `PETrackII.pileup\_bdg` now wraps pileup outputs in `array.array`
before passing them to `bedGraphTrackI`, preventing runtime type
errors when writing bedGraph tracks from FRAG inputs.
2) `PETrackII.sample\_percent\*` now correctly allows zero-percent
...
4) Fixed a `pvalue\_stat` issue that could yield `nan` when a region
...
please read the [advanced usage](docs/Advanced\_Step-by-step\_Peak\_Calling.md). There is a
[Q&A](docs/qa.md) document where we collected some common questions
from users.
## Contribute
Please read our [CODE OF CONDUCT](CODE\_OF\_CONDUCT.md) and [How to
...
:hidden:
docs/INSTALL.md
docs/subcommands\_index.md
docs/fileformats\_index.md
docs/tutorial.md
docs/qa.md
