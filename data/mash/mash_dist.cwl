...
baseCommand:
  ...
  - dist
label: mash_dist
doc: "Estimate the distance of each query sequence to the reference. Both the reference
  and queries can be fasta or fastq, gzipped or not, or Mash sketch files (.msh) with
  matching k-mer sizes. Query files can also be files of file names (see -l). Whole
  files are compared by default (see -i). The output fields are [reference-ID, query-ID,
  distance, p-value, shared-hashes].\n\nTool homepage: https://github.com/marbl/Mash"
inputs:
  - id: reference
    type: File
  ...
  - id: query
    type:
      type: array
      items: File
    doc: Query file(s) (fasta, fastq, or Mash sketch, or file of file names with
      -l)
    inputBinding:
      position: 2
  - id: alphabet
