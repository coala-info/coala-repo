...
[SummarizedExperiment](https://bioconductor.org/packages/SummarizedExperiment) package. This allows developers to take
...
values111 For simplicity’s sake, we won’t worry about enforcing integer type, as fractional values are possible, e.g., when dealing with expected counts..

## 2.2 Defining the class and its constructor
...
## 2.3 Defining a validity method
...
`setValidity2` from the [S4Vectors](https://bioconductor.org/packages/S4Vectors)
package222 This allows us to turn off the validity checks in internal functions where intermediate objects may not be valid within the scope of the function..
...
generics that do this task, e.g., in [BiocGenerics](https://bioconductor.org/packages/BiocGenerics) or
[S4Vectors](https://bioconductor.org/packages/S4Vectors)333 If you have an idea for a generally applicable generic that is not yet available, please contact the Bioconductor core team..
...
class444 The `...` in the generic function definition means that custom arguments like `withDimnames=` can be provided for specific methods, if necessary.:
...
## 2.5 Some comments on package organization
...
[Writing R Extensions](https://cran.r-project.org/doc/manuals/r-release/R-exts.html#The-DESCRIPTION-file) for more details.

# 3 Deriving a class with custom slots
...
argument to specify the new custom slots555 It does no harm to repeat the Roxygen tags, which explicitly specifies the imports required for each class and function..
...
## 3.3 Creating getter methods

### 3.3.1 For 1D data structures
...
### 3.3.3 For `SummarizedExperiment` slots

The getter methods defined in [SummarizedExperiment](https://bioconductor.org/packages/SummarizedExperiment) can be directly
...
## 3.4 Defining the validity method
...
implementation666 This protects against changes to the slot names, and simplifies development when the storage mode differs from the conceptual meaning of the data, e.g., for efficiency purposes..
...
We use the `NCOL` and `NROW` methods from [BiocGenerics](https://bioconductor.org/packages/BiocGenerics) as these
support various Bioconductor objects, whereas the base methods do not.

## 3.5 Creating a `show` method
...
### 3.6.3 For `SummarizedExperiment` slots

Again, we can use the setter methods defined in
[SummarizedExperiment](https://bioconductor.org/packages/SummarizedExperiment) to modify slots in the base class. These
...
### 3.6.4 Other types of modifying functions
...
`normalize` function, using the generic from [BiocGenerics](https://bioconductor.org/packages/BiocGenerics).
...
## 3.7 Enabling subsetting operations

### 3.7.1 Getting a subset
...
## 3.8 Defining combining methods
...
## 3.9 Defining coercion methods

### 3.9.1 Coercion from `SummarizedExperiment`
...
### 3.9.2 Deriving from a `RangedSummarizedExperiment`

Note that, if we were deriving from a `RangedSummarizedExperiment` (e.g., for some `ExampleClassRanged`),
it would be necessary to define explicit conversions from both `RangedSummarizedExperiment` *and* `SummarizedExperment` to `ExampleClassRanged`.
...
1. Use the existing `SummarizedExperiment` to `RangedSummarizedExperiment` converter defined in *[SummarizedExperiment](https://bioconductor.org/packages/3.22/SummarizedExperiment)*, and then
...
[testthat](https://cran.r-project.org/package%3Dtestthat) package. Each function will test an expression and will
