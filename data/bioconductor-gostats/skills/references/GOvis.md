...
R. Gentleman and Sonali Kumari1
...
* [Introduction](#introduction)
  + [0.1 Graphs](#graphs)
  + [0.2 Some problems](#some-problems)
  + [0.3 An Example](#sec:example)
* [1 Visualization](#visualization)
* [2 GO induced Distances](#go-induced-distances)
* [3 Operations on GO Graphs](#operations-on-go-graphs)
* [References](#references)

# Introduction

The basic characteristics of the GO (The Gene Ontology Consortium, [2000](#ref-GO)) data are described in The Gene Ontology Consortium ([2000](#ref-GO)) and the
...
mappings of genes to GO terms that are provide by GOA (Camon *et al.*, [2004](#ref-GOA)). We examine some
...
(Chiaretti *et al.*, [2004](#ref-Chiaretti2004)).
...
The results reported in this document are based on hgu95av2.db 3.13.0
version of the *[hgu95av2.db](https://bioconductor.org/packages/3.22/hgu95av2.db)* data package and on GO.db 3.22.0
of the *[GO.db](https://bioconductor.org/packages/3.22/GO.db)* package. If you have different versions your results
...
of genes one may use the mappings provided by GOA (Camon *et al.*, [2004](#ref-GOA)) to find the most
...
\(G\_1\) and \(G\_2\) are two different GO graphs from the same ontology then we can
form their union, \(G\_1 \cup G\_2\) which is the union of the nodes in the two
...
To demonstrate some of the tools that are included in the *[GOstats](https://bioconductor.org/packages/3.22/GOstats)*
...
GeneChip arrays (Chiaretti *et al.*, [2004](#ref-Chiaretti2004)). The data were normalized using quantile
normalization and expression estimates were computed using RMA (Irizarry *et al.*, [2003](#ref-Irizarry2003)).
...
We refer readers to Heydebreck *et al.* ([2004](#ref-WileyEncy)) for a general discussion of some of the issues
...
100 units. Applying both of these transformations yielded 771
probes that were differentially expressed.

Next restricting attention to just these 771 probes we
use the *mt.maxT* function from the *[multtest](https://bioconductor.org/packages/3.22/multtest)*
package to compute adjusted \(p\)-values for F-test comparisons. We found
151 of these to have adjusted \(p\)-values less
than 0.05. We will deem these to be the differentially
...
though, we note that these 151 probes map to
131 distinct Entrez Gene identifiers and since
...
criteria. In Storch *et al.* ([2002](#ref-Storch)) they considered using color for the nodes. The studied
...
interest were mapped to terms in the Biological Process arm of GO The Gene Ontology Consortium ([2000](#ref-GO)). They then
...
![ The induced MF GO graph for the selected genes.](data:image/png;base64...)

Figure 1: The induced MF GO graph for the selected genes

![ The induced BP GO graph for the selected genes.](data:image/png;base64...)

Figure 2: The induced BP GO graph for the selected genes

![The induced CC GO graph for the selected genes.](data:image/png;base64...)
...
a basis for assigning similarity. Readers should consult W.Lord *et al.* ([2003](#ref-Lordetal)) for a general
review of the area and for some suggestions. Code contributions that implement
some of their methods would be most welcome.

Two methods based on graph similarity have been implemented in *[GOstats](https://bioconductor.org/packages/3.22/GOstats)*
...
separately. For example, suppose that we have a gene, \(G\_1\) that performs a
...
You should refer to the documentation for the *[graph](https://bioconductor.org/packages/3.22/graph)* and
*[RBGL](https://bioconductor.org/packages/3.22/RBGL)* packages for details on how to manipulate *graph*
...
```
## [1] "GO:0097225" "GO:0070062" "GO:0098978" "GO:0008076" "GO:0030672"
```

# References

Camon,E. *et al.* (2004) The Gene Ontology annotation (GOA) database: Sharing knowledge in Uniprot with Gene Ontology. *Nucleic Acids Research*, **32**, D262–D266.
...
Heydebreck,A. von *et al.* (2004) Differential expression with the bioconductor project. In, *Encyclopedia of genetics, genomics, proteomics and bioinformatics*. John Wiley; Sons.
...
Storch,K.-F. *et al.* (2002) Extensive and divergent circadian gene expression in liver and heart. *Nature*, **417**, 79–82.

The Gene Ontology Consortium (2000) Gene Ontology: Tool for the unification of biology. *Nature Genetics*, **25**, 25–29.

W.Lord,P. *et al.* (2003) Investigating semantic similarity measures across the Gene Ontology: The relationship between sequence and annotation. *Bioinformatics*, **19**, 1275–83.