...
label: gatk_CompareDuplicatesSpark
doc: Determine if two potentially identical BAMs have the same duplicate reads. 
  This tool is useful for checking if two BAMs that seem identical have the same
...
inputs:
  ...
  - id: output
    type: string
    doc: If output is given, the tool will return a bam with all the mismatching
      duplicate groups in the first specified file
    inputBinding:
      position: 101
      prefix: --output
  ...
  - id: output2
    type: string
    doc: If output is given, the tool will return a bam with all the mismatching
      duplicate groups in the second specified input file
    inputBinding:
      position: 101
      prefix: --output2
  - id: print_summary
  ...
  - id: throw_on_diff
    type:
      - 'null'
      - boolean
    doc: Throw error if any differences were found
    inputBinding:
      position: 101
