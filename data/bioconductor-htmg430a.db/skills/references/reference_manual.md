...
htmg430aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
htmg430aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(htmg430aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the htmg430a.db annotation Package. The purpose of this package is to provide detailed
information about the htmg430a platform. This package is updated biannually.
...
htmg430aCHR is an R object that provides mappings between a manufacturer identifier and the
...
htmg430aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
htmg430aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
htmg430aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
htmg430aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
htmg430aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. htmg430aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the htmg430aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the htmg430aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In htmg430aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert htmg430aENZYME2PROBE to a list to see inside

htmg430aGENENAME

9

x <- htmg430aENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
htmg430aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
htmg430aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (htmg430aGO2PROBE)
...
htmg430aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If htmg430aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If htmg430aGO2ALLPROBES or htmg430aGO2PROBE is cast as a list, each GO term maps to a
...
x <- htmg430aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert htmg430aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package htmg430a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

htmg430aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package htmg430a.db.
...
htmg430aMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for htmg430a

Description

htmg430aORGANISM is an R object that contains a single item: a character string that names
the organism for which htmg430a was built. htmg430aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, htmg430aORGANISM
provides a simple way to programmatically extract the organism name. htmg430aORGPKG pro-
...
htmg430aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

htmg430aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- htmg430aPATH
mapped_probes <- mappedkeys(x)
# Now convert the htmg430aPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htmg430aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
htmg430aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. htmg430aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When htmg430aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When htmg430aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- htmg430aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object htmg430aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htmg430aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
htmg430aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
htmg430aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
htmg430aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
htmg430a_dbconn()
htmg430a_dbfile()
htmg430a_dbschema(file="", show.indices=FALSE)
htmg430a_dbInfo()
...
htmg430a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by htmg430a_dbconn or you will break all
the AnnDbObj objects defined in this package!

htmg430a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

htmg430a_dbschema prints the schema definition of the package annotation DB.

htmg430a_dbInfo prints other information about the package annotation DB.

Value

htmg430a_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

htmg430a_dbfile: a character string with the path to the package annotation DB.

htmg430a_dbschema: none (invisible NULL).

htmg430a_dbInfo: none (invisible NULL).
...
dbGetQuery(htmg430a_dbconn(), "SELECT COUNT(*) FROM probes")
...
htmg430a_dbconn, 20
htmg430aACCNUM, 1
htmg430aALIAS2PROBE, 2
htmg430aCHR, 3
htmg430aCHRLENGTHS, 4
htmg430aCHRLOC, 4
htmg430aENSEMBL, 5
htmg430aENTREZID, 6
htmg430aENZYME, 7
...
htmg430a (htmg430a.db), 2
htmg430a.db, 2
htmg430a_dbconn, 20
htmg430a_dbfile (htmg430a_dbconn), 20
htmg430a_dbInfo (htmg430a_dbconn), 20

22

htmg430a_dbschema (htmg430a_dbconn), 20
htmg430aACCNUM, 1
htmg430aALIAS2PROBE, 2
htmg430aCHR, 3
htmg430aCHRLENGTHS, 4
htmg430aCHRLOC, 4
htmg430aCHRLOCEND (htmg430aCHRLOC), 4
htmg430aENSEMBL, 5
htmg430aENSEMBL2PROBE

(htmg430aENSEMBL), 5

htmg430aENTREZID, 6
htmg430aENZYME, 7
htmg430aENZYME2PROBE (htmg430aENZYME), 7
htmg430aGENENAME, 9
htmg430aGO, 10
htmg430aGO2ALLPROBES, 11
htmg430aGO2ALLPROBES (htmg430aGO), 10
htmg430aGO2PROBE (htmg430aGO), 10
htmg430aLOCUSID (htmg430aENTREZID), 6
htmg430aMAPCOUNTS, 12
htmg430aMGI, 12
htmg430aMGI2PROBE (htmg430aMGI), 12
htmg430aORGANISM, 13
htmg430aORGPKG (htmg430aORGANISM), 13
htmg430aPATH, 14
htmg430aPATH2PROBE (htmg430aPATH), 14
htmg430aPFAM, 15
htmg430aPMID, 15
htmg430aPMID2PROBE (htmg430aPMID), 15
htmg430aPROSITE, 17
htmg430aREFSEQ, 17
htmg430aSYMBOL, 18
htmg430aUNIPROT, 19
htmg430aUNIPROT2PROBE
