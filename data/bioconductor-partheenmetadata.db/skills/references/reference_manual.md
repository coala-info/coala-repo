...
PartheenMetaDataACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
PartheenMetaDataALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(PartheenMetaDataALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the PartheenMetaData.db annotation Package. The purpose of this package is to pro-
vide detailed information about the PartheenMetaData platform. This package is updated biannu-
ally.
...
## use of select().
ls("package:PartheenMetaData.db")
...
PartheenMetaDataCHR is an R object that provides mappings between a manufacturer identifier
...
PartheenMetaDataCHRLOC

PartheenMetaDataCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

PartheenMetaDataCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
PartheenMetaDataCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
PartheenMetaDataENSEMBL is an R object that contains mappings between manufacturer identi-
...
# Convert to a list
xx <- as.list(PartheenMetaDataENSEMBL2PROBE)
if(length(xx) > 0){
...
PartheenMetaDataENTREZID is an R object that provides mappings between manufacturer identi-
...
PartheenMetaDataENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. PartheenMetaDataENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the PartheenMetaDataENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the PartheenMetaDataENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In PartheenMetaDataENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert PartheenMetaDataENZYME2PROBE to a list to see inside
xx <- as.list(PartheenMetaDataENZYME2PROBE)
if(length(xx) > 0){

# Get the probe identifiers for the first five enzyme

PartheenMetaDataGENENAME
...
PartheenMetaDataGENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
PartheenMetaDataGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(PartheenMetaDataGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

PartheenMetaDataGO2ALLPROBES is an R object that provides mappings between a given GO
...
PartheenMetaDataGO2PROBE.

Details

If PartheenMetaDataGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If PartheenMetaDataGO2ALLPROBES or PartheenMetaDataGO2PROBE is cast as a list, each
...
PartheenMetaDataMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
PartheenMetaDataMAPCOUNTS

13

PartheenMetaDataMAPCOUNTS

Number of mapped keys for the maps in package PartheenMeta-
Data.db
...
PartheenMetaDataMAPCOUNTS provides the "map count" (i.e.
each map in package PartheenMetaData.db.
...
PartheenMetaDataOMIM Map between Manufacturer Identifiers and Mendelian Inheritance in

Man (MIM) identifiers

Description

PartheenMetaDataOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for PartheenMetaData

Description

PartheenMetaDataORGANISM is an R object that contains a single item: a character string that
names the organism for which PartheenMetaData was built. PartheenMetaDataORGPKG is an R
...
Although the package name is suggestive of the organism for which it was built, PartheenMeta-
DataORGANISM provides a simple way to programmatically extract the organism name. Partheen-
MetaDataORGPKG provides a simple way to programmatically extract the name of the parent or-
ganism package. The parent organism package is a strict dependency for chip packages as this is
...
## Bimap interface:
PartheenMetaDataORGANISM
PartheenMetaDataORGPKG

PartheenMetaDataPATH

15

PartheenMetaDataPATH Mappings between probe identifiers and KEGG pathway identifiers
...
PartheenMetaDataPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

PartheenMetaDataPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the PartheenMetaDataPATH2PROBE object to a list
xx <- as.list(PartheenMetaDataPATH2PROBE)
if(length(xx) > 0){
...
PartheenMetaDataPMID

PartheenMetaDataPFAM Maps between Manufacturer Identifiers and PFAM Identifiers

Description

PartheenMetaDataPFAM is an R object that provides mappings between manufacturer identifiers
...
PartheenMetaDataPMID Maps between Manufacturer Identifiers and PubMed Identifiers

Description

PartheenMetaDataPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. PartheenMetaDataPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When PartheenMetaDataPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When PartheenMetaDataPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object PartheenMetaDataPMID2PROBE to a list
xx <- as.list(PartheenMetaDataPMID2PROBE)
if(length(xx) > 0){
...
PartheenMetaDataPROSITE is an R object that provides mappings between manufacturer identi-
...
PartheenMetaDataREFSEQ is an R object that provides mappings between manufacturer identifiers
...
PartheenMetaDataSYMBOL is an R object that provides mappings between manufacturer identi-
...
PartheenMetaDataUNIGENE

x <- PartheenMetaDataSYMBOL
...
PartheenMetaDataUNIGENE is an R object that provides mappings between manufacturer identi-
...
PartheenMetaDataUNIPROT is an R object that contains mappings between the manufacturer iden-
...
PartheenMetaData_dbconn()
PartheenMetaData_dbfile()
PartheenMetaData_dbschema(file="", show.indices=FALSE)
...
PartheenMetaData_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by PartheenMetaData_dbconn
or you will break all the AnnDbObj objects defined in this package!
PartheenMetaData_dbfile returns the path (character string) to the package annotation DB (this
is an SQLite file).
PartheenMetaData_dbschema prints the schema definition of the package annotation DB.
PartheenMetaData_dbInfo prints other information about the package annotation DB.

Value

PartheenMetaData_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.
PartheenMetaData_dbfile: a character string with the path to the package annotation DB.
PartheenMetaData_dbschema: none (invisible NULL).
PartheenMetaData_dbInfo: none (invisible NULL).
...
dbGetQuery(PartheenMetaData_dbconn(), "SELECT COUNT(*) FROM probes")
...
PartheenMetaData.db, 2
PartheenMetaData_dbconn, 22
PartheenMetaDataACCNUM, 1
PartheenMetaDataALIAS2PROBE, 2
PartheenMetaDataCHR, 3
PartheenMetaDataCHRLENGTHS, 4
PartheenMetaDataCHRLOC, 4
PartheenMetaDataENSEMBL, 5
PartheenMetaDataENTREZID, 6
PartheenMetaDataENZYME, 7
PartheenMetaDataGENENAME, 9
PartheenMetaDataGO, 10
PartheenMetaDataMAP, 12
PartheenMetaDataMAPCOUNTS, 13
PartheenMetaDataOMIM, 13
PartheenMetaDataORGANISM, 14
PartheenMetaDataPATH, 15
PartheenMetaDataPFAM, 16
PartheenMetaDataPMID, 16
PartheenMetaDataPROSITE, 17
PartheenMetaDataREFSEQ, 18
PartheenMetaDataSYMBOL, 19
PartheenMetaDataUNIGENE, 20
PartheenMetaDataUNIPROT, 21
...
PartheenMetaData (PartheenMetaData.db),
...
PartheenMetaDataCHRLENGTHS, 4
PartheenMetaDataCHRLOC, 4
PartheenMetaDataCHRLOCEND

(PartheenMetaDataCHRLOC), 4

PartheenMetaDataENSEMBL, 5
PartheenMetaDataENSEMBL2PROBE

(PartheenMetaDataENSEMBL), 5

PartheenMetaDataENTREZID, 6
PartheenMetaDataENZYME, 7
PartheenMetaDataENZYME2PROBE

(PartheenMetaDataENZYME), 7

PartheenMetaDataGENENAME, 9
PartheenMetaDataGO, 10
PartheenMetaDataGO2ALLPROBES, 11
PartheenMetaDataGO2ALLPROBES
...
PartheenMetaDataPATH, 15
PartheenMetaDataPATH2PROBE

(PartheenMetaDataPATH), 15

PartheenMetaDataPFAM, 16
PartheenMetaDataPMID, 16
PartheenMetaDataPMID2PROBE
