...
hgug4112aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgug4112aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4112aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4112aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4112a.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4112a platform. This package is updated biannually.
...
hgug4112aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4112aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4112aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgug4112aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgug4112aENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4112aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4112aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgug4112aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgug4112aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgug4112aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hgug4112aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgug4112aENZYME2PROBE to a list to see inside
xx <- as.list(hgug4112aENZYME2PROBE)
if(length(xx) > 0){
...
hgug4112aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4112aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgug4112aGO2PROBE)
...
hgug4112aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgug4112aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgug4112aGO2ALLPROBES or hgug4112aGO2PROBE is cast as a list, each GO term maps to
...
# Convert hgug4112aGO2ALLPROBES to a list
xx <- as.list(hgug4112aGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4112aMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4112a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgug4112aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4112a.db.
...
hgug4112aOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4112a

Description

hgug4112aORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4112a was built. hgug4112aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgug4112aORGANISM
provides a simple way to programmatically extract the organism name. hgug4112aORGPKG pro-
...
hgug4112aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgug4112aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hgug4112aPATH2PROBE object to a list
xx <- as.list(hgug4112aPATH2PROBE)
if(length(xx) > 0){
...
hgug4112aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgug4112aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgug4112aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgug4112aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgug4112aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgug4112aPMID2PROBE to a list
xx <- as.list(hgug4112aPMID2PROBE)
if(length(xx) > 0){
...
hgug4112aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgug4112aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4112aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4112aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4112aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgug4112a_dbconn()
hgug4112a_dbfile()
hgug4112a_dbschema(file="", show.indices=FALSE)
hgug4112a_dbInfo()
...
hgug4112a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4112a_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgug4112a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgug4112a_dbschema prints the schema definition of the package annotation DB.
hgug4112a_dbInfo prints other information about the package annotation DB.

Value

hgug4112a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
hgug4112a_dbfile: a character string with the path to the package annotation DB.
hgug4112a_dbschema: none (invisible NULL).
hgug4112a_dbInfo: none (invisible NULL).
...
hgug4112a_dbconn, 21
hgug4112aACCNUM, 1
hgug4112aALIAS2PROBE, 2
hgug4112aCHR, 3
hgug4112aCHRLENGTHS, 4
hgug4112aCHRLOC, 4
hgug4112aENSEMBL, 5
hgug4112aENTREZID, 6
hgug4112aENZYME, 7
...
hgug4112aGO, 10
hgug4112aMAP, 12
hgug4112aMAPCOUNTS, 13
hgug4112aOMIM, 13
hgug4112aORGANISM, 14
hgug4112aPATH, 15
hgug4112aPFAM, 16
hgug4112aPMID, 16
hgug4112aPROSITE, 17
...
hgug4112a (hgug4112a.db), 2
hgug4112a.db, 2
hgug4112a_dbconn, 21

hgug4112a_dbfile (hgug4112a_dbconn), 21
hgug4112a_dbInfo (hgug4112a_dbconn), 21
hgug4112a_dbschema (hgug4112a_dbconn),
...
hgug4112aCHRLENGTHS, 4
hgug4112aCHRLOC, 4
hgug4112aCHRLOCEND (hgug4112aCHRLOC), 4
hgug4112aENSEMBL, 5
hgug4112aENSEMBL2PROBE

(hgug4112aENSEMBL), 5

hgug4112aENTREZID, 6
hgug4112aENZYME, 7
hgug4112aENZYME2PROBE

(hgug4112aENZYME), 7

hgug4112aGENENAME, 9
hgug4112aGO, 10
hgug4112aGO2ALLPROBES, 11
hgug4112aGO2ALLPROBES (hgug4112aGO), 10
hgug4112aGO2PROBE (hgug4112aGO), 10
hgug4112aLOCUSID (hgug4112aENTREZID), 6
hgug4112aMAP, 12
hgug4112aMAPCOUNTS, 13
hgug4112aOMIM, 13
hgug4112aORGANISM, 14
hgug4112aORGPKG (hgug4112aORGANISM), 14
hgug4112aPATH, 15
hgug4112aPATH2PROBE (hgug4112aPATH), 15
hgug4112aPFAM, 16
hgug4112aPMID, 16
hgug4112aPMID2PROBE (hgug4112aPMID), 16
hgug4112aPROSITE, 17
hgug4112aREFSEQ, 18
hgug4112aSYMBOL, 19
hgug4112aUNIGENE, 20
hgug4112aUNIPROT, 21
hgug4112aUNIPROT2PROBE
