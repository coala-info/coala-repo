...
rgug4131aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
rgug4131aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

rgug4131aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(rgug4131aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the rgug4131a.db annotation Package. The purpose of this package is to provide de-
tailed information about the rgug4131a platform. This package is updated biannually.
...
rgug4131aCHR is an R object that provides mappings between a manufacturer identifier and the
...
rgug4131aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rgug4131aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
rgug4131aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(rgug4131aENSEMBL2PROBE)
if(length(xx) > 0){
...
rgug4131aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
rgug4131aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. rgug4131aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the rgug4131aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the rgug4131aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In rgug4131aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert rgug4131aENZYME2PROBE to a list to see inside
xx <- as.list(rgug4131aENZYME2PROBE)
if(length(xx) > 0){
...
rgug4131aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rgug4131aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (rgug4131aGO2PROBE)
...
rgug4131aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If rgug4131aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If rgug4131aGO2ALLPROBES or rgug4131aGO2PROBE is cast as a list, each GO term maps to
...
# Convert rgug4131aGO2ALLPROBES to a list
xx <- as.list(rgug4131aGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package rgug4131a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rgug4131aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package rgug4131a.db.
...
The Organism information for rgug4131a

Description

rgug4131aORGANISM is an R object that contains a single item: a character string that names
the organism for which rgug4131a was built. rgug4131aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, rgug4131aORGANISM
provides a simple way to programmatically extract the organism name. rgug4131aORGPKG pro-
...
rgug4131aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

rgug4131aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
rgug4131aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rgug4131aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rgug4131aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When rgug4131aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When rgug4131aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object rgug4131aPMID2PROBE to a list
xx <- as.list(rgug4131aPMID2PROBE)
if(length(xx) > 0){
...
rgug4131aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
rgug4131aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
rgug4131aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
rgug4131aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
rgug4131aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
rgug4131a_dbconn()
rgug4131a_dbfile()
rgug4131a_dbschema(file="", show.indices=FALSE)
rgug4131a_dbInfo()
...
rgug4131a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by rgug4131a_dbconn or you will break all
the AnnDbObj objects defined in this package!

rgug4131a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

rgug4131a_dbschema prints the schema definition of the package annotation DB.

rgug4131a_dbInfo prints other information about the package annotation DB.

Value

rgug4131a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

rgug4131a_dbfile: a character string with the path to the package annotation DB.

rgug4131a_dbschema: none (invisible NULL).

rgug4131a_dbInfo: none (invisible NULL).
...
dbGetQuery(rgug4131a_dbconn(), "SELECT COUNT(*) FROM probes")
...
rgug4131a_dbconn, 20
rgug4131aACCNUM, 1
rgug4131aALIAS2PROBE, 2
rgug4131aCHR, 3
rgug4131aCHRLENGTHS, 4
rgug4131aCHRLOC, 4
rgug4131aENSEMBL, 5
rgug4131aENTREZID, 6
rgug4131aENZYME, 7
rgug4131aGENENAME, 9
rgug4131aGO, 10
rgug4131aMAPCOUNTS, 12
rgug4131aORGANISM, 12
rgug4131aPATH, 13
rgug4131aPFAM, 14
rgug4131aPMID, 14
rgug4131aPROSITE, 16
...
rgug4131a (rgug4131a.db), 2
rgug4131a.db, 2
rgug4131a_dbconn, 20
rgug4131a_dbfile (rgug4131a_dbconn), 20
rgug4131a_dbInfo (rgug4131a_dbconn), 20

22

rgug4131a_dbschema (rgug4131a_dbconn),
...
rgug4131aCHRLENGTHS, 4
rgug4131aCHRLOC, 4
rgug4131aCHRLOCEND (rgug4131aCHRLOC), 4
rgug4131aENSEMBL, 5
rgug4131aENSEMBL2PROBE

(rgug4131aENSEMBL), 5

rgug4131aENTREZID, 6
rgug4131aENZYME, 7
rgug4131aENZYME2PROBE

(rgug4131aENZYME), 7

rgug4131aGENENAME, 9
rgug4131aGO, 10
rgug4131aGO2ALLPROBES, 11
rgug4131aGO2ALLPROBES (rgug4131aGO), 10
rgug4131aGO2PROBE (rgug4131aGO), 10
rgug4131aLOCUSID (rgug4131aENTREZID), 6
rgug4131aMAPCOUNTS, 12
rgug4131aORGANISM, 12
rgug4131aORGPKG (rgug4131aORGANISM), 12
rgug4131aPATH, 13
rgug4131aPATH2PROBE (rgug4131aPATH), 13
rgug4131aPFAM, 14
rgug4131aPMID, 14
rgug4131aPMID2PROBE (rgug4131aPMID), 14
rgug4131aPROSITE, 16
rgug4131aREFSEQ, 16
rgug4131aSYMBOL, 17
rgug4131aUNIGENE, 18
rgug4131aUNIPROT, 19
rgug4131aUNIPROT2PROBE
