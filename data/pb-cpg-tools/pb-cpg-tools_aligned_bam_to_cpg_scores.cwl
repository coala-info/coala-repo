...
inputs:
  - id: bam
    type: File
    doc: Alignment file for input sample in BAM or CRAM format. Alignment file 
      must be mapped and indexed, with MM/ML tags specifying 5mC methylation. If
      a CRAM file is provided, then `--ref` must also be specified
    inputBinding:
      position: 101
      prefix: --bam
  - id: hap_tag
    type:
      - 'null'
      - string
    doc: The 2-letter SAM aux tag used for per-read haplotype ids in the input 
      bam
    inputBinding:
      position: 101
      prefix: --hap-tag
  - id: min_coverage
    type:
      - 'null'
      - int
    doc: Minimum site coverage. The tensorflow prediction models have their own 
      hard-coded minimum coverage of 4, so any value below this level should 
      have no effect in 'model' pileup mode
    inputBinding:
      position: 101
      prefix: --min-coverage
  - id: min_mapq
  ...
  - id: ref
    type:
      - 'null'
      - File
    doc: Genome reference in FASTA format. This is required if either (1) 
      'reference' modsite mode is selected or (2) input alignments are in CRAM 
      format
    inputBinding:
      position: 101
      prefix: --ref
