...
moe430aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
moe430aALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the moe430a.db annotation Package. The purpose of this package is to provide detailed
information about the moe430a platform. This package is updated biannually.
...
moe430aCHR is an R object that provides mappings between a manufacturer identifier and the
...
moe430aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
moe430aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
moe430aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
moe430aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
moe430aENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. moe430aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the moe430aENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the moe430aENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In moe430aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert moe430aENZYME2PROBE to a list to see inside

moe430aGENENAME

9

x <- moe430aENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
moe430aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
moe430aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (moe430aGO2PROBE)
...
moe430aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If moe430aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If moe430aGO2ALLPROBES or moe430aGO2PROBE is cast as a list, each GO term maps to a
...
x <- moe430aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert moe430aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package moe430a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

moe430aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package moe430a.db.
...
moe430aMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for moe430a

Description

moe430aORGANISM is an R object that contains a single item: a character string that names the
organism for which moe430a was built. moe430aORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, moe430aORGANISM
provides a simple way to programmatically extract the organism name. moe430aORGPKG pro-
...
moe430aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

moe430aPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- moe430aPATH
mapped_probes <- mappedkeys(x)
# Now convert the moe430aPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
moe430aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
moe430aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. moe430aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When moe430aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When moe430aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- moe430aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object moe430aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
moe430aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
moe430aREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
moe430aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
moe430aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
moe430a_dbconn()
moe430a_dbfile()
moe430a_dbschema(file="", show.indices=FALSE)
moe430a_dbInfo()
...
moe430a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by moe430a_dbconn or you will break all the
AnnDbObj objects defined in this package!

moe430a_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

moe430a_dbschema prints the schema definition of the package annotation DB.

moe430a_dbInfo prints other information about the package annotation DB.

Value

moe430a_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

moe430a_dbfile: a character string with the path to the package annotation DB.

moe430a_dbschema: none (invisible NULL).

moe430a_dbInfo: none (invisible NULL).
...
dbGetQuery(moe430a_dbconn(), "SELECT COUNT(*) FROM probes")
...
moe430a_dbschema (moe430a_dbconn), 20
moe430aACCNUM, 1
moe430aALIAS2PROBE, 2
moe430aCHR, 3
moe430aCHRLENGTHS, 4
moe430aCHRLOC, 4
moe430aCHRLOCEND (moe430aCHRLOC), 4
moe430aENSEMBL, 5
moe430aENSEMBL2PROBE (moe430aENSEMBL), 5
moe430aENTREZID, 6
moe430aENZYME, 7
moe430aENZYME2PROBE (moe430aENZYME), 7
moe430aGENENAME, 9
moe430aGO, 10
moe430aGO2ALLPROBES, 11
moe430aGO2ALLPROBES (moe430aGO), 10
moe430aGO2PROBE (moe430aGO), 10
moe430aLOCUSID (moe430aENTREZID), 6
moe430aMAPCOUNTS, 12
moe430aMGI, 12
moe430aMGI2PROBE (moe430aMGI), 12
moe430aORGANISM, 13
moe430aORGPKG (moe430aORGANISM), 13
moe430aPATH, 14
moe430aPATH2PROBE (moe430aPATH), 14
moe430aPFAM, 15
moe430aPMID, 15
moe430aPMID2PROBE (moe430aPMID), 15
moe430aPROSITE, 17
moe430aREFSEQ, 17
moe430aSYMBOL, 18
moe430aUNIPROT, 19
moe430aUNIPROT2PROBE (moe430aUNIPROT),
...
moe430a (moe430a.db), 2
moe430a.db, 2
moe430a_dbconn, 20
moe430a_dbfile (moe430a_dbconn), 20
moe430a_dbInfo (moe430a_dbconn), 20
