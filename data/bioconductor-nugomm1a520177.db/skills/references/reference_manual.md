...
nugomm1a520177ACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
nugomm1a520177ALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(nugomm1a520177ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the nugomm1a520177.db annotation Package. The purpose of this package is to pro-
vide detailed information about the nugomm1a520177 platform. This package is updated biannu-
ally.
...
nugomm1a520177CHR is an R object that provides mappings between a manufacturer identifier
...
nugomm1a520177CHRLENGTHS

A named vector for the length of each of the chromosomes

Description

nugomm1a520177CHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
tt <- nugomm1a520177CHRLENGTHS
# Length of chromosome 1
tt["1"]

4

nugomm1a520177ENSEMBL

nugomm1a520177CHRLOC Map Manufacturer IDs to Chromosomal Location

Description

nugomm1a520177CHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
nugomm1a520177ENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

nugomm1a520177ENSEMBL is an R object that contains mappings between manufacturer identi-
fiers and Ensembl gene accession numbers.

nugomm1a520177ENTREZID
...
# Convert to a list
xx <- as.list(nugomm1a520177ENSEMBL2PROBE)
if(length(xx) > 0){
...
nugomm1a520177ENTREZID is an R object that provides mappings between manufacturer identi-
...
nugomm1a520177ENZYME Maps between Manufacturer IDs and Enzyme Commission (EC) Num-

bers

Description

nugomm1a520177ENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. nugomm1a520177ENZYME2PROBE is an R object that maps Enzyme
Commission (EC) numbers to manufacturer identifiers.

Details

When the nugomm1a520177ENZYME maping viewed as a list, each manufacturer identifier maps
...
For the nugomm1a520177ENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In nugomm1a520177ENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert nugomm1a520177ENZYME2PROBE to a list to see inside
xx <- as.list(nugomm1a520177ENZYME2PROBE)
if(length(xx) > 0){
...
nugomm1a520177GENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
nugomm1a520177GO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(nugomm1a520177GO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

nugomm1a520177GO2ALLPROBES is an R object that provides mappings between a given GO
...
nugomm1a520177GO2PROBE.

Details

If nugomm1a520177GO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If nugomm1a520177GO2ALLPROBES or nugomm1a520177GO2PROBE is cast as a list, each
...
nugomm1a520177MAPCOUNTS

# For the reverse map:
# Convert to a list
xx <- as.list(nugomm1a520177GO2PROBE)
if(length(xx) > 0){
...
# Convert nugomm1a520177GO2ALLPROBES to a list
xx <- as.list(nugomm1a520177GO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package nugomm1a520177.db

Description

nugomm1a520177MAPCOUNTS provides the "map count" (i.e.
each map in package nugomm1a520177.db.
...
nugomm1a520177MAPCOUNTS
mapnames <- names(nugomm1a520177MAPCOUNTS)
nugomm1a520177MAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package nugomm1a520177.db
checkMAPCOUNTS("nugomm1a520177.db")
...
nugomm1a520177MGI is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(nugomm1a520177MGI2PROBE)
if(length(xx) > 0){
...
The Organism information for nugomm1a520177

Description

nugomm1a520177ORGANISM is an R object that contains a single item: a character string that
names the organism for which nugomm1a520177 was built. nugomm1a520177ORGPKG is an R
...
Although the package name is suggestive of the organism for which it was built, nugomm1a520177ORGANISM
provides a simple way to programmatically extract the organism name. nugomm1a520177ORGPKG
...
nugomm1a520177PATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

nugomm1a520177PATH2PROBE is an R object that provides mappings between KEGG identifiers
...
# Now convert the nugomm1a520177PATH2PROBE object to a list
xx <- as.list(nugomm1a520177PATH2PROBE)
if(length(xx) > 0){
...
nugomm1a520177PMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. nugomm1a520177PMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When nugomm1a520177PMID is viewed as a list each manufacturer identifier is mapped to a
...
When nugomm1a520177PMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
# Now convert the reverse map object nugomm1a520177PMID2PROBE to a list
xx <- as.list(nugomm1a520177PMID2PROBE)
if(length(xx) > 0){
...
nugomm1a520177REFSEQ Map between Manufacturer Identifiers and RefSeq Identifiers

Description

nugomm1a520177REFSEQ is an R object that provides mappings between manufacturer identifiers
...
nugomm1a520177SYMBOL Map between Manufacturer Identifiers and Gene Symbols

Description

nugomm1a520177SYMBOL is an R object that provides mappings between manufacturer identi-
...
nugomm1a520177UNIGENE

x <- nugomm1a520177SYMBOL
...
nugomm1a520177UNIGENE Map between Manufacturer Identifiers and UniGene cluster identifiers

Description

nugomm1a520177UNIGENE is an R object that provides mappings between manufacturer identi-
...
nugomm1a520177UNIPROT

17

nugomm1a520177UNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

nugomm1a520177UNIPROT is an R object that contains mappings between the manufacturer iden-
...
nugomm1a520177_dbconn Collect information about the package annotation DB
...
nugomm1a520177_dbconn()
nugomm1a520177_dbfile()
nugomm1a520177_dbschema(file="", show.indices=FALSE)
...
nugomm1a520177_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by nugomm1a520177_dbconn or you
will break all the AnnDbObj objects defined in this package!

nugomm1a520177_dbfile returns the path (character string) to the package annotation DB (this is
an SQLite file).

nugomm1a520177_dbschema prints the schema definition of the package annotation DB.

nugomm1a520177_dbInfo prints other information about the package annotation DB.

Value

nugomm1a520177_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

nugomm1a520177_dbfile: a character string with the path to the package annotation DB.

nugomm1a520177_dbschema: none (invisible NULL).

nugomm1a520177_dbInfo: none (invisible NULL).
...
dbGetQuery(nugomm1a520177_dbconn(), "SELECT COUNT(*) FROM probes")

nugomm1a520177_dbschema()
...
nugomm1a520177.db, 2
nugomm1a520177_dbconn, 17
nugomm1a520177ACCNUM, 1
nugomm1a520177ALIAS2PROBE, 2
nugomm1a520177CHR, 3
nugomm1a520177CHRLENGTHS, 3
nugomm1a520177CHRLOC, 4
nugomm1a520177ENSEMBL, 4
nugomm1a520177ENTREZID, 5
nugomm1a520177ENZYME, 6
nugomm1a520177GENENAME, 7
nugomm1a520177GO, 8
nugomm1a520177MAPCOUNTS, 10
nugomm1a520177MGI, 11
nugomm1a520177ORGANISM, 11
nugomm1a520177PATH, 12
nugomm1a520177PMID, 13
nugomm1a520177REFSEQ, 14
nugomm1a520177SYMBOL, 15
nugomm1a520177UNIGENE, 16
nugomm1a520177UNIPROT, 17
...
nugomm1a520177 (nugomm1a520177.db), 2
nugomm1a520177.db, 2
nugomm1a520177_dbconn, 17
...
nugomm1a520177CHRLENGTHS, 3
nugomm1a520177CHRLOC, 4
nugomm1a520177CHRLOCEND

(nugomm1a520177CHRLOC), 4

nugomm1a520177ENSEMBL, 4
nugomm1a520177ENSEMBL2PROBE

(nugomm1a520177ENSEMBL), 4

nugomm1a520177ENTREZID, 5
nugomm1a520177ENZYME, 6
nugomm1a520177ENZYME2PROBE

(nugomm1a520177ENZYME), 6

nugomm1a520177GENENAME, 7
nugomm1a520177GO, 8
nugomm1a520177GO2ALLPROBES, 9
nugomm1a520177GO2ALLPROBES
...
nugomm1a520177LOCUSID

(nugomm1a520177ENTREZID), 5

nugomm1a520177MAPCOUNTS, 10
nugomm1a520177MGI, 11
nugomm1a520177MGI2PROBE
...
nugomm1a520177PATH, 12
nugomm1a520177PATH2PROBE

(nugomm1a520177PATH), 12

nugomm1a520177PMID, 13
nugomm1a520177PMID2PROBE
