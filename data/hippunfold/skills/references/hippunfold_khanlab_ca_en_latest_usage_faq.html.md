...
* Frequently asked questions
  + [Why is the workflow stopping at the run\_inference step?](#why-is-the-workflow-stopping-at-the-run-inference-step)
  + [Why do I get the error, `No input images found for T1w`, or `No input images found for T2w`](#why-do-i-get-the-error-no-input-images-found-for-t1w-or-no-input-images-found-for-t2w)
  + [Why is the HippUnfold Docker/Singularity/Apptainer container so large?](#why-is-the-hippunfold-docker-singularity-apptainer-container-so-large)
  + [Why do I end up with large files in `~/.cache/hippunfold` after running HippUnfold?](#why-do-i-end-up-with-large-files-in-cache-hippunfold-after-running-hippunfold)
...
* Frequently asked questions
* [View page source](../_sources/usage/faq.md.txt)

---

# Frequently asked questions[](#frequently-asked-questions "Permalink to this heading")

1. [Why is the workflow stopping at the run\_inference step?](#run-inference-mem)
2. [Why do I get the error, No input images found for T1w, or No input images found for T2w](#no-input-images)
3. [Why is the HippUnfold Docker/Singularity/Apptainer container so large?](#container-size)
4. [Why do I end up with large files in ~/.cache/hippunfold after running HippUnfold?](#model-files)

## Why is the workflow stopping at the run\_inference step?[](#why-is-the-workflow-stopping-at-the-run-inference-step "Permalink to this heading")

If you are getting an error in the run\_inference step, e.g. as follows:
...
## Why do I get the error, `No input images found for T1w`, or `No input images found for T2w`[](#why-do-i-get-the-error-no-input-images-found-for-t1w-or-no-input-images-found-for-t2w "Permalink to this heading")
...
## Why do I end up with large files in `~/.cache/hippunfold` after running HippUnfold?[](#why-do-i-end-up-with-large-files-in-cache-hippunfold-after-running-hippunfold "Permalink to this heading")

This folder is where the nnU-net model parameters are stored by default. You can override the location with the `HIPPUNFOLD_CACHE_DIR` environment variable. See [Deep learning nnU-net model files](../contributing/contributing.html#deep-learning-nnu-net-model-files) for more details.

[Previous](templates.html "Template-base segmentation")
[Next](../pipeline/pipeline.html "Pipeline Details")
