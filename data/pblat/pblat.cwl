...
baseCommand: pblat
label: pblat
doc: "BLAT with parallel supports v. 36x2 fast sequence search command line tool\n
  \nTool homepage: https://github.com/icebert/pblat"
inputs:
  - id: database
    type: File
    doc: Database .fa file
    inputBinding:
      position: 1
  - id: query
    type: File
  ...
  - id: mask
    type:
      - 'null'
      - string
    doc: Mask out repeats (lower, upper, out, or file.out).
    inputBinding:
      position: 103
      prefix: -mask
  - id: max_gap
  ...
  - id: one_off
    type:
      - 'null'
      - int
    doc: If set to 1, this allows one mismatch in tile and still triggers an alignments.
    default: 0
    inputBinding:
      position: 103
      prefix: -oneOff
  ...
  - id: repeats
    type:
      - 'null'
      - string
    doc: Type is same as mask types. Repeat bases will be reported separately.
    inputBinding:
      position: 103
      prefix: -repeats
  - id: step_size
...
outputs:
  - id: output
    type: File
    doc: Name of the output file (e.g., output.psl)
    outputBinding:
      glob: '*.out'
  - id: make_ooc
    type:
...
hints:
  - class: DockerRequirement
    dockerPull: quay.io/biocontainers/pblat:2.5.1--h41f7678_4
