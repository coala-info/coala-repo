...
在开发csvtk之前的两三年间，我已经写了几个可以复用的Python/Perl脚本（https://github.com/shenwei356/datakit） ，包括csv2tab、csvtk\_grep、csv\_join、csv\_melt，intersection，unique。所以我的计划是首先集成这些已有的功能，随后根据需求进行扩展。
...
6. csvtk默认以`#`开始的为注释行，若标题行含`#`，请给全局参数`-C`指定另一个不常见的字符（如`$`）
...
3. 转为markdown
   ...
   | id | first\_name | last\_name | username |
   | --- | --- | --- | --- |
   | 11 | Rob | Pike | rob |
   | 2 | Ken | Thompson | ken |
   | 4 | Robert | Griesemer | gri |
