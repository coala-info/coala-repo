...
hints:
  SoftwareRequirement:
    packages:
      busco:
        version: ["5.7.0"]
        specs: ["https://anaconda.org/bioconda/busco", "doi.org/10.1093/molbev/msab199"]
  DockerRequirement:
...
inputs:
  ...
  # by default add a --skip_bbtools flag to avoid fail due to a bbtools bug, see https://gitlab.com/ezlab/busco/-/issues/705
  skip_bbtools:
    type: boolean
    label: Skip BBTools
...
outputs:
  ...
  batch_summary:
    label: Batch summary
    type: File? # Was optional ? It will not be generated if input is a single file
