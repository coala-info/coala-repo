...
baseCommand:
  - pdftk
label: pdftk_shuffle
doc: "Collates pages from input PDFs to create a new PDF. Works like the cat operation
  except that it takes one page at a time from each page range to assemble the output
...
inputs:
  - id: input_pdf_files
    type:
      type: array
      items: File
    doc: A list of the input PDF files or handles (e.g., A=in1.pdf B=in2.pdf).
    inputBinding:
      position: 1
  - id: operation
    type: string
    doc: The operation to perform (shuffle).
    inputBinding:
      position: 2
  - id: page_ranges
    type:
      ...
      - type: array
        items: string
    doc: Page ranges to shuffle (e.g., A1-12 B1-end).
    inputBinding:
      position: 3
  - id: allow
    type:
  ...
  - id: flatten
    type:
      - 'null'
      - boolean
    doc: Merge an input PDF's interactive form fields with the PDF's pages.
    inputBinding:
      position: 104
      prefix: flatten
  - id: input_pw
