...
Source: [`vignettes/web-only/programming-efficiency.Rmd`](https://github.com/bnprks/BPCells/tree/main/r/vignettes/web-only/programming-efficiency.Rmd)

`programming-efficiency.Rmd`

## Normalizations and PCA

* Avoid dense matrices whenever possible. Put normalizations that
  ...
  more entries with a dense matrix.
* For most operations, we recommend using lazy evaluation to avoid
  ...
  + For storage efficiency, keep any sparsity-breaking normalizations
    ...
    with `[write_matrix_dir()](../../reference/matrix_io.html)` then apply the sparsity-breaking
    normalizations
* Adding values to the rows/columns of a matrix has very little
...
* Sparse matrices can be stored in a row-major or column-major
  ...
  operations:
  + Marker features can only be computed on a matrix indexed by
    gene/feature.
  + Sparse matrix multiplication can only be performed between matrices
    with the same storage order
  + Sparse matrix multiplication performance can change dramatically
    ...
    left/right preferences are reversed.
* You can check the storage order for a matrix by printing it out in
  the R terminal
* When calling the `[t()](https://rdrr.io/pkg/BiocGenerics/man/t.html)` function, BPCells just flips a
  boolean flag for whether the matrix is row-major or column-major. This
  does not affect the underlying storage order.
* To adjust the underlying storage order, call
  `[transpose_storage_order()](../../reference/transpose_storage_order.html)`. This is a slower operation, that
  requires writing a new copy of the data to disk.

## Other tips

* When running disk-backed analysis, always try to store your working
  ...
  system to a physically attached SSD for best performance.
* Use a single call to `[matrix_stats()](../../reference/matrix_stats.html)` to calculate mean +
  variance in a single pass through the matrix when possible. See the
  function reference for details.
* For ATAC-seq data, you can calculate variable features on the tile
