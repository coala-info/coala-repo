...
with *PureCN*. These tools implement highly recommended best practices. For
...
*[PureCN](https://bioconductor.org/packages/3.22/PureCN)* is backward compatible with input generated by
...
When using `--model betabin` in `PureCN.R`, we recommend for all previous
...
install *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* with suggested dependencies:
...
To use the alternative and in many cases recommended *[PSCBS](https://CRAN.R-project.org/package%3DPSCBS)*
segmentation:
...
*[deconstructSigs](https://CRAN.R-project.org/package%3DdeconstructSigs)* package:
...
*GATK4 GenomicsDB*, follow the installations instructions from
[GenomicsDB-R](https://github.com/nalinigans/GenomicsDB-R).

The *GATK4* segmentation requires the `gatk` binary in path. Versions 4.1.7.0
...
* Generate an interval file from a BED file containing baits coordinates (not
  ...
  Section [5](#run-purecn-with-third-party-segmentation)):
...
Internally, this script uses *[rtracklayer](https://bioconductor.org/packages/3.22/rtracklayer)* to parse the
`--in-file`. Make sure that the file format matches the file extension.
See the *[rtracklayer](https://bioconductor.org/packages/3.22/rtracklayer)* documentation for problems loading the file.
...
We do not recommend padding the baits file manually unless the coverages are
very low (<30X) where the increased counts from the padded regions might decrease
sampling variance slightly. Note that we do however strongly recommend running the
variant caller with a padding of at least 50bp to increase the number of
informative SNPs, see below in the VCF section. Double check that the genome
version of the `--in-file` is correct - many assays are still designed using
older references and might need to be lifted over to the pipeline reference.
If possible, do NOT use a BED file that contains the targeted exons, instead use
the coordinates of the baits. These are optimized for GC-content and mappability and
will produce cleaner coverage profiles.
...
*[TxDb.Hsapiens.UCSC.hg19.knownGene](https://bioconductor.org/packages/3.22/TxDb.Hsapiens.UCSC.hg19.knownGene)* in R.
...
*[rtracklayer](https://bioconductor.org/packages/3.22/rtracklayer)* format is supported). This is useful when the coverages
are calculated with third-party tools like GATK.

The `--mappability` argument should provide a *[rtracklayer](https://bioconductor.org/packages/3.22/rtracklayer)*
...
* [GCA\_000001405.15\_GRCh38\_no\_alt\_analysis\_set\_76.bw](https://s3.amazonaws.com/purecn/GCA_000001405.15_GRCh38_no_alt_analysis_set_76.bw)
* [GCA\_000001405.15\_GRCh38\_no\_alt\_analysis\_set\_100.bw](https://s3.amazonaws.com/purecn/GCA_000001405.15_GRCh38_no_alt_analysis_set_100.bw)
...
*[PureCN](https://bioconductor.org/packages/3.22/PureCN)* does not ship with a variant caller. Use a third-party
tool to generate a VCF for each sample.

Important recommendations:

* Use *MuTect 1.1.7* if possible; *Mutect 2* from *GATK 4.1.7+* is now out of
  alpha and VCFs generated following the best practices somatic workflow should
  work (earlier *Mutect 2* versions are not supported and will not work).
* VCFs from most other tumor-only callers such as *VarScan2* and
  *FreeBayes* are supported, but only very limited artifact filtering will be
...
* Since germline SNPs are needed to infer allele-specific copy numbers, the
  ...
  usuable output without those flags. Since *Mutect 2* from *GATK 4.2.0+*,
  average base quality scores can be very low and variants will be too
  aggressively removed by *PureCN*. You will need to set `--min-base-quality 20` in *PureCN.R* to keep them.
* Run the variant caller with a 50-75 base pair interval padding to increase
  ...
  `--interval-padding` in *Mutect 1.1.7* and *Mutect 2*, respectively). For
  very high coverages beyond 1000X, it is safe to increase this value up to
  200bp.

# 4 Run PureCN with internal segmentation

The following describes *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* runs with internal copy number
...
* When supported third-party tools are used to calculate coverage (currently
  *CNVkit*, *GATK3* and *GATK4*), it is possible to GC-normalize those
...
* In particular, do not mix normal data obtained with different capture kits
  (e.g. *Agilent SureSelect v4* and *v6*)
...
* A warning pointing to the likely use of a wrong baits file means that more
  ...
  with the low coverage targets will be generated in `--out-dir`. If for any
...
run *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* to normalize, segment and determine purity and ploidy:
...
* The *MuTect 1.1.7* stats file (the main output file besides the VCF) should be
  ...
  provide this file for *Mutect 2*.
* The `--post-optimize` flag defines that purity should be optimized using both
...
* If `--out` is a directory, it will use the sample id as file prefix for all
  output files. Otherwise *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* will use `--out` as prefix.
* The `--parallel` flag will enable the parallel fitting of local optima. See
  *[BiocParallel](https://bioconductor.org/packages/3.22/BiocParallel)* for details. This script will use the default
  backend. `--cores` is a short cut to use the specified number of CPUs instead
...
* `--fun-segmentation PSCBS` is the new recommendation in 1.22. Support for
  interval weights currently requires a patch (see Section
  [1.2](#installation)). See below for some more details on the best choice of
  the method.
...
* Defaults are well calibrated and should produce close to ideal results for
  ...
  + High purity and high quality: For cancer types with a high expected
    ...
    (*[PureCN](https://bioconductor.org/packages/3.22/PureCN)* reports copy numbers greater than this value, but
  ...
  + Small panels with high coverage: `--interval-padding 100` (or higher), requires
    ...
    `--max-homozygous-loss` parameter might also need some adjustment for very
  ...
  + Cell lines: Safely skip the search for low purity solutions in cell lines:
    `--max-copy-number 8`, `--min-purity 0.9`, `--max-purity 0.99`. Add
    `--model-homozygous` to find regions of LOH in samples without normal
    contamination (do not provide this flag when matched normal data are
    available in the VCF).
  + cfDNA: `--min-purity 0.1`, `--min-af 0.01` (or lower) and `--error 0.0005`
  ...
  + All assays: `--max-segments` should set to a value so that with few
    ...
    `--max-non-clonal` to 0.3-0.4 (this will increase the runtime significantly
  ...
  + The choice of the segmentation function can also make a significant
    ...
    - GATK4: Most recent addition. Not yet well tested in
      *[PureCN](https://bioconductor.org/packages/3.22/PureCN)*, but theoretically best choice with a larger
      number of SNPs per intervals, for example assays with copy number
      backbones. We appreciate feedback.
    - CBS: Simple, fast and well tested. Does not fully support SNP
      information, so only recommended for settings with a very small
      SNPs/intervals ratio, for example small targeted panels (<1Mb) with
      healthy off-target coverage (<150kb resolution and similar log ratio
      noise compared to on-target).
    - copynumber: For cases with multiple time points or biopsies. This is
      automatically chosen with `--additional-tumors` and currently not
...
* A few recommendations for checks whether the *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* setup is
  correct:
  ...
  + The “Mean standard deviation of log-ratios” reported in the log file
    ...
    noisy).
  + Related to that, a warning is thrown when less than 10% of all intervals
    passing filters are off-target intervals. Whole-exome sequencing is
    usually around that value. If the log-ratio standard deviation is similar
    or even lower than the one for on-target, it is worth keeping off-target
    regions. Otherwise off-target might add more noise than signal.
    Off-target information is automatically ignored when the passing rate
    falls below 5% of all intervals.
  + The fraction of targets with SNPs should be between 10 and 15 percent.
    ...
    default 25 base quality feature, you might be using *Mutect 2* and need
    to re-run *PureCN.R* with `--min-base-quality 20`.
  + “Initial testing for significant sample cross-contamination” in the log
    ...
    most samples, not “maybe”. Insufficient artifact removal can result in
    too many false SNPs calls with low allelic fractions, confusing the
    contamination caller.
  + Read all warnings.

# 5 Run PureCN with third-party segmentation

Our internal *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* normalization combined with the *PSCBS* or
*GATK4* segmentation should produce highly competitive results and we encourage
...
therefore made it relatively easy to use *[PureCN](https://bioconductor.org/packages/3.22/PureCN)* with third-party
tools. We provide examples for *CNVkit* and *GATK4* and it should be
...
*GATK4*, *EXCAVATOR2*). For a minimal test run:
...
* The `--fun-segmentation` argument controls if the data should to be
...
* In test runs, especially when the input VCF contains matched normal
  information, `--mapping-bias-file` can be skipped
* *CNVkit* runs without normal reference samples are not recommended
* The `--stats-file` is only supported for *Mutect 1.1.7*. *Mutect 2* provides
  the filter flags directly in the VCF.

## 5.3 Recommended *GATK4* usage
...
* The `--fun-segmentation` can be set to none in most cases. This will keep the
...
* Run *GATK CallableLoci* with `--minDepth N` where N is roughly 20% of the mean
  target coverage of all samples.
* If `--callable` is missing, all intervals passing filters are assumed to be
  callable.
...
| `--parallel` |  | Use default *[BiocParallel](https://bioconductor.org/packages/3.22/BiocParallel)* backend when multiple BAMs are provided |
| `--seed` |  |  |
| `--version -v` |  |  |
| `--force -f` |  |  |
| `--help -h` |  |  |
