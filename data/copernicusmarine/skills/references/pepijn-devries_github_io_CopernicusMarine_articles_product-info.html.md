...
Source: [`vignettes/product-info.Rmd`](https://github.com/pepijn-devries/CopernicusMarine/blob/master/vignettes/product-info.Rmd)

`product-info.Rmd`

Before you start downloading data, you first need to know which data
you need and for which purpose. And not unimportant: you need to know
where to find it. Although data discovery is not the primary objective
of this package, it provides several instruments to obtain product
information.

## The Web Browser

Perhaps the easiest way to find products and layers is via the [online catalogue](https://data.marine.copernicus.eu/products).
To streamline your workflow using the web browser, you should check
`[vignette("translate")](../articles/translate.html)`. That vignette explains how you can
copy a request from the web browser catalogue and use it in R.

## Listing Products

To get a complete overview of all available products, you can list
them with either `[cms_products_list()](../reference/cms_products_list.html)` or
`[cms_products_list2()](../reference/cms_products_list.html)`:
...
The first returns a `data.frame` complete with al sorts of
meta information about the type of variables in the product, the
spatio-temporal coverage, number of vertical layers, etc. You can use
this `data.frame` to narrow your search by applying a
`[dplyr::filter()](https://dplyr.tidyverse.org/reference/filter.html)` on it.
...
The reason this is poorly documented is because this function does
not use the formal API. Instead it uses the web-form used by the online
catalogue. Users should therefore not rely on it too much as it may get
discontinued or altered at any time.

Instead, users can use `[cms_products_list2()](../reference/cms_products_list.html)` which
produces a list of products, by using the official API. Unfortunately,
this list does not contain any additional information. For this purpose
users can refer to `[cms_product_metadata()](../reference/cms_product_metadata.html)`,
`[cms_product_details()](../reference/cms_product_details.html)`, or
`[cms_product_services()](../reference/cms_product_services.html)` as described below.

## Product Details

With `[cms_product_details()](../reference/cms_product_details.html)` you will get some descriptive
information about your product. Nothing too fancy, but it will help you
...
When subsetting a dataset with `[cms_download_subset()](../reference/cms_download_subset.html)`,
the most tricky thing is discovering what the available ranges are for
its dimensions. You can use `[cms_product_metadata()](../reference/cms_product_metadata.html)` for this
purpose. It returns a named list.
...
Another way to get the dimension ranges is by setting up a stars
proxy object (see `[vignette("proxy")](../articles/proxy.html)`) and call
`[st_dimensions()](https://r-spatial.github.io/stars/reference/st_dimensions.html)` on it:
...
`[cms_product_services()](../reference/cms_product_services.html)`. It will present a
`data.frame` for your product with all services provided by
Copernicus. You can use any of the columns with the `"_href"`
suffix to get an URL of a specific service. If you want to access those
directly, you are on your own. It is easier to use any of the wrappers
provided by this package to access the data.
