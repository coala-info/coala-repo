cwlVersion: v1.2
class: CommandLineTool
baseCommand: solvebio import
label: solvebio_import
doc: "Import files into SolveBio datasets.\n\nTool homepage: https://github.com/solvebio/solvebio-python"
inputs:
  - id: full_path
    type: string
    doc: 'The full path to the dataset in the format: "domain:vault:/path/dataset".'
    inputBinding:
      position: 1
  - id: files
    type:
      type: array
      items: File
    doc: One or more files to import. Can be local files, folders, globs or 
      remote URLs. Pass --remote-source in order to list remote full_paths or 
      path globs on the SolveBio file system.
    inputBinding:
      position: 2
  - id: access_token
    type:
      - 'null'
      - string
    doc: Manually provide a SolveBio OAuth2 access token
    inputBinding:
      position: 103
      prefix: --access-token
  - id: api_host
    type:
      - 'null'
      - string
    doc: Override the default SolveBio API host
    inputBinding:
      position: 103
      prefix: --api-host
  - id: api_key
    type:
      - 'null'
      - string
    doc: Manually provide a SolveBio API key
    inputBinding:
      position: 103
      prefix: --api-key
  - id: capacity
    type:
      - 'null'
      - string
    doc: 'Specifies the capacity of the created dataset: small (default, <100M records),
      medium (<500M), large (>=500M)'
    inputBinding:
      position: 103
      prefix: --capacity
  - id: commit_mode
    type:
      - 'null'
      - string
    doc: Commit mode to use when importing data. Options are "append" (default),
      "overwrite","upsert", or "delete"
    inputBinding:
      position: 103
      prefix: --commit-mode
  - id: create_dataset
    type:
      - 'null'
      - boolean
    doc: Create the dataset if it doesn't exist
    inputBinding:
      position: 103
      prefix: --create-dataset
  - id: create_vault
    type:
      - 'null'
      - boolean
    doc: Create the vault if it doesn't exist
    inputBinding:
      position: 103
      prefix: --create-vault
  - id: dry_run
    type:
      - 'null'
      - boolean
    doc: Dry run mode will not create any datasets or import any files.
    inputBinding:
      position: 103
      prefix: --dry-run
  - id: follow
    type:
      - 'null'
      - boolean
    doc: Follow the import's progress until it completes
    inputBinding:
      position: 103
      prefix: --follow
  - id: metadata
    type:
      - 'null'
      - type: array
        items: string
    doc: Dataset metadata in the format KEY=VALUE
    inputBinding:
      position: 103
      prefix: --metadata
  - id: metadata_json_file
    type:
      - 'null'
      - File
    doc: Metadata key value pairs in JSON format
    inputBinding:
      position: 103
      prefix: --metadata-json-file
  - id: remote_source
    type:
      - 'null'
      - boolean
    doc: File paths are remote globs or full paths on the SolveBio file system.
    inputBinding:
      position: 103
      prefix: --remote-source
  - id: tag
    type:
      - 'null'
      - type: array
        items: string
    doc: 'A tag to be added. Tags are case insensitive strings. Example tags: --tag
      GRCh38 --tag Tissue --tag "Foundation Medicine"'
    inputBinding:
      position: 103
      prefix: --tag
  - id: template_file
    type:
      - 'null'
      - File
    doc: A local template file to be used when creating a new dataset (via 
      --create-dataset)
    inputBinding:
      position: 103
      prefix: --template-file
  - id: template_id
    type:
      - 'null'
      - string
    doc: The template ID used when creating a new dataset (via --create-dataset)
    inputBinding:
      position: 103
      prefix: --template-id
outputs:
  - id: stdout
    type: stdout
    doc: Standard output
hints:
  - class: DockerRequirement
    dockerPull: quay.io/biocontainers/solvebio:2.34.0--pyh7e72e81_0
stdout: solvebio_import.out
