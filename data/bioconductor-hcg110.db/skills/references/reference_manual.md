...
hcg110ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hcg110ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hcg110.db annotation Package. The purpose of this package is to provide detailed
information about the hcg110 platform. This package is updated biannually.
...
hcg110CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hcg110CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hcg110CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hcg110ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hcg110ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hcg110ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hcg110ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hcg110ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hcg110ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hcg110ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hcg110ENZYME2PROBE to a list to see inside

hcg110GENENAME

9

x <- hcg110ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hcg110GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hcg110GO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hcg110GO2PROBE)
...
hcg110GO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hcg110GO2PROBE.

Details

If hcg110GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hcg110GO2ALLPROBES or hcg110GO2PROBE is cast as a list, each GO term maps to a named
...
x <- hcg110GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hcg110GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hcg110MAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hcg110.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hcg110MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hcg110.db.
...
hcg110OMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hcg110

Description

hcg110ORGANISM is an R object that contains a single item: a character string that names the
organism for which hcg110 was built. hcg110ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hcg110ORGANISM
provides a simple way to programmatically extract the organism name. hcg110ORGPKG provides
...
hcg110PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hcg110PATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- hcg110PATH
mapped_probes <- mappedkeys(x)
# Now convert the hcg110PATH2PROBE object to a list
...
hcg110PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hcg110PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hcg110PMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hcg110PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hcg110PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- hcg110PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hcg110PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hcg110PROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hcg110REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hcg110SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hcg110UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hcg110_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hcg110_dbconn or you will break all the
AnnDbObj objects defined in this package!

hcg110_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hcg110_dbschema prints the schema definition of the package annotation DB.

hcg110_dbInfo prints other information about the package annotation DB.

Value

hcg110_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

hcg110_dbfile: a character string with the path to the package annotation DB.

hcg110_dbschema: none (invisible NULL).

hcg110_dbInfo: none (invisible NULL).
...
hcg110_dbInfo (hcg110_dbconn), 21
hcg110_dbschema (hcg110_dbconn), 21
hcg110ACCNUM, 1
hcg110ALIAS2PROBE, 2
hcg110CHR, 3
hcg110CHRLENGTHS, 4
hcg110CHRLOC, 4
hcg110CHRLOCEND (hcg110CHRLOC), 4
hcg110ENSEMBL, 5
hcg110ENSEMBL2PROBE (hcg110ENSEMBL), 5
hcg110ENTREZID, 6
hcg110ENZYME, 7
hcg110ENZYME2PROBE (hcg110ENZYME), 7
hcg110GENENAME, 9
hcg110GO, 10
hcg110GO2ALLPROBES, 11
hcg110GO2ALLPROBES (hcg110GO), 10
hcg110GO2PROBE (hcg110GO), 10
hcg110LOCUSID (hcg110ENTREZID), 6
hcg110MAP, 12
hcg110MAPCOUNTS, 13
hcg110OMIM, 13
hcg110ORGANISM, 14
hcg110ORGPKG (hcg110ORGANISM), 14
hcg110PATH, 15
hcg110PATH2PROBE (hcg110PATH), 15
hcg110PFAM, 16
hcg110PMID, 16
hcg110PMID2PROBE (hcg110PMID), 16
hcg110PROSITE, 17
hcg110REFSEQ, 18
hcg110SYMBOL, 19
hcg110UNIPROT, 20
hcg110UNIPROT2PROBE (hcg110UNIPROT), 20
...
hcg110.db, 2
hcg110_dbconn, 21
hcg110_dbfile (hcg110_dbconn), 21
