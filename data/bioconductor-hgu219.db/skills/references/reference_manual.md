...
hgu219ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu219ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgu219.db annotation Package. The purpose of this package is to provide detailed
information about the hgu219 platform. This package is updated biannually.
...
hgu219CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgu219CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu219CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu219ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgu219ENSEMBL2PROBE)
if(length(xx) > 0){
...
hgu219ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu219ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu219ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu219ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu219ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu219ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu219ENZYME2PROBE to a list to see inside
xx <- as.list(hgu219ENZYME2PROBE)
if(length(xx) > 0){
...
hgu219GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu219GO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hgu219GO2PROBE)
...
hgu219GO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu219GO2PROBE.

Details

If hgu219GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu219GO2ALLPROBES or hgu219GO2PROBE is cast as a list, each GO term maps to a named
...
# Convert hgu219GO2ALLPROBES to a list
xx <- as.list(hgu219GO2ALLPROBES)
if(length(xx) > 0){
...
hgu219MAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu219.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu219MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu219.db.
...
hgu219OMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu219

Description

hgu219ORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu219 was built. hgu219ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu219ORGANISM
provides a simple way to programmatically extract the organism name. hgu219ORGPKG provides
...
hgu219PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu219PATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the hgu219PATH2PROBE object to a list
xx <- as.list(hgu219PATH2PROBE)
if(length(xx) > 0){
...
hgu219PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu219PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu219PMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu219PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgu219PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
# Now convert the reverse map object hgu219PMID2PROBE to a list
xx <- as.list(hgu219PMID2PROBE)
if(length(xx) > 0){
...
hgu219PROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hgu219REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu219SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu219UNIGENE is an R object that provides mappings between manufacturer identifiers and Uni-
Gene identifiers.
...
hgu219UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu219_dbconn()
hgu219_dbfile()
hgu219_dbschema(file="", show.indices=FALSE)
hgu219_dbInfo()
...
hgu219_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu219_dbconn or you will break all the
AnnDbObj objects defined in this package!
hgu219_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
hgu219_dbschema prints the schema definition of the package annotation DB.
hgu219_dbInfo prints other information about the package annotation DB.

Value

hgu219_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.
hgu219_dbfile: a character string with the path to the package annotation DB.
hgu219_dbschema: none (invisible NULL).
hgu219_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu219_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu219_dbfile (hgu219_dbconn), 21
hgu219_dbInfo (hgu219_dbconn), 21
hgu219_dbschema (hgu219_dbconn), 21
hgu219ACCNUM, 1
hgu219ALIAS2PROBE, 2
hgu219CHR, 3
hgu219CHRLENGTHS, 4
hgu219CHRLOC, 4
hgu219CHRLOCEND (hgu219CHRLOC), 4
hgu219ENSEMBL, 5
hgu219ENSEMBL2PROBE (hgu219ENSEMBL), 5
hgu219ENTREZID, 6
hgu219ENZYME, 7
hgu219ENZYME2PROBE (hgu219ENZYME), 7
hgu219GENENAME, 9
hgu219GO, 10
hgu219GO2ALLPROBES, 11
hgu219GO2ALLPROBES (hgu219GO), 10
hgu219GO2PROBE (hgu219GO), 10
hgu219LOCUSID (hgu219ENTREZID), 6
hgu219MAP, 12
hgu219MAPCOUNTS, 13
hgu219OMIM, 13
hgu219ORGANISM, 14
hgu219ORGPKG (hgu219ORGANISM), 14
hgu219PATH, 15
hgu219PATH2PROBE (hgu219PATH), 15
hgu219PFAM, 16
hgu219PMID, 16
hgu219PMID2PROBE (hgu219PMID), 16
hgu219PROSITE, 17
hgu219REFSEQ, 18
hgu219SYMBOL, 19
hgu219UNIGENE, 20
hgu219UNIPROT, 21
hgu219UNIPROT2PROBE (hgu219UNIPROT), 21
