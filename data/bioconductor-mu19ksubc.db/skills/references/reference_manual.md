...
mu19ksubcACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mu19ksubcALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mu19ksubcALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mu19ksubcALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mu19ksubc.db annotation Package. The purpose of this package is to provide
detailed information about the mu19ksubc platform. This package is updated biannually.
...
mu19ksubcCHR is an R object that provides mappings between a manufacturer identifier and the
...
mu19ksubcCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mu19ksubcCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mu19ksubcENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mu19ksubcENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mu19ksubcENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mu19ksubcENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mu19ksubcENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mu19ksubcENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mu19ksubcENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mu19ksubcENZYME2PROBE to a list to see inside

mu19ksubcGENENAME

9

x <- mu19ksubcENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mu19ksubcGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mu19ksubcGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mu19ksubcGO2PROBE)
...
mu19ksubcGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If mu19ksubcGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mu19ksubcGO2ALLPROBES or mu19ksubcGO2PROBE is cast as a list, each GO term maps to
...
x <- mu19ksubcGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mu19ksubcGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package mu19ksubc.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mu19ksubcMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mu19ksubc.db.
...
mu19ksubcMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
The Organism information for mu19ksubc

Description

mu19ksubcORGANISM is an R object that contains a single item: a character string that names
the organism for which mu19ksubc was built. mu19ksubcORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mu19ksubcORGANISM
provides a simple way to programmatically extract the organism name. mu19ksubcORGPKG pro-
...
mu19ksubcPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mu19ksubcPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- mu19ksubcPATH
mapped_probes <- mappedkeys(x)
# Now convert the mu19ksubcPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mu19ksubcPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mu19ksubcPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mu19ksubcPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mu19ksubcPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mu19ksubcPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- mu19ksubcPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mu19ksubcPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mu19ksubcPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mu19ksubcREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mu19ksubcSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mu19ksubcUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mu19ksubc_dbconn()
mu19ksubc_dbfile()
mu19ksubc_dbschema(file="", show.indices=FALSE)
mu19ksubc_dbInfo()
...
mu19ksubc_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mu19ksubc_dbconn or you will break all
the AnnDbObj objects defined in this package!

mu19ksubc_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

mu19ksubc_dbschema prints the schema definition of the package annotation DB.

mu19ksubc_dbInfo prints other information about the package annotation DB.

Value

mu19ksubc_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

mu19ksubc_dbfile: a character string with the path to the package annotation DB.

mu19ksubc_dbschema: none (invisible NULL).

mu19ksubc_dbInfo: none (invisible NULL).
...
dbGetQuery(mu19ksubc_dbconn(), "SELECT COUNT(*) FROM probes")
...
mu19ksubc_dbconn, 20
mu19ksubcACCNUM, 1
mu19ksubcALIAS2PROBE, 2
mu19ksubcCHR, 3
mu19ksubcCHRLENGTHS, 4
mu19ksubcCHRLOC, 4
mu19ksubcENSEMBL, 5
mu19ksubcENTREZID, 6
mu19ksubcENZYME, 7
mu19ksubcGENENAME, 9
mu19ksubcGO, 10
mu19ksubcMAPCOUNTS, 12
mu19ksubcMGI, 12
mu19ksubcORGANISM, 13
mu19ksubcPATH, 14
mu19ksubcPFAM, 15
mu19ksubcPMID, 15
mu19ksubcPROSITE, 17
...
mu19ksubc (mu19ksubc.db), 2
mu19ksubc.db, 2
mu19ksubc_dbconn, 20
mu19ksubc_dbfile (mu19ksubc_dbconn), 20
mu19ksubc_dbInfo (mu19ksubc_dbconn), 20

22

mu19ksubc_dbschema (mu19ksubc_dbconn),
...
mu19ksubcCHRLENGTHS, 4
mu19ksubcCHRLOC, 4
mu19ksubcCHRLOCEND (mu19ksubcCHRLOC), 4
mu19ksubcENSEMBL, 5
mu19ksubcENSEMBL2PROBE

(mu19ksubcENSEMBL), 5

mu19ksubcENTREZID, 6
mu19ksubcENZYME, 7
mu19ksubcENZYME2PROBE

(mu19ksubcENZYME), 7

mu19ksubcGENENAME, 9
mu19ksubcGO, 10
mu19ksubcGO2ALLPROBES, 11
mu19ksubcGO2ALLPROBES (mu19ksubcGO), 10
mu19ksubcGO2PROBE (mu19ksubcGO), 10
mu19ksubcLOCUSID (mu19ksubcENTREZID), 6
mu19ksubcMAPCOUNTS, 12
mu19ksubcMGI, 12
mu19ksubcMGI2PROBE (mu19ksubcMGI), 12
mu19ksubcORGANISM, 13
mu19ksubcORGPKG (mu19ksubcORGANISM), 13
mu19ksubcPATH, 14
mu19ksubcPATH2PROBE (mu19ksubcPATH), 14
mu19ksubcPFAM, 15
mu19ksubcPMID, 15
mu19ksubcPMID2PROBE (mu19ksubcPMID), 15
mu19ksubcPROSITE, 17
mu19ksubcREFSEQ, 17
mu19ksubcSYMBOL, 18
mu19ksubcUNIPROT, 19
mu19ksubcUNIPROT2PROBE
