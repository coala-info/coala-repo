...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2FCOMBINE-lab%2Fpiscem%2Fblob%2Fmain%2Fbuild.rs)
...
[History](/COMBINE-lab/piscem/commits/main/build.rs)
...
[Raw](https://github.com/COMBINE-lab/piscem/raw/refs/heads/main/build.rs)
...
std::fs::read\_to\_string("Cargo.lock").expect("Failed to read Cargo.lock");

let lock: toml::Value = lock\_contents.parse().expect("Failed to parse Cargo.lock");
...
.find(|p| p["name"].as\_str() == Some("cf1-rs"))

.and\_then(|p| p["version"].as\_str())

.unwrap\_or("unknown");

println!("cargo:rustc-env=CF1\_RS\_VERSION={version}");
