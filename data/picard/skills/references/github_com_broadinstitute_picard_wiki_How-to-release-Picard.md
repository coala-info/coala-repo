...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fbroadinstitute%2Fpicard%2Fwiki%2FHow-to-release-Picard)
...
Louis Bergelson edited this page Oct 9, 2024
·
[33 revisions](/broadinstitute/picard/wiki/How-to-release-Picard/_history)
...
* Set up your sonatype account
  ...
  + If you don't already have a sonatype account, create one by going to [https://issues.sonatype.org/secure/Signup!default.jspa](https://issues.sonatype.org/secure/Signup%21default.jspa).
...
* Decide on a version number for the new Picard release (for example, `2.6.1`). Typically we follow a very loose interpretation of semantic versioning, bumping the third number for bug fix releases (eg., `2.6.0` -> `2.6.1`), the second number for releases with significant new features but no major breaking changes (eg., `2.6.1` -> `2.7.0`), and the first number when there are major breaking changes (eg., `2.6.0` -> `3.0.0`).
  ...
  + A good way to examine the changes that have occurred since the last release is to go to <https://github.com/broadinstitute/picard/releases> and click on the `X commits to master since this release` link for the most recent release.
* Tag the release by running `git tag -sa VERSION_NUMBER`, where `VERSION_NUMBER` is the version you've chosen for the new release (eg., `2.6.1`). Your text editor will pop up and ask you to write a commit message for the new tag. (This can just be the release number for now). After doing so, save and then exit. `git describe` should now print the new version.
