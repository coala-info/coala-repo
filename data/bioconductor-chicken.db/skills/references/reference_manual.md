...
chickenACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
chickenALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the chicken.db annotation Package. The purpose of this package is to provide detailed
information about the chicken platform. This package is updated biannually.
...
chickenCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
chickenCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
chickenCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
Mappings were based on data provided by: UCSC Genome Bioinformatics (Gallus gallus)
...
chickenENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
chickenENTREZID is an R object that provides mappings between manufacturer identifiers and
...
chickenENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. chickenENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the chickenENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the chickenENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In chickenENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
chickenGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
chickenGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (chick-
enGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only the
directly evidenced terms are represented here.

chickenGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than chick-
enGO2PROBE.

Details

If chickenGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If chickenGO2ALLPROBES or chickenGO2PROBE is cast as a list, each GO term maps to a named
...
Number of mapped keys for the maps in package chicken.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

chickenMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package chicken.db.
...
The Organism information for chicken

Description

chickenORGANISM is an R object that contains a single item: a character string that names the
organism for which chicken was built. chickenORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, chickenORGAN-
ISM provides a simple way to programmatically extract the organism name. chickenORGPKG
...
chickenPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

chickenPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
chickenPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
chickenPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. chickenPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When chickenPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When chickenPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
chickenPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
chickenREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
chickenSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
chickenUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
chicken_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by chicken_dbconn or you will break all the
AnnDbObj objects defined in this package!

chicken_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
...
chicken_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
...
chickenENZYME, 7
chickenENZYME2PROBE (chickenENZYME), 7
chickenGENENAME, 9
chickenGO, 10
chickenGO2ALLPROBES, 11
chickenGO2ALLPROBES (chickenGO), 10
chickenGO2PROBE (chickenGO), 10
chickenLOCUSID (chickenENTREZID), 6
chickenMAPCOUNTS, 12
chickenORGANISM, 12
chickenORGPKG (chickenORGANISM), 12
chickenPATH, 13
chickenPATH2PROBE (chickenPATH), 13
chickenPFAM, 14
chickenPMID, 14
