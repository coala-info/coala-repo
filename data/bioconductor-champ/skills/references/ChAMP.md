...
* [3 Test Data](#section-test-data)
* [4 ChAMP Pipeline](#section-champ-pipeline)
  + [4.1 Pipeline Introduction](#section-pipeline-introduction)
  + [4.2 Full Pipeline](#section-full-pipeline)
  + [4.3 Separated Steps](#section-separated-steps)
  + [4.4 EPIC pipeline](#section-epic-pipeline)
  + [4.5 Computational Requirements](#section-computational-requirements)
* [5 Description of ChAMP Pipelines](#section-description-of-champ-pipelines)
  + [5.1 Loading Data](#section-loading-data)
  + [5.2 Filtering Data](#section-filtering-data)
  + [5.3 Further quality control and exploratory analysis](#section-further-quality-control-and-exploratory-analysis)
  + [5.4 Normalization](#section-normalization)
  + [5.5 SVD Plot](#section-svd-plot)
  + [5.6 Batch Effect Correction](#section-batch-effect-correction)
  + [5.7 Differential Methylation Probes](#section-differential-methylation-probes)
    - [5.7.1 DMP.GUI performance on numeric variable](#section-dmp.gui-performance-on-numeric-variable)
    - [5.7.2 Hydroxymethylation Analysis](#section-hydroxymethylation-analysis)
  + [5.8 Differential Methylation Regions](#section-differential-methylation-regions)
  + [5.9 Differential Methylation Blocks](#section-differential-methylation-blocks)
  + [5.10 Gene Set Enrichment Analysis](#section-gene-set-enrichment-analysis)
  + [5.11 Copy Number Variation](#section-copy-number-variation)
  + [5.12 Cell Type Heterogeneity](#section-cell-type-heterogeneity)
* [6 Summary](#section-summary)
* [7 Citing ChAMP](#section-citing-champ)
...
We note that champ.process() may not always run successfully, which is likely due to particularities of specific data sets . For example, the covariate of interest in the pd file may not be denoted “Sample\_Group” but another category, for example, “Disease”, but in the ChAMP function, “Sample\_Group” is set as default as the variable of interest. If you encounter any problems when using champ.process(), we recommend you apply it in a step-by-step fashion:
...
It is important to check the pd file (Sample\_Sheet.csv). Most pd files are similar in format, but their most valuable information are not always stored in same column, for example, for most pd files, Sample\_Group means groups of phenotype wants to be researched, but in some pd files, similar information may be stored in columns named as “Diagnose” or “CancerType”, so the user **MUST** understand their pd file well to achieve a successful analysis.
...
As we can see here, the Sample\_Group contains two phenotypes: C and T, which is what we want to compare. In this data set, C means “Control”, while T means “Tumor”.
...
* Since low quality samples (e.g. those with many undetected probes) may be removed during the process of filtering, it is important to ensure that the Sample\_Name identifiers in your pd file are exactly the same as colnames of your data matrix
...
* **Fifth, by default setting, ChAMP will filter all multi-hit probes.** The multi-hit probe list comes from [Nordlund’s Genome Biology Paper in 2013](https://genomebiology.biomedcentral.com/articles/10.1186/gb-2013-14-9-r105)22.
...
* Covariates are not associated with name, Sample\_Name or File\_Name
...
4. Fortin JP, Labbe A, Lemire M, et al. Functional normalization of 450k methylation array data improves replication in large cancer studies. *Genome Biology*. 2014;15(11):1-17.
...
6. Fortin JP, Triche T, Hansen K. Preprocessing, normalization and integration of the illumina humanmethylationepic array. *bioRxiv*. Published online 2016. doi:[10.1101/065490](https://doi.org/10.1101/065490)
...
18. Davis S, Du P, Bilke S, Triche T, Jr., Bootwalla M. Methylumi: Handle illumina methylation data. Published online 2013.

19. Assenov Y, Muller F, Lutsik P. RnBeads - Comprehensive DNA Methylation Analysis. Published online 2012.

20. Wong C, Pidsley R, Schalkwyk L. The wateRmelon Package. Published online 2012.

21. Zhou W, Laird PW, Shen H. Comprehensive characterization, annotation and innovative use of infinium dna methylation beadchip probes. *Nucleic Acids Research*. Published online 2016. doi:[10.1093/nar/gkw967](https://doi.org/10.1093/nar/gkw967)

22. Nordlund J, Bäcklin CL, Wahlberg P, et al. Genome-wide signatures of differential dna methylation in pediatric acute lymphoblastic leukemia. *Genome Biology*. 2013;14(9):r105. doi:[10.1186/gb-2013-14-9-r105](https://doi.org/10.1186/gb-2013-14-9-r105)
