...
htmg430bACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
htmg430bALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(htmg430bALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the htmg430b.db annotation Package. The purpose of this package is to provide detailed
information about the htmg430b platform. This package is updated biannually.
...
htmg430bCHR is an R object that provides mappings between a manufacturer identifier and the
...
htmg430bCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
htmg430bCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
htmg430bENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
htmg430bENTREZID is an R object that provides mappings between manufacturer identifiers and
...
htmg430bENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. htmg430bENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the htmg430bENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the htmg430bENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In htmg430bENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert htmg430bENZYME2PROBE to a list to see inside

htmg430bGENENAME

9

x <- htmg430bENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
htmg430bGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
htmg430bGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (htmg430bGO2PROBE)
...
htmg430bGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If htmg430bGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If htmg430bGO2ALLPROBES or htmg430bGO2PROBE is cast as a list, each GO term maps to a
...
x <- htmg430bGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert htmg430bGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package htmg430b.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

htmg430bMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package htmg430b.db.
...
htmg430bMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for htmg430b

Description

htmg430bORGANISM is an R object that contains a single item: a character string that names
the organism for which htmg430b was built. htmg430bORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, htmg430bORGANISM
provides a simple way to programmatically extract the organism name. htmg430bORGPKG pro-
...
htmg430bPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

htmg430bPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- htmg430bPATH
mapped_probes <- mappedkeys(x)
# Now convert the htmg430bPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htmg430bPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
htmg430bPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. htmg430bPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When htmg430bPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When htmg430bPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- htmg430bPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object htmg430bPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
htmg430bPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
htmg430bREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
htmg430bSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
htmg430bUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
htmg430b_dbconn()
htmg430b_dbfile()
htmg430b_dbschema(file="", show.indices=FALSE)
htmg430b_dbInfo()
...
htmg430b_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by htmg430b_dbconn or you will break all
the AnnDbObj objects defined in this package!

htmg430b_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

htmg430b_dbschema prints the schema definition of the package annotation DB.

htmg430b_dbInfo prints other information about the package annotation DB.

Value

htmg430b_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

htmg430b_dbfile: a character string with the path to the package annotation DB.

htmg430b_dbschema: none (invisible NULL).

htmg430b_dbInfo: none (invisible NULL).
...
dbGetQuery(htmg430b_dbconn(), "SELECT COUNT(*) FROM probes")
...
htmg430b_dbconn, 20
htmg430bACCNUM, 1
htmg430bALIAS2PROBE, 2
htmg430bCHR, 3
htmg430bCHRLENGTHS, 4
htmg430bCHRLOC, 4
htmg430bENSEMBL, 5
htmg430bENTREZID, 6
htmg430bENZYME, 7
...
htmg430b (htmg430b.db), 2
htmg430b.db, 2
htmg430b_dbconn, 20
htmg430b_dbfile (htmg430b_dbconn), 20
htmg430b_dbInfo (htmg430b_dbconn), 20

22

htmg430b_dbschema (htmg430b_dbconn), 20
htmg430bACCNUM, 1
htmg430bALIAS2PROBE, 2
htmg430bCHR, 3
htmg430bCHRLENGTHS, 4
htmg430bCHRLOC, 4
htmg430bCHRLOCEND (htmg430bCHRLOC), 4
htmg430bENSEMBL, 5
htmg430bENSEMBL2PROBE

(htmg430bENSEMBL), 5

htmg430bENTREZID, 6
htmg430bENZYME, 7
htmg430bENZYME2PROBE (htmg430bENZYME), 7
htmg430bGENENAME, 9
htmg430bGO, 10
htmg430bGO2ALLPROBES, 11
htmg430bGO2ALLPROBES (htmg430bGO), 10
htmg430bGO2PROBE (htmg430bGO), 10
htmg430bLOCUSID (htmg430bENTREZID), 6
htmg430bMAPCOUNTS, 12
htmg430bMGI, 12
htmg430bMGI2PROBE (htmg430bMGI), 12
htmg430bORGANISM, 13
htmg430bORGPKG (htmg430bORGANISM), 13
htmg430bPATH, 14
htmg430bPATH2PROBE (htmg430bPATH), 14
htmg430bPFAM, 15
htmg430bPMID, 15
htmg430bPMID2PROBE (htmg430bPMID), 15
htmg430bPROSITE, 17
htmg430bREFSEQ, 17
htmg430bSYMBOL, 18
htmg430bUNIPROT, 19
htmg430bUNIPROT2PROBE
