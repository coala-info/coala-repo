...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Febi-gene-expression-group%2Fbioconductor-sc3-scripts%2Fblob%2Fdevelop%2Fsc3-sc3-calc-consens.R)
...
[History](/ebi-gene-expression-group/bioconductor-sc3-scripts/commits/develop/sc3-sc3-calc-consens.R)
...
[Raw](https://github.com/ebi-gene-expression-group/bioconductor-sc3-scripts/raw/refs/heads/develop/sc3-sc3-calc-consens.R)
...
opt <- wsc\_parse\_args(option\_list, mandatory = c('input\_object\_file', 'output\_object\_file', 'output\_text\_file'))

# Check parameter values defined

if ( ! file.exists(opt$input\_object\_file)){

stop((paste('SC3 file object', opt$input\_object\_file, 'does not exist.')))
...
SingleCellExperiment <- sc3\_calc\_consens(object = SingleCellExperiment)
...
for (cluster\_col in colnames(clusters)){

k <- sub('.\*\_(\\d+)\_.\*', '\\1', cluster\_col)

# Write a summary for this K

cluster\_table <- data.frame(table(clusters[[cluster\_col]]))

colnames(cluster\_table) <- c('Cluster', 'No. cells')

rownames(cluster\_table) <- cluster\_table$Cluster
...
capture.output(cluster\_table[, 2, drop = FALSE]),

paste(length(which(is.na(clusters[[cluster\_col]]))), 'cells fall outside clusters'),
...
write.table(clusters, file = opt$output\_text\_file, sep = "\t", row.names = FALSE, quote = FALSE, na='')
