# Code example from 'variancePartition' vignette. See references/ for full tutorial.
...
## ----vpa1, echo=TRUE, cache=TRUE, results='hide'--------------------------------------------------
form <- ~ (1 | Tissue) + (1 | Individual) + (1 | Batch) + Age
varPart <- fitExtractVarPartModel(geneExpr, form, info)

## ----vpa2, echo=TRUE, cache=TRUE, results='hide'--------------------------------------------------
library("limma")
...
## ----vpa3, echo=TRUE, cache=TRUE, results='hide'--------------------------------------------------
...
## ----vpa4, echo=TRUE, cache=TRUE, results='hide'--------------------------------------------------
...
## ----limma, echo=TRUE, cache=TRUE, results='hide'-------------------------------------------------
library("limma")
library("edgeR")
...
## ----DESeq2, echo=TRUE, cache=TRUE, results='hide'------------------------------------------------
library("DESeq2")
...
## ----tximport, echo=TRUE, cache=TRUE, results='hide', eval=FALSE----------------------------------
...
## ----ballgown, echo=TRUE, cache=TRUE, results='hide'----------------------------------------------
library("ballgown")
...
## ----sim, echo=FALSE, cache=TRUE, results='hide'--------------------------------------------------
library("variancePartition")
...
## ----siteDominant, echo=FALSE, fig.height=8, fig.width=8, fig.show="hold", warnings=TRUE----------
set.seed(1)
...
## ----cowplot, echo=FALSE, fig.width=8-------------------------------------------------------------
...
## ----IndivDominant, echo=FALSE, fig.height=8, fig.width=8-----------------------------------------
set.seed(1)
...
## ----cowplot2, echo=FALSE, fig.width=8------------------------------------------------------------
fig1 = plotVar(data$geneExpr, data$info)
fig2 = plotVarCross(data$geneExpr, data$info, label.angle = 60)

cowplot::plot_grid(fig1 + ggtitle("Variance partitioning"), fig2 + ggtitle("variancePartition - within Tissue"), axis="tblr", align="hv")

## ----stop, echo=FALSE, cache=FALSE----------------------------------------------------------------
...
## ----DE, echo=FALSE, cache=TRUE, fig.height=5, fig.width=8, warnings=FALSE------------------------
set.seed(1)
