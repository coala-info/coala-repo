...
mgu74bACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
mgu74bALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the mgu74b.db annotation Package. The purpose of this package is to provide detailed
information about the mgu74b platform. This package is updated biannually.
...
mgu74bCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
mgu74bCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
mgu74bCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
mgu74bENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
mgu74bENTREZID is an R object that provides mappings between manufacturer identifiers and
...
mgu74bENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. mgu74bENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the mgu74bENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the mgu74bENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In mgu74bENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mgu74bENZYME2PROBE to a list to see inside

mgu74bGENENAME

9

x <- mgu74bENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
mgu74bGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
mgu74bGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mgu74bGO2PROBE)
...
mgu74bGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than mgu74bGO2PROBE.

Details

If mgu74bGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If mgu74bGO2ALLPROBES or mgu74bGO2PROBE is cast as a list, each GO term maps to a
...
x <- mgu74bGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert mgu74bGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mgu74b.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mgu74bMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package mgu74b.db.
...
mgu74bMGI is an R object that contains mappings between manufacturer identifiers and Jackson
...
The Organism information for mgu74b

Description

mgu74bORGANISM is an R object that contains a single item: a character string that names the
organism for which mgu74b was built. mgu74bORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, mgu74bORGANISM
provides a simple way to programmatically extract the organism name. mgu74bORGPKG provides
...
mgu74bPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

mgu74bPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- mgu74bPATH
mapped_probes <- mappedkeys(x)
# Now convert the mgu74bPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74bPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
mgu74bPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. mgu74bPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When mgu74bPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When mgu74bPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
x <- mgu74bPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object mgu74bPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
mgu74bPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mgu74bREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
mgu74bSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
mgu74bUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
mgu74b_dbconn()
mgu74b_dbfile()
mgu74b_dbschema(file="", show.indices=FALSE)
mgu74b_dbInfo()
...
mgu74b_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by mgu74b_dbconn or you will break all the
AnnDbObj objects defined in this package!

mgu74b_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

mgu74b_dbschema prints the schema definition of the package annotation DB.

mgu74b_dbInfo prints other information about the package annotation DB.

Value

mgu74b_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

mgu74b_dbfile: a character string with the path to the package annotation DB.

mgu74b_dbschema: none (invisible NULL).

mgu74b_dbInfo: none (invisible NULL).
...
dbGetQuery(mgu74b_dbconn(), "SELECT COUNT(*) FROM probes")
...
mgu74b_dbschema (mgu74b_dbconn), 20
mgu74bACCNUM, 1
mgu74bALIAS2PROBE, 2
mgu74bCHR, 3
mgu74bCHRLENGTHS, 4
mgu74bCHRLOC, 4
mgu74bCHRLOCEND (mgu74bCHRLOC), 4
mgu74bENSEMBL, 5
mgu74bENSEMBL2PROBE (mgu74bENSEMBL), 5
mgu74bENTREZID, 6
mgu74bENZYME, 7
mgu74bENZYME2PROBE (mgu74bENZYME), 7
mgu74bGENENAME, 9
mgu74bGO, 10
mgu74bGO2ALLPROBES, 11
mgu74bGO2ALLPROBES (mgu74bGO), 10
mgu74bGO2PROBE (mgu74bGO), 10
mgu74bLOCUSID (mgu74bENTREZID), 6
mgu74bMAPCOUNTS, 12
mgu74bMGI, 12
mgu74bMGI2PROBE (mgu74bMGI), 12
mgu74bORGANISM, 13
mgu74bORGPKG (mgu74bORGANISM), 13
mgu74bPATH, 14
mgu74bPATH2PROBE (mgu74bPATH), 14
mgu74bPFAM, 15
mgu74bPMID, 15
mgu74bPMID2PROBE (mgu74bPMID), 15
mgu74bPROSITE, 17
mgu74bREFSEQ, 17
mgu74bSYMBOL, 18
mgu74bUNIPROT, 19
mgu74bUNIPROT2PROBE (mgu74bUNIPROT), 19
...
mgu74b.db, 2
mgu74b_dbconn, 20
mgu74b_dbfile (mgu74b_dbconn), 20
mgu74b_dbInfo (mgu74b_dbconn), 20
