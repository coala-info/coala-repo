...
* Classification (ganon classify)
  + [Profiling](#profiling)
  + [Binning](#binning)
  + [Reads with multiple matches](#reads-with-multiple-matches)
  + [Classifying more reads](#classifying-more-reads)
  + [Multiple and Hierarchical classification](#multiple-and-hierarchical-classification)
  + [Parameter details](#parameter-details)
    - [input reads/sequences](#input-readssequences)
    - [batch classification](#batch-classification)
    - [cutoff and filter (--rel-cutoff, --rel-filter)](#cutoff-and-filter-rel-cutoff-rel-filter)
    - [False positive of a query (--fpr-query)](#false-positive-of-a-query-fpr-query)
...
### Binning[:)](#binning "Permanent link")
...
## Reads with multiple matches[:)](#reads-with-multiple-matches "Permanent link")
...
## Classifying more reads[:)](#classifying-more-reads "Permanent link")
...
## Multiple and Hierarchical classification[:)](#multiple-and-hierarchical-classification "Permanent link")
...
In this example, classification will be performed with different `--rel-cutoff` for each database. For each hierarchy levels (`1\_first` and `2\_second`) a different `--rel-filter` will be used:
...
## Parameter details[:)](#parameter-details "Permanent link")

### input reads/sequences[:)](#input-readssequences "Permanent link")
...
### batch classification[:)](#batch-classification "Permanent link")
...
### cutoff and filter (--rel-cutoff, --rel-filter)[:)](#cutoff-and-filter-rel-cutoff-rel-filter "Permanent link")
...
### False positive of a query (--fpr-query)[:)](#false-positive-of-a-query-fpr-query "Permanent link")
...
[Previous](../custom_databases/ "Custom databases (ganon build-custom)")
[Next](../reports/ "Reports (ganon report)")
