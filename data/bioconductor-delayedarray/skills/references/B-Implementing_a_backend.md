...
* [2 Implementing the seed class](#implementing-the-seed-class)
  + [2.1 Class definition](#class-definition)
  + [2.2 Constructor](#constructor)
  + [2.3 The seed contract](#the-seed-contract)
    - [2.3.1 dim() and dimnames()](#dim-and-dimnames)
    - [2.3.2 extract\_array()](#extract_array)
  + [2.4 What to import?](#what-to-import)
  + [2.5 Testing](#testing)
* [3 Implementing high-level classes ADSArray and ADSMatrix](#implementing-high-level-classes-adsarray-and-adsmatrix)
  + [3.1 ADSArray class definition](#adsarray-class-definition)
  + [3.2 The ADSArray() constructor](#the-adsarray-constructor)
  + [3.3 ADSMatrix class definition](#adsmatrix-class-definition)
  + [3.4 Going from ADSArray to ADSMatrix](#going-from-adsarray-to-adsmatrix)
  + [3.5 Going from ADSMatrix to ADSArray](#going-from-adsmatrix-to-adsarray)
  + [3.6 Implementing optimized backend-specific methods](#implementing-optimized-backend-specific-methods)
  + [3.7 What to export?](#what-to-export)
* [4 Testing](#testing-1)

# 1 Introduction

The DelayedArray framework currently supports a small number of on-disk
backends: HDF5 (via the *HDF5Array* package), GDS (via the *GDSArray* package),
and VCF (via the *VCFArray* package). This can be extended to support other
...
[*HDF5Array*](https://github.com/Bioconductor/HDF5Array) package
...
Note that `file_path_as_absolute()` is defined in the *tools* package
so it needs to be imported by adding the following to the NAMESPACE file
of the *ADSArray* package:

```
importFrom(tools, file_path_as_absolute)
```

and adding *tools* to the `Imports` field of the DESCRIPTION file of the
package.
...
* Each element in the returned list is either `NULL` or a character vector
  ...
  or integer vectors are not allowed and *will* break downstream code.

### 2.3.2 extract\_array()

`extract_array()` is a generic function defined in the *DelayedArray*
package:
...
return its 2nd column as an M x 1 *ordinary matrix* of type `complex`.
...
Make sure the NAMESPACE file of the *ADSArray* package contains at least
...
things from the *methods*, *BiocGenerics*, *S4Vectors*, *IRanges*, and
*DelayedArray* packages. This will only complicate maintenance of the
*ADSArray* package in the long run and has no real benefits (contrary
to popular belief).

Add *methods*, *BiocGenerics*, and *DelayedArray* to the `Depends` field
of the DESCRIPTION file of the package, and *tools*, *S4Vectors*, and
*IRanges* to its `Imports` field.
...
## 3.6 Implementing optimized backend-specific methods
