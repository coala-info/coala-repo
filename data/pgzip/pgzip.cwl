...
inputs:
  - id: input_file
    type:
      ...
      - type: array
        items: File
    doc: Files to compress or decompress. If no file is specified, read from 
      standard input.
    inputBinding:
      position: 1
  - id: best
  ...
  - id: force
    type:
      - 'null'
      - boolean
    doc: Force compression or decompression even if the file has multiple links 
  ...
  - id: list
    type:
      - 'null'
      - boolean
    doc: For each compressed file, list the uncompressed size, compressed size, 
  ...
  - id: verbose
    type:
      - 'null'
      - boolean
    doc: Verbose mode; display the name and percentage reduction for each file 
      processed.
    inputBinding:
      position: 102
      prefix: --verbose
