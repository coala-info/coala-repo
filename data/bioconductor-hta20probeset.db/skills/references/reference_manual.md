...
hta20probesetACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hta20probesetALIAS is an R object that provides mappings between common gene symbol identi-
...
xx <- as.list(hta20probesetALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hta20probeset.db annotation Package. The purpose of this package is to provide
detailed information about the hta20probeset platform. This package is updated biannually.
...
hta20probesetCHR is an R object that provides mappings between a manufacturer identifier and the
...
hta20probesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hta20probesetCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hta20probesetCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
hta20probesetENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

hta20probesetENSEMBL is an R object that contains mappings between manufacturer identifiers
...
hta20probesetENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

hta20probesetENTREZID is an R object that provides mappings between manufacturer identifiers
...
hta20probesetENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. hta20probesetENZYME2PROBE is an R object that maps Enzyme Commission
(EC) numbers to manufacturer identifiers.

Details

When the hta20probesetENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the hta20probesetENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In hta20probesetENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hta20probesetENZYME2PROBE to a list to see inside

hta20probesetGENENAME

9

x <- hta20probesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hta20probesetGENENAME Map between Manufacturer IDs and Genes

Description

hta20probesetGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hta20probesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hta20probesetGO2PROBE) do NOT associate the child terms from the GO ontology with the gene.
Only the directly evidenced terms are represented here.

hta20probesetGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
hta20probesetGO2PROBE.

Details

If hta20probesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hta20probesetGO2ALLPROBES or hta20probesetGO2PROBE is cast as a list, each GO term
...
hta20probesetMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package hta20probeset.db

hta20probesetMAPCOUNTS
...
hta20probesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package hta20probeset.db.
...
hta20probesetOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
hta20probesetORGANISM The Organism information for hta20probeset

Description

hta20probesetORGANISM is an R object that contains a single item: a character string that names
the organism for which hta20probeset was built. hta20probesetORGPKG is an R object that con-
...
Although the package name is suggestive of the organism for which it was built, hta20probesetORGANISM
provides a simple way to programmatically extract the organism name. hta20probesetORGPKG
...
hta20probesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

hta20probesetPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hta20probesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the hta20probesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hta20probesetPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hta20probesetPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hta20probesetPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hta20probesetPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hta20probesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hta20probesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hta20probesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hta20probesetPROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers
...
hta20probesetPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hta20probesetREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hta20probesetSYMBOL is an R object that provides mappings between manufacturer identifiers
...
## Bimap interface:
x <- hta20probesetSYMBOL

20

hta20probesetUNIPROT
...
hta20probesetUNIPROT Map Uniprot accession numbers with Entrez Gene identifiers

Description

hta20probesetUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hta20probeset_dbconn()
hta20probeset_dbfile()
hta20probeset_dbschema(file="", show.indices=FALSE)
hta20probeset_dbInfo()
...
hta20probeset_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by hta20probeset_dbconn or you will
break all the AnnDbObj objects defined in this package!

hta20probeset_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hta20probeset_dbschema prints the schema definition of the package annotation DB.

hta20probeset_dbInfo prints other information about the package annotation DB.

Value

hta20probeset_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.

hta20probeset_dbfile: a character string with the path to the package annotation DB.

hta20probeset_dbschema: none (invisible NULL).

hta20probeset_dbInfo: none (invisible NULL).
...
dbGetQuery(hta20probeset_dbconn(), "SELECT COUNT(*) FROM probes")

hta20probeset_dbschema()
...
hta20probeset.db, 2
hta20probeset_dbconn, 21
hta20probesetACCNUM, 1
hta20probesetALIAS2PROBE, 2
hta20probesetCHR, 3
hta20probesetCHRLENGTHS, 4
hta20probesetCHRLOC, 4
hta20probesetENSEMBL, 5
hta20probesetENTREZID, 6
hta20probesetENZYME, 7
hta20probesetGENENAME, 9
hta20probesetGO, 10
hta20probesetMAP, 12
hta20probesetMAPCOUNTS, 13
hta20probesetOMIM, 13
hta20probesetORGANISM, 14
hta20probesetPATH, 15
hta20probesetPFAM, 16
hta20probesetPMID, 16
hta20probesetPROSITE, 17
hta20probesetREFSEQ, 18
hta20probesetSYMBOL, 19
hta20probesetUNIPROT, 20
...
hta20probeset (hta20probeset.db), 2
hta20probeset.db, 2
hta20probeset_dbconn, 21
hta20probeset_dbfile
...
hta20probesetCHRLENGTHS, 4
hta20probesetCHRLOC, 4
hta20probesetCHRLOCEND

(hta20probesetCHRLOC), 4

hta20probesetENSEMBL, 5
hta20probesetENSEMBL2PROBE

(hta20probesetENSEMBL), 5

hta20probesetENTREZID, 6
hta20probesetENZYME, 7
hta20probesetENZYME2PROBE

(hta20probesetENZYME), 7

hta20probesetGENENAME, 9
hta20probesetGO, 10
hta20probesetGO2ALLPROBES, 11
hta20probesetGO2ALLPROBES
...
hta20probesetPATH, 15
hta20probesetPATH2PROBE

(hta20probesetPATH), 15

hta20probesetPFAM, 16
hta20probesetPMID, 16
hta20probesetPMID2PROBE
