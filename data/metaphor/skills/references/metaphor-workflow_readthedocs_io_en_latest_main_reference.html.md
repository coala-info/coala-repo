...
## qc.smk[¶](#qc-smk "Link to this heading")
...
## assembly.smk[¶](#assembly-smk "Link to this heading")

**concatenate\_merged\_reads**
...
## annotation.smk[¶](#annotation-smk "Link to this heading")
...
## mapping.smk[¶](#mapping-smk "Link to this heading")

**concatenate\_contigs**

```
concatenate.py -m {params.sequence_length_cutoff} {output} {input} &> {log}
```

**decompress\_catalogue**

```
pigz -d -f -p {threads} -k {input.catalogue_gz} &> {log}
```

**concatenate\_proteins**

Used by DAS\_Tool (skips the Prodigal run).

```
cat {input} > {output}
```

**create\_contigs\_index**

```
minimap2 -d {output} {input} &> {log}
```

**create\_genes\_index**
...
## binning.smk[¶](#binning-smk "Link to this heading")
...
* Reference
  + [qc.smk](#qc-smk)
  + [assembly.smk](#assembly-smk)
  + [annotation.smk](#annotation-smk)
  + [mapping.smk](#mapping-smk)
  + [binning.smk](#binning-smk)

### Related Topics

* [Documentation overview](../index.html)
  + Previous: [Contributing](contributing.html "previous chapter")
...
|
[Page source](../_sources/main/reference.md.txt)