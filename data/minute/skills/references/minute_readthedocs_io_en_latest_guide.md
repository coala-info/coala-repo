...
* User guide
  + [Standard run](#standard-run)
    ...
    - [Running minute](#running-minute)
    - [Result folders and files](#result-folders-and-files)
  + [Advanced options](#advanced-options)
    - [Custom configuration](#custom-configuration)
    - [Configuration files](#configuration-files)
      * [The libraries.tsv file](#the-librariestsv-file)
      * [The groups.tsv file](#the-groupstsv-file)
      * [The minute.yaml file](#the-minuteyaml-file)
  + [Downloading data from the Sequence Read Archive (SRA)](#downloading-data-from-the-sequence-read-archive-sra)
  + [Running on SLURM clusters](#running-on-slurm-clusters)
...
4. **Genome reference**. Reference identifier to map the reads to. This identifier should match an
   identifier in the `minute.yaml` [file](./#the-minuteyaml-file).

#### Configuration steps

1. Create a barcodes `barcodes.tsv` file describing the barcodes for each
   condition of your experiment as described.
2. Activate `minute` conda environment (see the [installation guide](../install/)).
...
3. Run:
   ...
   ```
4. Edit [`minute.yaml`](./#the-minuteyaml-file) if required. References present in `barcodes.tsv` must be
...
so it is still recommended to first configure a [standard run](./#standard-run).
...
Once the custom configuration is done, you can proceed to [run it](./#running-minute).
...
information about the matched FASTQ file pair, since no two pairs sample\_name
...
configuring your run using the [standard run](./#standard-run) will
...
used to automatically download the listed samples from the SRA, see [downloading data from SRA](./#downloading-data-from-the-sequence-read-archive-sra).
...
2. Assign a `-t` (time) parameter long enough, as this w