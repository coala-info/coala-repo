...
* [bioCancer Package](#biocancer-package)
  + [Pipeline Overview](#pipeline-overview)
  + [How to run bioCancer](#how-to-run-biocancer)
* [Portal Panel](#portal-panel)
  + [Display available Cancer Studies in Table](#display-available-cancer-studies-in-table)
    - [Studies Panel](#studies-panel)
    - [Filters in Table](#filters-in-table)
    - [Download table as csv file](#download-table-as-csv-file)
    - [Side bar Menu](#side-bar-menu)
  + [Show Clinical Data in Table](#show-clinical-data-in-table)
    - [Side bar menu](#side-bar-menu-1)
  + [Show Profiles Data in Table](#show-profiles-data-in-table)
  + [Manage data and state: Load data into bioCancer, Save data to disk, Remove a dataset from memory, or Save/Load the full state of the app](#manage-data-and-state-load-data-into-biocancer-save-data-to-disk-remove-a-dataset-from-memory-or-saveload-the-full-state-of-the-app)
    - [Datasets](#datasets)
    - [Load data](#load-data)
    - [Save data](#save-data)
    - [Save and load state](#save-and-load-state)
    - [Remove data from memory](#remove-data-from-memory)
    - [Using commands to load and save data](#using-commands-to-load-and-save-data)
  + [Show data in table form](#show-data-in-table-form)
    - [Datasets](#datasets-1)
    - [Select columns](#select-columns)
    - [Browse the data](#browse-the-data-1)
    - [Sort](#sort-1)
    - [Filter](#filter)
    - [Column filters and Search](#column-filters-and-search)
  + [Visualize data](#visualize-data)
    - [Filter](#filter-1)
    - [Plot-type](#plot-type)
    - [Box plots](#box-plots)
    - [Sub-plots and heat-maps](#sub-plots-and-heat-maps)
    - [Line, loess, and jitter](#line-loess-and-jitter)
    - [Axis scale](#axis-scale)
    - [Flip axes](#flip-axes)
    - [Plot height and width](#plot-height-and-width)
    - [Customizing plots in R > Report](#customizing-plots-in-r-report)
  + [Create pivot tables to explore your data](#create-pivot-tables-to-explore-your-data)
    - [Filter](#filter-2)
  + [Summarize and explore your data](#summarize-and-explore-your-data)
    - [Filter](#filter-3)
    - [Transform command log](#transform-command-log)
    - [Filter](#filter-4)
    - [Type](#type)
    - [Transform](#transform)
    - [Create](#create)
    ...
    - [Clipboard](#clipboard)
    - [Normalize](#normalize)
    - [Reorder or remove columns](#reorder-or-remove-columns)
    - [Reorder or remove levels](#reorder-or-remove-levels)
    - [Remove missing values](#remove-missing-values)
    - [Remove duplicates](#remove-duplicates)
    - [Show duplicates](#show-duplicates)
  + [Combine two datasets](#combine-two-datasets)
    - [Inner join (superheroes, publishers)](#inner-join-superheroes-publishers)
    - [Left join (superheroes, publishers)](#left-join-superheroes-publishers)
    - [Right join (superheroes, publishers)](#right-join-superheroes-publishers)
    - [Full join (superheroes, publishers)](#full-join-superheroes-publishers)
    - [Semi join (superheroes, publishers)](#semi-join-superheroes-publishers)
    - [Anti join (superheroes, publishers)](#anti-join-superheroes-publishers)
    - [Dataset order](#dataset-order)
    - [Inner join (publishers, superheroes)](#inner-join-publishers-superheroes)
    - [Left and Right join (publishers, superheroes)](#left-and-right-join-publishers-superheroes)
    - [Full join (publishers, superheroes)](#full-join-publishers-superheroes)
    - [Semi join (publishers, superheroes)](#semi-join-publishers-superheroes)
    - [Anti join (publishers, superheroes)](#anti-join-publishers-superheroes)
    - [Additional tools to combine datasets (avengers, superheroes)](#additional-tools-to-combine-datasets-avengers-superheroes)
    - [Bind rows](#bind-rows)
    - [Bind columns](#bind-columns)
    - [Intersect](#intersect)
    - [Union](#union)
    - [Setdiff](#setdiff)
* [Enrichment Panel](#enrichment-panel)
  + [Show multi-Omics Data in Circular Layout](#show-multi-omics-data-in-circular-layout)
  + [Genes / Diseases / Pathways Classification and clustering](#genes-diseases-pathways-classification-and-clustering)
    - [Classification](#classification)
    - [Plot Clusters](#plot-clusters)
  + [Function Interaction Network Enrichment](#function-interaction-network-enrichment)
...
There are in general but not always, 6 types of genetic profiles: \* Copy Number Alteration ([CNA](https://en.wikipedia.org/wiki/Copy-number_variation)). \* mRNA expression ([mRNA](https://en.wikipedia.org/wiki/Gene_expression)) \* Mutations ([Mut](https://en.wikipedia.org/wiki/Mutation)) \* Methylation ([Met](https://en.wikipedia.org/wiki/DNA_methylation)): There are two probes `HM_27` and `HM_450` \* microRNA expression ([miRNA](https://en.wikipedia.org/wiki/MicroRNA)) \* Reverse Phase Protein Array ([RPPA](https://en.wikipedia.org/wiki/Reverse_phase_protein_lysate_microarray))

It is possible to find other kind of data related to one of listed types. For example the `log` or [z\_score](https://docs.cbioportal.org/user-guide/faq/#what-are-mrna-and-microrna-z-scores) of mRNA expression.
...
It is good practice to add a description of the data and variables to each file you use. For the files that are part of bioCancer you will see a brief overview of the variables etc. below the table of the first 10 rows of the data. If you would like to add a description for your own data check the ‘Add/edit data description’ check-box. A window will open below that data table where you can add text in [markdown](https://pandoc.org/MANUAL.html) format. The descriptions of the data included with bioCancer should serve as a good starting point. When you save the data as an rda file the description you created (or edited) will automatically be added to the file.
...
This is convenient if you want to save your work to be completed at another time, perhaps on another computer, or to review any assignments you completed using bioCancer. You can also share the file with others that would like to replicate your analyses. As an example, download and then load the state\_file [`RadiantState.rda`](https://radiant-rstats.github.io/radiant.data/articles/pkgdown/manage.html). Go to `Data > View`, `Data > Visualize` to see some of the settings loaded from the statefile. There is also a report in `R > Report` created using the Radiant interface. The html file [`RadiantState.html`](https://radiant-rstats.github.io/radiant.data/articles/pkgdown/manage.html) contains the output.
...
![Box-plot](data:image/png;base64...) [Source](https://en.wikipedia.org/wiki/File%3ABoxplot_vs_PDF.svg)
...
See the ggplot2 documentation page for available options [https://docs.ggplot2.org](https://ggplot2.tidyverse.org/).
...
8. As date/time (mdy\_hms): convert a variable to a date if the dates are ordered as month-day-year-hour-minute-second
9. As date/time (mdy\_hm): convert a variable to a date if the dates are ordered as month-day-year-hour-minute
...
xmRNA\_rc = ifelse(xmRNA > 400, NA, sales)
...
15. Calculating the distance between two locations using lat-long information
    ...
    trip\_distance = as\_distance(lat1, long1, lat2, long2)
...
There are six *join* (or *merge*) options available in bioCancer from the [dplyr](https://www.rdocumentation.org/packages/dplyr) package developed by Hadley Wickham and Romain Francois on [GitHub](https://github.com/tidyverse/dplyr).

The examples below are adapted from [Cheatsheet for dplyr join functions](https://posit.co/resources/cheatsheets/) by Jenny Bryan and focus on three small datasets, `superheroes`, `publishers`, and `avengers`, to illustrate the different *join* types and other ways to combine datasets in R and bioCancer. The data is also available in csv format through the links below:

[superheroes.csv](https://github.com/vnijs/radiant/blob/gh-pages/examples/superheroes.csv)

[publishers.csv](https://github.com/vnijs/radiant/blob/gh-pages/examples/publishers.csv)

[avengers.csv](https://github.com/vnijs/radiant/blob/gh-pages/examples/avengers.csv)
...
In the table above we lose *Hellboy* because, although this hero does appear in `superheroes`, the publisher (*Dark Horse Comics*) does not appear in `publishers`. The join result has all variables from `superheroes`, plus *yr\_founded*, from `publishers`. We can visualize an inner join with the venn-diagram below:
...
The join result contains `superheroes` with variable `yr_founded` from `publishers`. *Hellboy*, whose publisher does not appear in `publishers`, has an `NA` for *yr\_founded*. We can visualize a left join with the venn-diagram below:
...
We now get **only** *Hellboy*, the only superhero not in `publishers` and we do not get the variable *yr\_founded* either. We can visualize an anti join with the venn-diagram below:
...
**Table1: Ranking Genes by Study** ![Classifier table](data:image/png;base64...)

### Plot Clusters

#### Gene Diseases Association

`GeneList/Diseases` predicts Wich disease are involving your GeneList. It uses annotations from [DisGeNET](https://www.disgenet.org/) [2] and Methods from [clusterProfiler](https://bioconductor.org/packages/release/bioc/html/clusterProfiler.html) package [3].
...
**Figure 1: Genes / Diseases Association** ![Plot_enrich](data:image/png;base64...)

The `Diseases Onthology` uses genes/Study groups computed by `Classifier` (Table 1). The dotplot position indicates wish Diseases are annotated for genes/study [4]. The dot size indicates the ratio of genes involved in the disease for the same genes groups (lihc\_tcga has 2/3 genes involved for the 4 disease). The color indicates the P-Value.

**Figure 2: Diseases Onthology** ![DO](data:image/png;base64...)

The same process is possible with Gene Onthology (GO) and KEGG.

**Figure 3: GO Pathway Enrishment** ![GO](data:image/png;base64...)

**Figure 4: KEGG Pathway Enrishment**  ![KEGG](data:image/png;base64...)
