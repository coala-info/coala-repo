...
msa\_split chr1.maf --refseq chr1.fa --in-format MAF \
...
format.
msa\_split mydata.fa --features conserved.bed --by-category \
--out-root mydata
(Output will be to mydata.background-0.fa and mydata.bed\_feature-1.fa
...
msa\_split chr22.maf --in-format MAF --features chr22.gff \
...
GFF file. Assume genes are defined by the tag "transcript\_id".
msa\_split cftr.fa --features cftr.gff --by-group transcript\_id
5. Obtain a sub-alignment for each of a set of regulatory regions,
as defined in a BED file.
msa\_split chr22.maf --in-format MAF --refseq chr22.fa \
--features chr22.reg.bed --for-features \
--out-root chr22.reg
OPTIONS:
(Splitting options)
--windows, -w
Split the alignment into "windows" of size  bases,
overlapping by .
--by-category, -L
...
20-. Note that the indices are relative to the
...
indices will not be moved more than  sites.
--features, -g
...
--out-root, -r
Filename root for output files (default "msa\_split").
--sub-features, -f
...
gaps, or gaps in the specified sequence (; indexing
...
--min-informative, -I
Only output alignments having at least  informative sites
...
--by-category, insert tuple\_size-1 columns of missing data
