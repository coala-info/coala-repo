...
![cgc-home](data:image/png;base64...)

You can find login/registration on the Cancer Genomics Cloud homepage <https://www.cancergenomicscloud.org>. Follow the [signup tutorial](http://docs.cancergenomicscloud.org/docs/sign-up-for-the-cgc) if you need to access TCGA Controlled Data on the CGC via NIH eRA Commons.
...
![shinyapps-token](data:image/png;base64...)

## 2.5 Report issues

This package is under active development, will bring many new features as well, at any moment, if you have questions or problem about this R package, please file issues on [GitHub](https://github.com/sbg/sevenbridges-r/issues).

If you have question regarding the Cancer Genomics Cloud or other Seven Bridges platforms, we have a different channel for each platform, for example, Cancer Genomics Cloud have lots [documentation](http://docs.cancergenomicscloud.org/docs) and a [forum](http://docs.cancergenomicscloud.org/discuss).
...
![quickstart-flow](data:image/png;base64...)
...
![ggvis](data:image/png;base64...)

A differential expression table

![de-table](data:image/png;base64...)
...
![html-report](data:image/png;base64...)
...
![cgc-projects](data:image/png;base64...)
...
## 3.2 Build a RNA-Seq tool: from bam to report

### 3.2.1 Step 1: Have a plan

To demonstrate I will use a RNA-seq [workflow](https://www.bioconductor.org/help/workflows/rnaseqGene/) from Bioconductor, when you open this link, you will notice a full example written in Markdown I want to make it into an app and allow you to input new files and generate new report base on this template.
...
![rnaseq-diagram](data:image/png;base64...)
...
### 3.2.2 Step 2: Create Docker container for your tool
...
1. First you need to check is there any existing container you can directly use, so you don’t have to make a new one. If you don’t know what to use, I will suggest you start with `rocker/hadleyverse`, it has lots of stuff like Markdown, knitr need and other Hadley’s popular packages.
...
### 3.2.3 Step 3: Create your command line interface
...
You are ready to build Docker container and push it to the registry. You can choose to use registry like dockerhub or use CGC’s own Docker registry (`cgc-images.sbgenomics.com`). To learn how to use CGC Image registry, please read out [tutorial](http://docs.cancergenomicscloud.org/docs/the-cgc-image-registry).

### 3.2.5 Step 5: Describe your tool in R into CWL
...
### 3.2.6 Step 6: Execute your tool with a new task via R API
...
```
Warning in readLines(download.fl): incomplete final line found on
'/tmp/Rtmp3nM4lg/Rinst2fae666801c266/sevenbridges/extdata/download.txt'
```
...
## 3.3 Build a reporting Tool

### 3.3.1 What is report tool?
...
![report-tool](data:image/png;base64...)
...
2. In this root folder, you have `data`, `src`, `www` folders, on the interface, you can connect output into those folders, so data flows into it.
...
![cgc-workflow](data:image/png;base64...)

And next run a task on the platform via UI like this:

![rnaseq-runtask](data:image/png;base64...)

# 4 Exercise: bring your own tool
...
or on the [vignettes page](https://sbg.github.io/sevenbridges-r/articles/).