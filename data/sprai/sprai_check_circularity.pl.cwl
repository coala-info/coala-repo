cwlVersion: v1.2
class: CommandLineTool
baseCommand: sprai_check_circularity.pl
label: sprai_check_circularity.pl
doc: "A tool to check the circularity of sequences, typically used in the SPRAI (Single
  Pass Read Accuracy Improver) pipeline for genome assembly.\n\nTool homepage: http://zombie.cb.k.u-tokyo.ac.jp/sprai/"
inputs: []
outputs:
  - id: stdout
    type: stdout
    doc: Standard output
hints:
  - class: DockerRequirement
    dockerPull: quay.io/biocontainers/sprai:0.9.9.23--py27pl5.22.0_0
stdout: sprai_check_circularity.pl.out
