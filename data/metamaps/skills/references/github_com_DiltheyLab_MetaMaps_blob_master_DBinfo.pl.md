...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2FDiltheyLab%2FMetaMaps%2Fblob%2Fmaster%2FDBinfo.pl)
...
[Sign up](/signup?ref_cta=Sign+up&ref_loc=header+logged+out&ref_page=%2F%3Cuser-name%3E%2F%3Crepo-name%3E%2Fblob%2Fshow&source=header-repo&source_repo=DiltheyLab%2FMetaMaps)
...
[History](/DiltheyLab/MetaMaps/commits/master/DBinfo.pl)
...
[Raw](https://github.com/DiltheyLab/MetaMaps/raw/refs/heads/master/DBinfo.pl)
...
my %taxonID\_2\_contigs;

my %contigLength;

Util::read\_taxonIDs\_and\_contigs($DB, \%taxonID\_2\_contigs, \%contigLength);
...
my @ancestors = ($taxonID, taxTree::get\_ancestors($taxonomy, $taxonID));
...
$taxonID\_ranks{$rank} = taxTree::taxon\_id\_get\_name($nodeID, $taxonomy);
...
next unless((exists $taxonID\_ranks{$limitTo1}) and ($taxonID\_ranks{$limitTo1} eq $limitTo2));

}

$taxonID\_ranks{$reportLevel} = 'Undefined' unless(defined $taxonID\_ranks{$reportLevel});

$report{$taxonID\_ranks{$reportLevel}}[0]++;

$report{$taxonID\_ranks{$reportLevel}}[1] += $contigLength{$contigID};

$report{$taxonID\_ranks{$reportLevel}}[2]{$taxonID}++;
...
my $n\_genomes = scalar(keys %{$report{$v}[2]});

print "\t - ${v}: $n\_genomes genomes ($report{$v}[0] contigs), ", sprintf("%.2f", $report{$v}[1] / (1024\*\*2)), "mb.\n";
