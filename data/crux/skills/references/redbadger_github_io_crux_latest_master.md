...
presentation layer and effect execution (but not *orchestration*, that is part of the behaviour
...
To get playing with Crux quickly, follow Part I of the book, from the [Getting Started](./part-1/getting_started.html) chapter onward. It will take you from zero to a basic working app on your preferred platform quickly. From there, continue on to Part II – building the [Weather App](./guide/weather_app.html), which builds on the basics and covers the more advanced features and patterns needed in a real world app.

If you just want to understand why we set out to build Crux in the first place and what problems it tries to solve, before you spend any time trying it (no hard feelings, we would too), read our original [Motivation](./motivation.html).
...
Crux uses [managed side-effects](./part-2/effects.html) – the Core requests side-effects
...
more [about our motivation](./motivation.html). The overall goals of Crux are to:
