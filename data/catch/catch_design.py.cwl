...
baseCommand: design.py
label: catch_design.py
doc: "Design probes for targeted sequencing experiments.\n\nTool homepage: https://github.com/broadinstitute/catch"
inputs:
  - id: datasets
    type:
      type: array
  ...
  - id: custom_hybridization_fn_tolerant
    ...
    doc: A more tolerant model than the one implemented in 
      ...
      genomes are avoided. See --custom-hybridization- fn for details of how 
      this function should be implemented and provided.
    inputBinding:
      position: 102
  ...
  - id: kmer_probe_map_k
    ...
    doc: Use this value (KMER_PROBE_LENGTH_K) as the k-mer length when 
      ...
      exists, it will use a randomized approach with KMER_PROBE_LENGTH_K as the 
