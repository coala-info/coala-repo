...
baseCommand:
  ...
  - FlagStat
label: gatk_FlagStat
doc: Accumulate flag statistics given a BAM file, e.g. total number of reads 
  with QC failure flag set, number of duplicates, percentage mapped etc. and 
  output summary to standard output (and optionally to a file).
inputs:
  - id: input
    type:
      type: array
