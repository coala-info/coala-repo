...
hu35ksubbACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hu35ksubbALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hu35ksubbALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hu35ksubbALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hu35ksubb.db annotation Package. The purpose of this package is to provide de-
tailed information about the hu35ksubb platform. This package is updated biannually.
...
hu35ksubbCHR is an R object that provides mappings between a manufacturer identifier and the
...
hu35ksubbCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hu35ksubbCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hu35ksubbENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hu35ksubbENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hu35ksubbENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hu35ksubbENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hu35ksubbENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hu35ksubbENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hu35ksubbENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hu35ksubbENZYME2PROBE to a list to see inside

hu35ksubbGENENAME

9

x <- hu35ksubbENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hu35ksubbGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hu35ksubbGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hu35ksubbGO2PROBE)
...
hu35ksubbGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hu35ksubbGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hu35ksubbGO2ALLPROBES or hu35ksubbGO2PROBE is cast as a list, each GO term maps to
...
x <- hu35ksubbGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hu35ksubbGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hu35ksubbMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hu35ksubb.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hu35ksubbMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hu35ksubb.db.
...
hu35ksubbOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hu35ksubb

Description

hu35ksubbORGANISM is an R object that contains a single item: a character string that names
the organism for which hu35ksubb was built. hu35ksubbORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hu35ksubbORGANISM
provides a simple way to programmatically extract the organism name. hu35ksubbORGPKG pro-
...
hu35ksubbPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hu35ksubbPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hu35ksubbPATH
mapped_probes <- mappedkeys(x)
# Now convert the hu35ksubbPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubbPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hu35ksubbPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hu35ksubbPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hu35ksubbPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hu35ksubbPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hu35ksubbPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hu35ksubbPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubbPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hu35ksubbREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hu35ksubbSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hu35ksubbUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hu35ksubb_dbconn()
hu35ksubb_dbfile()
hu35ksubb_dbschema(file="", show.indices=FALSE)
hu35ksubb_dbInfo()
...
hu35ksubb_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hu35ksubb_dbconn or you will break all
the AnnDbObj objects defined in this package!

hu35ksubb_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hu35ksubb_dbschema prints the schema definition of the package annotation DB.

hu35ksubb_dbInfo prints other information about the package annotation DB.

Value

hu35ksubb_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

hu35ksubb_dbfile: a character string with the path to the package annotation DB.

hu35ksubb_dbschema: none (invisible NULL).

hu35ksubb_dbInfo: none (invisible NULL).
...
dbGetQuery(hu35ksubb_dbconn(), "SELECT COUNT(*) FROM probes")
...
hu35ksubb_dbconn, 21
hu35ksubbACCNUM, 1
hu35ksubbALIAS2PROBE, 2
hu35ksubbCHR, 3
hu35ksubbCHRLENGTHS, 4
hu35ksubbCHRLOC, 4
hu35ksubbENSEMBL, 5
hu35ksubbENTREZID, 6
hu35ksubbENZYME, 7
...
hu35ksubbGO, 10
hu35ksubbMAP, 12
hu35ksubbMAPCOUNTS, 13
hu35ksubbOMIM, 13
hu35ksubbORGANISM, 14
hu35ksubbPATH, 15
hu35ksubbPFAM, 16
hu35ksubbPMID, 16
hu35ksubbPROSITE, 17
...
hu35ksubb (hu35ksubb.db), 2
hu35ksubb.db, 2
hu35ksubb_dbconn, 21
hu35ksubb_dbfile (hu35ksubb_dbconn), 21

23

hu35ksubb_dbInfo (hu35ksubb_dbconn), 21
hu35ksubb_dbschema (hu35ksubb_dbconn),
...
hu35ksubbCHRLENGTHS, 4
hu35ksubbCHRLOC, 4
hu35ksubbCHRLOCEND (hu35ksubbCHRLOC), 4
hu35ksubbENSEMBL, 5
hu35ksubbENSEMBL2PROBE

(hu35ksubbENSEMBL), 5

hu35ksubbENTREZID, 6
hu35ksubbENZYME, 7
hu35ksubbENZYME2PROBE

(hu35ksubbENZYME), 7

hu35ksubbGENENAME, 9
hu35ksubbGO, 10
hu35ksubbGO2ALLPROBES, 11
hu35ksubbGO2ALLPROBES (hu35ksubbGO), 10
hu35ksubbGO2PROBE (hu35ksubbGO), 10
hu35ksubbLOCUSID (hu35ksubbENTREZID), 6
hu35ksubbMAP, 12
hu35ksubbMAPCOUNTS, 13
hu35ksubbOMIM, 13
hu35ksubbORGANISM, 14
hu35ksubbORGPKG (hu35ksubbORGANISM), 14
hu35ksubbPATH, 15
hu35ksubbPATH2PROBE (hu35ksubbPATH), 15
hu35ksubbPFAM, 16
hu35ksubbPMID, 16
hu35ksubbPMID2PROBE (hu35ksubbPMID), 16
hu35ksubbPROSITE, 17
hu35ksubbREFSEQ, 18
hu35ksubbSYMBOL, 19
hu35ksubbUNIPROT, 20
hu35ksubbUNIPROT2PROBE
