...
N. Gopalakrishnan and Halimat C. Atanda1
...
* [1 graphBAM class](#graphbam-class)
  + [1.1 Introduction](#introduction)
  + [1.2 A simple graph represented using graphBAM class](#a-simple-graph-represented-using-graphbam-class)
  + [1.3 Mice gene interaction data for brain tissue (SAGE data)](#mice-gene-interaction-data-for-brain-tissue-sage-data)
* [2 MultiGraphs](#multigraphs)
  + [2.1 Introduction](#introduction-1)
  + [2.2 A simple MultiGraph example](#a-simple-multigraph-example)
  + [2.3 MultiGraph representation of mice gene interaction data. (SAGE)](#multigraph-representation-of-mice-gene-interaction-data.-sage)
...
We shall load the *[graph](https://bioconductor.org/packages/3.22/graph)* and *[RBGL](https://bioconductor.org/packages/3.22/RBGL)* packages that we will be using. We will then create a *MultiGraph* object and then spend some time examining some of the different functions that can be applied to *MultiGraph* objects.
...
We are interested in the gene “10024416717” and the sum of the weights along the path that connects this genes to the other genes for the brain tissue. Since the algorithms in the *[RBGL](https://bioconductor.org/packages/3.22/RBGL)* package that we will use to find the edges that are connected to the gene “10024416717” do not work directly with *MultiGraph* objects, we proceed to create `graphBAM` objects from the male and female edge sets for the brain tissue.
...
We then identify the genes connected to gene “10024416717” as well as the sum of the weights along the path that connect the identified genes using the function `bellman.ford.sp` function from the *[RBGL](https://bioconductor.org/packages/3.22/RBGL)* package.
