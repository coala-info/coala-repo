...
hugene11stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

hugene11stprobesetACCNUM is an R object that contains mappings between a manufacturer’s
...
hugene11stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(hugene11stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
hugene11stprobeset.db Bioconductor annotation data package

Description

Welcome to the hugene11stprobeset.db annotation Package. The purpose of this package is to
provide detailed information about the hugene11stprobeset platform. This package is updated bian-
nually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

hugene11stprobesetCHR
...
## use of select().
ls("package:hugene11stprobeset.db")

hugene11stprobesetCHR Map Manufacturer IDs to Chromosomes

Description

hugene11stprobesetCHR is an R object that provides mappings between a manufacturer identifier
...
hugene11stprobesetCHRLOC

hugene11stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hugene11stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
## Bimap interface:
tt <- hugene11stprobesetCHRLENGTHS
...
hugene11stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting posi-
...
hugene11stprobesetENSEMBL is an R object that contains mappings between manufacturer iden-
...
x <- hugene11stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hugene11stprobesetENTREZID is an R object that provides mappings between manufacturer iden-
tifiers and Entrez Gene identifiers.

hugene11stprobesetENZYME
...
hugene11stprobesetENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. hugene11stprobesetENZYME2PROBE is an R object that maps Enzyme
...
When the hugene11stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the hugene11stprobesetENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In hugene11stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert hugene11stprobesetENZYME2PROBE to a list to see inside
x <- hugene11stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hugene11stprobesetGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- hugene11stprobesetGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

hugene11stprobesetGO
...
hugene11stprobesetGO Maps between manufacturer IDs and Gene Ontology (GO) IDs

Description

hugene11stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hugene11stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

hugene11stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
hugene11stprobesetGO2PROBE.

Details

If hugene11stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If hugene11stprobesetGO2ALLPROBES or hugene11stprobesetGO2PROBE is cast as a list, each
...
• hugene11stprobesetGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
hugene11stprobesetMAP

x <- hugene11stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
hugene11stprobesetMAP Map between Manufacturer Identifiers and cytogenetic maps/bands

Description

hugene11stprobesetMAP is an R object that provides mappings between manufacturer identifiers
...
hugene11stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package hugene11stprobeset.db.
...
hugene11stprobesetOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for hugene11stprobeset

Description

hugene11stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which hugene11stprobeset was built. hugene11stprobesetORGPKG is an R
...
hugene11stprobesetPATH

Details

15

Although the package name is suggestive of the organism for which it was built, hugene11stprobesetORGANISM
provides a simple way to programmatically extract the organism name. hugene11stprobesetORGPKG
...
hugene11stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

hugene11stprobesetPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- hugene11stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the hugene11stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene11stprobesetPFAM is an R object that provides mappings between manufacturer identifiers
...
hugene11stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. hugene11stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When hugene11stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a
...
When hugene11stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hugene11stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hugene11stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hugene11stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

hugene11stprobesetPROSITE is an R object that provides mappings between manufacturer identi-
...
hugene11stprobesetREFSEQ is an R object that provides mappings between manufacturer identi-
...
hugene11stprobesetSYMBOL is an R object that provides mappings between manufacturer identi-
...
hugene11stprobesetUNIPROT is an R object that contains mappings between the manufacturer
...
hugene11stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by hugene11stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
hugene11stprobeset_dbfile returns the path (character string) to the package annotation DB
(this is an SQLite file).
hugene11stprobeset_dbschema prints the schema definition of the package annotation DB.
hugene11stprobeset_dbInfo prints other information about the package annotation DB.

Value

hugene11stprobeset_dbconn: a DBIConnection object representing an open connection to the
package annotation DB.
hugene11stprobeset_dbfile: a character string with the path to the package annotation DB.
hugene11stprobeset_dbschema: none (invisible NULL).
hugene11stprobeset_dbInfo: none (invisible NULL).
...
hugene11stprobeset.db, 2
hugene11stprobeset_dbconn, 21
hugene11stprobesetACCNUM, 1
hugene11stprobesetALIAS2PROBE, 2
hugene11stprobesetCHR, 3
hugene11stprobesetCHRLENGTHS, 4
hugene11stprobesetCHRLOC, 4
hugene11stprobesetENSEMBL, 5
hugene11stprobesetENTREZID, 6
hugene11stprobesetENZYME, 7
hugene11stprobesetGENENAME, 9
hugene11stprobesetGO, 10
hugene11stprobesetMAP, 12
hugene11stprobesetMAPCOUNTS, 13
hugene11stprobesetOMIM, 13
hugene11stprobesetORGANISM, 14
hugene11stprobesetPATH, 15
hugene11stprobesetPFAM, 16
hugene11stprobesetPMID, 17
hugene11stprobesetPROSITE, 18
hugene11stprobesetREFSEQ, 18
hugene11stprobesetSYMBOL, 20
hugene11stprobesetUNIPROT, 21
...
hugene11stprobesetCHRLENGTHS, 4
hugene11stprobesetCHRLOC, 4
hugene11stprobesetCHRLOCEND

(hugene11stprobesetCHRLOC), 4

hugene11stprobesetENSEMBL, 5
hugene11stprobesetENSEMBL2PROBE

(hugene11stprobesetENSEMBL), 5

hugene11stprobesetENTREZID, 6
hugene11stprobesetENZYME, 7
hugene11stprobesetENZYME2PROBE

(hugene11stprobesetENZYME), 7

hugene11stprobesetGENENAME, 9
hugene11stprobesetGO, 10
hugene11stprobesetGO2ALLPROBES, 11
hugene11stprobesetGO2ALLPROBES
...
hugene11stprobesetPATH, 15
hugene11stprobesetPATH2PROBE

(hugene11stprobesetPATH), 15

hugene11stprobesetPFAM, 16
hugene11stprobesetPMID, 17
hugene11stprobesetPMID2PROBE
...
hugene11stprobesetSYMBOL, 20
hugene11stprobesetUNIPROT, 21
hugene11stprobesetUNIPROT2PROBE
