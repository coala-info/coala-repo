...
hgu133aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu133aALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the hgu133a.db annotation Package. The purpose of this package is to provide detailed
information about the hgu133a platform. This package is updated biannually.
...
hgu133aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgu133aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu133aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu133aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu133aENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu133aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu133aENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu133aENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu133aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu133aENZYME2PROBE to a list to see inside

hgu133aGENENAME

9

x <- hgu133aENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu133aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu133aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgu133aGO2PROBE)
...
hgu133aGO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu133aGO2PROBE.

Details

If hgu133aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu133aGO2ALLPROBES or hgu133aGO2PROBE is cast as a list, each GO term maps to a
...
x <- hgu133aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu133aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hgu133aMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu133a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu133aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu133a.db.
...
hgu133aOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu133a

Description

hgu133aORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu133a was built. hgu133aORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu133aORGANISM
provides a simple way to programmatically extract the organism name. hgu133aORGPKG provides
...
hgu133aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu133aPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- hgu133aPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu133aPATH2PROBE object to a list
...
hgu133aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu133aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu133aPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu133aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgu133aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hgu133aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu133aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu133aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgu133aREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu133aSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu133aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu133a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu133a_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgu133a_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgu133a_dbschema prints the schema definition of the package annotation DB.

hgu133a_dbInfo prints other information about the package annotation DB.

Value

hgu133a_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

hgu133a_dbfile: a character string with the path to the package annotation DB.

hgu133a_dbschema: none (invisible NULL).

hgu133a_dbInfo: none (invisible NULL).
...
hgu133a (hgu133a.db), 2
hgu133a.db, 2
hgu133a_dbconn, 21
hgu133a_dbfile (hgu133a_dbconn), 21

23

hgu133a_dbInfo (hgu133a_dbconn), 21
hgu133a_dbschema (hgu133a_dbconn), 21
hgu133aACCNUM, 1
hgu133aALIAS2PROBE, 2
hgu133aCHR, 3
hgu133aCHRLENGTHS, 4
hgu133aCHRLOC, 4
hgu133aCHRLOCEND (hgu133aCHRLOC), 4
hgu133aENSEMBL, 5
hgu133aENSEMBL2PROBE (hgu133aENSEMBL), 5
hgu133aENTREZID, 6
hgu133aENZYME, 7
hgu133aENZYME2PROBE (hgu133aENZYME), 7
hgu133aGENENAME, 9
hgu133aGO, 10
hgu133aGO2ALLPROBES, 11
hgu133aGO2ALLPROBES (hgu133aGO), 10
hgu133aGO2PROBE (hgu133aGO), 10
hgu133aLOCUSID (hgu133aENTREZID), 6
hgu133aMAP, 12
hgu133aMAPCOUNTS, 13
hgu133aOMIM, 13
hgu133aORGANISM, 14
hgu133aORGPKG (hgu133aORGANISM), 14
hgu133aPATH, 15
hgu133aPATH2PROBE (hgu133aPATH), 15
hgu133aPFAM, 16
hgu133aPMID, 16
hgu133aPMID2PROBE (hgu133aPMID), 16
hgu133aPROSITE, 17
hgu133aREFSEQ, 18
hgu133aSYMBOL, 19
hgu133aUNIPROT, 20
hgu133aUNIPROT2PROBE (hgu133aUNIPROT),
