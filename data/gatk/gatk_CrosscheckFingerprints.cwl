...
label: gatk_CrosscheckFingerprints
...
inputs:
  ...
  - id: crosscheck_by
    type:
      - 'null'
      - string
    doc: 'Specifies which data-type should be used as the basic comparison unit. Possible
      values: {FILE, SAMPLE, LIBRARY, READGROUP}'
    inputBinding:
      position: 101
  ...
  - id: genotyping_error_rate
    type:
      - 'null'
      - float
    doc: DEPRECATED. Assumed genotyping error rate that provides a floor on the 
      probability that a genotype comes from the expected sample.
    inputBinding:
      position: 101
  ...
  - id: input_sample_file_map
    type:
      - 'null'
      - File
    doc: A tsv with two columns representing the sample as it should be used for
      comparisons to SECOND_INPUT and the source file for the fingerprint.
    inputBinding:
      position: 101
  ...
  - id: input_sample_map
    type:
      - 'null'
      - File
    doc: A tsv with two columns representing the sample as it appears in the 
      INPUT data and the sample as it should be used for comparisons to 
      SECOND_INPUT.
    inputBinding:
      position: 101
  ...
  - id: lod_threshold
    type:
      - 'null'
      - float
    doc: If any two groups (with the same sample name) match with a LOD score 
      lower than the threshold the tool will exit with a non-zero code.
    inputBinding:
      position: 101
  ...
  - id: second_input_index_map
    type:
      - 'null'
      - File
    doc: A tsv with two columns and no header which maps the second input files 
  ...
  - id: second_input_sample_map
    type:
      ...
      - File
    doc: A tsv with two columns representing the sample as it appears in the 
      SECOND_INPUT data and the sample as it should be used for comparisons to 
      INPUT.
    inputBinding:
      position: 101
