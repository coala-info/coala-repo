...
huex10stprobesetACCNUM

Map Manufacturer identifiers to Accession Numbers

Description

huex10stprobesetACCNUM is an R object that contains mappings between a manufacturer’s iden-
...
huex10stprobesetALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(huex10stprobesetALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the huex10stprobeset.db annotation Package. The purpose of this package is to provide
detailed information about the huex10stprobeset platform. This package is updated biannually.
...
## use of select().
ls("package:huex10stprobeset.db")
...
huex10stprobesetCHR is an R object that provides mappings between a manufacturer identifier and
...
huex10stprobesetCHRLOC

huex10stprobesetCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

huex10stprobesetCHRLENGTHS provides the length measured in base pairs for each of the chro-
mosomes.
...
huex10stprobesetCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
huex10stprobesetENSEMBL is an R object that contains mappings between manufacturer identi-
...
x <- huex10stprobesetENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
huex10stprobesetENTREZID is an R object that provides mappings between manufacturer identi-
fiers and Entrez Gene identifiers.

huex10stprobesetENZYME
...
huex10stprobesetENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. huex10stprobesetENZYME2PROBE is an R object that maps Enzyme Commis-
...
When the huex10stprobesetENZYME maping viewed as a list, each manufacturer identifier maps
...
will contain an NA.
For the huex10stprobesetENZYME2PROBE, each EC number maps to a named vector containing
...
numbers. In huex10stprobesetENZYME2PROBE, EC is dropped from the Enzyme Commission
numbers.
...
# Now convert huex10stprobesetENZYME2PROBE to a list to see inside
x <- huex10stprobesetENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
huex10stprobesetGENENAME is an R object that maps manufacturer identifiers to the correspond-
ing gene name.
...
huex10stprobesetGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(huex10stprobesetGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

huex10stprobesetGO2ALLPROBES is an R object that provides mappings between a given GO
...
huex10stprobesetGO2PROBE.

Details

If huex10stprobesetGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If huex10stprobesetGO2ALLPROBES or huex10stprobesetGO2PROBE is cast as a list, each GO
...
x <- huex10stprobesetGO2ALLPROBES
mapped_genes <- mappedkeys(x)
...
huex10stprobesetMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package huex10stprobeset.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

huex10stprobesetMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package huex10stprobeset.db.
...
huex10stprobesetOMIM Map between Manufacturer Identifiers and Mendelian Inheritance in

Man (MIM) identifiers

Description

huex10stprobesetOMIM is an R object that provides mappings between manufacturer identifiers
...
The Organism information for huex10stprobeset

Description

huex10stprobesetORGANISM is an R object that contains a single item: a character string that
names the organism for which huex10stprobeset was built. huex10stprobesetORGPKG is an R
...
huex10stprobesetPATH

Details

15

Although the package name is suggestive of the organism for which it was built, huex10stprobesetORGANISM
provides a simple way to programmatically extract the organism name. huex10stprobesetORGPKG
...
## Bimap interface:
huex10stprobesetORGANISM
huex10stprobesetORGPKG

huex10stprobesetPATH Mappings between probe identifiers and KEGG pathway identifiers
...
huex10stprobesetPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

huex10stprobesetPATH2PROBE is an R object that provides mappings between KEGG identifiers
...
x <- huex10stprobesetPATH
mapped_probes <- mappedkeys(x)
# Now convert the huex10stprobesetPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
huex10stprobesetPFAM Maps between Manufacturer Identifiers and PFAM Identifiers

Description

huex10stprobesetPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
huex10stprobesetPMID

17

huex10stprobesetPMID Maps between Manufacturer Identifiers and PubMed Identifiers

Description

huex10stprobesetPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. huex10stprobesetPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When huex10stprobesetPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When huex10stprobesetPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- huex10stprobesetPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object huex10stprobesetPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
huex10stprobesetPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

huex10stprobesetPROSITE is an R object that provides mappings between manufacturer identifiers
...
huex10stprobesetREFSEQ is an R object that provides mappings between manufacturer identifiers
...
huex10stprobesetSYMBOL is an R object that provides mappings between manufacturer identifiers
...
huex10stprobesetUNIPROT is an R object that contains mappings between the manufacturer iden-
...
huex10stprobeset_dbschema(file="", show.indices=FALSE)
...
huex10stprobeset_dbconn returns a connection object to the package annotation DB. IMPOR-
TANT: Don’t call dbDisconnect on the connection object returned by huex10stprobeset_dbconn
or you will break all the AnnDbObj objects defined in this package!
huex10stprobeset_dbfile returns the path (character string) to the package annotation DB (this
is an SQLite file).
huex10stprobeset_dbschema prints the schema definition of the package annotation DB.
huex10stprobeset_dbInfo prints other information about the package annotation DB.

Value

huex10stprobeset_dbconn: a DBIConnection object representing an open connection to the pack-
age annotation DB.
huex10stprobeset_dbfile: a character string with the path to the package annotation DB.
huex10stprobeset_dbschema: none (invisible NULL).
huex10stprobeset_dbInfo: none (invisible NULL).
...
dbGetQuery(huex10stprobeset_dbconn(), "SELECT COUNT(*) FROM probes")
...
huex10stprobeset.db, 2
huex10stprobeset_dbconn, 21
huex10stprobesetACCNUM, 1
huex10stprobesetALIAS2PROBE, 2
huex10stprobesetCHR, 3
huex10stprobesetCHRLENGTHS, 4
huex10stprobesetCHRLOC, 4
huex10stprobesetENSEMBL, 5
huex10stprobesetENTREZID, 6
huex10stprobesetENZYME, 7
huex10stprobesetGENENAME, 9
huex10stprobesetGO, 10
huex10stprobesetMAP, 12
huex10stprobesetMAPCOUNTS, 13
huex10stprobesetOMIM, 13
huex10stprobesetORGANISM, 14
huex10stprobesetPATH, 15
huex10stprobesetPFAM, 16
huex10stprobesetPMID, 17
huex10stprobesetPROSITE, 18
huex10stprobesetREFSEQ, 18
huex10stprobesetSYMBOL, 20
huex10stprobesetUNIPROT, 21
...
huex10stprobeset (huex10stprobeset.db),
...
huex10stprobesetCHRLENGTHS, 4
huex10stprobesetCHRLOC, 4
huex10stprobesetCHRLOCEND

(huex10stprobesetCHRLOC), 4

huex10stprobesetENSEMBL, 5
huex10stprobesetENSEMBL2PROBE

(huex10stprobesetENSEMBL), 5

huex10stprobesetENTREZID, 6
huex10stprobesetENZYME, 7
huex10stprobesetENZYME2PROBE

(huex10stprobesetENZYME), 7

huex10stprobesetGENENAME, 9
huex10stprobesetGO, 10
huex10stprobesetGO2ALLPROBES, 11
huex10stprobesetGO2ALLPROBES
...
huex10stprobesetPATH, 15
huex10stprobesetPATH2PROBE

(huex10stprobesetPATH), 15

huex10stprobesetPFAM, 16
huex10stprobesetPMID, 17
huex10stprobesetPMID2PROBE
