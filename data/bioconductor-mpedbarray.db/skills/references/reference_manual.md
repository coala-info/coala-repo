...
mpedbarrayACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
mpedbarrayALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

mpedbarrayALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(mpedbarrayALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the mpedbarray.db annotation Package. The purpose of this package is to provide
detailed information about the mpedbarray platform. This package is updated biannually.
...
mpedbarrayCHR is an R object that provides mappings between a manufacturer identifier and the
...
mpedbarrayCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

mpedbarrayCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
mpedbarrayCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
mpedbarrayENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(mpedbarrayENSEMBL2PROBE)
if(length(xx) > 0){
...
mpedbarrayENTREZID is an R object that provides mappings between manufacturer identifiers
...
mpedbarrayENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. mpedbarrayENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the mpedbarrayENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the mpedbarrayENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In mpedbarrayENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert mpedbarrayENZYME2PROBE to a list to see inside
xx <- as.list(mpedbarrayENZYME2PROBE)
if(length(xx) > 0){
...
mpedbarrayGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
mpedbarrayGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (mpedbar-
rayGO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only the
directly evidenced terms are represented here.

mpedbarrayGO2ALLPROBES is an R object that provides mappings between a given GO identi-
fier and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than mpedbar-
rayGO2PROBE.

Details

If mpedbarrayGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If mpedbarrayGO2ALLPROBES or mpedbarrayGO2PROBE is cast as a list, each GO term maps to
...
# Convert mpedbarrayGO2ALLPROBES to a list
xx <- as.list(mpedbarrayGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package mpedbarray.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

mpedbarrayMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package mpedbarray.db.
...
mpedbarrayMGI is an R object that contains mappings between manufacturer identifiers and Jack-
...
# Convert to a list
xx <- as.list(mpedbarrayMGI2PROBE)
if(length(xx) > 0){
...
The Organism information for mpedbarray

Description

mpedbarrayORGANISM is an R object that contains a single item: a character string that names
the organism for which mpedbarray was built. mpedbarrayORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, mpedbarrayOR-
GANISM provides a simple way to programmatically extract the organism name. mpedbarray-
ORGPKG provides a simple way to programmatically extract the name of the parent organism
...
mpedbarrayPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

mpedbarrayPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the mpedbarrayPATH2PROBE object to a list
xx <- as.list(mpedbarrayPATH2PROBE)
if(length(xx) > 0){
...
mpedbarrayPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
mpedbarrayPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. mpedbarrayPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When mpedbarrayPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When mpedbarrayPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object mpedbarrayPMID2PROBE to a list
xx <- as.list(mpedbarrayPMID2PROBE)
if(length(xx) > 0){
...
mpedbarrayPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
mpedbarrayREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
mpedbarraySYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
mpedbarrayUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
mpedbarrayUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
mpedbarray_dbconn()
mpedbarray_dbfile()
mpedbarray_dbschema(file="", show.indices=FALSE)
mpedbarray_dbInfo()
...
mpedbarray_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by mpedbarray_dbconn or you will
break all the AnnDbObj objects defined in this package!
mpedbarray_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
mpedbarray_dbschema prints the schema definition of the package annotation DB.
mpedbarray_dbInfo prints other information about the package annotation DB.

Value

mpedbarray_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
mpedbarray_dbfile: a character string with the path to the package annotation DB.
mpedbarray_dbschema: none (invisible NULL).
mpedbarray_dbInfo: none (invisible NULL).
...
dbGetQuery(mpedbarray_dbconn(), "SELECT COUNT(*) FROM probes")
...
mpedbarray.db, 2
mpedbarray_dbconn, 20
mpedbarrayACCNUM, 1
mpedbarrayALIAS2PROBE, 2
mpedbarrayCHR, 3
mpedbarrayCHRLENGTHS, 4
mpedbarrayCHRLOC, 4
mpedbarrayENSEMBL, 5
mpedbarrayENTREZID, 6
mpedbarrayENZYME, 7
mpedbarrayGENENAME, 9
mpedbarrayGO, 10
mpedbarrayMAPCOUNTS, 12
mpedbarrayMGI, 12
mpedbarrayORGANISM, 13
mpedbarrayPATH, 14
mpedbarrayPFAM, 15
mpedbarrayPMID, 15
mpedbarrayPROSITE, 16
mpedbarrayREFSEQ, 17
mpedbarraySYMBOL, 18
mpedbarrayUNIGENE, 19
mpedbarrayUNIPROT, 20
...
mpedbarray_dbInfo (mpedbarray_dbconn),
...
mpedbarrayCHRLENGTHS, 4
mpedbarrayCHRLOC, 4
mpedbarrayCHRLOCEND (mpedbarrayCHRLOC),

4
mpedbarrayENSEMBL, 5
mpedbarrayENSEMBL2PROBE

(mpedbarrayENSEMBL), 5

mpedbarrayENTREZID, 6
mpedbarrayENZYME, 7
mpedbarrayENZYME2PROBE

(mpedbarrayENZYME), 7

mpedbarrayGENENAME, 9
mpedbarrayGO, 10
mpedbarrayGO2ALLPROBES, 11
mpedbarrayGO2ALLPROBES (mpedbarrayGO),

10

mpedbarrayGO2PROBE (mpedbarrayGO), 10
mpedbarrayLOCUSID (mpedbarrayENTREZID),

6

mpedbarrayMAPCOUNTS, 12
mpedbarrayMGI, 12
mpedbarrayMGI2PROBE (mpedbarrayMGI), 12
mpedbarrayORGANISM, 13
mpedbarrayORGPKG (mpedbarrayORGANISM),

13
mpedbarrayPATH, 14
mpedbarrayPATH2PROBE (mpedbarrayPATH),

14
mpedbarrayPFAM, 15
mpedbarrayPMID, 15
mpedbarrayPMID2PROBE (mpedbarrayPMID),

mpedbarray (mpedbarray.db), 2
mpedbarray.db, 2
mpedbarray_dbconn, 20
mpedbarray_dbfile (mpedbarray_dbconn),
