...
rta10transcriptclusterACCNUM is an R object that contains mappings between a manufacturer’s
...
## Bimap interface:
x <- rta10transcriptclusterACCNUM
...
rta10transcriptclusterALIAS2PROBE
...
rta10transcriptclusterALIAS is an R object that provides mappings between common gene symbol
...
xx <- as.list(rta10transcriptclusterALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the rta10transcriptcluster.db annotation Package. The purpose of this package is to
provide detailed information about the rta10transcriptcluster platform. This package is updated
biannually.
Objects in this package are accessed using the select() interface. See ?select in the AnnotationDbi
package for details.

rta10transcriptclusterCHR
...
## use of select().
ls("package:rta10transcriptcluster.db")
...
rta10transcriptclusterCHR is an R object that provides mappings between a manufacturer identifier
...
rta10transcriptclusterCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

rta10transcriptclusterCHRLENGTHS provides the length measured in base pairs for each of the
chromosomes.
...
## Bimap interface:
tt <- rta10transcriptclusterCHRLENGTHS
...
rta10transcriptclusterCHRLOC is an R object that maps manufacturer identifiers to the starting
...
## Bimap interface:
x <- rta10transcriptclusterCHRLOC
...
rta10transcriptclusterENSEMBL is an R object that contains mappings between manufacturer iden-
...
rta10transcriptclusterENTREZID
...
## Bimap interface:
x <- rta10transcriptclusterENSEMBL
...
x <- rta10transcriptclusterENSEMBL2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
rta10transcriptclusterENTREZID is an R object that provides mappings between manufacturer
identifiers and Entrez Gene identifiers.

rta10transcriptclusterENZYME
...
## Bimap interface:
x <- rta10transcriptclusterENTREZID
...
rta10transcriptclusterENZYME is an R object that provides mappings between manufacturer iden-
tifiers and EC numbers. rta10transcriptclusterENZYME2PROBE is an R object that maps Enzyme
...
When the rta10transcriptclusterENZYME maping viewed as a list, each manufacturer identifier
...
vector will contain an NA.
For the rta10transcriptclusterENZYME2PROBE, each EC number maps to a named vector contain-
...
numbers. In rta10transcriptclusterENZYME2PROBE, EC is dropped from the Enzyme Commis-
sion numbers.
...
## Bimap interface:
x <- rta10transcriptclusterENZYME
...
rta10transcriptclusterGENENAME
...
# Now convert rta10transcriptclusterENZYME2PROBE to a list to see inside
x <- rta10transcriptclusterENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
rta10transcriptclusterGENENAME is an R object that maps manufacturer identifiers to the corre-
sponding gene name.
...
## Bimap interface:
x <- rta10transcriptclusterGENENAME
# Get the probe identifiers that are mapped to a gene name
mapped_probes <- mappedkeys(x)

10

rta10transcriptclusterGO
...
rta10transcriptclusterGO is an R object that provides mappings between manufacturer identifiers
and the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(rta10transcriptclusterGO2PROBE) do NOT associate the child terms from the GO ontology with
the gene. Only the directly evidenced terms are represented here.

rta10transcriptclusterGO2ALLPROBES is an R object that provides mappings between a given GO
...
rta10transcriptclusterGO2PROBE.

Details

If rta10transcriptclusterGO is cast as a list, each manufacturer identifier is mapped to a list of lists.
...
If rta10transcriptclusterGO2ALLPROBES or rta10transcriptclusterGO2PROBE is cast as a list,
...
• rta10transcriptclusterGO2ALLPROBES
• AnnotationDb-class for use of the select() interface.
...
# For the reverse map:
x <- rta10transcriptclusterGO2PROBE
mapped_genes <- mappedkeys(x)
# Convert to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
rta10transcriptclusterORGANISM

x <- rta10transcriptclusterGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert rta10transcriptclusterGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
rta10transcriptclusterMAPCOUNTS
...
rta10transcriptclusterMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for
each map in package rta10transcriptcluster.db.
...
The Organism information for rta10transcriptcluster

Description

rta10transcriptclusterORGANISM is an R object that contains a single item: a character string that
names the organism for which rta10transcriptcluster was built. rta10transcriptclusterORGPKG is
...
rta10transcriptclusterPATH

Details

13

Although the package name is suggestive of the organism for which it was built, rta10transcriptclusterORGANISM
provides a simple way to programmatically extract the organism name. rta10transcriptclusterORGPKG
...
rta10transcriptclusterPATH maps probe identifiers to the identifiers used by KEGG for pathways in
which the genes represented by the probe identifiers are involved

rta10transcriptclusterPATH2PROBE is an R object that provides mappings between KEGG identi-
...
x <- rta10transcriptclusterPATH
mapped_probes <- mappedkeys(x)
# Now convert the rta10transcriptclusterPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rta10transcriptclusterPFAM is an R object that provides mappings between manufacturer identifiers
...
rta10transcriptclusterPMID is an R object that provides mappings between manufacturer identifiers
and PubMed identifiers. rta10transcriptclusterPMID2PROBE is an R object that provides mappings
between PubMed identifiers and manufacturer identifiers.

Details

When rta10transcriptclusterPMID is viewed as a list each manufacturer identifier is mapped to a
...
When rta10transcriptclusterPMID2PROBE is viewed as a list each PubMed identifier is mapped
...
x <- rta10transcriptclusterPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object rta10transcriptclusterPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rta10transcriptclusterPROSITE

Maps between Manufacturer Identifiers and PROSITE Identifiers

Description

rta10transcriptclusterPROSITE is an R object that provides mappings between manufacturer iden-
...
rta10transcriptclusterREFSEQ is an R object that provides mappings between manufacturer identi-
...
## Bimap interface:
x <- rta10transcriptclusterREFSEQ
...
rta10transcriptclusterSYMBOL is an R object that provides mappings between manufacturer iden-
...
## Bimap interface:
x <- rta10transcriptclusterSYMBOL
...
rta10transcriptclusterUNIPROT is an R object that contains mappings between the manufacturer
...
## Bimap interface:
x <- rta10transcriptclusterUNIPROT
...
rta10transcriptcluster_dbconn returns a connection object to the package annotation DB. IM-
PORTANT: Don’t call dbDisconnect on the connection object returned by rta10transcriptcluster_dbconn
or you will break all the AnnDbObj objects defined in this package!
rta10transcriptcluster_dbfile returns the path (character string) to the package annotation
DB (this is an SQLite file).
rta10transcriptcluster_dbschema prints the schema definition of the package annotation DB.
rta10transcriptcluster_dbInfo prints other information about the package annotation DB.

Value

rta10transcriptcluster_dbconn: a DBIConnection object representing an open connection to
the package annotation DB.
rta10transcriptcluster_dbfile: a character string with the path to the package annotation DB.
rta10transcriptcluster_dbschema: none (invisible NULL).
rta10transcriptcluster_dbInfo: none (invisible NULL).
...
rta10transcriptcluster_dbconn, 19
rta10transcriptclusterACCNUM, 1
rta10transcriptclusterALIAS2PROBE,

2

rta10transcriptclusterCHR, 3
rta10transcriptclusterCHRLENGTHS,

4

rta10transcriptclusterCHRLOC, 4
rta10transcriptclusterENSEMBL, 5
rta10transcriptclusterENTREZID, 6
rta10transcriptclusterENZYME, 7
rta10transcriptclusterGENENAME, 9
rta10transcriptclusterGO, 10
rta10transcriptclusterMAPCOUNTS,

12

rta10transcriptclusterORGANISM, 12
rta10transcriptclusterPATH, 13
rta10transcriptclusterPFAM, 14
rta10transcriptclusterPMID, 15
rta10transcriptclusterPROSITE, 16
rta10transcriptclusterREFSEQ, 16
rta10transcriptclusterSYMBOL, 18
rta10transcriptclusterUNIPROT, 19
...
rta10transcriptclusterACCNUM, 1
rta10transcriptclusterALIAS2PROBE, 2
rta10transcriptclusterCHR, 3
rta10transcriptclusterCHRLENGTHS, 4
rta10transcriptclusterCHRLOC, 4
rta10transcriptclusterCHRLOCEND

(rta10transcriptclusterCHRLOC),
4

rta10transcriptclusterENSEMBL, 5
rta10transcriptclusterENSEMBL2PROBE
...
rta10transcriptclusterENTREZID, 6
rta10transcriptclusterENZYME, 7
rta10transcriptclusterENZYME2PROBE
...
rta10transcriptclusterGENENAME, 9
rta10transcriptclusterGO, 10
rta10transcriptclusterGO2ALLPROBES, 11
rta10transcriptclusterGO2ALLPROBES
...
rta10transcriptclusterLOCUSID

(rta10transcriptclusterENTREZID),
6

rta10transcriptclusterMAPCOUNTS, 12
rta10transcriptclusterORGANISM, 12
rta10transcriptclusterORGPKG
...
rta10transcriptclusterPATH2PROBE
...
rta10transcriptclusterPFAM, 14
rta10transcriptclusterPMID, 15
rta10transcriptclusterPMID2PROBE
...
rta10transcriptclusterSYMBOL, 18
rta10transcriptclusterUNIPROT, 19
rta10transcriptclusterUNIPROT2PROBE
