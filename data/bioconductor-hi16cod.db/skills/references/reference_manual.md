...
hi16codACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hi16codALIAS is an R object that provides mappings between common gene symbol identifiers
...
Welcome to the hi16cod.db annotation Package. The purpose of this package is to provide detailed
information about the hi16cod platform. This package is updated biannually.
...
hi16codCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hi16codCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hi16codCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hi16codENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hi16codENSEMBL2PROBE)
if(length(xx) > 0){
...
hi16codENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hi16codENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hi16codENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hi16codENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hi16codENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hi16codENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hi16codENZYME2PROBE to a list to see inside
xx <- as.list(hi16codENZYME2PROBE)
if(length(xx) > 0){
...
hi16codGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hi16codGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hi16codGO2PROBE)
...
hi16codGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hi16codGO2PROBE.

Details

If hi16codGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hi16codGO2ALLPROBES or hi16codGO2PROBE is cast as a list, each GO term maps to a
...
# Convert hi16codGO2ALLPROBES to a list
xx <- as.list(hi16codGO2ALLPROBES)
if(length(xx) > 0){
...
hi16codMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hi16cod.db

Description

hi16codMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hi16cod.db.
...
hi16codMAPCOUNTS
mapnames <- names(hi16codMAPCOUNTS)
hi16codMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package hi16cod.db
checkMAPCOUNTS("hi16cod.db")
...
hi16codOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hi16cod

Description

hi16codORGANISM is an R object that contains a single item: a character string that names the
organism for which hi16cod was built. hi16codORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hi16codORGANISM
provides a simple way to programmatically extract the organism name. hi16codORGPKG provides
...
hi16codPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hi16codPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the hi16codPATH2PROBE object to a list
xx <- as.list(hi16codPATH2PROBE)
if(length(xx) > 0){
...
hi16codPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hi16codPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hi16codPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hi16codPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
# Now convert the reverse map object hi16codPMID2PROBE to a list

hi16codREFSEQ

15

xx <- as.list(hi16codPMID2PROBE)
if(length(xx) > 0){
...
hi16codREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hi16codSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hi16codUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hi16codUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hi16cod_dbconn()
hi16cod_dbfile()
hi16cod_dbschema(file="", show.indices=FALSE)
hi16cod_dbInfo()
...
hi16cod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hi16cod_dbconn or you will break all the
AnnDbObj objects defined in this package!
hi16cod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
hi16cod_dbschema prints the schema definition of the package annotation DB.
hi16cod_dbInfo prints other information about the package annotation DB.

Value

hi16cod_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
hi16cod_dbfile: a character string with the path to the package annotation DB.
hi16cod_dbschema: none (invisible NULL).
hi16cod_dbInfo: none (invisible NULL).
...
dbGetQuery(hi16cod_dbconn(), "SELECT COUNT(*) FROM probes")
...
hi16cod (hi16cod.db), 2
hi16cod.db, 2
hi16cod_dbconn, 18
hi16cod_dbfile (hi16cod_dbconn), 18

20

hi16cod_dbInfo (hi16cod_dbconn), 18
hi16cod_dbschema (hi16cod_dbconn), 18
hi16codACCNUM, 1
hi16codALIAS2PROBE, 2
hi16codCHR, 3
hi16codCHRLENGTHS, 3
hi16codCHRLOC, 4
hi16codCHRLOCEND (hi16codCHRLOC), 4
hi16codENSEMBL, 4
hi16codENSEMBL2PROBE (hi16codENSEMBL), 4
hi16codENTREZID, 5
hi16codENZYME, 6
hi16codENZYME2PROBE (hi16codENZYME), 6
hi16codGENENAME, 7
hi16codGO, 8
hi16codGO2ALLPROBES, 9
hi16codGO2ALLPROBES (hi16codGO), 8
hi16codGO2PROBE (hi16codGO), 8
hi16codLOCUSID (hi16codENTREZID), 5
hi16codMAP, 10
hi16codMAPCOUNTS, 11
hi16codOMIM, 11
hi16codORGANISM, 12
hi16codORGPKG (hi16codORGANISM), 12
hi16codPATH, 13
hi16codPATH2PROBE (hi16codPATH), 13
hi16codPMID, 14
hi16codPMID2PROBE (hi16codPMID), 14
hi16codREFSEQ, 15
hi16codSYMBOL, 16
hi16codUNIGENE, 17
hi16codUNIPROT, 17
hi16codUNIPROT2PROBE (hi16codUNIPROT),
