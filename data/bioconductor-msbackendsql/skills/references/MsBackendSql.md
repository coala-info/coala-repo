...
**Package**: *[MsBackendSql](https://bioconductor.org/packages/3.22/MsBackendSql)*
...
**Compiled**: Mon Nov 3 18:55:09 2025

# 1 Introduction

The *[Spectra](https://bioconductor.org/packages/3.22/Spectra)* Bioconductor package provides a flexible and
...
*[MsBackendSql](https://bioconductor.org/packages/3.22/MsBackendSql)* package provides backends to store data from whole
...
## Database: /tmp/RtmpC7fSDt/file820cd6a9d44fb
```
...
##  ... 35 more variables/columns.
## Processing:
##  Switch backend from MsBackendOfflineSql to MsBackendMemory [Mon Nov  3 18:55:20 2025]
```
...
##  ... 35 more variables/columns.
##  Use  'spectraVariables' to list all of them.
## Database: /tmp/RtmpC7fSDt/file820cd54577c11
## Processing:
##  Switch backend from MsBackendOfflineSql to MsBackendMemory [Mon Nov  3 18:55:20 2025]
##  Switch backend from MsBackendMemory to MsBackendOfflineSql [Mon Nov  3 18:55:20 2025]
```
...
Performance comparison have been made for small and large data sets using
different SQL database systems and *MsBackendSql* has been optimized based on
these results. For *MariaDB* database systems, for example, the
[Aria](https://mariadb.com/docs/server/server-usage/storage-engines/aria)
storage engine is used by default as it has considerable advantages over other
MariaDB engines.

Performance of *MariaDB* and *SQLite* is comparable, even for very large data
sets/databases. See this [GitHub
issue](https://github.com/rformassspectrometry/MsBackendSql/issues/15) for
performance comparison between MariaDB and SQLite.

Performance evaluation of *SQLite* and *duckdb* are provided in this [GitHub
issue](https://github.com/rformassspectrometry/MsBackendSql/issues/26).
*MsBackendSql* long format database layout (see next section for details on
available database layouts) with *duckdb* is clearly faster than with
*SQLite*. For the *blob2* database layout *SQLite* has advantages. Also,
extracting individual spectra variables or filtering by e.g. retention time is
slower for *duckdb*.
...
##      msLevel(sps) 8214.177 8904.1915 9585.53110 9279.514 9922.9145 14654.251
##  msLevel(sps_mzr)  615.618  659.7465  710.88096  685.854  750.2915   993.371
##   msLevel(sps_im)   14.339   27.4600   39.01911   41.275   50.0540    76.636
##  neval cld
##    100 a
##    100  b
##    100   c
...
## Unit: microseconds
##                                         expr        min         lq       mean
##      peaksData(sps, BPPARAM = SerialParam())  61116.682  65260.119 232410.735
##  peaksData(sps_mzr, BPPARAM = SerialParam()) 754786.573 763237.518 774855.997
##   peaksData(sps_im, BPPARAM = SerialParam())    571.914    794.876   3154.053
##      median         uq       max neval cld
##  104181.497 430184.455 551926.68    10 a
##  770621.998 788497.268 799954.39    10  b
##    1226.092   1421.421  20507.05    10   c
```
...
##      peaksData(sps, BPPARAM = m2)  58.803833  68.364609  150.616171  83.249863
##  peaksData(sps_mzr, BPPARAM = m2) 648.860562 669.851170 1077.358761 746.710112
##   peaksData(sps_im, BPPARAM = m2)   1.117974   1.244154    1.479758   1.327181
##           uq         max neval cld
##    96.463024  781.029335    10  a
##  1707.493239 2210.360764    10   b
##     1.788325    1.968326    10  a
```
...
## Unit: microseconds
##                                expr      min       lq      mean   median
##      filterRt(sps, rt = c(50, 100)) 2848.986 2960.337 3352.6823 3018.552
##  filterRt(sps_mzr, rt = c(50, 100)) 1953.555 2064.823 2222.1288 2177.143
##   filterRt(sps_im, rt = c(50, 100))  607.399  671.291  856.4695  706.214
##        uq       max neval cld
##  3133.059 24947.420   100 a
##  2289.818  4773.844   100  b
##   748.397 14766.912   100   c
```
...
##      sps[idx]  201.204  222.669  260.3827  254.8645  265.8525 1545.526   100
##  sps_mzr[idx] 1025.662 1055.394 1075.2057 1062.6330 1076.3805 1944.211   100
##   sps_im[idx]  304.235  325.105  345.9781  349.2225  360.5375  398.288   100
##  cld
##  a
##   b
##    c
...
##      peaksData(sps_10)  1289.267  1366.867  1682.1900  1628.932  1848.890
##  peaksData(sps_mzr_10) 11410.758 11591.941 12180.0183 12300.123 12494.204
##   peaksData(sps_im_10)   323.975   344.620   378.9944   390.466   400.356
##        max neval cld
##   2553.339    10 a
##  13037.741    10  b
##    445.226    10   c
...
## [43] fs_1.6.6               IRanges_2.44.0         bit_4.6.0
...
## [58] prettyunits_1.2.0      MetaboCoreUtils_1.18.0
```