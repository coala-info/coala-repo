...
Bernat Gel (bgel@igtp.cat), Anna Diez-Villanueva (adiez@iconcologia.net) and Roberto Malinverni (rmalinverni@carrerasresearch.org)
...
* [3 Permutation tests](#permutation-tests)
  + [3.1 How does a permutation test work?](#how-does-a-permutation-test-work)
  + [3.2 How to perform a permutation test with *regioneR*](#how-to-perform-a-permutation-test-with-regioner)
  + [3.3 A note on the number of permutations](#a-note-on-the-number-of-permutations)
  + [3.4 Randomization Functions](#randomization-functions)
    - [3.4.1 randomizeRegions](#randomizeregions)
    - [3.4.2 circularRandomizeRegions](#circularrandomizeregions)
    - [3.4.3 resampleRegions](#resampleregions)
  + [3.5 Evaluation Functions](#evaluation-functions)
    - [3.5.1 numOverlaps](#numoverlaps)
    - [3.5.2 meanDistance](#meandistance)
    - [3.5.3 meanInRegion](#meaninregion)
  + [3.6 Custom Functions](#custom-functions)
    - [3.6.1 Custom Evaluation](#custom-evaluation)
    - [3.6.2 Custom Randomization](#custom-randomization)
  + [3.7 A note on reproducibility](#a-note-on-reproducibility)
* [4 Local Z-score](#local-z-score)
* [5 Region Sets](#region-sets)
* [6 Genomes and Masks](#genomes-and-masks)
  + [6.1 Genomes](#genomes)
  + [6.2 Masks](#masks)
  + [6.3 How to retrieve a genome and mask](#how-to-retrieve-a-genome-and-mask)
  + [6.4 Filtering Chromosomes](#filtering-chromosomes)
* [7 Region Set Helper Functions](#region-set-helper-functions)
  + [7.1 Functions operating on a single RS](#functions-operating-on-a-single-rs)
  + [7.2 Functions operating on two RS](#functions-operating-on-two-rs)
  + [7.3 Other Functions: functions not returning a `GRanges` object](#other-functions-functions-not-returning-a-granges-object)
* [8 Usage Examples](#usage-examples)
  + [8.1 Example 1: CpG Islands and Gene Promoters. A basic example.](#example-1-cpg-islands-and-gene-promoters.-a-basic-example.)
    - [8.1.1 Question: Are Cpg islands and promoters associated?](#question-are-cpg-islands-and-promoters-associated)
  + [8.2 Example 2: Analysis of ChIP-seq peaks. Advanced usage and local Z-score.](#example-2-analysis-of-chip-seq-peaks.-advanced-usage-and-local-z-score.)
    - [8.2.1 Question: In the cell line Hepg2 (Human hepatocellular carcinoma ), is Rad21 (Cohesin) associated with CTCF? And with gene promoters? Are these associations different?](#question-in-the-cell-line-hepg2-human-hepatocellular-carcinoma-is-rad21-cohesin-associated-with-ctcf-and-with-gene-promoters-are-these-associations-different)
* [9 Benchmarks and Performance](#benchmarks-and-performance)
  + [9.1 Randomization Functions](#randomization-functions-1)
* [10 Technical Considerations](#technical-considerations)
  + [10.1 Memoisation](#memoisation)
...
In addition, *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* includes a set of helper functions based on Bioconductor’s *[GenomicRanges](https://bioconductor.org/packages/3.22/GenomicRanges)* infrastructure to manage and manipulate region sets with a simple and consistent interface.

# 2 Quick Start

One of the most common uses of *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* is to answer the question: Do the regions in set A overlap with the regions in B more than expected? For example: Are my ChIP-seq peaks on the promoters of active genes? or Are the break points in repetitive regions? To answer question like these we can use the convenience `overlapPermTest` function in regioneR, that evaluates the number of overlaps between two sets of regions.
...
Which in this case shows that moving the regions in A produces a drop in the z-scores and so, shows that the association is dependant on the exact position of the regions and is not a regional effect. More information about the `localZScore` function can be found at section [Local Z-score](#local-z-score).
...
## 3.1 How does a permutation test work?
...
To help with that, many randomization functions provided by *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* accept a mask, indicating where a random region cannot be placed. In any case, selecting the best randomization strategy is a key part of a permutation test and can have a great impact in the final results.

The least restricted function included in *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* is `randomizeRegions`, that given a RS, a genome and an optional mask, returns a new RS with the same number of regions and of the same width as the original ones but randomly placed along the non-masked parts of the genome. This is also the slowest of the randomization functions available.
...
## 3.2 How to perform a permutation test with *[regioneR](https://bioconductor.org/packages/3.22/regioneR)*

The main function to perform a permutation test with *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* is `permTest`, a function taking a a region set (RS) in any of the accepted formats (see section 4 - Region Sets), a randomization function and an evaluation function and returning a `permTestResults` object.
...
## Tests Results:
##                 pvalue zscore    test
## numOverlaps 0.01960784 9.2666 greater
```
...
In *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* , a randomization function is any function that takes a RS as an argument and returns a RS with the randomized regions. Different randomization functions are included in the package, but it is also possible to create custom randomization functions for more specific needs (see section 4 - Region Sets).

### 3.4.1 randomizeRegions
...
##          <Rle>         <IRanges>  <Rle>
##   [1]     chr5 54829332-54831332      *
##   [2]     chr4 62031963-62051963      *
##   [3]     chrX   3133032-3433032      *
##   -------
##   seqinfo: 298 sequences from an unspecified genome; no seqlengths
```
...
##     seqnames              ranges strand
##        <Rle>           <IRanges>  <Rle>
##   1     chr1 146003475-146005475      *
##   2     chr1 146033475-146053475      *
##   3     chr1 146083475-146383475      *
##   -------
##   seqinfo: 1 sequence from an unspecified genome; no seqlengths
```

### 3.4.3 resampleRegions
...
The evaluation functions are the functions that define what is being tested in the permutation test. Some evaluation functions are provided in *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* covering the most common uses but it is possible (and easy) to create your own custom evaluation functions (see section 2.6.1). In general, an evaluation function is simply a function taking a RS and returning a single numeric value.
...
## 3.7 A note on reproducibility

In order to create reproducible code with functions that use random numbers such as the permutation testing in *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* , it is necessary to use `set.seed`. However, since regioneR uses *[parallel](https://bioconductor.org/packages/3.22/parallel)* to perform the test it is also necessary to set the `mc.set.seed` parameter to FALSE to ensure reproducibility.
...
```
## $numOverlaps
## P-value: 0.0909090909090909
## Z-score: 5.8304
## Number of iterations: 10
## Note: less than 20 iterations might produce unreliable results
## Alternative: greater
## Evaluation of the original region set: 26
## Evaluation function: numOverlaps
## Randomization function: randomizeRegions
...
In *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* , a genomic region is a part of a genome and is defined by three parameters: chromosome, start and end, where start is less than or equal to end and both are inside the limits of the chromosome. This definition includes from single bases (start equal to end) to whole chromosomes. In addition, genomic regions can have associated data or annotations which will usually depend on the nature of the data. A region set, then, is defined as a group of one or more genomic regions and is referred to as RS along this document.

Internally, *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* uses `GRanges` to represent a RS, and all functions returning an RS return a `GRanges` object. However, *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* accepts different data formats as input thanks to its `toGRanges` function. In particular, it accepts:

* **GRanges:** A `GRanges` object from the *[GenomicRanges](https://bioconductor.org/packages/3.22/GenomicRanges)* Bioconductor package.
...
* **A file:** a file name or connection to a file in any of the formats accepted by *[rtracklayer](https://bioconductor.org/packages/3.22/rtracklayer)*’s import function (bed, gff, …)
...
Two utility functions are provided in *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* to manage the transformation to and from `GRanges`: `toGRanges` and `toDataframe`.
...
## 6.3 How to retrieve a genome and mask
...
*[regioneR](https://bioconductor.org/packages/3.22/regioneR)* includes a set of helper functions based on the *[GenomicRanges](https://bioconductor.org/packages/3.22/GenomicRanges)* infrastructure to manage and manipulate RS: `joinRegions`, `commonRegions`, `mergeRegions`, `overlapRegions`, etc… All these functions share a standard and simple signature, with one or two RS in any of the accepted formats and almost all of them (except `splitRegions` and `overlapRegions`) return a `GRanges` object.

## 7.1 Functions operating on a single RS
...
## 7.2 Functions operating on two RS
...
## 7.3 Other Functions: functions not returning a `GRanges` object

* **overlapRegions:** `overlapRegions` is one of most powerful and useful RS management functions in *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* . It has a number of parameters and types of returned values. For a thorough description, refer to the regioneR user guide.
...
In this section we will present a series of examples of how to use *[regioneR](https://bioconductor.org/packages/3.22/regioneR)* to answer real biological questions.

## 8.1 Example 1: CpG Islands and Gene Promoters. A basic example.

### 8.1.1 Question: Are Cpg islands and promoters associated?

#### 8.1.1.1 Brief Problem Description:
...
We will use the CpG islands described in Wu et al. 2010 and the promoters from UCSC defined as -2000 to +200 base pairs to the TSS according to the genome assembly hg19. The promoters file can be dowloaded from <http://gattaca.imppc.org/regioner/data/UCSC.promoters.hg19.bed>.
...
Association between CpG islands and gene promoters
...
## 8.2 Example 2: Analysis of ChIP-seq peaks. Advanced usage and local Z-score.

### 8.2.1 Question: In the cell line Hepg2 (Human hepatocellular carcinoma ), is Rad21 (Cohesin) associated with CTCF? And with gene promoters? Are these associations different?

#### 8.2.1.1 Brief Problem Description:
...
Association of Rad21 peaks with CTCF and gene promoters.
...
Local Z-score analysis of the association of Rad21 peaks with CTCF and gene promoters with a 1Kbp window.
...
Local Z-score analysis of the association of Rad21 peaks with CTCF and gene promoters with a 10Kbp window.
...
The effect of the subset size in the association testing.
...
## [13] cigarillo_1.0.0             GenomeInfoDb_1.46.0
## [15] restfulr_0.0.16             tinytex_0.57
## [17] BiocManager_1.30.26         httr_1.4.7
## [19] UCSC.utils_1.6.0            XML_3.99-0.19
...
## [35] memoise_2.0.1               Rsamtools_2.26.0
## [37] SummarizedExperiment_1.40.0 curl_7.0.0
## [39] R6_2.6.1                    magick_2.9.0
## [41] matrixStats_1.5.0           lifecycle_1.0.4
## [43] bslib_0.9.0                 Rcpp_1.1.0
## [45] xfun_0.53                   GenomicAlignments_1.46.0
## [47] MatrixGenerics_1.22.0       rjson_0.2.23
## [49] htmltools_0.5.8.1           rmarkdown_2.30
## [51] compiler_4.5.1              RCurl_1.98-1.17
```