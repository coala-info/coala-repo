...
* [Tutorial](tutorial.html)
  + Advanced Step-by-step peak calling using MACS3 commands
    - [Step 1: Filter duplicates](#step-1-filter-duplicates)
    - [Step 2: Decide the fragment length `d`](#step-2-decide-the-fragment-length-d)
    - [Step 3: Extend ChIP sample to get ChIP coverage track](#step-3-extend-chip-sample-to-get-chip-coverage-track)
    - [Step 4: Build local bias track from control](#step-4-build-local-bias-track-from-control)
      * [The `d` background](#the-d-background)
      * [The slocal background](#the-slocal-background)
      * [The llocal background](#the-llocal-background)
      * [The genome background](#the-genome-background)
      * [Combine and generate the maximum background noise](#combine-and-generate-the-maximum-background-noise)
    - [Step 5: Scale the ChIP and control to the same sequencing depth](#step-5-scale-the-chip-and-control-to-the-same-sequencing-depth)
    - [Step 6: Compare ChIP and local lambda to get the scores in pvalue or qvalue](#step-6-compare-chip-and-local-lambda-to-get-the-scores-in-pvalue-or-qvalue)
    - [Step 7: Call peaks on score track using a cutoff](#step-7-call-peaks-on-score-track-using-a-cutoff)
  + [Cutoff Analysis](cutoffanalysis.html)
* [Common Q & A](qa.html)
...
* [Tutorial](tutorial.html)
* Advanced Step-by-step peak calling using MACS3 commands
* [View page source](../_sources/docs/Advanced_Step-by-step_Peak_Calling.md.txt)

---

# Advanced Step-by-step peak calling using MACS3 commands[](#advanced-step-by-step-peak-calling-using-macs3-commands "Link to this heading")
...
## Step 1: Filter duplicates[](#step-1-filter-duplicates "Link to this heading")
...
details, check `macs3 filterdup -h`. If you opt for the `--keep-dup auto` setting, ensure the genome size is set appropriately. MACS3 will
...
output in `BEDPE` format. Check [this document for detail](BEDPE.html)
on this `BEDPE` format.

## Step 2: Decide the fragment length `d`[](#step-2-decide-the-fragment-length-d "Link to this heading")
...
This function will print out that `# Average insertion length of all pairs is 253 bps`. You should write down this number `253` for
building the control bias track for pair-end data in Step 4.

## Step 3: Extend ChIP sample to get ChIP coverage track[](#step-3-extend-chip-sample-to-get-chip-coverage-track "Link to this heading")
...
## Step 4: Build local bias track from control[](#step-4-build-local-bias-track-from-control "Link to this heading")
...
### The `d` background[](#the-d-background "Link to this heading")
...
### The slocal background[](#the-slocal-background "Link to this heading")
...
### The llocal background[](#the-llocal-background "Link to this heading")
...
### The genome background[](#the-genome-background "Link to this heading")
...
### Combine and generate the maximum background noise[](#combine-and-generate-the-maximum-background-noise "Link to this heading")
...
## Step 5: Scale the ChIP and control to the same sequencing depth[](#step-5-scale-the-chip-and-control-to-the-same-sequencing-depth "Link to this heading")
...
## Step 6: Compare ChIP and local lambda to get the scores in pvalue or qvalue[](#step-6-compare-chip-and-local-lambda-to-get-the-scores-in-pvalue-or-qvalue "Link to this heading")
...
```
$ macs3 bdgcmp -t CTCF_ChIP_200K_filterdup.pileup.bdg -c local_lambda.bdg -m p