...
baseCommand:
  - pdftk
label: pdftk_generate_fdf
doc: "Reads a single input PDF file and generates an FDF file suitable for fill_form
  out of it to the given output filename or (if no output is given) to stdout. Does
  not create a new PDF.\n\nTool homepage: https://github.com/mikehaertl/php-pdftk"
inputs:
  - id: input_pdf
    type: File
    doc: The input PDF file from which to generate the FDF data.
    inputBinding:
      position: 1
  - id: operation
    type: string
  ...
  - id: input_pw
    type:
      - 'null'
      - string
    doc: Input PDF owner password(s). Use handles if necessary (e.g., 
      A=password).
    inputBinding:
      position: 103
      prefix: input_pw
  - id: keep_final_id
    type:
      - 'null'
      - boolean
    doc: Copy the document ID from the final input document into the output.
    inputBinding:
      position: 103
  ...
  - id: keep_first_id
    type:
      - 'null'
      - boolean
    doc: Copy the document ID from the first input document into the output.
    inputBinding:
      position: 103
  ...
  - id: need_appearances
    type:
      - 'null'
      - boolean
    doc: Sets a flag for Reader/Acrobat to generate new field appearances.
    inputBinding:
      position: 103
  ...
  - id: owner_pw
    type:
      - 'null'
      - string
    doc: Set the owner password for the output file.
    inputBinding:
      position: 103
      prefix: owner_pw
  - id: uncompress
  ...
  - id: user_pw
    type:
      - 'null'
      - string
    doc: Set the user password for the output file.
    inputBinding:
      position: 103
      prefix: user_pw
  - id: verbose
...
outputs:
  - id: output
    type:
      - 'null'
      - File
    doc: The output FDF filename. If omitted, output goes to stdout.
    outputBinding:
      glob: $(inputs.output)
