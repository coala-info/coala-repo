...
inputs:
  ...
  - id: rep_names
    type:
      - 'null'
      - File
    doc: An input file that the sample names of replicates can be found. The 
      first element of each row contains the name of a base sample, and every 
      other element in a row contains the name of a sample based off the base 
      sample. This file is required to run -a, --get_avgs.
    inputBinding:
      position: 101
      prefix: --rep_names
  - id: col_sums_path
...
outputs:
  ...
  - id: col_sums
    ...
    outputBinding:
      glob: $(inputs.col_sums_path)
  - id: get_avgs
    type:
      - 'null'
      - File
    doc: Name of a file to which the average of different replicate values 
      should be written. Output will be a tab-delimted file with sample names as
      the column headers and peptide names as row names. Each entry consists of 
      the average of the replicate values for the given sample and peptide.
    outputBinding:
      glob: $(inputs.get_avgs_path)
  - id: logfile
    type:
      - 'null'
      - File
    doc: Designated file to which the module's processes are logged. By default,
      the logfile's name will include the module's name and the time the module 
      started running.
    outputBinding:
