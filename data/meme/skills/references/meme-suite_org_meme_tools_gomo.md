The motifs to scan with and determine associated GO terms.

[ close ]

Select the species' whose promoters wll be scanned for matches
...
See the GOMo
[species database documentation](../db/gomo)
for more information on the available promoter databases.

[ close ]

The threshold above which results are not considered significant.

[ close ]

GOMo calculates empirical *p*-values by shuffling the association
...
Some datasets are setup to use multiple genomes but you can force GOMo
to only use the main genome by deselecting this option.
...
![GOMo Logo](../doc/images/gomo_icon.png)
...
Perform GO term association with DNA motifs.

## Input the motifs

#### Upload or enter nucleotide motifs to analyze for association with GO terms.
...
#### Choose which species' promoters to analyze.
...
### Set the number of score shuffling rounds

Rounds:

### Use multiple genomes?

[x]
Use multiple genomes when available.
