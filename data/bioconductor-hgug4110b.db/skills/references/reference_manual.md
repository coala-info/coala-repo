...
hgug4110bACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgug4110bALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4110bALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4110bALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4110b.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4110b platform. This package is updated biannually.
...
hgug4110bCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4110bCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4110bCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
hgug4110bENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgug4110bENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4110bENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4110bENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgug4110bENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgug4110bENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgug4110bENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In hgug4110bENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgug4110bENZYME2PROBE to a list to see inside
xx <- as.list(hgug4110bENZYME2PROBE)
if(length(xx) > 0){
...
hgug4110bGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4110bGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgug4110bGO2PROBE)
...
hgug4110bGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgug4110bGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgug4110bGO2ALLPROBES or hgug4110bGO2PROBE is cast as a list, each GO term maps to
...
# Convert hgug4110bGO2ALLPROBES to a list
xx <- as.list(hgug4110bGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4110bMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4110b.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgug4110bMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4110b.db.
...
hgug4110bOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4110b

Description

hgug4110bORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4110b was built. hgug4110bORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgug4110bORGANISM
provides a simple way to programmatically extract the organism name. hgug4110bORGPKG pro-
...
hgug4110bPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgug4110bPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hgug4110bPATH2PROBE object to a list
xx <- as.list(hgug4110bPATH2PROBE)
if(length(xx) > 0){
...
hgug4110bPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgug4110bPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgug4110bPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgug4110bPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgug4110bPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgug4110bPMID2PROBE to a list
xx <- as.list(hgug4110bPMID2PROBE)
if(length(xx) > 0){
...
hgug4110bPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgug4110bREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4110bSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4110bUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4110bUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgug4110b_dbconn()
hgug4110b_dbfile()
hgug4110b_dbschema(file="", show.indices=FALSE)
hgug4110b_dbInfo()
...
hgug4110b_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4110b_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgug4110b_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgug4110b_dbschema prints the schema definition of the package annotation DB.
hgug4110b_dbInfo prints other information about the package annotation DB.

Value

hgug4110b_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
hgug4110b_dbfile: a character string with the path to the package annotation DB.
hgug4110b_dbschema: none (invisible NULL).
hgug4110b_dbInfo: none (invisible NULL).
...
dbGetQuery(hgug4110b_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgug4110b_dbconn, 21
hgug4110bACCNUM, 1
hgug4110bALIAS2PROBE, 2
hgug4110bCHR, 3
hgug4110bCHRLENGTHS, 4
hgug4110bCHRLOC, 4
hgug4110bENSEMBL, 5
hgug4110bENTREZID, 6
hgug4110bENZYME, 7
...
hgug4110bGO, 10
hgug4110bMAP, 12
hgug4110bMAPCOUNTS, 13
hgug4110bOMIM, 13
hgug4110bORGANISM, 14
hgug4110bPATH, 15
hgug4110bPFAM, 16
hgug4110bPMID, 16
hgug4110bPROSITE, 17
...
hgug4110b (hgug4110b.db), 2
hgug4110b.db, 2
hgug4110b_dbconn, 21

hgug4110b_dbfile (hgug4110b_dbconn), 21
hgug4110b_dbInfo (hgug4110b_dbconn), 21
hgug4110b_dbschema (hgug4110b_dbconn),
...
hgug4110bCHRLENGTHS, 4
hgug4110bCHRLOC, 4
hgug4110bCHRLOCEND (hgug4110bCHRLOC), 4
hgug4110bENSEMBL, 5
hgug4110bENSEMBL2PROBE

(hgug4110bENSEMBL), 5

hgug4110bENTREZID, 6
hgug4110bENZYME, 7
hgug4110bENZYME2PROBE

(hgug4110bENZYME), 7

hgug4110bGENENAME, 9
hgug4110bGO, 10
hgug4110bGO2ALLPROBES, 11
hgug4110bGO2ALLPROBES (hgug4110bGO), 10
hgug4110bGO2PROBE (hgug4110bGO), 10
hgug4110bLOCUSID (hgug4110bENTREZID), 6
hgug4110bMAP, 12
hgug4110bMAPCOUNTS, 13
hgug4110bOMIM, 13
hgug4110bORGANISM, 14
hgug4110bORGPKG (hgug4110bORGANISM), 14
hgug4110bPATH, 15
hgug4110bPATH2PROBE (hgug4110bPATH), 15
hgug4110bPFAM, 16
hgug4110bPMID, 16
hgug4110bPMID2PROBE (hgug4110bPMID), 16
hgug4110bPROSITE, 17
hgug4110bREFSEQ, 18
hgug4110bSYMBOL, 19
hgug4110bUNIGENE, 20
hgug4110bUNIPROT, 21
hgug4110bUNIPROT2PROBE
