...
hgug4101aACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hgug4101aALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hgug4101aALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgug4101aALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgug4101a.db annotation Package. The purpose of this package is to provide
detailed information about the hgug4101a platform. This package is updated biannually.
...
hgug4101aCHR is an R object that provides mappings between a manufacturer identifier and the
...
hgug4101aCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hgug4101aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgug4101aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(hgug4101aENSEMBL2PROBE)
if(length(xx) > 0){
...
hgug4101aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgug4101aENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgug4101aENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgug4101aENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgug4101aENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hgug4101aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgug4101aENZYME2PROBE to a list to see inside
xx <- as.list(hgug4101aENZYME2PROBE)
if(length(xx) > 0){
...
hgug4101aGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hgug4101aGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgug4101aGO2PROBE)
...
hgug4101aGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgug4101aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgug4101aGO2ALLPROBES or hgug4101aGO2PROBE is cast as a list, each GO term maps to
...
# Convert hgug4101aGO2ALLPROBES to a list
xx <- as.list(hgug4101aGO2ALLPROBES)
if(length(xx) > 0){
...
hgug4101aMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgug4101a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgug4101aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hgug4101a.db.
...
hgug4101aOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hgug4101a

Description

hgug4101aORGANISM is an R object that contains a single item: a character string that names
the organism for which hgug4101a was built. hgug4101aORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgug4101aORGANISM
provides a simple way to programmatically extract the organism name. hgug4101aORGPKG pro-
...
hgug4101aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgug4101aPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hgug4101aPATH2PROBE object to a list
xx <- as.list(hgug4101aPATH2PROBE)
if(length(xx) > 0){
...
hgug4101aPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hgug4101aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgug4101aPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgug4101aPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgug4101aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hgug4101aPMID2PROBE to a list
xx <- as.list(hgug4101aPMID2PROBE)
if(length(xx) > 0){
...
hgug4101aPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgug4101aREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgug4101aSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgug4101aUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hgug4101aUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hgug4101a_dbconn()
hgug4101a_dbfile()
hgug4101a_dbschema(file="", show.indices=FALSE)
hgug4101a_dbInfo()
...
hgug4101a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgug4101a_dbconn or you will break all
the AnnDbObj objects defined in this package!
hgug4101a_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hgug4101a_dbschema prints the schema definition of the package annotation DB.
hgug4101a_dbInfo prints other information about the package annotation DB.

Value

hgug4101a_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.
hgug4101a_dbfile: a character string with the path to the package annotation DB.
hgug4101a_dbschema: none (invisible NULL).
hgug4101a_dbInfo: none (invisible NULL).
...
dbGetQuery(hgug4101a_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgug4101a_dbconn, 21
hgug4101aACCNUM, 1
hgug4101aALIAS2PROBE, 2
hgug4101aCHR, 3
hgug4101aCHRLENGTHS, 4
hgug4101aCHRLOC, 4
hgug4101aENSEMBL, 5
hgug4101aENTREZID, 6
hgug4101aENZYME, 7
...
hgug4101aGO, 10
hgug4101aMAP, 12
hgug4101aMAPCOUNTS, 13
hgug4101aOMIM, 13
hgug4101aORGANISM, 14
hgug4101aPATH, 15
hgug4101aPFAM, 16
hgug4101aPMID, 16
hgug4101aPROSITE, 17
...
hgug4101a (hgug4101a.db), 2
hgug4101a.db, 2
hgug4101a_dbconn, 21

hgug4101a_dbfile (hgug4101a_dbconn), 21
hgug4101a_dbInfo (hgug4101a_dbconn), 21
hgug4101a_dbschema (hgug4101a_dbconn),
...
hgug4101aCHRLENGTHS, 4
hgug4101aCHRLOC, 4
hgug4101aCHRLOCEND (hgug4101aCHRLOC), 4
hgug4101aENSEMBL, 5
hgug4101aENSEMBL2PROBE

(hgug4101aENSEMBL), 5

hgug4101aENTREZID, 6
hgug4101aENZYME, 7
hgug4101aENZYME2PROBE

(hgug4101aENZYME), 7

hgug4101aGENENAME, 9
hgug4101aGO, 10
hgug4101aGO2ALLPROBES, 11
hgug4101aGO2ALLPROBES (hgug4101aGO), 10
hgug4101aGO2PROBE (hgug4101aGO), 10
hgug4101aLOCUSID (hgug4101aENTREZID), 6
hgug4101aMAP, 12
hgug4101aMAPCOUNTS, 13
hgug4101aOMIM, 13
hgug4101aORGANISM, 14
hgug4101aORGPKG (hgug4101aORGANISM), 14
hgug4101aPATH, 15
hgug4101aPATH2PROBE (hgug4101aPATH), 15
hgug4101aPFAM, 16
hgug4101aPMID, 16
hgug4101aPMID2PROBE (hgug4101aPMID), 16
hgug4101aPROSITE, 17
hgug4101aREFSEQ, 18
hgug4101aSYMBOL, 19
hgug4101aUNIGENE, 20
hgug4101aUNIPROT, 21
hgug4101aUNIPROT2PROBE
