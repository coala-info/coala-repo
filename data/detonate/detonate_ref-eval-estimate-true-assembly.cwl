...
doc: "A program to estimate the \"true\" assembly of a set of reads, relative to a
...
inputs:
  - id: alignment_policy
    ...
    doc: 'The policy used to choose which alignment(s) of each read to use in constructing
      ...
      the alignment that maximizes the posterior probability mentioned above. Ties
      are broken arbitrarily but deterministically (the first alignment in the BAM
      file is used). all: For each read, use all its alignments. Some reads might
      end up with more than one alignment. In that case, contigs will be made assuming
      that the read aligns to each place. (In other words, the read is effectively
      duplicated, with one copy per alignment.) This policy is applied after the thresholding
      implied by --min-alignment-prob. For example, if "--min-alignment-prob=0.10
      --alignment-policy=sample" is given, then (first) all alignments with posterior
      probability less than 0.10 will be discarded, and (second), for each read, an
      alignment will be sampled from among the remaining alignments, with the posterior
  ...
  - id: min_alignment_prob
    type:
      - 'null'
      - float
    doc: A number between 0 and 1 (inclusive). Any alignment (of a read to a 
  ...
  - id: paired_end
    type:
      - 'null'
      - boolean
    doc: If you have paired-end data, and you want to estimate the "true" 
