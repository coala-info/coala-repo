...
* Example Workflow
  ...
  + [Step 3 : Change your miniconda path](#step-3-change-your-miniconda-path)
  + [Step 4: Edit snakemake/cluster.yaml](#step-4-edit-snakemake-cluster-yaml)
...
[Manual](cmd-manual.html "next chapter")

### This Page

* [Show Source](_sources/example_workflow.rst.txt)

1. [Docs](index.html)
2. Example Workflow

# Example Workflow[¶](#example-workflow "Permalink to this headline")

We will be working with the first published Ribo-seq dataset [GSE13750](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE13750) from Ingolia et al. (2009) which has samples for [both](https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP000637) mRNA-seq and Ribo-seq from **Yeast** grown in starved and nutrient rich media.

At this point, we assume you have already completed all
the steps under “Installing dependencies” section of the README.

## Step 1: Downloading datasets[¶](#step-1-downloading-datasets "Permalink to this headline")
...
[![_images/dag.svg](_images/dag.svg)](_images/dag.svg)

## Step 2: Copy template[¶](#step-2-copy-template "Permalink to this headline")
...
## Step 3 : Change your miniconda path[¶](#step-3-change-your-miniconda-path "Permalink to this headline")

Edit [Line4 snakemake/jobscript.sh](https://github.com/saketkc/riboraptor/blob/47c8a50753c2bcc96b57d43b525a47bb8fde2d04/snakemake/jobscript.sh#L4) pointing to your conda root directory.
...
## Step 4: Edit snakemake/cluster.yaml[¶](#step-4-edit-snakemake-cluster-yaml "Permalink to this headline")

Edit [Line6 snakemake/cluster.yaml](https://github.com/saketkc/riboraptor/blob/47c8a50753c2bcc96b57d43b525a47bb8fde2d04/snakemake/cluster.yaml#L6) and [Line7 snakemake/cluster.yaml](https://github.com/saketkc/riboraptor/blob/47c8a50753c2bcc96b57d43b525a47bb8fde2d04/snakemake/cluster.yaml#L7) to point
...
## Step 5: Submit job[¶](#step-5-submit-job "Permalink to this headline")
...
## Visualizing Results[¶](#visualizing-results "Permalink to this headline")

When the entire pipeline as run, it will create an html file riboraptor\_report.html as output. You can copy it locally to visualize metagene profiles and read length distribution for all samples.

[Installation](installation.html "previous chapter (use the left arrow)")

[Manual](cmd-manual.html "next chapter (use the right arrow)")
