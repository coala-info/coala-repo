...
- id: verbosity
  ...
  inputBinding:
    position: 102
    prefix: --verbosity
- id: read_other_out_name
  type: string?
  doc: filename for reads designated READ_OTHER
  inputBinding:
    position: 102
    prefix: '0'
- id: read1_out_name
  type: string?
  doc: filename for reads designated READ1
  inputBinding:
    position: 102
    prefix: "-1"
- id: read2_out_name
  type: string?
  doc: filename for reads designated READ2
  inputBinding:
    position: 102
    prefix: "-2"
- id: output_file_name
  type: string?
  doc: filename for reads designated READ1 or READ2
  inputBinding:
    position: 102
    prefix: -o
- id: singleton_out_name
  type: string?
  doc: filename for singleton reads designated READ1 or READ2
  inputBinding:
    position: 102
    prefix: -s
- id: index1_out_name
  type: string?
  doc: filename for first index reads
  inputBinding:
    position: 102
    prefix: --index1
- id: index2_out_name
  type: string?
  doc: filename for second index reads
  inputBinding:
    position: 102
    prefix: --index2
outputs:
- id: read_other_out
  type: File?
  outputBinding:
    glob: $(inputs.read_other_out_name)
- id: read1_out
  type: File?
  outputBinding:
    glob: $(inputs.read1_out_name)
- id: read2_out
  type: File?
  outputBinding:
    glob: $(inputs.read2_out_name)
- id: output_file
  type: File?
  outputBinding:
...
- id: singleton_out
  type: File?
  outputBinding:
    glob: $(inputs.singleton_out_name)
- id: index1_out
  type: File?
  outputBinding:
    glob: $(inputs.index1_out_name)
- id: index2_out
  type: File?
  outputBinding:
    glob: $(inputs.index2_out_name)
- id: stdout_file
  type: stdout
stdout: output.fasta
hints:
