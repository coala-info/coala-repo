...
* Articles
  + [Ascertain diagnoses](articles/ascertain_diagnoses.html)
  + [Label fields](articles/label_fields.html)
  + [Spark functions](articles/spark_functions.html)
...
* 🧬 [Genetics](#genetic-variants): extract genotypes from Bulk data, create polygenic score
* 📋 [Diagnoses](#ascertain-diagnoses): ascertain from health records and self-reported illness data, determine date first diagnosed
* 🛠️ [Utilities](#utility-functions): Check field names, label data fields, upload/download files from RAP, and pull phenotypes from Spark
...
`[extract_variants()](reference/extract_variants.html)` by default uses [bgenix](https://enkre.net/cgi-bin/code/bgen) and [plink](https://www.cog-genomics.org/plink/) to subset the [imputed BGEN files](https://biobank.ctsu.ox.ac.uk/crystal/field.cgi?id=22828) and read it quickly and easily into R.

The only required input is a data frame (or path to file) containing “rsid” and “chr” variables. See function documentation for further details/options (including the available `[make_imputed_bed()](reference/make_imputed_bed.html)` and `[load_bed()](reference/load_bed.html)` internal functions).
...
You can use function `[get_imputed_variant_info()](reference/get_imputed_variant_info.html)` to get the UK Biobank imputed genotype variant IDs, alleles, MAF, and INFO from the MFI files. Provide a data.frame containing `chr` and `pos` fields (in build 37).
...
The highlight of developing this feature was naming the internal function `[make_dragen_bed()](reference/make_dragen_bed.html)` 🐉 🛏️
...
`[create_pgs()](reference/create_pgs.html)` takes a data frame containing a list of variant associations with a trait and creates a weighted allele score using [plink](https://www.cog-genomics.org/plink/1.9/score). By default it uses the [imputed](https://biobank.ctsu.ox.ac.uk/crystal/field.cgi?id=22828) genotypes.

The only required input is a data frame (or path to file) containing rsid, chr, pos, effect\_allele, other\_allele, beta. For DRAGEN pos is build 38.
...
This only needs to happen once per project. Run `[export_tables()](reference/export_tables.html)` to submit the `table-exporter` jobs to save the required files to the RAP persistent storage. ~10Gb of text files are created, costing ~£0.15 per month to store.
...
Identify the date first diagnosed for each participant from any of datasets searched with `[get_diagnoses()](reference/get_diagnoses.html)` (cause of death, HES diagnoses, GP clinical, cancer registry, HES operations, and self-reported illness fields).
...
The default `[get_df()](reference/get_df.html)` behaviour is to use all available codes. However the most time-efficient way to get multiple conditions is to run `[get_diagnoses()](reference/get_diagnoses.html)` once for all codes for the conditions you wish to ascertain, then get the “date first diagnosed” for each condition separately. In the codes data frame you just need a field indicating the condition name, that will become the variable prefixes.
...
* Check if field IDs are valid and return phenotype names with `[fields_to_phenos()](reference/fields_to_phenos.html)`
* Label UK Biobank data fields with `[label_ukb_fields()](reference/label_ukb_fields.html)`
* Upload/download files between worker and RAP with `[upload_to_rap()](reference/upload_to_rap.html)` and `[download_from_rap()](reference/download_from_rap.html)`
* Pull phenotypes from Spark instance with `[get_rap_phenos()](reference/get_rap_phenos.html)`
...
* [Browse source code](https://github.com/lcpilling/ukbrapR/)
* [Report a bug](https://github.com/lcpilling/ukbrapR/issues)
...
* [Citing ukbrapR](authors.html#citation)

## Developers

* [Luke Pilling](https://lcpilling.github.io)
   Author, maintainer
...
Developed by [Luke Pilling](https://lcpilling.github.io).
