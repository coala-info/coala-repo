...
hthgu133plusaACCNUM is an R object that contains mappings between a manufacturer’s identi-
...
hthgu133plusaALIAS is an R object that provides mappings between common gene symbol identi-
...
xx <- as.list(hthgu133plusaALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hthgu133plusa.db annotation Package. The purpose of this package is to provide
detailed information about the hthgu133plusa platform. This package is updated biannually.
...
hthgu133plusaCHR is an R object that provides mappings between a manufacturer identifier and
...
hthgu133plusaCHRLENGTHS

A named vector for the length of each of the chromosomes

Description

hthgu133plusaCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hthgu133plusaCHRLOC is an R object that maps manufacturer identifiers to the starting position
...
hthgu133plusaENSEMBL Map Ensembl gene accession numbers with Entrez Gene identifiers

Description

hthgu133plusaENSEMBL is an R object that contains mappings between manufacturer identifiers
...
hthgu133plusaENTREZID Map between Manufacturer Identifiers and Entrez Gene

Description

hthgu133plusaENTREZID is an R object that provides mappings between manufacturer identifiers
...
hthgu133plusaENZYME is an R object that provides mappings between manufacturer identifiers
and EC numbers. hthgu133plusaENZYME2PROBE is an R object that maps Enzyme Commission
(EC) numbers to manufacturer identifiers.

Details

When the hthgu133plusaENZYME maping viewed as a list, each manufacturer identifier maps to
...
For the hthgu133plusaENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In hthgu133plusaENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hthgu133plusaENZYME2PROBE to a list to see inside

hthgu133plusaGENENAME

9

x <- hthgu133plusaENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hthgu133plusaGENENAME Map between Manufacturer IDs and Genes

Description

hthgu133plusaGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hthgu133plusaGO is an R object that provides mappings between manufacturer identifiers and
the GO identifiers that they are directly associated with. This mapping and its reverse mapping
(hthgu133plusaGO2PROBE) do NOT associate the child terms from the GO ontology with the
gene. Only the directly evidenced terms are represented here.

hthgu133plusaGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
hthgu133plusaGO2PROBE.

Details

If hthgu133plusaGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hthgu133plusaGO2ALLPROBES or hthgu133plusaGO2PROBE is cast as a list, each GO term
...
x <- hthgu133plusaGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hthgu133plusaGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hthgu133plusaMAP is an R object that provides mappings between manufacturer identifiers and
cytoband locations.
...
Number of mapped keys for the maps in package hthgu133plusa.db

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hthgu133plusaMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each
map in package hthgu133plusa.db.
...
hthgu133plusaOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
hthgu133plusaORGANISM The Organism information for hthgu133plusa

Description

hthgu133plusaORGANISM is an R object that contains a single item: a character string that names
the organism for which hthgu133plusa was built. hthgu133plusaORGPKG is an R object that con-
...
Although the package name is suggestive of the organism for which it was built, hthgu133plusaORGANISM
provides a simple way to programmatically extract the organism name. hthgu133plusaORGPKG
...
hthgu133plusaPATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

hthgu133plusaPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hthgu133plusaPATH
mapped_probes <- mappedkeys(x)
# Now convert the hthgu133plusaPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133plusaPFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hthgu133plusaPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hthgu133plusaPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hthgu133plusaPMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hthgu133plusaPMID2PROBE is viewed as a list each PubMed identifier is mapped to a
...
x <- hthgu133plusaPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hthgu133plusaPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hthgu133plusaPROSITE Maps between Manufacturer Identifiers and PROSITE Identifiers
...
hthgu133plusaPROSITE is an R object that provides mappings between manufacturer identifiers
...
hthgu133plusaREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
