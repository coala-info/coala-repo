...
* [Auspice](../index.html)
* [Releases](index.html)
* [View page source](../_sources/releases/v2.rst.txt)

---

# Auspice Version 2.0[](#auspice-version-2-0 "Link to this heading")
...
## Pie charts to represent discrete variables on a map[](#pie-charts-to-represent-discrete-variables-on-a-map "Link to this heading")
...
![pie-charts](../_images/v2-pie-charts.png) *Notice the difference? Auspice v1 (left) & v2 (right)*

## New dataset JSON format[](#new-dataset-json-format "Link to this heading")
...
### Strings parsed unchanged[](#strings-parsed-unchanged "Link to this heading")

For historical reasons, we used to “prettify” certain strings in Auspice - e.g. “usa” became “USA”, “new\_zealand” became “New Zealand”. We dug ourselves into a bit of a hole here, as you can imagine the list of exceptions kept growing and growing. Auspice v2 now generally parses strings as they appear in the JSON, allowing you full control over how things appear. (For backwards compatability, we’ve kept the “prettifying” function in the v1-v2 conversion logic so the look of v1 JSONs shouldn’t change.)

### Both metadata and tree data in a single JSON[](#both-metadata-and-tree-data-in-a-single-json "Link to this heading")
...
### Gene / Genome definitions are now in GFF format.[](#gene-genome-definitions-are-now-in-gff-format "Link to this heading")

v1 Auspice JSONs used 0-based starts for the gene positions and `1`/`-1` for which strand the gene was on. We now use one-based start & end coordinates, following GFF format, and a “+” or “-” to denote the strand. These definitions are now under the `genome_annotations` property (formerly `annotations`). See [PR #770](https://github.com/nextstrain/auspice/pull/770).

### Changes to how node data is stored[](#changes-to-how-node-data-is-stored "Link to this heading")
...
### Colorings, geographic resolutions, and defaults[](#colorings-geographic-resolutions-and-defaults "Link to this heading")

The `colorings` property (formerly `color_options`) is now an array of objects, the properties of which are easier to understand. This guarantees the ordering appears in Auspice as you define it in the JSON. See [PR #748](https://github.com/nextstrain/auspice/pull/748).

The `geo_resolutions` property (formerly `geo`) is similarly an array of objects.

The `display_defaults` property (formerly `defaults`) now contains keys which are snake\_case instead of camelCase.

### Multiple maintainers[](#multiple-maintainers "Link to this heading")
...
### Continous, Categorical, Ordinal, and Boolean Color Scales[](#continous-categorical-ordinal-and-boolean-color-scales "Link to this heading")
...
## More information in tree info boxes[](#more-information-in-tree-info-boxes "Link to this heading")
...
![more-tree-info](../_images/v2-tree-info.png) *Auspice v1 (left) & v2 (right). v2 shows more information on both tree hover (upper panel) & when clicking on tips (lower panel).*

## Display of second trees[](#display-of-second-trees "Link to this heading")

Auspice has had the ability to display two trees side-by-side for a while now (and [finally documented](../advanced-functionality/second-trees.html)). If you wanted to, say, compare influenza HA & NA trees, the URL used to look like “flu/seasonal/h3n2/ha:na/2y”. This turned out to be problematic when coming up with suitable candidates for potential second-trees, and also made it impossible to compare, for instance, “ha/2y” with “na/3y”
...
P.S. The list of available second trees, which is displayed in the sidebar, is now handed to Auspice by the [getAvailable API request](../server/api.html#server-api-charon-getavailable).

## Display better dates on the tree axis[](#display-better-dates-on-the-tree-axis "Link to this heading")

Internally, we use decimal dates (e.g. 2012.3 is around the start of may) so that’s what we displayed on the tree. It turns out this is pretty hard to interpret when looking at small timespans! We now (a) show dates on the tree’s x-axis using months & days, depending on the timespan displayed, and (b) try to use more informative grid spacings. These help with the interpretation of trees over smaller time scales. See [PR #804](https://github.com/nextstrain/auspice/pull/804).

![time-labels](../_images/v2-time-labels.png) *Above: Auspice v1’s decimal labels were somewhat hard to interprete. Below: v2 displays calendar dates as appropriate, and uses more intelligent grid spacing.*

## Map “reset zoom” button zooms to include all demes[](#map-reset-zoom-button-zooms-to-include-all-demes "Link to this heading")

There’s now a button at the top-right of the map which will trigger the map to reset the zoom. See [PR #802](https://github.com/nextstrain/auspice/pull/802).

## Consistent colouring of missing data in the tree[](#consistent-colouring-of-missing-data-in-the-tree "Link to this heading")

If your analysis produces results in `-` (gaps), `X` (unknown residue) or `N` (unknown nucelotide) then we now colour these grey, making it much easier to see when data is missing. See [PR #799](https://github.com/nextstrain/auspice/pull/799).

![base-colours](../_images/v2-base-colours.png) *Same analysis, different colour schemes, different interpretation.*

## Removal of Twitter & Google Analytics[](#removal-of-twitter-google-analytics "Link to this heading")

These were a holdover from the early days when [nextstrain.org](https://nextstrain.org) and Auspice were the same thing. We’ve now removed all calls to Twitter, and made Google Analytics opt in. See [requests made from the client](../customise-client/requests.html) for details on exactly what requests are made and how to opt-in to Google Analytics if you desire.

## Improvements in the entropy panel[](#improvements-in-the-entropy-panel "Link to this heading")

We improved the usability of the entropy (genomic diversity) panel, as well as fixing a few hidden bugs – see [PR #771](https://github.com/nextstrain/auspice/pull/771). For instance, you can now see which codon a nucleotide codes for (and vice-versa).

![entropy](../_images/v2-entropy.gif)

entropy[](#id1 "Link to this image")

## Auspice responds to server redirects for datasets[](#auspice-responds-to-server-redirects-for-datasets "Link to this heading")

This allows custom servers ([nextstrain.org](https://nextstrain.org), for instance!) to smoothly inform Auspice that, e.g., a `getDataset` request to “/flu” (which doesn’t actually exist) should be “/flu/seasonal/h3n2/ha/3y”. See [PR #778](https://github.com/nextstrain/auspice/pull/778).

## Importing (server) code from Auspice[](#importing-server-code-from-auspice "Link to this heading")

Auspice now makes a few helper commands available for those who are writing custom Auspice servers. See [these docs](../server/api.html#server-api-importing-code-from-auspice) for more info.

## New Auspice subcommand: `auspice convert`[](#new-auspice-subcommand-auspice-convert "Link to this heading")
...
Right now, `auspice view` will automatically convert “v1” JSONs into “v2” JSONs, so there’s no need to do this yoursel