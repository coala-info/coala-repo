...
* [GNU Datamash](https://www.gnu.org/software/datamash/) - Numeric, textual and statistical operations on TSV files. This tool has many similarities to [tsv-summarize](/tsv-utils/docs/tool_reference/tsv-summarize.html). Written in C.
...
Tradeoffs between file formats is its own topic. Appropriate choice of format is often dependent on the specifics of the environment and tasks being performed. See [Comparing TSV and CSV formats](/tsv-utils/docs/comparing-tsv-and-csv.html) for a discussion of TSV and CSV formats. The [brendano/tsvutils README](https://github.com/brendano/tsvutils#the-philosophy-of-tsvutils) (Brendan O'Conner) has a nice discussion of the rationale for using TSV files.