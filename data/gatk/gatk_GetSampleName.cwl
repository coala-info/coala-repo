...
baseCommand:
  - gatk
  - GetSampleName
label: gatk_GetSampleName
doc: Emit a single sample name from the bam header into an output file. The 
  sample name is that in the read group (RG) sample (SM) field
inputs:
  - id: input
    type:
      type: array
  ...
  - id: use_url_encoding
    type:
      - 'null'
      - boolean
    doc: Apply URL encoding to convert spaces and other special characters in 
      sample name.
    inputBinding:
      position: 101
      prefix: --use-url-encoding
