# pmultiqc CWL Generation Report

## pmultiqc_multiqc

### Tool Description
MultiQC aggregates results from bioinformatics analyses across many samples into a single report. It searches a given directory for analysis logs and compiles an HTML report. It's a general use tool, perfect for summarising the output from numerous bioinformatics tools. To run, supply with one or more directory to scan for analysis results. For example, to run in the current working directory, use 'multiqc .'

### Metadata
- **Docker Image**: quay.io/biocontainers/pmultiqc:0.0.40--pyhdfd78af_0
- **Homepage**: https://github.com/bigbio/pmultiqc/
- **Package**: https://anaconda.org/channels/bioconda/packages/pmultiqc/overview
- **Validation**: PASS

- **Conda**: https://anaconda.org/channels/bioconda/packages/pmultiqc/overview
...
│ --affix-type                      Prefix (default) or suffix (TEXT)          │
│ --condition                       Create conditions from provided (e.g.,     │
│                                   factor) columns. (TEXT)                    │
│ --contaminant-affix               The contaminant prefix or suffix used or   │
│                                   to be used (default: CONT) (TEXT)          │
│ --decoy-affix                     The decoy prefix or suffix used or to be   │
│                                   used (default: DECOY_) (TEXT)              │
│ --diann-plugin                    Enable DIA-NN plugin                       │
│ --disable-hoverinfo               Disable hoverinfo                          │
│ --disable-table                   disable protein/peptide table plots for    │
│                                   large dataset                              │
│ --fragpipe-plugin                 Enable FragPipe plugin                     │
│ --ignored-idxml                   ignored idxml files for faster running     │
│ --keep-raw                        Keep filenames in experimental design      │
│                                   output as raw.                             │
│ --maxquant-plugin                 Enable MaxQuant plugin                     │
│ --mzid-plugin                     Extract mzIdentML                          │
│ --pmultiqc-version                                                           │
│ --proteobench-plugin              Enable ProteoBench plugin                  │
│ --quantification-method           The quantification method for LFQ          │
│                                   experiment (default: feature_intensity)    │
│                                   (feature_intensity|spectral_counting)      │
│ --quantms-plugin                  Enable quantms plugin                      │
│ --remove-decoy/--no-remove-decoy  Remove decoy peptides when counting        │
│                                   (default: True). Use --no-remove-decoy to  │
│                                   disable.                                   │
╰──────────────────────────────────────────────────────────────────────────────╯
