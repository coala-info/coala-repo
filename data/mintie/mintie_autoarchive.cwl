...
baseCommand: bpipe
label: mintie_autoarchive
doc: "Manage and execute pipelines\n\nTool homepage: https://github.com/Oshlack/MINTIE"
inputs:
  - id: command
    type: string
    doc: Command to execute (e.g., run, test, debug, retry, remake, resume, 
      stop, history, log, jobs, checks, override, status, cleanup, query, 
      preallocate, archive, autoarchive, preserve, register, diagram, 
      diagrameditor)
    inputBinding:
      position: 1
  - id: archive_file
    type:
      - 'null'
      - File
    doc: Path to the zip file for archive command
    inputBinding:
      position: 2
  - id: job_id
    type:
      - 'null'
      - string
    doc: Job ID for retry or log command
    inputBinding:
      position: 3
  - id: output_files
    type:
      ...
      - type: array
        items: File
    doc: Output files for remake command
    inputBinding:
      position: 4
  - id: pipeline
    type:
