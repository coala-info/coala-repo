...
ri16codACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
ri16codALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the ri16cod.db annotation Package. The purpose of this package is to provide detailed
information about the ri16cod platform. This package is updated biannually.
...
ri16codCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
ri16codCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
ri16codCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
ri16codENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(ri16codENSEMBL2PROBE)
if(length(xx) > 0){
...
ri16codENTREZID is an R object that provides mappings between manufacturer identifiers and
...
ri16codENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. ri16codENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the ri16codENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the ri16codENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In ri16codENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert ri16codENZYME2PROBE to a list to see inside
xx <- as.list(ri16codENZYME2PROBE)
if(length(xx) > 0){
...
ri16codGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
ri16codGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (ri16codGO2PROBE)
...
ri16codGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than ri16codGO2PROBE.

Details

If ri16codGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If ri16codGO2ALLPROBES or ri16codGO2PROBE is cast as a list, each GO term maps to a named
...
# Convert ri16codGO2ALLPROBES to a list
xx <- as.list(ri16codGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package ri16cod.db

Description

ri16codMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package ri16cod.db.
...
ri16codMAPCOUNTS
mapnames <- names(ri16codMAPCOUNTS)
ri16codMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package ri16cod.db
checkMAPCOUNTS("ri16cod.db")
...
The Organism information for ri16cod

Description

ri16codORGANISM is an R object that contains a single item: a character string that names the
organism for which ri16cod was built. ri16codORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, ri16codORGANISM
provides a simple way to programmatically extract the organism name. ri16codORGPKG provides
...
ri16codPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

ri16codPATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
# Now convert the ri16codPATH2PROBE object to a list
xx <- as.list(ri16codPATH2PROBE)
if(length(xx) > 0){
...
ri16codPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. ri16codPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When ri16codPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When ri16codPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
# Now convert the reverse map object ri16codPMID2PROBE to a list
xx <- as.list(ri16codPMID2PROBE)
if(length(xx) > 0){
...
ri16codREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
ri16codSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
ri16codUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
ri16codUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
ri16cod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by ri16cod_dbconn or you will break all the
AnnDbObj objects defined in this package!

ri16cod_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
...
ri16cod_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
...
ri16cod_dbschema (ri16cod_dbconn), 16
ri16codACCNUM, 1
ri16codALIAS2PROBE, 2
ri16codCHR, 3
ri16codCHRLENGTHS, 3
ri16codCHRLOC, 4
ri16codCHRLOCEND (ri16codCHRLOC), 4
ri16codENSEMBL, 4
ri16codENSEMBL2PROBE (ri16codENSEMBL), 4
ri16codENTREZID, 5
ri16codENZYME, 6
ri16codENZYME2PROBE (ri16codENZYME), 6
ri16codGENENAME, 7
ri16codGO, 8
ri16codGO2ALLPROBES, 9
ri16codGO2ALLPROBES (ri16codGO), 8
ri16codGO2PROBE (ri16codGO), 8
ri16codLOCUSID (ri16codENTREZID), 5
ri16codMAPCOUNTS, 10
ri16codORGANISM, 11
ri16codORGPKG (ri16codORGANISM), 11
ri16codPATH, 11
ri16codPATH2PROBE (ri16codPATH), 11
ri16codPMID, 12
ri16codPMID2PROBE (ri16codPMID), 12
ri16codREFSEQ, 13
ri16codSYMBOL, 14
ri16codUNIGENE, 15
ri16codUNIPROT, 16
ri16codUNIPROT2PROBE (ri16codUNIPROT),
...
ri16cod (ri16cod.db), 2
ri16cod.db, 2
ri16cod_dbconn, 16
ri16cod_dbfile (ri16cod_dbconn), 16
ri16cod_dbInfo (ri16cod_dbconn), 16
