...
| assembly | Assemble linked reads into a genome. The linked-read barcodes must be in BX:Z or BC:Z FASTQ header tags. It is strongly recommended to first deconvolve the input FASTQ files with harpy deconvolve. |
| convert | [deprecated] Convert between linked-read formats. This module of Harpy has been deprecated and its function has been moved to the Djinn package. |
| deconvolve | Resolve barcode sharing in unrelated molecules. Provide the input fastq files and/or directories at the end of the command. |
| demultiplex | Demultiplex haplotagged FASTQ files. Check that you are using the correct haplotagging method/technology, since the different barcoding approaches have very different demultiplexing strategies. |
| deps | Locally install workflow dependencies. These commands are intended only for situations on HPCs where conda cannot be installed or the worker nodes do not have internet access to download conda/apptainer workflow dependencies. |
| diagnose | Attempt to resolve workflow errors |
| impute | Impute variant genotypes from alignments. Provide the parameter file followed by the input VCF and the input alignment files/directories (.bam) at the end of the command. |
| metassembly | Assemble linked reads into a metagenome. The linked-read barcodes must be in BX:Z or BC:Z FASTQ header tags. It is strongly recommended to first deconvolve the input FASTQ files with harpy deconvolve. |
| phase | Phase SNPs into haplotypes. Provide the vcf file followed by the input alignment (.bam) files and/or directories. |
| qc | FASTQ adapter removal, quality filtering, etc. Linked-read presence and type is auto-detected. |
| resume | Continue an incomplete Harpy workflow by bypassing preprocessing steps and executing the Snakemake command present in the target directory. |
...
| snp | Call SNPs and small indels from alignments. Provide an additional subcommand mpileup or freebayes to get more information on using those variant callers. They are both robust variant callers, but freebayes is recommended when ploidy is greater than 2. |
| sv | Call inversions, deletions, and duplications from alignments using LEVIATHAN or NAIBR. |
| template | Create files and HPC configs for workflows. All commands write to stdout. Use hpc-* and impute without arguments. |
| validate | File format checks for linked-read data. This is useful to make sure your input files are formatted correctly for the processing pipeline before you are surprised by errors hours into an analysis. |
| view | View a workflow's components. These convenient commands let you view/edit the latest workflow log file, snakefile, snakemake parameter file, workflow config file in a directory that was used for the output of a Harpy run. |
