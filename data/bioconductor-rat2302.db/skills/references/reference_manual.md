...
rat2302ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
rat2302ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the rat2302.db annotation Package. The purpose of this package is to provide detailed
information about the rat2302 platform. This package is updated biannually.
...
rat2302CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
rat2302CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rat2302CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
rat2302ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
rat2302ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
rat2302ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. rat2302ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the rat2302ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the rat2302ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In rat2302ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert rat2302ENZYME2PROBE to a list to see inside

rat2302GENENAME

9

x <- rat2302ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
rat2302GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rat2302GO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (rat2302GO2PROBE)
...
rat2302GO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than rat2302GO2PROBE.

Details

If rat2302GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If rat2302GO2ALLPROBES or rat2302GO2PROBE is cast as a list, each GO term maps to a named
...
Number of mapped keys for the maps in package rat2302.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rat2302MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package rat2302.db.
...
The Organism information for rat2302

Description

rat2302ORGANISM is an R object that contains a single item: a character string that names the
organism for which rat2302 was built. rat2302ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, rat2302ORGANISM
provides a simple way to programmatically extract the organism name. rat2302ORGPKG provides
...
rat2302PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

rat2302PATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- rat2302PATH
mapped_probes <- mappedkeys(x)
# Now convert the rat2302PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rat2302PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rat2302PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rat2302PMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When rat2302PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When rat2302PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
x <- rat2302PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object rat2302PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rat2302PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
rat2302REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
rat2302SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
rat2302UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
rat2302_dbconn()
rat2302_dbfile()
rat2302_dbschema(file="", show.indices=FALSE)
rat2302_dbInfo()
...
rat2302_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by rat2302_dbconn or you will break all the
AnnDbObj objects defined in this package!

rat2302_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

rat2302_dbschema prints the schema definition of the package annotation DB.

rat2302_dbInfo prints other information about the package annotation DB.

Value

rat2302_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

rat2302_dbfile: a character string with the path to the package annotation DB.

rat2302_dbschema: none (invisible NULL).

rat2302_dbInfo: none (invisible NULL).
...
dbGetQuery(rat2302_dbconn(), "SELECT COUNT(*) FROM probes")
...
rat2302ACCNUM, 1
rat2302ALIAS2PROBE, 2
rat2302CHR, 3
rat2302CHRLENGTHS, 4
rat2302CHRLOC, 4
rat2302CHRLOCEND (rat2302CHRLOC), 4
rat2302ENSEMBL, 5
rat2302ENSEMBL2PROBE (rat2302ENSEMBL), 5
rat2302ENTREZID, 6
rat2302ENZYME, 7
rat2302ENZYME2PROBE (rat2302ENZYME), 7
rat2302GENENAME, 9
rat2302GO, 10
rat2302GO2ALLPROBES, 11
rat2302GO2ALLPROBES (rat2302GO), 10
rat2302GO2PROBE (rat2302GO), 10
rat2302LOCUSID (rat2302ENTREZID), 6
rat2302MAPCOUNTS, 12
rat2302ORGANISM, 12
rat2302ORGPKG (rat2302ORGANISM), 12
rat2302PATH, 13
rat2302PATH2PROBE (rat2302PATH), 13
rat2302PFAM, 14
rat2302PMID, 14
rat2302PMID2PROBE (rat2302PMID), 14
rat2302PROSITE, 16
rat2302REFSEQ, 16
rat2302SYMBOL, 17
rat2302UNIPROT, 18
rat2302UNIPROT2PROBE (rat2302UNIPROT),
...
rat2302 (rat2302.db), 2
rat2302.db, 2
rat2302_dbconn, 19
rat2302_dbfile (rat2302_dbconn), 19
rat2302_dbInfo (rat2302_dbconn), 19
rat2302_dbschema (rat2302_dbconn), 19
