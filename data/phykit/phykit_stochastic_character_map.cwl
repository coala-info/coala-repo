...
label: phykit_stochastic_character_map
doc: Perform Stochastic Character Mapping (SIMMAP) of discrete traits onto a 
  phylogeny, analogous to R's phytools::make.simmap(). Fits a continuous-time 
  Markov chain (CTMC) rate matrix Q via maximum likelihood, then simulates 
  character histories conditioned on tip states.
inputs:
  - id: tree
    type: File
    doc: a tree file
  ...
  - id: trait
    type: string
    doc: column name for discrete character trait
    inputBinding:
      position: 101
      prefix: --trait
  - id: model
    type:
      - 'null'
      - string
    doc: 'substitution model: ER, SYM, or ARD'
    inputBinding:
      position: 101
      prefix: --model
  - id: nsim
    type:
      - 'null'
      - int
    doc: number of stochastic mapping simulations
    inputBinding:
      position: 101
      prefix: --nsim
  - id: seed
