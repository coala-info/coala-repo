...
hgu133a2ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu133a2ALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hgu133a2ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hgu133a2.db annotation Package. The purpose of this package is to provide detailed
information about the hgu133a2 platform. This package is updated biannually.
...
hgu133a2CHR is an R object that provides mappings between a manufacturer identifier and the
...
hgu133a2CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu133a2CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu133a2ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu133a2ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu133a2ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hgu133a2ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hgu133a2ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hgu133a2ENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hgu133a2ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu133a2ENZYME2PROBE to a list to see inside

hgu133a2GENENAME

9

x <- hgu133a2ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu133a2GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu133a2GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hgu133a2GO2PROBE)
...
hgu133a2GO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hgu133a2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hgu133a2GO2ALLPROBES or hgu133a2GO2PROBE is cast as a list, each GO term maps to a
...
x <- hgu133a2GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu133a2GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hgu133a2MAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hgu133a2.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu133a2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu133a2.db.
...
hgu133a2OMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu133a2

Description

hgu133a2ORGANISM is an R object that contains a single item: a character string that names
the organism for which hgu133a2 was built. hgu133a2ORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu133a2ORGANISM
provides a simple way to programmatically extract the organism name. hgu133a2ORGPKG pro-
...
hgu133a2PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu133a2PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hgu133a2PATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu133a2PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu133a2PFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu133a2PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu133a2PMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hgu133a2PMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hgu133a2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hgu133a2PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu133a2PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu133a2PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hgu133a2REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hgu133a2SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hgu133a2UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu133a2_dbconn()
hgu133a2_dbfile()
hgu133a2_dbschema(file="", show.indices=FALSE)
hgu133a2_dbInfo()
...
hgu133a2_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu133a2_dbconn or you will break all
the AnnDbObj objects defined in this package!

hgu133a2_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hgu133a2_dbschema prints the schema definition of the package annotation DB.

hgu133a2_dbInfo prints other information about the package annotation DB.

Value

hgu133a2_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.

hgu133a2_dbfile: a character string with the path to the package annotation DB.

hgu133a2_dbschema: none (invisible NULL).

hgu133a2_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu133a2_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu133a2_dbconn, 21
hgu133a2ACCNUM, 1
hgu133a2ALIAS2PROBE, 2
hgu133a2CHR, 3
hgu133a2CHRLENGTHS, 4
hgu133a2CHRLOC, 4
hgu133a2ENSEMBL, 5
hgu133a2ENTREZID, 6
hgu133a2ENZYME, 7
...
hgu133a2 (hgu133a2.db), 2
hgu133a2.db, 2
hgu133a2_dbconn, 21
hgu133a2_dbfile (hgu133a2_dbconn), 21

23

hgu133a2_dbInfo (hgu133a2_dbconn), 21
hgu133a2_dbschema (hgu133a2_dbconn), 21
hgu133a2ACCNUM, 1
hgu133a2ALIAS2PROBE, 2
hgu133a2CHR, 3
hgu133a2CHRLENGTHS, 4
hgu133a2CHRLOC, 4
hgu133a2CHRLOCEND (hgu133a2CHRLOC), 4
hgu133a2ENSEMBL, 5
hgu133a2ENSEMBL2PROBE

(hgu133a2ENSEMBL), 5

hgu133a2ENTREZID, 6
hgu133a2ENZYME, 7
hgu133a2ENZYME2PROBE (hgu133a2ENZYME), 7
hgu133a2GENENAME, 9
hgu133a2GO, 10
hgu133a2GO2ALLPROBES, 11
hgu133a2GO2ALLPROBES (hgu133a2GO), 10
hgu133a2GO2PROBE (hgu133a2GO), 10
hgu133a2LOCUSID (hgu133a2ENTREZID), 6
hgu133a2MAP, 12
hgu133a2MAPCOUNTS, 13
hgu133a2OMIM, 13
hgu133a2ORGANISM, 14
hgu133a2ORGPKG (hgu133a2ORGANISM), 14
hgu133a2PATH, 15
hgu133a2PATH2PROBE (hgu133a2PATH), 15
hgu133a2PFAM, 16
hgu133a2PMID, 16
hgu133a2PMID2PROBE (hgu133a2PMID), 16
hgu133a2PROSITE, 17
hgu133a2REFSEQ, 18
hgu133a2SYMBOL, 19
hgu133a2UNIPROT, 20
hgu133a2UNIPROT2PROBE
