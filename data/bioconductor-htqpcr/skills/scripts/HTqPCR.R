# Code example from 'HTqPCR' vignette. See references/ for full tutorial.

### R code from vignette source 'HTqPCR.Rnw'
...
###################################################
### code chunk number 12: HTqPCR.Rnw:245-248
###################################################
par(mfrow=c(2,1))
g <- featureNames(raw)[1:10]
plotCtOverview(raw, genes=g, xlim=c(0,50), groups=files$Treatment, conf.int=TRUE,
...
###################################################
### code chunk number 15: HTqPCR.Rnw:270-271
###################################################
plotCtCard(raw, col.range=c(10,35), well.size=2.6)


###################################################
### code chunk number 16: HTqPCR.Rnw:273-274
###################################################
featureClass(raw) <- factor(c("Marker", "TF", "Kinase")[sample(c(1,1,2,2,1,3),
384, replace=TRUE)])
plotCtCard(raw, plot="class", well.size=2.6)
...
###################################################
### code chunk number 18: HTqPCR.Rnw:291-292
###################################################
plotCtReps(qPCRraw, card=2, percent=20)
...
###################################################
### code chunk number 22: HTqPCR.Rnw:331-332
###################################################
raw.mix <- raw
#exprs(raw.mix)[,6] <- sample(exprs(raw[,6]))
plotCtVariation(raw.mix, variation="sd", log=TRUE, main="SD of replicated features", col="lightgrey")
...
###################################################
### code chunk number 23: HTqPCR.Rnw:334-335
###################################################
raw.variation <- plotCtVariation(raw.mix, type="detail", add.featurenames=TRUE, pch=" ", cex=1.2)
...
###################################################
### code chunk number 26: HTqPCR.Rnw:376-379
###################################################
par(mfrow=c(2,1))
raw.cat <- raw
plotCtCategory(raw.cat)
...
###################################################
### code chunk number 28: HTqPCR.Rnw:394-395
###################################################
plotCtCategory(raw.cat, by.feature=TRUE, cexRow=0.1)
...
###################################################
### code chunk number 31: HTqPCR.Rnw:447-463
###################################################
...
###################################################
### code chunk number 33: HTqPCR.Rnw:543-544
###################################################
plotCtCor(raw, main="Ct correlation")
...
###################################################
### code chunk number 37: HTqPCR.Rnw:574-577
###################################################
...
###################################################
### code chunk number 38: HTqPCR.Rnw:588-595
###################################################
...
###################################################
### code chunk number 40: HTqPCR.Rnw:610-611
###################################################
plotCtBoxes(sr.norm, stratify="class")
...
###################################################
### code chunk number 43: HTqPCR.Rnw:631-634
###################################################
par(mfrow=c(1,2), mar=c(3,3,2,1))
plotCtScatter(sr.norm, cards=c(1,2), col="type", diag=TRUE)
plotCtScatter(sr.norm, cards=c(1,4), col="class", diag=TRUE)
...
###################################################
### code chunk number 45: HTqPCR.Rnw:652-653
###################################################
plotCtPairs(sr.norm, col="type", diag=TRUE)
...
###################################################
### code chunk number 47: HTqPCR.Rnw:672-673
###################################################
plotCtHeatmap(raw, gene.names="", dist="euclidean")
...
###################################################
### code chunk number 49: HTqPCR.Rnw:691-694
###################################################
...
###################################################
### code chunk number 51: HTqPCR.Rnw:720-721
###################################################
clusterCt(sr.norm, type="samples")
...
###################################################
### code chunk number 53: HTqPCR.Rnw:740-741
###################################################
cluster.list <- clusterCt(sr.norm, type="genes", n.cluster=6, cex=0.5)
...
###################################################
### code chunk number 55: HTqPCR.Rnw:759-762
###################################################
par(mfrow=c(1,2), mar=c(2,2,2,1))
plotCtPCA(qPCRraw)
plotCtPCA(qPCRraw, features=FALSE)
...
###################################################
### code chunk number 66: HTqPCR.Rnw:1082-1083
###################################################
plotCtArray(raw1)
...
###################################################
### code chunk number 68: sessionInfo
###################################################
toLatex(sessionInfo())
