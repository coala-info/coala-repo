...
[Sign in](/login?return_to=https%3A%2F%2Fgithub.com%2Fmourisl%2FT1K%2Fblob%2Fmaster%2FKmerCount.hpp)
...
[History](/mourisl/T1K/commits/master/KmerCount.hpp)
...
[Raw](https://github.com/mourisl/T1K/raw/refs/heads/master/KmerCount.hpp)
...
#ifndef \_LSONG\_KMERCOUNT\_HEADER

#define \_LSONG\_KMERCOUNT\_HEADER
...
int GetHash( uint64\_t k )

{

return k % KCOUNT\_HASH\_MAX ;
...
count = new std::map<uint64\_t, int>[KCOUNT\_HASH\_MAX] ;
...
uint64\_t kcode = kmerCode.GetCanonicalKmerCode() ;
...
uint64\_t kcode = kmerCode.GetCode() ;
...
for ( i = 0 ; i < KCOUNT\_HASH\_MAX ; ++i )

{

for ( std::map<uint64\_t, int>::iterator it = count[i].begin() ; it != count[i].end() ; ++it )
...
for (std::map<uint64\_t, int>::iterator it = b.count[i].begin() ; it != b.count[i].end() ; ++it)
