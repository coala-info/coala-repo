...
hguqiagenv3ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hguqiagenv3ALIAS is an R object that provides mappings between common gene symbol identi-
...
xx <- as.list(hguqiagenv3ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hguqiagenv3.db annotation Package. The purpose of this package is to provide
detailed information about the hguqiagenv3 platform. This package is updated biannually.
...
hguqiagenv3CHR is an R object that provides mappings between a manufacturer identifier and the
...
hguqiagenv3CHRLENGTHS A named vector for the length of each of the chromosomes

Description

hguqiagenv3CHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hguqiagenv3CHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
hguqiagenv3ENSEMBL is an R object that contains mappings between manufacturer identifiers
...
# Convert to a list
xx <- as.list(hguqiagenv3ENSEMBL2PROBE)
if(length(xx) > 0){
...
hguqiagenv3ENTREZID is an R object that provides mappings between manufacturer identifiers
...
hguqiagenv3ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hguqiagenv3ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hguqiagenv3ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hguqiagenv3ENZYME2PROBE, each EC number maps to a named vector containing all
...
bers. In hguqiagenv3ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hguqiagenv3ENZYME2PROBE to a list to see inside
xx <- as.list(hguqiagenv3ENZYME2PROBE)
if(length(xx) > 0){
...
hguqiagenv3GENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
hguqiagenv3GO is an R object that provides mappings between manufacturer identifiers and the
GO identifiers that they are directly associated with. This mapping and its reverse mapping (hguqi-
agenv3GO2PROBE) do NOT associate the child terms from the GO ontology with the gene. Only
the directly evidenced terms are represented here.

hguqiagenv3GO2ALLPROBES is an R object that provides mappings between a given GO identi-
fier and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than hguqia-
genv3GO2PROBE.

Details

If hguqiagenv3GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hguqiagenv3GO2ALLPROBES or hguqiagenv3GO2PROBE is cast as a list, each GO term maps
...
# For the reverse map:
# Convert to a list
xx <- as.list(hguqiagenv3GO2PROBE)
if(length(xx) > 0){
...
# Convert hguqiagenv3GO2ALLPROBES to a list
xx <- as.list(hguqiagenv3GO2ALLPROBES)
if(length(xx) > 0){
...
hguqiagenv3MAP is an R object that provides mappings between manufacturer identifiers and cy-
toband locations.
...
hguqiagenv3MAPCOUNTS

13

hguqiagenv3MAPCOUNTS

Number of mapped keys for the maps in package hguqiagenv3.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hguqiagenv3MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hguqiagenv3.db.
...
hguqiagenv3OMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hguqiagenv3

Description

hguqiagenv3ORGANISM is an R object that contains a single item: a character string that names
the organism for which hguqiagenv3 was built. hguqiagenv3ORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hguqiagenv3ORGANISM
provides a simple way to programmatically extract the organism name. hguqiagenv3ORGPKG pro-
...
hguqiagenv3PATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

hguqiagenv3PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the hguqiagenv3PATH2PROBE object to a list
xx <- as.list(hguqiagenv3PATH2PROBE)
if(length(xx) > 0){
...
hguqiagenv3PFAM is an R object that provides mappings between manufacturer identifiers and
PFAM identifiers.
...
hguqiagenv3PMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. hguqiagenv3PMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When hguqiagenv3PMID is viewed as a list each manufacturer identifier is mapped to a named
...
When hguqiagenv3PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object hguqiagenv3PMID2PROBE to a list
xx <- as.list(hguqiagenv3PMID2PROBE)
if(length(xx) > 0){
...
hguqiagenv3PROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hguqiagenv3REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hguqiagenv3SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hguqiagenv3UNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
hguqiagenv3UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hguqiagenv3_dbconn()
hguqiagenv3_dbfile()
hguqiagenv3_dbschema(file="", show.indices=FALSE)
hguqiagenv3_dbInfo()
...
hguqiagenv3_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by hguqiagenv3_dbconn or you will
break all the AnnDbObj objects defined in this package!
hguqiagenv3_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
hguqiagenv3_dbschema prints the schema definition of the package annotation DB.
hguqiagenv3_dbInfo prints other information about the package annotation DB.

Value

hguqiagenv3_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
hguqiagenv3_dbfile: a character string with the path to the package annotation DB.
hguqiagenv3_dbschema: none (invisible NULL).
hguqiagenv3_dbInfo: none (invisible NULL).
...
dbGetQuery(hguqiagenv3_dbconn(), "SELECT COUNT(*) FROM probes")

hguqiagenv3_dbschema()
...
hguqiagenv3.db, 2
hguqiagenv3_dbconn, 21
hguqiagenv3ACCNUM, 1
hguqiagenv3ALIAS2PROBE, 2
hguqiagenv3CHR, 3
hguqiagenv3CHRLENGTHS, 4
hguqiagenv3CHRLOC, 4
hguqiagenv3ENSEMBL, 5
hguqiagenv3ENTREZID, 6
hguqiagenv3ENZYME, 7
hguqiagenv3GENENAME, 9
hguqiagenv3GO, 10
hguqiagenv3MAP, 12
hguqiagenv3MAPCOUNTS, 13
hguqiagenv3OMIM, 13
hguqiagenv3ORGANISM, 14
hguqiagenv3PATH, 15
hguqiagenv3PFAM, 16
hguqiagenv3PMID, 16
hguqiagenv3PROSITE, 17
hguqiagenv3REFSEQ, 18
hguqiagenv3SYMBOL, 19
hguqiagenv3UNIGENE, 20
hguqiagenv3UNIPROT, 21
...
hguqiagenv3CHRLENGTHS, 4
hguqiagenv3CHRLOC, 4
hguqiagenv3CHRLOCEND

(hguqiagenv3CHRLOC), 4

hguqiagenv3ENSEMBL, 5
hguqiagenv3ENSEMBL2PROBE

(hguqiagenv3ENSEMBL), 5

hguqiagenv3ENTREZID, 6
hguqiagenv3ENZYME, 7
hguqiagenv3ENZYME2PROBE

(hguqiagenv3ENZYME), 7

hguqiagenv3GENENAME, 9
hguqiagenv3GO, 10
hguqiagenv3GO2ALLPROBES, 11
hguqiagenv3GO2ALLPROBES

(hguqiagenv3GO), 10

hguqiagenv3GO2PROBE (hguqiagenv3GO), 10
hguqiagenv3LOCUSID
...
hguqiagenv3PATH, 15
hguqiagenv3PATH2PROBE

(hguqiagenv3PATH), 15

hguqiagenv3PFAM, 16
hguqiagenv3PMID, 16
hguqiagenv3PMID2PROBE
