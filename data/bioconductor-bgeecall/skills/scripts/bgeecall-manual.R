# Code example from 'bgeecall-manual' vignette. See references/ for full tutorial.
...
unlink(file.path(getWorkingPath(user_BgeeCall), paste0(getIntergenicPrefix(bgee), "*"), user_BgeeCall@species_id, "intergenic_ids.txt"))
unlink(file.path(getWorkingPath(user_BgeeCall), paste0(getIntergenicPrefix(bgee), "*"), user_BgeeCall@species_id, "bgee_intergenic.fa.gz"))
unlink(file.path(getWorkingPath(user_BgeeCall), paste0(getIntergenicPrefix(bgee), "*"), "all_results", "SRX099901_subset", "gene_*"), recursive = TRUE)
unlink(file.path(getWorkingPath(user_BgeeCall), paste0(getIntergenicPrefix(bgee), "*"), "all_results", "SRX099901_subset", "S4_slots_summary.tsv"), recursive = TRUE)
