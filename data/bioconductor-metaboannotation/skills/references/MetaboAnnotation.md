...
MetaboAnnotation 1.14.0

**Package**: *[MetaboAnnotation](https://bioconductor.org/packages/3.22/MetaboAnnotation)*
**Authors**: Michael Witting [aut] (ORCID: <https://orcid.org/0000-0002-1462-4426>),
...
Nir Shachaf [ctb]
**Compiled**: Thu Oct 30 00:55:28 2025

# 1 Introduction

The *[MetaboAnnotation](https://bioconductor.org/packages/3.22/MetaboAnnotation)* package defines high-level user
functionality to support and facilitate annotation of MS-based metabolomics data
(Rainer et al. [2022](#ref-rainer_modular_2022)).
...
instance (as created by the *[CompoundDb](https://bioconductor.org/packages/3.22/CompoundDb)* package) would also be
supported.
...
Results from LC-MS preprocessing (e.g. by the *[xcms](https://bioconductor.org/packages/3.22/xcms)*
...
*[Spectra](https://bioconductor.org/packages/3.22/Spectra)* package (see
...
## PestMix1_DDA.mzML
## Processing:
##  Filter: select MS level(s) 2 [Thu Oct 30 00:56:02 2025]
```
...
*[CompoundDb](https://bioconductor.org/packages/3.22/CompoundDb)* package) with parameter `target`. See the
...
resources from Bioconductor’s *[AnnotationHub](https://bioconductor.org/packages/3.22/AnnotationHub)*.
...
`FUN` (see the help on `compareSpectra()` in the *[Spectra](https://bioconductor.org/packages/3.22/Spectra)* package
for more details).
In addition, we can *pre-filter* the target spectra for each individual query
spectrum to speed-up the calculations. By setting `requirePrecursor = TRUE` we
compare below each query spectrum only to target spectra with matching
precursor m/z (accepting a deviation defined by parameters `ppm` and
`tolerance`). By default, `matchSpectra()` with `CompareSpectraParam`
...
*[xcms](https://bioconductor.org/packages/3.22/xcms)* package. Note also that retention times could in addition be
...
### 4.4.1 Using alternative spectra similarity functions

The previous examples used the default spectra similarity scoring function used
by `compareSpectra()` which is the normalized dot product
`MsCoreUtils::ndotproduct()`. It is however also possible to use other
similarity functions, either those defined in the *[MsCoreUtils](https://bioconductor.org/packages/3.22/MsCoreUtils)*
package (see `?MsCoreUtils::distance` for a list of available options), or
functions defined by other, external, packages. Here it is however very
important to carefully evaluate how this similarity function works and which
parameters are needed/defined. As an example we use the spectral entropy
similarity function `msentropy_similarity()` from the
[*msentropy*](https://cran.r-project.org/web/packages/msentropy/index.html)
package. This function performs both, the mapping between the fragment peaks of
the compared spectra **and** the similarity calculation. It is therefore crucial
to **disable** the default mapping of fragment spectra by setting `MAPFUN = joinPeaksNone` in the `CompareSpectraParam()` call. In addition,
`msentropy_similarity()` uses different parameters and parameter names to
configure the ppm and tolerance for the mapping. These parameters are
`ms2_tolerance_in_ppm` (instead of `ppm`) and `ms2_tolerance_in_da` (for
`tolerance`). To perform the spectra matching using the *msentropy* similarity
score we thus configure the `CompareSpectraParam` as shown below:

* disable the default mapping of fragment peaks: use `MAPFUN = joinPeaksNone`
  instead of the default `MAPFUN = joinPeaks`.
* use `FUN = msentropy_similarity`: this will use the respective function from
  the *msentropy* function that performs peak mapping (eventual spectra
  cleaning) and similarity calculation. See `?msentropy_similarity` for more
  details and parameters.
* define the ppm tolerance for the mapping of peaks between compared spectra
  using `ms2_tolerance_in_ppm`.
* define the absolute tolerance for the mapping of peaks between spectra using
  `ms2_tolerance_in_da` - setting this to `-1` has the same effect as `tolerance = 0` has with the default `joinPeaks()` mapping function.
...
Note that we still configured the `ppm` and `tolerance` parameters, as these
are used for the similarity between the spectra’s precursor m/z. Similarly, to
use GNPS-like similarity calculation, `MAPFUN = joinPeaksGnps` and `FUN = MsCoreUtils::gnps` would need to be used.

### 4.4.2 Query against multiple reference databases
...
* Annotation resources that provide their data as a `CompDb` database (defined
  by the *[CompoundDb](https://bioconductor.org/packages/3.22/CompoundDb)*) package. These are supported by
...
## Processing:
##  Switch backend from MsBackendCompDb to MsBackendDataFrame [Thu Oct 30 00:56:13 2025]
```
...
### 4.4.3 Finding MS2 spectra for selected m/z and retention times
...
### 4.4.4 Performance and parallel processing
...
## Unit: milliseconds
##                                                   expr      min       lq
##          compareSpectra(pest_ms2, minimb, param = csp) 69.75898 70.34398
##  compareSpectra(pest_ms2_mem, minimb_mem, param = csp) 41.63783 42.28028
##      mean   median       uq       max neval cld
##  81.52406 72.31343 74.39372 120.81020     5  a
##  49.02622 48.89281 50.20585  62.11432     5   b
```
...
##  [1] MetaboCoreUtils_1.18.0      microbenchmark_1.5.0
##  [3] msentropy_0.1.4             Rcpp_1.1.0
##  [5] msdata_0.49.0               QFeatures_1.20.0
##  [7] MultiAssayExperiment_1.36.0 SummarizedExperiment_1.40.0
##  [9] Biobase_2.70.0              GenomicRanges_1.62.0
## [11] Seqinfo_1.0.0               IRanges_2.44.0
## [13] MatrixGenerics_1.22.0       matrixStats_1.5.0
## [15] Spectra_1.20.0              BiocParallel_1.44.0
## [17] S4Vectors_0.48.0            MetaboAnnotation_1.14.0
...
##   [4] httr2_1.2.1             sandwich_3.1-1          rlang_1.1.6
##   [7] magrittr_2.0.4          multcomp_1.4-29         clue_0.3-66
##  [10] compiler_4.5.1          RSQLite_2.4.3           reshape2_1.4.4
##  [13] png_0.1-8               vctrs_0.6.5             stringr_1.5.2
##  [16] ProtGenerics_1.42.0     pkgconfig_2.0.3         crayon_1.5.3
##  [19] fastmap_1.2.0           magick_2.9.0            XVector_0.50.0
##  [22] rmarkdown_2.30          tinytex_0.57            purrr_1.1.0
##  [25] bit_4.6.0               xfun_0.53               cachem_1.1.0
##  [28] ChemmineR_3.62.0        jsonlite_2.0.0          blob_1.2.4
##  [31] DelayedArray_0.36.0     parallel_4.5.1          cluster_2.1.8.1
##  [34] R6_2.6.1                bslib_0.9.0             stringi_1.8.7
##  [37] RColorBrewer_1.1-3      jquerylib_0.1.4         bookdown_0.45
##  [40] knitr_1.50              zoo_1.8-14              base64enc_0.1-3
##  [43] BiocBaseUtils_1.12.0    splines_4.5.1           igraph_2.2.1
...
##  [55] tibble_3.3.0            withr_3.0.2             KEGGREST_1.50.0
##  [58] S7_0.2.0                evaluate_1.0.5          survival_3.8-3
##  [61] xml2_1.4.1              Biostrings_2.78.0       pillar_1.11.1
##  [64] BiocManager_1.30.26     filelock_1.0.3          DT_0.34.0
##  [67] ncdf4_1.24              RCurl_1.98-1.17         BiocVersion_3.22.0
##  [70] ggplot2_4.0.0           scales_1.4.0            glue_1.8.0
##  [73] lazyeval_0.2.2          tools_4.5.1             mzR_2.44.0
##  [76] mvtnorm_1.3-3           fs_1.6.6                grid_4.5.1
##  [79] tidyr_1.3.1             MsCoreUtils_1.22.0      AnnotationDbi_1.72.0
##  [82] cli_3.6.5               rappdirs_0.3.3          rsvg_2.7.0
##  [85] S4Arrays_1.10.0         dplyr_1.1.4             AnnotationFilter_1.34.0
##  [88] gtable_0.3.6            sass_0.4.10             digest_0.6.37
##  [91] TH.data_1.1-4           SparseArray_1.10.0      rjson_0.2.23
##  [94] htmlwidgets_1.6.4       farver_2.1.2            memoise_2.0.1
##  [97] htmltools_0.5.8.1       lifecycle_1.0.4         httr_1.4.7
## [100] CompoundDb_1.14.0       bit64_4.6.0-1           MASS_7.3-65
```
