...
* [3 Module identification for single layer network](#module-identification-for-single-layer-network)
* [4 Module identification for two-layer network](#module-identification-for-two-layer-network)
* [5 Module identification for multi-layer network](#module-identification-for-multi-layer-network)
* [6 Module identification for real-world data](#module-identification-for-real-world-data)
* [7 High-performance considerations](#high-performance-considerations)
* [8 Biological explanation](#biological-explanation)
* [9 Developer page](#developer-page)
* [10 Session Information](#session-information)
* [References](#references)

# 1 Motivation

There are various ways to detect modules from weighted networks. Conventional approaches such as clustering or graph partitioning purely use the network topology to define modules (Fortunato [2010](#ref-fortunato2010community)). But we may need additional information such as increasing high-throughput omics data. On the one hand, the construction of reliable networks, especially for specific tissues, is relatively slow. On the other hand, integrating omics data with network topology has become a paradigm in system biology community in the past decade (Mitra et al. [2013](#ref-mitra2013integrative)). Weighted gene co-expression network (WGCN) is a pure data-driven gene network, which only relies on gene expression profiles. There is no rigorous definition of *active modules* in WGCN, but the module itself should be more *compact* and *informative* compared with random subnetworks. AMOUNTAIN (Li et al. [2016](#ref-Li056952)) provides a convex optimization based approach to identify such modules. Here we embed parts of the examples from the corresponding package [AMOUNTAIN](https://bioconductor.org/packages/AMOUNTAIN) help pages into a single document.

# 2 Network simulation

We follow (Li et al. [2011](#ref-li2011integrative)) to construct gene co-expression networks for simulation study. Let \(n\) be the number of genes, and edge weights \(W\) as well as node score \(z\) follow the uniform distribution in range \([0,1]\). A module contains \(k\) genes inside which the edge weights as well as node score follow the uniform distribution in range \([\theta,1]\), where \(\theta=\{0.5,0.6,0.7,0.8,0.9\}\).
...
\[\min\_{{\bf x}\in \mathbb{R}\_+^n}\ F({\bf x})=-{\bf x}^TW{\bf x}-\lambda{\bf z}^T{\bf x}\quad s.t.\quad \alpha\|{\bf x}\|\_1+(1-\alpha)\|{\bf x}\|\_2^2=1\]

where the module membership vector \({\bf x}\in\mathbb{R}\_+^n\) is relaxed from a \(0-1\) vector in which \(x\_i\neq0\) means node \(i\) is in the module. And \(\alpha\) is the parameter to balance \(\ell\_1\)-norm and \(\ell\_2\)-norm which actually controls the module size. Larger \(\alpha\) means a more sparse vector, corresponding smaller module in this case. We adopt the euclidean projection based technique (Gong, Gai, and Zhang [2011](#ref-gong2011efficient)) to solve the problem.
...
## [1] qgraph_1.9.8     AMOUNTAIN_1.36.0 BiocStyle_2.38.0
##
...
## [25] stringr_1.5.2       tinytex_0.57        mnormt_2.1.1
## [28] htmltools_0.5.8.1   glasso_1.11         sass_0.4.10
## [31] fdrtool_1.2.18      yaml_2.3.10         htmlTable_2.4.3
## [34] Formula_1.2-5       pillar_1.11.1       jquerylib_0.1.4
## [37] cachem_1.1.0        magick_2.9.0        Hmisc_5.2-4
## [40] abind_1.4-8         rpart_4.1.24        nlme_3.1-168
## [43] lavaan_0.6-20       gtools_3.9.5        tidyselect_1.2.1
## [46] digest_0.6.37       stringi_1.8.7       dplyr_1.1.4
## [49] reshape2_1.4.4      bookdown_0.45       fastmap_1.2.0
## [52] grid_4.5.1          colorspace_2.1-2    cli_3.6.5
## [55] magrittr_2.0.4      base64enc_0.1-3     dichromat_2.0-0.1
## [58] pbivnorm_0.6.0      foreign_0.8-90      corpcor_1.6.10
## [61] scales_1.4.0        backports_1.5.0     rmarkdown_2.30
## [64] jpeg_0.1-11         igraph_2.2.1        nnet_7.3-20
## [67] gridExtra_2.3       png_0.1-8           pbapply_1.7-4
## [70] evaluate_1.0.5      knitr_1.50          rlang_1.1.6
...
Gong, Pinghua, Kun Gai, and Changshui Zhang. 2011. “Efficient Euclidean Projections via Piecewise Root Finding and Its Application in Gradient Projection.” *Neurocomputing* 74 (17): 2754–66.

Li, Dong, Shan He, Zhisong Pan, and Guyu Hu. 2016. “Active Modules for Multilayer Weighted Gene Co-Expression Networks: A Continuous Optimization Approach.” *bioRxiv*. <https://doi.org/10.1101/056952>.

Li, Wenyuan, Chun-Chi Liu, Tong Zhang, Haifeng Li, Michael S Waterman, and Xianghong Jasmine Zhou. 2011. “Integrative Analysis of Many Weighted Co-Expression Networks Using Tensor Computation.” *PLoS Comput Biol* 7 (6): e1001106.
