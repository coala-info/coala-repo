...
* ←
  [6.1.2. bedextract](bedextract.html "Previous document")
* [6.1.4. Nested elements](nested-elements.html "Next document")
  →
...
# 6.1.3. closest-features[¶](#closest-features "Permalink to this headline")
...
An application of this tool in our own research is [finding the nearest DNase hypersensitive sites](../../usage-examples/distance-frequencies.html#distance-frequencies) upstream and downstream from a given SNP, as well as signed distances. The [closest-features](#closest-features) program can report both results.
...
## 6.1.3.1. Inputs and outputs[¶](#inputs-and-outputs "Permalink to this headline")

### 6.1.3.1.1. Input[¶](#input "Permalink to this headline")

The [closest-features](#closest-features) program takes two sorted BED files (a so-called *reference* file and a *map* file), as well as optional arguments for modifying behavior and outputs.

Alternatively, [closest-features](#closest-features) can accept [Starch-formatted archives](../file-management/compression/starch.html#starch) as inputs, with no need to extract archive data to intermediate BED files!
...
### 6.1.3.1.2. Output[¶](#output "Permalink to this headline")

The [closest-features](#closest-features) program returns summary data to standard output, which may include reference and nearest elements and distance values (depending on provided options).

## 6.1.3.2. Usage[¶](#usage "Permalink to this headline")
...
## 6.1.3.3. Per-chromosome operations (–chrom)[¶](#per-chromosome-operations-chrom "Permalink to this headline")

All operations on inputs can be restricted to one chromosome, by adding the `--chrom <val>` operator.

Tip

This option is highly useful for cluster-based work, where operations on large BED inputs can be split up by chromosome and pushed to separate cluster nodes.

To demonstrate the use of this option, we take two sample Starch-archived BED datasets `A` and `B` (refer to the [Downloads](#closest-features-downloads) section for sample inputs) which contain regions from multiple chromosomes:
...
## 6.1.3.4. Error checking[¶](#error-checking "Permalink to this headline")
...
Using `--ec` will slow down analysis considerably. We recommend using this option to test and debug pipelines and then removing it for use in production.

## 6.1.3.5. Downloads[¶](#downloads "Permalink to this headline")

* Sample dataset [`A`](../../../_downloads/d1a71b58a405da850b1c619bd66a13d4/reference_closestfeatures_a.starch)
* Sample dataset [`B`](../../../_downloads/db6c4ffb6988a6d1a5a0a021c1b997a2/reference_closestfeatures_b.starch)
...
* 6.1.3. closest-features
  + [6.1.3.1. Inputs and outputs](#inputs-and-outputs)
    - [6.1.3.1.1. Input](#input)
    - [6.1.3.1.2. Output](#output)
  + [6.1.3.2. Usage](#usage)
  + [6.1.3.3. Per-chromosome operations (–chrom)](#per-chromosome-operations-chrom)
  + [6.1.3.4. Error checking](#error-checking)
  + [6.1.3.5. Downloads](#downloads)
