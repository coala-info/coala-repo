...
baseCommand:
  - bioconvert
  - fasta2fasta_agp
label: bioconvert_fasta2fasta_agp
doc: "Convert file from '('FASTA',)' to '('FASTA', 'AGP')' format. See bioconvert.readthedocs.io
  for details\n\nTool homepage: http://bioconvert.readthedocs.io/"
inputs:
  - id: input_file
    type: File
    doc: Path to the input scaffold FASTA file.
    inputBinding:
      position: 1
  - id: output_file
    type: File
  ...
  - id: min_scaffold_length
    ...
    inputBinding:
      position: 103
      prefix: --min-scaffold-length
  - id: min_stretch_of_n
    type:
      - 'null'
      - int
    doc: minimum stretch of Ns to split a scaffold
    inputBinding:
      position: 103
      prefix: --min-stretch-of-N
  - id: raise_exception
    type:
      - 'null'
      - boolean
