...
baseCommand: extract
label: tigr-glimmer_extract
doc: Read fasta-format <sequence-file> and extract from it the subsequences 
  specified by <coords>. By default, <coords> is the name of a file containing 
  lines of the form <tag> <start> <stop> [<frame>] ... Coordinates are inclusive
  counting from 1, e.g., "1 3" represents the 1st 3 characters of the sequence. 
  For each line the corresponding region of <sequence-file> is extracted and 
  output (after reverse-complementing if necessary) Output goes to stdout in 
  multi-fasta format, unless the -2 option is specified
inputs:
  - id: sequence_file
    type: File
  ...
  - id: coords
    type: File
    doc: File containing coordinates (tag, start, stop, [frame])
    inputBinding:
      position: 2
  - id: direction
    type:
...
stdout: tigr-glimmer_extract.out
