...
[View this page](_sources/query_assignment.rst.txt "View this page")

# Query assignment (`poppunk_assign`)[¶](#query-assignment-poppunk-assign "Link to this heading")

This is the recommended mode to use PopPUNK, as long as a database is available for
your species. If there is no database available, you can fit your own ([Fitting new models (--fit-model)](model_fitting.html)).

Briefly, [download your reference database](https://www.bacpop.org/poppunk-databases/) and run:
...
## Downloading a database[¶](#downloading-a-database "Link to this heading")
...
## Clustering your genomes[¶](#clustering-your-genomes "Link to this heading")
...
[Data quality control (--qc-db)](qc.html) does not apply to query sequences. A test for maximum accessory distance
...
`--exact-count`) and specify your input as `--strand-preserved`. See [Sketching (--create-db)](sketching.html) for
more information on these options.

Next, core and accessory distances between your input sketches and those in the database
will be computed. This has complexity \(O(RQ)\) where \(R\) is the number of
samples in `database_references.refs` and \(Q\) is the number in `qfile.txt`. These distances
...
merged, the new cluster would be called 23\_38.
...
wish to know their new cluster IDs, use `--update-db` ([Updating the database](#update-db)).
...
### Adding external cluster labels (MLST, CC etc)[¶](#adding-external-cluster-labels-mlst-cc-etc "Link to this heading")
...
### Using a model fitted with `--indiv-refine`[¶](#using-a-model-fitted-with-indiv-refine "Link to this heading")
...
## Increasing speed[¶](#increasing-speed "Link to this heading")

Query assignment is the most efficient mode in which to run PopPUNK, typically requiring \(Q\) sketches and
\(RQ\) distances. If you are updating the database, this increases to \(Q^2 + RQ\)
distances. If you are assigning a very large number of queries you can run `poppunk_assign`
with `--update-db` repeatedly for batches of query input, as the \(Q^2\) term will
...
## Updating the database[¶](#updating-the-database "Link to this heading")
...
as it will calculate all \(Q^2\) query-query distances, rather than
...
See [Troubleshooting](troubleshooting.html) for more details.

## Visualising results[¶](#visualising-results "Link to this heading")
...
See [Creating visualisations](visualisation.html) for more details.
...
* Query assignment (`poppunk_assign`)
  ...
  + [Clustering your genomes](#clustering-your-genomes)
    - [Adding external cluster labels (MLST, CC etc)](#adding-external-cluster-labels-mlst-cc-etc)
    - [Using a model fitted with `--indiv-refine`](#using-a-model-fitted-with-indiv-refine)
