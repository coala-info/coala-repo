...
hgu95cACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu95cALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgu95c.db annotation Package. The purpose of this package is to provide detailed
information about the hgu95c platform. This package is updated biannually.
...
hgu95cCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgu95cCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu95cCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu95cENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu95cENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu95cENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu95cENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu95cENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu95cENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu95cENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu95cENZYME2PROBE to a list to see inside

hgu95cGENENAME

9

x <- hgu95cENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu95cGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu95cGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hgu95cGO2PROBE)
...
hgu95cGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu95cGO2PROBE.

Details

If hgu95cGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu95cGO2ALLPROBES or hgu95cGO2PROBE is cast as a list, each GO term maps to a named
...
x <- hgu95cGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu95cGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hgu95cMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu95c.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu95cMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu95c.db.
...
hgu95cOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu95c

Description

hgu95cORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu95c was built. hgu95cORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu95cORGANISM
provides a simple way to programmatically extract the organism name. hgu95cORGPKG provides
...
hgu95cPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu95cPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- hgu95cPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu95cPATH2PROBE object to a list
...
hgu95cPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu95cPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu95cPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu95cPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgu95cPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- hgu95cPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu95cPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu95cPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hgu95cREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu95cSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu95cUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu95c_dbconn()
hgu95c_dbfile()
hgu95c_dbschema(file="", show.indices=FALSE)
hgu95c_dbInfo()
...
hgu95c_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu95c_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgu95c_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgu95c_dbschema prints the schema definition of the package annotation DB.

hgu95c_dbInfo prints other information about the package annotation DB.

Value

hgu95c_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

hgu95c_dbfile: a character string with the path to the package annotation DB.

hgu95c_dbschema: none (invisible NULL).

hgu95c_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu95c_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu95c_dbInfo (hgu95c_dbconn), 21
hgu95c_dbschema (hgu95c_dbconn), 21
hgu95cACCNUM, 1
hgu95cALIAS2PROBE, 2
hgu95cCHR, 3
hgu95cCHRLENGTHS, 4
hgu95cCHRLOC, 4
hgu95cCHRLOCEND (hgu95cCHRLOC), 4
hgu95cENSEMBL, 5
hgu95cENSEMBL2PROBE (hgu95cENSEMBL), 5
hgu95cENTREZID, 6
hgu95cENZYME, 7
hgu95cENZYME2PROBE (hgu95cENZYME), 7
hgu95cGENENAME, 9
hgu95cGO, 10
hgu95cGO2ALLPROBES, 11
hgu95cGO2ALLPROBES (hgu95cGO), 10
hgu95cGO2PROBE (hgu95cGO), 10
hgu95cLOCUSID (hgu95cENTREZID), 6
hgu95cMAP, 12
hgu95cMAPCOUNTS, 13
hgu95cOMIM, 13
hgu95cORGANISM, 14
hgu95cORGPKG (hgu95cORGANISM), 14
hgu95cPATH, 15
hgu95cPATH2PROBE (hgu95cPATH), 15
hgu95cPFAM, 16
hgu95cPMID, 16
hgu95cPMID2PROBE (hgu95cPMID), 16
hgu95cPROSITE, 17
hgu95cREFSEQ, 18
hgu95cSYMBOL, 19
hgu95cUNIPROT, 20
hgu95cUNIPROT2PROBE (hgu95cUNIPROT), 20
...
hgu95c.db, 2
hgu95c_dbconn, 21
hgu95c_dbfile (hgu95c_dbconn), 21
