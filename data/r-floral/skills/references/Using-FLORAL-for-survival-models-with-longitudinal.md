...
Source: [`vignettes/Using-FLORAL-for-survival-models-with-longitudinal-microbiome-data.Rmd`](https://github.com/vdblab/FLORAL/blob/master/vignettes/Using-FLORAL-for-survival-models-with-longitudinal-microbiome-data.Rmd)

`Using-FLORAL-for-survival-models-with-longitudinal-microbiome-data.Rmd`
...
In this vignette, we illustrate how to apply `FLORAL` to
...
We will use the built-in simulation function `[simu()](../reference/simu.html)` to
...
based on a piece-wise exponential distribution as described by [Hendry 2014](https://doi.org/10.1002/sim.5945).
...
* First of all, please use `longitudinal = TRUE` such that
  the algorithm would use the appropriate method to handle longitudinal
  data.
* The feature matrix input `x` should be the count matrix
  where rows specify samples and columns specify features.
...
* The vector of sample collection times corresponding to the rows of
  `x` should be input as `tobs`.
* The `Surv` object (`Surv(time,status)`) of
...
* Start with patient metadata which includes survival data (time and
  ...
  variables for the `Surv` object for input as
  `y`.
* Curate the microbiome feature data matrix, sorted by patient IDs and
  ...
  collection vectors for `id` and `tobs`. Save the
