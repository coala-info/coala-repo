...
canine2ACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
canine2ALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the canine2.db annotation Package. The purpose of this package is to provide detailed
information about the canine2 platform. This package is updated biannually.
...
canine2CHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
canine2CHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
canine2CHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
canine2ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
canine2ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
canine2ENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. canine2ENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the canine2ENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the canine2ENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In canine2ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert canine2ENZYME2PROBE to a list to see inside

canine2GENENAME

9

x <- canine2ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
canine2GENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
canine2GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (canine2GO2PROBE)
...
canine2GO2ALLPROBES is an R object that provides mappings between a given GO identifier
and all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD
NODES in the GO ontology. Thus, this mapping is much larger and more inclusive than ca-
nine2GO2PROBE.

Details

If canine2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If canine2GO2ALLPROBES or canine2GO2PROBE is cast as a list, each GO term maps to a
...
x <- canine2GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert canine2GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
Number of mapped keys for the maps in package canine2.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

canine2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package canine2.db.
...
The Organism information for canine2

Description

canine2ORGANISM is an R object that contains a single item: a character string that names the
organism for which canine2 was built. canine2ORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, canine2ORGANISM
provides a simple way to programmatically extract the organism name. canine2ORGPKG provides
...
canine2PATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

canine2PATH2PROBE is an R object that provides mappings between KEGG identifiers and man-
...
x <- canine2PATH
mapped_probes <- mappedkeys(x)
# Now convert the canine2PATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
canine2PMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. canine2PMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When canine2PMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When canine2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vec-
...
x <- canine2PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object canine2PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
canine2REFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
canine2SYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
canine2UNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
canine2_dbconn()
canine2_dbfile()
canine2_dbschema(file="", show.indices=FALSE)
canine2_dbInfo()
...
canine2_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by canine2_dbconn or you will break all the
AnnDbObj objects defined in this package!
canine2_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
canine2_dbschema prints the schema definition of the package annotation DB.
canine2_dbInfo prints other information about the package annotation DB.

Value

canine2_dbconn: a DBIConnection object representing an open connection to the package anno-
tation DB.
canine2_dbfile: a character string with the path to the package annotation DB.
canine2_dbschema: none (invisible NULL).
canine2_dbInfo: none (invisible NULL).
...
dbGetQuery(canine2_dbconn(), "SELECT COUNT(*) FROM probes")

canine2_dbschema()

canine2_dbInfo()

canine2GO2ALLPROBES, 11
canine2GO2ALLPROBES (canine2GO), 10
canine2GO2PROBE (canine2GO), 10
canine2LOCUSID (canine2ENTREZID), 6
canine2MAPCOUNTS, 12
canine2ORGANISM, 12
canine2ORGPKG (canine2ORGANISM), 12
canine2PATH, 13
canine2PATH2PROBE (canine2PATH), 13
canine2PMID, 14
canine2PMID2PROBE (canine2PMID), 14
canine2REFSEQ, 15
canine2SYMBOL, 17
canine2UNIPROT, 18
canine2UNIPROT2PROBE (canine2UNIPROT),
...
canine2 (canine2.db), 2
canine2.db, 2
canine2_dbconn, 18
canine2_dbfile (canine2_dbconn), 18
canine2_dbInfo (canine2_dbconn), 18
canine2_dbschema (canine2_dbconn), 18
canine2ACCNUM, 1
canine2ALIAS2PROBE, 2
canine2CHR, 3
canine2CHRLENGTHS, 4
canine2CHRLOC, 4
canine2CHRLOCEND (canine2CHRLOC), 4
canine2ENSEMBL, 5
canine2ENSEMBL2PROBE (canine2ENSEMBL), 5
canine2ENTREZID, 6
canine2ENZYME, 7
canine2ENZYME2PROBE (canine2ENZYME), 7
canine2GENENAME, 9
canine2GO, 10
