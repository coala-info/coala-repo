...
baseCommand:
  - grenedalf
  - fst-cathedral
label: grenedalf_fst-cathedral
doc: "Compute the data for an FST cathedral plot.\n\nTool homepage: https://github.com/lczech/grenedalf"
inputs:
  ...
  - id: cathedral_height
    type:
      - 'null'
      - int
    doc: Height of the plot, in pixels. This determines the number of different 
      window sizes displayed in the plot, from whole chromosome at the top to 
      finest resolution at the bottom.
    default: 500
    inputBinding:
      position: 101
      prefix: --cathedral-height
  - id: cathedral_width
    type:
      - 'null'
      - int
    doc: Width of the plot, in pixels. In particular, this determines the 
      resolution of the last row of the plot, where each pixel corresponds to a 
      window of the size `genome length / plot width`.
    default: 1500
    inputBinding:
      position: 101
      prefix: --cathedral-width
  - id: comparand
    type:
      - 'null'
      - string
  ...
  - id: filter_total_only_biallelic_snps
    ...
    doc: "Filter out any positions that do not have exactly two alleles across all
      ...
      a biallelic SNP, and ignored.\nBy default, we already filter out invariant sites,
      so that only SNPs remain. With this option however, this is further reduced
      to only biallelic (pure) SNPs. Note that with `--method karlsson`, we implicitly
      also activate to filter for biallelic SNPs only, as the method requires it."
    inputBinding:
      position: 101
  ...
  - id: method
    type: string
    doc: 'FST method to use for the computation: The unbiased pool-sequencing statistic
      in two variants, following the definition of Nei, and the definition of Hudson.'
    default: unbiased-nei
    inputBinding:
      position: 101
      prefix: --method
  ...
  - id: pool_sizes
    ...
    doc: "Pool sizes for all samples that are used (not filtered out). These are the
      ...
      for all samples, specified on the command line, or \n                      \
...
stdout: grenedalf_fst-cathedral.out
