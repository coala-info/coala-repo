...
hgu95aACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
hgu95aALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the hgu95a.db annotation Package. The purpose of this package is to provide detailed
information about the hgu95a platform. This package is updated biannually.
...
hgu95aCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
hgu95aCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
hgu95aCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hgu95aENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hgu95aENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hgu95aENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. hgu95aENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the hgu95aENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the hgu95aENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In hgu95aENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hgu95aENZYME2PROBE to a list to see inside

hgu95aGENENAME

9

x <- hgu95aENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hgu95aGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hgu95aGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (hgu95aGO2PROBE)
...
hgu95aGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than hgu95aGO2PROBE.

Details

If hgu95aGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If hgu95aGO2ALLPROBES or hgu95aGO2PROBE is cast as a list, each GO term maps to a named
...
x <- hgu95aGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hgu95aGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
hgu95aMAP is an R object that provides mappings between manufacturer identifiers and cytoband
locations.
...
Number of mapped keys for the maps in package hgu95a.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hgu95aMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package hgu95a.db.
...
hgu95aOMIM is an R object that provides mappings between manufacturer identifiers and OMIM
identifiers.
...
The Organism information for hgu95a

Description

hgu95aORGANISM is an R object that contains a single item: a character string that names the
organism for which hgu95a was built. hgu95aORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, hgu95aORGANISM
provides a simple way to programmatically extract the organism name. hgu95aORGPKG provides
...
hgu95aPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hgu95aPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- hgu95aPATH
mapped_probes <- mappedkeys(x)
# Now convert the hgu95aPATH2PROBE object to a list
...
hgu95aPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hgu95aPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hgu95aPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When hgu95aPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When hgu95aPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- hgu95aPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hgu95aPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hgu95aPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
hgu95aREFSEQ is an R object that provides mappings between manufacturer identifiers and Ref-
Seq identifiers.
...
hgu95aSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
hgu95aUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
hgu95a_dbconn()
hgu95a_dbfile()
hgu95a_dbschema(file="", show.indices=FALSE)
hgu95a_dbInfo()
...
hgu95a_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hgu95a_dbconn or you will break all the
AnnDbObj objects defined in this package!

hgu95a_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).

hgu95a_dbschema prints the schema definition of the package annotation DB.

hgu95a_dbInfo prints other information about the package annotation DB.

Value

hgu95a_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.

hgu95a_dbfile: a character string with the path to the package annotation DB.

hgu95a_dbschema: none (invisible NULL).

hgu95a_dbInfo: none (invisible NULL).
...
dbGetQuery(hgu95a_dbconn(), "SELECT COUNT(*) FROM probes")
...
hgu95a_dbInfo (hgu95a_dbconn), 21
hgu95a_dbschema (hgu95a_dbconn), 21
hgu95aACCNUM, 1
hgu95aALIAS2PROBE, 2
hgu95aCHR, 3
hgu95aCHRLENGTHS, 4
hgu95aCHRLOC, 4
hgu95aCHRLOCEND (hgu95aCHRLOC), 4
hgu95aENSEMBL, 5
hgu95aENSEMBL2PROBE (hgu95aENSEMBL), 5
hgu95aENTREZID, 6
hgu95aENZYME, 7
hgu95aENZYME2PROBE (hgu95aENZYME), 7
hgu95aGENENAME, 9
hgu95aGO, 10
hgu95aGO2ALLPROBES, 11
hgu95aGO2ALLPROBES (hgu95aGO), 10
hgu95aGO2PROBE (hgu95aGO), 10
hgu95aLOCUSID (hgu95aENTREZID), 6
hgu95aMAP, 12
hgu95aMAPCOUNTS, 13
hgu95aOMIM, 13
hgu95aORGANISM, 14
hgu95aORGPKG (hgu95aORGANISM), 14
hgu95aPATH, 15
hgu95aPATH2PROBE (hgu95aPATH), 15
hgu95aPFAM, 16
hgu95aPMID, 16
hgu95aPMID2PROBE (hgu95aPMID), 16
hgu95aPROSITE, 17
hgu95aREFSEQ, 18
hgu95aSYMBOL, 19
hgu95aUNIPROT, 20
hgu95aUNIPROT2PROBE (hgu95aUNIPROT), 20
...
hgu95a.db, 2
hgu95a_dbconn, 21
hgu95a_dbfile (hgu95a_dbconn), 21
