...
drosophila2ACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
drosophila2ALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(drosophila2ALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the drosophila2.db annotation Package. The purpose of this package is to provide
detailed information about the drosophila2 platform. This package is updated biannually.
...
drosophila2CHR is an R object that provides mappings between a manufacturer identifier and the
...
drosophila2CHRLENGTHS A named vector for the length of each of the chromosomes

Description

drosophila2CHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
drosophila2CHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
drosophila2ENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
drosophila2ENTREZID is an R object that provides mappings between manufacturer identifiers and
...
drosophila2ENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. drosophila2ENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the drosophila2ENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the drosophila2ENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In drosophila2ENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert drosophila2ENZYME2PROBE to a list to see inside

drosophila2FLYBASE

9

x <- drosophila2ENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
drosophila2FLYBASE is an R object that contains mappings between manufacturer identifiers and
...
# For the reverse map:
# Convert to a list
xx <- as.list(drosophila2FLYBASE2PROBE)
if(length(xx) > 0){
...
drosophila2FLYBASECG

drosophila2FLYBASECG Map FlyBase CG accession numbers with manufacturer identifiers
...
# Convert to a list
xx <- as.list(drosophila2FLYBASECG2PROBE)
if(length(xx) > 0){
...
drosophila2GENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
drosophila2GO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (drosophila2GO2PROBE)
...
drosophila2GO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If drosophila2GO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If drosophila2GO2ALLPROBES or drosophila2GO2PROBE is cast as a list, each GO term maps to
...
x <- drosophila2GO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert drosophila2GO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
drosophila2MAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
drosophila2MAPCOUNTS

15

drosophila2MAPCOUNTS

Number of mapped keys for the maps in package drosophila2.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

drosophila2MAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package drosophila2.db.
...
The Organism information for drosophila2

Description

drosophila2ORGANISM is an R object that contains a single item: a character string that names
the organism for which drosophila2 was built. drosophila2ORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, drosophila2ORGANISM
provides a simple way to programmatically extract the organism name. drosophila2ORGPKG pro-
...
drosophila2PATH maps probe identifiers to the identifiers used by KEGG for pathways in which
the genes represented by the probe identifiers are involved

drosophila2PATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- drosophila2PATH
mapped_probes <- mappedkeys(x)
# Now convert the drosophila2PATH2PROBE object to a list
...
drosophila2PMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. drosophila2PMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When drosophila2PMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When drosophila2PMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- drosophila2PMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object drosophila2PMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
drosophila2REFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
drosophila2SYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
drosophila2UNIPROT is an R object that contains mappings between the manufacturer identifiers
...
drosophila2_dbconn()
drosophila2_dbfile()
drosophila2_dbschema(file="", show.indices=FALSE)
drosophila2_dbInfo()
...
drosophila2_dbconn returns a connection object to the package annotation DB. IMPORTANT:
Don’t call dbDisconnect on the connection object returned by drosophila2_dbconn or you will
break all the AnnDbObj objects defined in this package!
drosophila2_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).
drosophila2_dbschema prints the schema definition of the package annotation DB.
drosophila2_dbInfo prints other information about the package annotation DB.
...
drosophila2_dbconn: a DBIConnection object representing an open connection to the package
annotation DB.
drosophila2_dbfile: a character string with the path to the package annotation DB.
drosophila2_dbschema: none (invisible NULL).
drosophila2_dbInfo: none (invisible NULL).
...
dbGetQuery(drosophila2_dbconn(), "SELECT COUNT(*) FROM probes")
...
drosophila2.db, 2
drosophila2_dbconn, 21
drosophila2ACCNUM, 1
drosophila2ALIAS2PROBE, 2
drosophila2CHR, 3
drosophila2CHRLENGTHS, 4
drosophila2CHRLOC, 4
drosophila2ENSEMBL, 5
drosophila2ENTREZID, 6
drosophila2ENZYME, 7
drosophila2FLYBASE, 9
drosophila2FLYBASECG, 10
drosophila2GENENAME, 11
drosophila2GO, 11
drosophila2MAP, 14
drosophila2MAPCOUNTS, 15
drosophila2ORGANISM, 15
drosophila2PATH, 16
drosophila2PMID, 17
drosophila2REFSEQ, 18
drosophila2SYMBOL, 19
drosophila2UNIPROT, 20
...
drosophila2CHRLENGTHS, 4
drosophila2CHRLOC, 4
drosophila2CHRLOCEND

(drosophila2CHRLOC), 4

drosophila2ENSEMBL, 5
drosophila2ENSEMBL2PROBE

(drosophila2ENSEMBL), 5

drosophila2ENTREZID, 6
drosophila2ENZYME, 7
drosophila2ENZYME2PROBE

(drosophila2ENZYME), 7

drosophila2FLYBASE, 9
drosophila2FLYBASE2PROBE

(drosophila2FLYBASE), 9

drosophila2FLYBASECG, 10
drosophila2FLYBASECG2PROBE

(drosophila2FLYBASECG), 10

drosophila2GENENAME, 11
drosophila2GO, 11
drosophila2GO2ALLPROBES, 13
drosophila2GO2ALLPROBES

(drosophila2GO), 11

drosophila2GO2PROBE (drosophila2GO), 11
drosophila2LOCUSID
...
drosophila2PATH, 16
drosophila2PATH2PROBE

(drosophila2PATH), 16

drosophila2PMID, 17
drosophila2PMID2PROBE
