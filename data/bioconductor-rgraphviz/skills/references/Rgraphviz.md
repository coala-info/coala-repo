...
Figure 3: rEG laid out with recipEdges set to combined (left) and distinct
(right).
...
abcdefghijabcdefghijABAB> plot(rEG, recipEdges="distinct")
...
Graphviz then attempts to find a layout where the specified subgraphs are plot-
ted with all nodes relatively close. This is particularly useful when laying out
graphs that represent some real physical entity (one biological example is path-
ways).

In the code below we construct three subgraphs that we will use to group

the corresponding nodes when g1 is rendered.
...
• attrs : A named vector of attributes, where the names are the attribute
...
abcdefghijabcdefghij> edgeNames(g1, recipEdges="distinct")
...
attributes for commonly desired tasks. For these we will use two lists nAttrs
and eAttrs.

9

abcdefghij> nAttrs <- list()
> eAttrs <- list()
...
InterfacesRwiththeATandTgraphvizlibraryforLabel 1Label 2weights, and unlist them, to provide them in vector format. Then, first we will
...
InterfacesRwiththeATandTgraphvizlibraryfor1111111211111111fillcolor defined, detailing what color the interior of the node should be. The
...
InterfacesRwiththeATandTgraphvizlibraryfor> edges[[1]]
...
InterfacesRwiththeATandTgraphvizlibraryfor1111111211111111Figure 11: Customized layout of graph z .
...
node is an object of class AgNode describing the node’s location and other
...
zzzzzzzzzzzzzA custom drawing function is free to ignore these arguments, but the argument
...
dot12345678910111213141516171819202122twopi12345678910111213141516171819202122neato12345678910111213141516171819202122Figure 14: A bipartite graph.
...
62514307chjgidaefbThe following workflow works well for an instance of Ragraph class obtained
