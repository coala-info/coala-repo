...
For the methods that don’t have adjusted p-values, we will calculate adjusted p-values from nominal p-values.
...
It is also possible to modify the facetting and/or label position by adding a facet\_wrap statement (note that the stratification variable is denoted “splitval”):
...
The plot\_overlap function generates venn diagram(s), using the vennDiagram function from the limma package. It can be provided with any additional arguments that are accepted by limma::vennDiagram, e.g. to change the size of the text:
...
The screenshot below shows the appearance of the iCOBRA shiny app, using the example data set.
