...
* Counting reads
* [Edit on GitHub](https://github.com/htseq/htseq/blob/main/doc/counting.rst)

---

# Counting reads[](#counting-reads "Permalink to this heading")
...
gene models and a SAM file and counts for each gene how many reads map to it; see Section [htseq-count: counting reads within features](htseqcount.html#htseqcount).
...
topic in the *Tour* ([A tour through HTSeq](tour.html#tour)) and give building blocks which should make it possible to write such scripts
also for bioinformaticians with only modest knowledge of Python.

## Preparing the feature array[](#preparing-the-feature-array "Permalink to this heading")

Our general approach is to define a [`GenomicArrayOfSets`](genomic.html#HTSeq.GenomicArrayOfSets "HTSeq.GenomicArrayOfSets") and fill it with all the features we would
...
## Counting ungapped single-end reads[](#counting-ungapped-single-end-reads "Permalink to this heading")
...
## Counting gapped single-end reads[](#counting-gapped-single-end-reads "Permalink to this heading")

### CIGAR Operations[](#cigar-operations "Permalink to this heading")
...
HTSeq parses the CIGAR string and presents it in the `cigar` slot of a class:SAM\_Alignment object
...
![_images/cigar.png](_images/cigar.png)
...
### Counting with gapped reads[](#counting-with-gapped-reads "Permalink to this heading")
...
the inner loop inspects the steps covered by the match operations and collects the gene\_ids in
...
### Dealing with multiple alignments[](#dealing-with-multiple-alignments "Permalink to this heading")
...
function:bundle\_multiple\_alignments. It takes an iterator over [`SAM_Alignment`](alignments.html#HTSeq.SAM_Alignment "HTSeq.SAM_Alignment") objects
(e.g., a `SAM_Reader` or [`BAM_Reader`](alignments.html#HTSeq.BAM_Reader "HTSeq.BAM_Reader") object) and returns an iterator over
lists of [`SAM_Alignment`](alignments.html#HTSeq.SAM_Alignment "HTSeq.SAM_Alignment") objects. Each list contains only records describing alignments
...
### Handling paired-end reads[](#handling-paired-end-reads "Permalink to this heading")
...
are exposed by the [`SAM_Alignment.pe_which`](alignments.html#HTSeq.SAM_Alignment.pe_which "HTSeq.SAM_Alignment.pe_which") slot of the [`SAM_Alignment`](alignments.html#HTSeq.SAM_Alignment "HTSeq.SAM_Alignment") class, which takes
...
which are exposed by the slot `SAM_Alignment.mate_pos`, which contains a [`GenomicPosition`](genomic.html#HTSeq.GenomicPosition "HTSeq.GenomicPosition")
object. The two records describing the two halves of a given alignment can be recognized as being corres