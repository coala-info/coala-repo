...
adme16codACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
adme16codALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

adme16codALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(adme16codALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the adme16cod.db annotation Package. The purpose of this package is to provide
detailed information about the adme16cod platform. This package is updated biannually.
...
adme16codCHR is an R object that provides mappings between a manufacturer identifier and the
...
adme16codCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
adme16codCHRLOC is an R object that maps manufacturer identifiers to the starting position of
...
adme16codENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
# Convert to a list
xx <- as.list(adme16codENSEMBL2PROBE)
if(length(xx) > 0){
...
adme16codENTREZID is an R object that provides mappings between manufacturer identifiers and
...
adme16codENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. adme16codENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the adme16codENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the adme16codENZYME2PROBE, each EC number maps to a named vector containing all of
...
bers. In adme16codENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert adme16codENZYME2PROBE to a list to see inside
xx <- as.list(adme16codENZYME2PROBE)
if(length(xx) > 0){
...
adme16codGENENAME is an R object that maps manufacturer identifiers to the corresponding
gene name.
...
adme16codGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (adme16codGO2PROBE)
...
adme16codGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If adme16codGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If adme16codGO2ALLPROBES or adme16codGO2PROBE is cast as a list, each GO term maps to
...
# Convert adme16codGO2ALLPROBES to a list
xx <- as.list(adme16codGO2ALLPROBES)
if(length(xx) > 0){
...
Number of mapped keys for the maps in package adme16cod.db

Description

adme16codMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package adme16cod.db.
...
adme16codMAPCOUNTS
mapnames <- names(adme16codMAPCOUNTS)
adme16codMAPCOUNTS[mapnames[1]]
x <- get(mapnames[1])
sum(!is.na(as.list(x)))
count.mappedkeys(x)
...
## Check the "map count" of all the maps in package adme16cod.db
checkMAPCOUNTS("adme16cod.db")
...
The Organism information for adme16cod

Description

adme16codORGANISM is an R object that contains a single item: a character string that names
the organism for which adme16cod was built. adme16codORGPKG is an R object that contains
a chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, adme16codORGANISM
provides a simple way to programmatically extract the organism name. adme16codORGPKG pro-
...
adme16codPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

adme16codPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
# Now convert the adme16codPATH2PROBE object to a list
xx <- as.list(adme16codPATH2PROBE)
if(length(xx) > 0){
...
adme16codPMID is an R object that provides mappings between manufacturer identifiers and
PubMed identifiers. adme16codPMID2PROBE is an R object that provides mappings between
PubMed identifiers and manufacturer identifiers.

Details

When adme16codPMID is viewed as a list each manufacturer identifier is mapped to a named vec-
...
When adme16codPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
# Now convert the reverse map object adme16codPMID2PROBE to a list
xx <- as.list(adme16codPMID2PROBE)
if(length(xx) > 0){
...
adme16codREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
adme16codSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
adme16codUNIGENE is an R object that provides mappings between manufacturer identifiers and
UniGene identifiers.
...
adme16codUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
adme16cod_dbconn()
adme16cod_dbfile()
adme16cod_dbschema(file="", show.indices=FALSE)
adme16cod_dbInfo()
...
adme16cod_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by adme16cod_dbconn or you will break all
the AnnDbObj objects defined in this package!

adme16cod_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

adme16cod_dbschema prints the schema definition of the package annotation DB.

adme16cod_dbInfo prints other information about the package annotation DB.

Value

adme16cod_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

adme16cod_dbfile: a character string with the path to the package annotation DB.

adme16cod_dbschema: none (invisible NULL).

adme16cod_dbInfo: none (invisible NULL).
...
dbGetQuery(adme16cod_dbconn(), "SELECT COUNT(*) FROM probes")
...
adme16codGENENAME, 7
adme16codGO, 8
adme16codGO2ALLPROBES, 9
adme16codGO2ALLPROBES (adme16codGO), 8
adme16codGO2PROBE (adme16codGO), 8
adme16codLOCUSID (adme16codENTREZID), 5
adme16codMAPCOUNTS, 10
adme16codORGANISM, 11
adme16codORGPKG (adme16codORGANISM), 11
adme16codPATH, 11
adme16codPATH2PROBE (adme16codPATH), 11
adme16codPMID, 12
adme16codPMID2PROBE (adme16codPMID), 12
adme16codREFSEQ, 13
adme16codSYMBOL, 14
adme16codUNIGENE, 15
adme16codUNIPROT, 16
adme16codUNIPROT2PROBE
...
adme16cod_dbconn, 16
adme16codACCNUM, 1
adme16codALIAS2PROBE, 2
adme16codCHR, 3
adme16codCHRLENGTHS, 3
adme16codCHRLOC, 4
adme16codENSEMBL, 4
adme16codENTREZID, 5
adme16codENZYME, 6
...
adme16cod (adme16cod.db), 2
adme16cod.db, 2
adme16cod_dbconn, 16
adme16cod_dbfile (adme16cod_dbconn), 16
adme16cod_dbInfo (adme16cod_dbconn), 16
adme16cod_dbschema (adme16cod_dbconn),
...
adme16codCHRLENGTHS, 3
adme16codCHRLOC, 4
adme16codCHRLOCEND (adme16codCHRLOC), 4
adme16codENSEMBL, 4
adme16codENSEMBL2PROBE

(adme16codENSEMBL), 4

adme16codENTREZID, 5
adme16codENZYME, 6
adme16codENZYME2PROBE
