...
inputs:
  - id: operations
    type:
      type: array
      items: string
    doc: Operation(s) to perform. Can be a primary operation, followed by other 
      operations. 'fld' is the input field to use. Multiple fields can be listed
      with a comma (e.g. 1,6,8). A range of fields can be listed with a dash 
      (e.g. 2-8). Use colons for operations which require a pair of fields (e.g.
      'pcov 2:6').
    inputBinding:
      position: 1
  ...
  - id: full
    ...
    doc: 'print entire input line before op results (default: print only the grouped
      keys). This option is only sensible for linewise operations.'
    inputBinding:
      position: 102
      prefix: --full
  - id: group
