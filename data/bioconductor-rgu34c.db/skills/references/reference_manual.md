...
rgu34cACCNUM is an R object that contains mappings between a manufacturer’s identifiers and
...
rgu34cALIAS is an R object that provides mappings between common gene symbol identifiers and
...
Welcome to the rgu34c.db annotation Package. The purpose of this package is to provide detailed
information about the rgu34c platform. This package is updated biannually.
...
rgu34cCHR is an R object that provides mappings between a manufacturer identifier and the chro-
...
rgu34cCHRLENGTHS provides the length measured in base pairs for each of the chromosomes.
...
rgu34cCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
rgu34cENSEMBL is an R object that contains mappings between manufacturer identifiers and En-
...
rgu34cENTREZID is an R object that provides mappings between manufacturer identifiers and
...
rgu34cENZYME is an R object that provides mappings between manufacturer identifiers and EC
numbers. rgu34cENZYME2PROBE is an R object that maps Enzyme Commission (EC) numbers
to manufacturer identifiers.

Details

When the rgu34cENZYME maping viewed as a list, each manufacturer identifier maps to a named
...
For the rgu34cENZYME2PROBE, each EC number maps to a named vector containing all of the
...
numbers. In rgu34cENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert rgu34cENZYME2PROBE to a list to see inside

rgu34cGENENAME

9

x <- rgu34cENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
rgu34cGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
rgu34cGO is an R object that provides mappings between manufacturer identifiers and the GO iden-
tifiers that they are directly associated with. This mapping and its reverse mapping (rgu34cGO2PROBE)
...
rgu34cGO2ALLPROBES is an R object that provides mappings between a given GO identifier and
all of the manufacturer identifiers annotated at that GO term OR TO ONE OF IT’S CHILD NODES
in the GO ontology. Thus, this mapping is much larger and more inclusive than rgu34cGO2PROBE.

Details

If rgu34cGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The names on
...
If rgu34cGO2ALLPROBES or rgu34cGO2PROBE is cast as a list, each GO term maps to a named
...
x <- rgu34cGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert rgu34cGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
if(length(xx) > 0){
...
Number of mapped keys for the maps in package rgu34c.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

rgu34cMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map in
package rgu34c.db.
...
The Organism information for rgu34c

Description

rgu34cORGANISM is an R object that contains a single item: a character string that names the
organism for which rgu34c was built. rgu34cORGPKG is an R object that contains a chararcter
vector with the name of the organism package that a chip package depends on for its gene-centric
annotation.

Details

Although the package name is suggestive of the organism for which it was built, rgu34cORGANISM
provides a simple way to programmatically extract the organism name. rgu34cORGPKG provides
...
rgu34cPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

rgu34cPATH2PROBE is an R object that provides mappings between KEGG identifiers and manu-
...
x <- rgu34cPATH
mapped_probes <- mappedkeys(x)
# Now convert the rgu34cPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rgu34cPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
rgu34cPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. rgu34cPMID2PROBE is an R object that provides mappings between PubMed identi-
fiers and manufacturer identifiers.

Details

When rgu34cPMID is viewed as a list each manufacturer identifier is mapped to a named vector of
...
When rgu34cPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named vector
...
x <- rgu34cPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object rgu34cPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
rgu34cPROSITE is an R object that provides mappings between manufacturer identifiers and PROSITE
identifiers.
...
rgu34cREFSEQ is an R object that provides mappings between manufacturer identifiers and RefSeq
identifiers.
...
rgu34cSYMBOL is an R object that provides mappings between manufacturer identifiers and gene
abbreviations.
...
rgu34cUNIPROT is an R object that contains mappings between the manufacturer identifiers and
...
rgu34c_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by rgu34c_dbconn or you will break all the
AnnDbObj objects defined in this package!

rgu34c_dbfile returns the path (character string) to the package annotation DB (this is an SQLite
file).
...
rgu34c_dbconn: a DBIConnection object representing an open connection to the package annota-
tion DB.
...
rgu34cCHRLENGTHS, 4
rgu34cCHRLOC, 4
rgu34cCHRLOCEND (rgu34cCHRLOC), 4
rgu34cENSEMBL, 5
rgu34cENSEMBL2PROBE (rgu34cENSEMBL), 5
rgu34cENTREZID, 6
rgu34cENZYME, 7
rgu34cENZYME2PROBE (rgu34cENZYME), 7
rgu34cGENENAME, 9
rgu34cGO, 10
rgu34cGO2ALLPROBES, 11
rgu34cGO2ALLPROBES (rgu34cGO), 10
rgu34cGO2PROBE (rgu34cGO), 10
rgu34cLOCUSID (rgu34cENTREZID), 6
rgu34cMAPCOUNTS, 12
rgu34cORGANISM, 12
rgu34cORGPKG (rgu34cORGANISM), 12
rgu34cPATH, 13
rgu34cPATH2PROBE (rgu34cPATH), 13
rgu34cPFAM, 14
rgu34cPMID, 14
rgu34cPMID2PROBE (rgu34cPMID), 14
rgu34cPROSITE, 16
rgu34cREFSEQ, 16
rgu34cSYMBOL, 17
rgu34cUNIPROT, 18
rgu34cUNIPROT2PROBE (rgu34cUNIPROT), 18
...
rgu34c.db, 2
rgu34c_dbconn, 19
rgu34c_dbfile (rgu34c_dbconn), 19
rgu34c_dbInfo (rgu34c_dbconn), 19
rgu34c_dbschema (rgu34c_dbconn), 19
