...
This vignette describes how to process quantitative mass spectrometry data with QFeatures: cleaning up unneeded feature variables, adding an experimental design, filtering out contaminants and reverse hits, managing missing values, log-transforming, normalising and aggregating data. This vignette is distributed under a CC BY-SA license.
...
conditions A and B (Paulovich et al. [2010](#ref-Paulovich:2010)). The peptide-level data, as
processed by MaxQuant (Cox and Mann [2008](#ref-Cox:2008)) is available in the `msdata`
package:
...
values (if any) (yellow on figure [1](#fig:miximp)) and a
deterministic minimum value imputation on the
35 proteins that display a non-random pattern
of missing values (brown on figure [1](#fig:miximp)).
...
![Quantitative data in its original scale (left) and log2-transformed (right).](data:image/png;base64...)
...
![Distribution of log2 peptide intensities before (left) and after (right) median normalisation.](data:image/png;base64...)

Figure 3: Distribution of log2 peptide intensities before (left) and after (right) median normalisation

# 10 Feature aggregation

At this stage, it is possible to directly use the peptide-level
intensities to perform a statistical analysis (Goeminne, Gevaert, and Clement [2016](#ref-Goeminne:2016)), or
...
* and finally **`fun`**, the function that will compute this
  ...
  `robustSummary` (Sticker et al. [2019](#ref-Sticker:2019)).
...
![Expression intensities for the protein *P02787ups|TRFE_HUMAN_UPS* (right, green) and its peptides (left) in groups A (circles) and B (triangles).](data:image/png;base64...)

Figure 4: Expression intensities for the protein *P02787ups|TRFE\_HUMAN\_UPS* (right, green) and its peptides (left) in groups A (circles) and B (triangles)
