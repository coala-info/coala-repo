...
hu35ksubaACCNUM is an R object that contains mappings between a manufacturer’s identifiers
...
hu35ksubaALIAS2PROBE Map between Common Gene Symbol Identifiers and Manufacturer

Identifiers

Description

hu35ksubaALIAS is an R object that provides mappings between common gene symbol identifiers
...
xx <- as.list(hu35ksubaALIAS2PROBE)
if(length(xx) > 0){
...
Welcome to the hu35ksuba.db annotation Package. The purpose of this package is to provide de-
tailed information about the hu35ksuba platform. This package is updated biannually.
...
hu35ksubaCHR is an R object that provides mappings between a manufacturer identifier and the
...
hu35ksubaCHRLENGTHS provides the length measured in base pairs for each of the chromo-
somes.
...
hu35ksubaCHRLOC is an R object that maps manufacturer identifiers to the starting position of the
...
hu35ksubaENSEMBL is an R object that contains mappings between manufacturer identifiers and
...
hu35ksubaENTREZID is an R object that provides mappings between manufacturer identifiers and
...
hu35ksubaENZYME is an R object that provides mappings between manufacturer identifiers and
EC numbers. hu35ksubaENZYME2PROBE is an R object that maps Enzyme Commission (EC)
numbers to manufacturer identifiers.

Details

When the hu35ksubaENZYME maping viewed as a list, each manufacturer identifier maps to a
...
For the hu35ksubaENZYME2PROBE, each EC number maps to a named vector containing all of
...
numbers. In hu35ksubaENZYME2PROBE, EC is dropped from the Enzyme Commission numbers.
...
# Now convert hu35ksubaENZYME2PROBE to a list to see inside

hu35ksubaGENENAME

9

x <- hu35ksubaENZYME2PROBE
mapped_probes <- mappedkeys(x)
## convert to a list
xx <- as.list(x[mapped_probes][1:3])
if(length(xx) > 0){
...
hu35ksubaGENENAME is an R object that maps manufacturer identifiers to the corresponding gene
name.
...
hu35ksubaGO is an R object that provides mappings between manufacturer identifiers and the GO
identifiers that they are directly associated with. This mapping and its reverse mapping (hu35ksubaGO2PROBE)
...
hu35ksubaGO2ALLPROBES is an R object that provides mappings between a given GO iden-
...
If hu35ksubaGO is cast as a list, each manufacturer identifier is mapped to a list of lists. The
...
If hu35ksubaGO2ALLPROBES or hu35ksubaGO2PROBE is cast as a list, each GO term maps to
...
x <- hu35ksubaGO2ALLPROBES
mapped_genes <- mappedkeys(x)
# Convert hu35ksubaGO2ALLPROBES to a list
xx <- as.list(x[mapped_genes][1:300])
...
hu35ksubaMAP is an R object that provides mappings between manufacturer identifiers and cyto-
band locations.
...
Number of mapped keys for the maps in package hu35ksuba.db

Description

DEPRECATED. Counts in the MAPCOUNT table are out of sync and should not be used.

hu35ksubaMAPCOUNTS provides the "map count" (i.e. the count of mapped keys) for each map
in package hu35ksuba.db.
...
hu35ksubaOMIM is an R object that provides mappings between manufacturer identifiers and
OMIM identifiers.
...
The Organism information for hu35ksuba

Description

hu35ksubaORGANISM is an R object that contains a single item: a character string that names
the organism for which hu35ksuba was built. hu35ksubaORGPKG is an R object that contains a
chararcter vector with the name of the organism package that a chip package depends on for its
gene-centric annotation.

Details

Although the package name is suggestive of the organism for which it was built, hu35ksubaORGANISM
provides a simple way to programmatically extract the organism name. hu35ksubaORGPKG pro-
...
hu35ksubaPATH maps probe identifiers to the identifiers used by KEGG for pathways in which the
genes represented by the probe identifiers are involved

hu35ksubaPATH2PROBE is an R object that provides mappings between KEGG identifiers and
...
x <- hu35ksubaPATH
mapped_probes <- mappedkeys(x)
# Now convert the hu35ksubaPATH2PROBE object to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubaPFAM is an R object that provides mappings between manufacturer identifiers and PFAM
identifiers.
...
hu35ksubaPMID is an R object that provides mappings between manufacturer identifiers and PubMed
identifiers. hu35ksubaPMID2PROBE is an R object that provides mappings between PubMed iden-
tifiers and manufacturer identifiers.

Details

When hu35ksubaPMID is viewed as a list each manufacturer identifier is mapped to a named vector
...
When hu35ksubaPMID2PROBE is viewed as a list each PubMed identifier is mapped to a named
...
x <- hu35ksubaPMID2PROBE
mapped_probes <- mappedkeys(x)
# Now convert the reverse map object hu35ksubaPMID2PROBE to a list
xx <- as.list(x[mapped_probes][1:300])
if(length(xx) > 0){
...
hu35ksubaPROSITE is an R object that provides mappings between manufacturer identifiers and
PROSITE identifiers.
...
hu35ksubaREFSEQ is an R object that provides mappings between manufacturer identifiers and
RefSeq identifiers.
...
hu35ksubaSYMBOL is an R object that provides mappings between manufacturer identifiers and
gene abbreviations.
...
hu35ksubaUNIPROT is an R object that contains mappings between the manufacturer identifiers
...
hu35ksuba_dbconn returns a connection object to the package annotation DB. IMPORTANT: Don’t
call dbDisconnect on the connection object returned by hu35ksuba_dbconn or you will break all
the AnnDbObj objects defined in this package!

hu35ksuba_dbfile returns the path (character string) to the package annotation DB (this is an
SQLite file).

hu35ksuba_dbschema prints the schema definition of the package annotation DB.

hu35ksuba_dbInfo prints other information about the package annotation DB.

Value

hu35ksuba_dbconn: a DBIConnection object representing an open connection to the package an-
notation DB.

hu35ksuba_dbfile: a character string with the path to the package annotation DB.

hu35ksuba_dbschema: none (invisible NULL).

hu35ksuba_dbInfo: none (invisible NULL).
...
hu35ksuba_dbconn, 21
hu35ksubaACCNUM, 1
hu35ksubaALIAS2PROBE, 2
hu35ksubaCHR, 3
hu35ksubaCHRLENGTHS, 4
hu35ksubaCHRLOC, 4
hu35ksubaENSEMBL, 5
hu35ksubaENTREZID, 6
hu35ksubaENZYME, 7
...
hu35ksubaGO, 10
hu35ksubaMAP, 12
hu35ksubaMAPCOUNTS, 13
hu35ksubaOMIM, 13
hu35ksubaORGANISM, 14
hu35ksubaPATH, 15
hu35ksubaPFAM, 16
hu35ksubaPMID, 16
hu35ksubaPROSITE, 17
...
hu35ksuba (hu35ksuba.db), 2
hu35ksuba.db, 2
hu35ksuba_dbconn, 21
hu35ksuba_dbfile (hu35ksuba_dbconn), 21

23

hu35ksuba_dbInfo (hu35ksuba_dbconn), 21
hu35ksuba_dbschema (hu35ksuba_dbconn),
...
hu35ksubaCHRLENGTHS, 4
hu35ksubaCHRLOC, 4
hu35ksubaCHRLOCEND (hu35ksubaCHRLOC), 4
hu35ksubaENSEMBL, 5
hu35ksubaENSEMBL2PROBE

(hu35ksubaENSEMBL), 5

hu35ksubaENTREZID, 6
hu35ksubaENZYME, 7
hu35ksubaENZYME2PROBE

(hu35ksubaENZYME), 7

hu35ksubaGENENAME, 9
hu35ksubaGO, 10
hu35ksubaGO2ALLPROBES, 11
hu35ksubaGO2ALLPROBES (hu35ksubaGO), 10
hu35ksubaGO2PROBE (hu35ksubaGO), 10
hu35ksubaLOCUSID (hu35ksubaENTREZID), 6
hu35ksubaMAP, 12
hu35ksubaMAPCOUNTS, 13
hu35ksubaOMIM, 13
hu35ksubaORGANISM, 14
hu35ksubaORGPKG (hu35ksubaORGANISM), 14
hu35ksubaPATH, 15
hu35ksubaPATH2PROBE (hu35ksubaPATH), 15
hu35ksubaPFAM, 16
hu35ksubaPMID, 16
hu35ksubaPMID2PROBE (hu35ksubaPMID), 16
hu35ksubaPROSITE, 17
hu35ksubaREFSEQ, 18
hu35ksubaSYMBOL, 19
hu35ksubaUNIPROT, 20
hu35ksubaUNIPROT2PROBE
