...
A table containing window sequences per metagenome/contig and marker gene. It may be in default form (a TSV with 6 columns, like below), or an extended form with more detail in further columns. The default OTU table output from [pipe](/singlem/tools/pipe), [renew](/singlem/tools/renew) and [summarise](/singlem/tools/summarise) subcommands has 6 columns, with one sequence per row. The extended form OTU table and archive OTU tables have further information (see below). Columns of a default OTU table:
...
6. A taxonomic classification for the OTU. This is a rough taxonomy which is refined during the "condense" step of the [pipe](/singlem/tools/pipe) workflow.
...
The extended OTU table form generated with the `--output-extras` option to the [pipe](/singlem/tools/pipe), [renew](/singlem/tools/renew) and [summarise](/singlem/tools/summarise) subcommands, has all the columns of a regular OTU table, but with several additional columns which contain more information about each OTU:

1. read\_names - the names of the reads which encode the OTU sequence
2. nucleotides\_aligned - the number of nucleotides which aligned to the window (usually 60, but can be more or less if there are gaps or inserts)
3. taxonomy\_by\_known? - whether the taxonomy of the OTU was determined by known genomes (TRUE) or by the reads themselves (FALSE). Currently this is a disused column and is always marked FALSE.
4. read\_unaligned\_sequences - the raw sequences of the reads which encode the OTU sequence
5. equal\_best\_hit\_taxonomies - the taxonomies of the best hits to the OTU sequence, if there are multiple equally good hits. This is a JSON array of strings.

## **Archive OTU table**

Similar to an extended form OTU table, but in JSON form for machine readability and with formatting version recorded. The [renew](/singlem/tools/renew) subcommand which re-analyses a dataset requires this format of OTU table rather than the default tab-separated OTU table format. The canonical file extension for SingleM packages is `.json`.
...
In a standard taxonomic profile output by [pipe](/singlem/tools/pipe), the coverage column is *unfilled* coverage. This means that the coverage of a taxon does not include the coverage of its sub-taxons. For example, in this profile:
...
An OTU table which has been converted to SQLite3 format and sequence similarity search indexes. Canonically SingleM databases are named with the `.sdb` extension, but this is not enforced. SingleM databases are created with the [makedb](/singlem/advanced/makedb) subcommand, and queried with the [query](/singlem/advanced/query) subcommand.

On this page

* [Glossary](#glossary)
* [Taxonomic profile](#taxonomic-profile)
* [OTU table](#otu-table)
* [OTU table (extended form)](#otu-table-(extended-form))
* [Archive OTU table](#archive-otu-table)
* [Coverage, unfilled coverage and filled coverage](#coverage,-unfilled-coverage-and-filled-coverage)
* [SingleM package (spkg)](#singlem-package-(spkg)-)
* [SingleM metapackage (smpkg)](#singlem-metapackage-(smpkg)-)
* [SingleM database](#singlem-database-)
